...
static void append_comma_and_scheme(gpointer scheme, gpointer user_data) {
    char **string;
...
format_supported_uri_schemes_for_display(GList *supported_uri_schemes) {
    ...
    string = g_strdup("");
    g_list_foreach(supported_uri_schemes, append_comma_and_scheme, &string);
    return string;
}

static void print_application(GAppInfo *application) {
    if (application == NULL) {
        puts("(none)");
    } else {
        printf("name: %s\ncommand: %s\nexpects_uris: %s\n",
...
static void print_application_list(GList *applications) {
    GList *p;
