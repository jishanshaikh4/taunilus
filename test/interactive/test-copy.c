...
static void copy_done(GHashTable *debuting_uris, gboolean success,
                      gpointer data) {
  g_print("Copy done\n");
}

static void changed_cb(NautilusProgressInfo *info, gpointer data) {
  g_print("Changed: %s -- %s\n", nautilus_progress_info_get_status(info),
          nautilus_progress_info_get_details(info));
}

static void progress_changed_cb(NautilusProgressInfo *info, gpointer data) {
  g_print("Progress changed: %f\n", nautilus_progress_info_get_progress(info));
}

static void finished_cb(NautilusProgressInfo *info, gpointer data) {
  g_print("Finished\n");
  gtk_main_quit();
}
...
int main(int argc, char *argv[]) {
  ...
  nautilus_file_operations_copy_async(sources, dest, GTK_WINDOW(window), NULL,
                                      copy_done, NULL);
  ...
  g_signal_connect(progress_info, "progress-changed",
                   (GCallback)progress_changed_cb, NULL);
