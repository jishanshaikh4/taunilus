...
G_DEFINE_TYPE_WITH_CODE(NautilusSearchEngineRecent,
                        nautilus_search_engine_recent, G_TYPE_OBJECT,
...
static void nautilus_search_engine_recent_finalize(GObject *object) {
...
static gboolean search_thread_add_hits_idle(gpointer user_data) {
...
static void search_add_hits_idle(NautilusSearchEngineRecent *self,
                                 GList *hits) {
...
static gboolean is_file_valid_recursive(NautilusSearchEngineRecent *self,
                                        GFile *file, GError **error) {
    g_autofree gchar *path = NULL;
    g_autoptr(GFileInfo) file_info = NULL;

    file_info = g_file_query_info(file, FILE_ATTRIBS, G_FILE_QUERY_INFO_NONE,
...
static gpointer recent_thread_func(gpointer user_data) {
    g_autoptr(NautilusSearchEngineRecent) self =
        NAUTILUS_SEARCH_ENGINE_RECENT(user_data);
    ...
    for (l = recent_items; l != NULL; l = l->next) {
        ...
        if (rank > 0) {
            ...
            if (gtk_recent_info_is_local(info)) {
                ...
                if (!is_file_valid_recursive(self, file, &error)) {
                    ...
                    if (error != NULL &&
                        ...
                        g_debug("Impossible to read recent file info: %s", error->message);
                    }
            ...
            if (date_range != NULL) {
                ...
                if (!nautilus_file_date_in_between(target_time, initial_date,
                                                   end_date)) {
                    continue;
                }
            }
...
static void nautilus_search_engine_recent_get_property(GObject *object,
        guint prop_id,
        GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_search_engine_recent_class_init(
    NautilusSearchEngineRecentClass *klass) {
