...
static GDebugKey keys[] = {{"Application", NAUTILUS_DEBUG_APPLICATION},
...
static void nautilus_debug_set_flags_from_env(void) {
  guint nkeys;
...
void nautilus_debug_set_flags(DebugFlags new_flags) {
  flags |= new_flags;
  initialized = TRUE;
}

gboolean nautilus_debug_flag_is_set(DebugFlags flag) { return flag & flags; }
...
nautilus_debug_valist(DebugFlags flag, const gchar *format, va_list args) {
...
nautilus_debug_files_valist(DebugFlags flag, GList *files, const gchar *format,
                            va_list args) {
  NautilusFile *file;
  GList *l;
  gchar *uri, *msg;
