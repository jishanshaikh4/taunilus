...
static void source_info_clear(SourceInfo *source_info) {
...
#define IS_IO_ERROR(__error, KIND)                                             \
  (((__error)->domain == G_IO_ERROR && (__error)->code == G_IO_ERROR_##KIND))
...
static gboolean is_all_button_text(const char *button_text) {
    ...
    return !strcmp(button_text, SKIP_ALL) || !strcmp(button_text, REPLACE_ALL) ||
           !strcmp(button_text, DELETE_ALL) || !strcmp(button_text, MERGE_ALL);
}

static void scan_sources(GList *files, SourceInfo *source_info, CommonJob *job,
                         OpKind kind);

static void empty_trash_thread_func(GTask *task, gpointer source_object,
                                    gpointer task_data,
                                    GCancellable *cancellable);

static void empty_trash_task_done(GObject *source_object, GAsyncResult *res,
                                  gpointer user_data);

static char *query_fs_type(GFile *file, GCancellable *cancellable);

static void nautilus_file_operations_copy(GTask *task, gpointer source_object,
        gpointer task_data,
        GCancellable *cancellable);

static void nautilus_file_operations_move(GTask *task, gpointer source_object,
        gpointer task_data,
        GCancellable *cancellable);
...
static gchar *get_formatted_time(int seconds) {
    int minutes;
    int hours;
    gchar *res;
    ...
    if (seconds < 60) {
        return g_strdup_printf(ngettext("%'d second", "%'d seconds", (int)seconds),
                               (int)seconds);
    }

    if (seconds < 60 * 60) {
        minutes = seconds / 60;
        return g_strdup_printf(ngettext("%'d minute", "%'d minutes", minutes),
                               minutes);
    }
 ...
 * and nth plurals, you can leave the st, nd, rd suffixes out and just make all
 * the translated strings look like "%s (copy %'d)%s".
 */
...
static char *extract_string_until(const char *original,
 ...
 */
static void parse_previous_duplicate_name(const char *name, char **name_base,
    ...
    *count = 0;
    /* ignore_extension was not used before to let above code handle case "dir
     * (copy).dir" for directories */
...
static char *make_next_duplicate_name(const char *base, const char *suffix,
                                      int count, int max_length) {
    const char *format;
    char *result;
    int unshortened_length;
    gboolean use_count;
...
static char *get_duplicate_name(const char *name, int count_increment,
                                int max_length, gboolean ignore_extension) {
    char *result;
    char *name_base;
    const char *suffix;
    int count;
...
static gchar *get_basename(GFile *file) {
    GFileInfo *info;
    ...
    if ((mount = nautilus_get_mounted_mount_for_root(file)) != NULL) {
        ...
        info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, 0,
                                 g_cancellable_get_current(), NULL);
        name = NULL;
        if (info) {
    ...
    if (name == NULL) {
        ...
        if (g_utf8_validate(basename, -1, NULL)) {
            ...
            name = g_uri_escape_string(basename, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH,
                                       TRUE);
            g_free(basename);
        }
    }
...
static gchar *get_truncated_parse_name(GFile *file) {
    ...
    return eel_str_middle_truncate(parse_name,
                                   MAXIMUM_DISPLAYED_FILE_NAME_LENGTH);
}

#define op_job_new(__type, parent_window, dbus_data)                           \
  ((__type *)(init_common(sizeof(__type), parent_window, dbus_data)))
...
static gpointer init_common(gsize job_size, GtkWindow *parent_window,
    ...
    common->cancellable =
        nautilus_progress_info_get_cancellable(common->progress);
...
static void skip_file(CommonJob *common, GFile *file) {
    if (common->skip_files == NULL) {
        common->skip_files = g_hash_table_new_full(
...
static void skip_readdir_error(CommonJob *common, GFile *dir) {
    if (common->skip_readdir_error == NULL) {
        common->skip_readdir_error = g_hash_table_new_full(
                                         g_file_hash, (GEqualFunc)g_file_equal, g_object_unref, NULL);
    }
...
static void set_transient_for(GdkWindow *child_window,
    ...
    if (GDK_IS_WAYLAND_DISPLAY(display)) {
        ...
        if (g_str_has_prefix(parent_handle, prefix)) {
            ...
            gdk_wayland_window_set_transient_for_exported(child_window,
                    (char *)handle);
        }
    }
#endif
...
static void dialog_realize_cb(GtkWidget *widget, gpointer user_data) {
    NautilusFileOperationsDBusData *dbus_data = user_data;
    const char *parent_handle;

    parent_handle =
        nautilus_file_operations_dbus_data_get_parent_handle(dbus_data);
    set_transient_for(gtk_widget_get_window(widget), parent_handle);
}

static gboolean is_long_job(CommonJob *job) {
...
static gboolean simple_dialog_button_activate(GtkWidget *button) {
    gtk_widget_set_sensitive(button, TRUE);
    return G_SOURCE_REMOVE;
}

static void simple_dialog_cb(GtkDialog *dialog, gint response_id,
                             gpointer user_data) {
...
static gboolean do_run_simple_dialog(gpointer _data) {
    ...
    /* Create the dialog. */
    dialog = gtk_message_dialog_new(*data->parent_window, 0, data->message_type,
                                    GTK_BUTTONS_NONE, NULL);

    g_object_set(dialog, "text", data->primary_text, "secondary-text",
                 data->secondary_text, NULL);
    ...
    if (data->dbus_data != NULL) {
        ...
        timestamp =
            nautilus_file_operations_dbus_data_get_timestamp(data->dbus_data);
        ...
        if (nautilus_file_operations_dbus_data_get_parent_handle(data->dbus_data) !=
                NULL) {
            g_signal_connect(dialog, "realize", G_CALLBACK(dialog_realize_cb),
                             data->dbus_data);
        }
...
static int run_simple_dialog_va(CommonJob *job, gboolean ignore_close_box,
                                GtkMessageType message_type, char *primary_text,
                                char *secondary_text, const char *details_text,
                                gboolean show_all, va_list varargs) {
...
static int run_error(CommonJob *job, char *primary_text, char *secondary_text,
    ...
    res = run_simple_dialog_va(job, FALSE, GTK_MESSAGE_ERROR, primary_text,
                               secondary_text, details_text, show_all, varargs);
    va_end(varargs);
    return res;
}

static int run_warning(CommonJob *job, char *primary_text, char *secondary_text,
                       const char *details_text, gboolean show_all, ...) {
    va_list varargs;
    int res;

    va_start(varargs, show_all);
    res = run_simple_dialog_va(job, FALSE, GTK_MESSAGE_WARNING, primary_text,
                               secondary_text, details_text, show_all, varargs);
    va_end(varargs);
    return res;
}

static int run_question(CommonJob *job, char *primary_text,
                        char *secondary_text, const char *details_text,
                        gboolean show_all, ...) {
    va_list varargs;
    int res;
    ...
    va_start(varargs, show_all);
    res = run_simple_dialog_va(job, FALSE, GTK_MESSAGE_QUESTION, primary_text,
                               secondary_text, details_text, show_all, varargs);
    va_end(varargs);
    return res;
}

static int run_cancel_or_skip_warning(CommonJob *job, char *primary_text,
                                      char *secondary_text,
                                      const char *details_text,
    ...
    if (total_operations == 1) {
        response = run_warning(job, primary_text, secondary_text, details_text,
                               FALSE, CANCEL, NULL);
    } else {
        response =
            run_warning(job, primary_text, secondary_text, details_text,
                        operations_remaining > 1, CANCEL, SKIP_ALL, SKIP, NULL);
    }
...
static void inhibit_power_manager(CommonJob *job, const char *message) {
    job->inhibit_cookie = gtk_application_inhibit(
...
static gboolean confirm_delete_from_trash(CommonJob *job, GList *files) {
    ...
    if (file_count == 1) {
        ...
        prompt =
            g_strdup_printf(_("Are you sure you want to permanently delete “%s” "
                              "from the trash?"),
                            basename);
    } else {
        prompt =
            g_strdup_printf(ngettext("Are you sure you want to permanently delete "
    ...
    response = run_warning(
                   job, prompt,
                   g_strdup(_("If you delete an item, it will be permanently lost.")), NULL,
                   FALSE, CANCEL, DELETE, NULL);
...
static gboolean confirm_empty_trash(CommonJob *job) {
    ...
    response = run_warning(
                   job, prompt,
                   g_strdup(_("All items in the Trash will be permanently deleted.")), NULL,
                   FALSE, CANCEL, EMPTY_TRASH, NULL);
...
static gboolean confirm_delete_directly(CommonJob *job, GList *files) {
    ...
    if (file_count == 1) {
        ...
        prompt = g_strdup_printf(
                     _("Are you sure you want to permanently delete “%s”?"), basename);
    } else {
        prompt =
            g_strdup_printf(ngettext("Are you sure you want to permanently delete "
                                     "the %'d selected item?",
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
static void report_delete_progress(CommonJob *job, SourceInfo *source_info,
    ...
    if (source_info->num_files == 1) {
        ...
        if (files_left == 0) {
            status = ngettext("Deleted %'d file", "Deleted %'d files",
                              source_info->num_files);
        } else {
            status = ngettext("Deleting %'d file", "Deleting %'d files",
                              source_info->num_files);
        }
        nautilus_progress_info_take_status(
            job->progress, g_strdup_printf(status, source_info->num_files));
    }
    ...
    if (elapsed > 0) {
        ...
        if (transfer_rate > 0) {
            remaining_time =
                (source_info->num_files - transfer_info->num_files) / transfer_rate;
        }
    }
    ...
    if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE ||
        ...
        if (files_left > 0) {
            ...
            details = g_strdup_printf(_("%'d / %'d"), transfer_info->num_files + 1,
                                      source_info->num_files);
        } else {
            /* To translators: %'d is the number of files completed for the operation,
             * so it will be something like 2/14. */
            details = g_strdup_printf(_("%'d / %'d"), transfer_info->num_files,
                                      source_info->num_files);
        }
    } else {
        if (files_left > 0) {
             ...
             * (i.e. the %s argument).
             */
            time_left_message = ngettext(
                                    "%'d / %'d \xE2\x80\x94 %s left", "%'d / %'d \xE2\x80\x94 %s left",
                                    seconds_count_format_time_units(remaining_time));
            transfer_rate += 0.5;
            files_per_second_message =
                ngettext("(%d file/sec)", "(%d files/sec)", (int)transfer_rate);
            concat_detail =
                g_strconcat(time_left_message, " ", files_per_second_message, NULL);

            formatted_time = get_formatted_time(remaining_time);
            details = g_strdup_printf(concat_detail, transfer_info->num_files + 1,
                                      source_info->num_files, formatted_time,
                                      (int)transfer_rate);
...
typedef void (*DeleteCallback)(GFile *file, GError *error,
                               gpointer callback_data);

static gboolean delete_file_recursively(GFile *file, GCancellable *cancellable,
    ...
    do {
        ...
        if (enumerator) {
            ...
            while (info != NULL) {
                ...
                success = success && delete_file_recursively(child, cancellable,
...
static void file_deleted_callback(GFile *file, GError *error,
                                  gpointer callback_data) {
    ...
    file_type = g_file_query_file_type(file, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                       job->cancellable);
    ...
    if (file_type == G_FILE_TYPE_DIRECTORY) {
        secondary =
            IS_IO_ERROR(error, PERMISSION_DENIED)
    ...
    response = run_cancel_or_skip_warning(
                   job, primary, secondary, details, source_info->num_files,
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
static void report_trash_progress(CommonJob *job, SourceInfo *source_info,
                                  TransferInfo *transfer_info) {
    int files_left;
    double elapsed, transfer_rate;
    int remaining_time;
    ...
    if (source_info->num_files == 1) {
        ...
        if (files_left > 0) {
            status = ngettext("Trashing %'d file", "Trashing %'d files",
                              source_info->num_files);
        } else {
            status = ngettext("Trashed %'d file", "Trashed %'d files",
                              source_info->num_files);
        }
        nautilus_progress_info_take_status(
            job->progress, g_strdup_printf(status, source_info->num_files));
    ...
    if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE ||
        ...
        if (files_left > 0) {
            ...
            time_left_message = ngettext(
                                    "%'d / %'d \xE2\x80\x94 %s left", "%'d / %'d \xE2\x80\x94 %s left",
                                    seconds_count_format_time_units(remaining_time));
            files_per_second_message = ngettext("(%d file/sec)", "(%d files/sec)",
...
static void trash_file(CommonJob *job, GFile *file, gboolean *skipped_file,
                       SourceInfo *source_info, TransferInfo *transfer_info,
                       gboolean toplevel, GList **to_delete) {
    GError *error;
    ...
    if (g_file_trash(file, job->cancellable, &error)) {
        ...
        if (job->undo_info != NULL) {
            nautilus_file_undo_info_trash_add_file(
                NAUTILUS_FILE_UNDO_INFO_TRASH(job->undo_info), file);
        }
    ...
    if (!IS_IO_ERROR(error, NOT_SUPPORTED)) {
        ...
        secondary = g_strdup(
                        _("This remote location does not support sending items to the trash."));
    }

    response =
        run_question(job, primary, secondary, details,
                     (source_info->num_files - transfer_info->num_files) > 1,
                     CANCEL, SKIP_ALL, SKIP, DELETE_ALL, DELETE, NULL);
...
static void source_info_remove_descendent_files_from_count(
    GFile *dir, SourceDirInfo *dir_info, SourceInfo *source_info) {
    GFile *other_dir;
    ...
    while (g_hash_table_iter_next(&dir_info_iter, (gpointer *)&other_dir,
                                  (gpointer *)&other_dir_info)) {
        g_assert(other_dir != NULL);
        g_assert(other_dir_info != NULL);

        if (other_dir_info != dir_info && g_file_has_parent(other_dir, dir)) {
            source_info_remove_descendent_files_from_count(other_dir, other_dir_info,
                    source_info);
        }
    }
}

static void source_info_remove_file_from_count(GFile *file, CommonJob *job,
        SourceInfo *source_info) {
    ...
    if (dir_info != NULL) {
        source_info_remove_descendent_files_from_count(file, dir_info, source_info);
    }
}
...
static void trash_files(CommonJob *job, GList *files, int *files_skipped) {
    ...
    for (l = files; l != NULL && !job_aborted(job); l = l->next) {
        ...
        skipped_file = FALSE;
        trash_file(job, file, &skipped_file, &source_info, &transfer_info, TRUE,
                   &to_delete);
        if (skipped_file) {
            (*files_skipped)++;
...
static void delete_task_done(GObject *source_object, GAsyncResult *res,
                             gpointer user_data) {
    DeleteJob *job;
    ...
    if (job->done_callback) {
        debuting_uris = g_hash_table_new_full(g_file_hash, (GEqualFunc)g_file_equal,
                                              g_object_unref, NULL);
        job->done_callback(debuting_uris, job->user_cancel,
...
static void trash_or_delete_internal(GTask *task, gpointer source_object,
                                     gpointer task_data,
                                     GCancellable *cancellable) {
...
static DeleteJob *setup_delete_job(GList *files, GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data,
                                   gboolean try_trash,
                                   NautilusDeleteCallback done_callback,
                                   gpointer done_callback_data) {
    ...
    if (!nautilus_file_undo_manager_is_operating() && try_trash) {
        job->common.undo_info =
            nautilus_file_undo_info_trash_new(g_list_length(files));
    }
...
static void
trash_or_delete_internal_sync(GList *files, GtkWindow *parent_window,
                              NautilusFileOperationsDBusData *dbus_data,
                              gboolean try_trash) {
    GTask *task;
    DeleteJob *job;
    ...
    job =
        setup_delete_job(files, parent_window, dbus_data, try_trash, NULL, NULL);

    task = g_task_new(NULL, NULL, NULL, job);
    g_task_set_task_data(task, job, NULL);
    g_task_run_in_thread_sync(task, trash_or_delete_internal);
    g_object_unref(task);
    /* Since g_task_run_in_thread_sync doesn't work with callbacks (in this case
     * not reaching delete_task_done) we need to set up the undo information
     * ourselves.
     */
    delete_task_done(NULL, NULL, job);
}

static void trash_or_delete_internal_async(
    GList *files, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data, gboolean try_trash,
    NautilusDeleteCallback done_callback, gpointer done_callback_data) {
    GTask *task;
    DeleteJob *job;

    job = setup_delete_job(files, parent_window, dbus_data, try_trash,
...
void nautilus_file_operations_trash_or_delete_async(
    GList *files, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusDeleteCallback done_callback, gpointer done_callback_data) {
    trash_or_delete_internal_async(files, parent_window, dbus_data, TRUE,
                                   done_callback, done_callback_data);
}

void nautilus_file_operations_delete_async(
    GList *files, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusDeleteCallback done_callback, gpointer done_callback_data) {
    trash_or_delete_internal_async(files, parent_window, dbus_data, FALSE,
                                   done_callback, done_callback_data);
}
...
static void unmount_mount_callback(GObject *source_object, GAsyncResult *res,
                                   gpointer user_data) {
    ...
    error = NULL;
    if (data->eject) {
        unmounted = g_mount_eject_with_operation_finish(G_MOUNT(source_object), res,
                    &error);
    } else {
    ...
    if (!unmounted) {
        if (error->code != G_IO_ERROR_FAILED_HANDLED) {
            ...
            show_dialog(primary, error->message, data->parent_window,
                        GTK_MESSAGE_ERROR);
            g_free(primary);
        }
    }
...
static void do_unmount(UnmountData *data) {
    ...
    if (data->eject) {
        g_mount_eject_with_operation(data->mount, 0, mount_op, NULL,
                                     unmount_mount_callback, data);
    } else {
        g_mount_unmount_with_operation(data->mount, 0, mount_op, NULL,
                                       unmount_mount_callback, data);
    }
...
static gboolean dir_has_files(GFile *dir) {
    ...
    enumerator = g_file_enumerate_children(dir, G_FILE_ATTRIBUTE_STANDARD_NAME, 0,
                                           NULL, NULL);
    if (enumerator) {
...
static GtkWidget *create_empty_trash_prompt(GtkWindow *parent_window) {
    GtkWidget *dialog;
    ...
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("Do _not Empty Trash"),
                           GTK_RESPONSE_REJECT, CANCEL, GTK_RESPONSE_CANCEL,
...
static void empty_trash_prompt_cb(GtkDialog *dialog, gint response_id,
                                  gpointer user_data) {
    UnmountData *data = user_data;
...
void nautilus_file_operations_unmount_mount_full(
    ...
    if (check_trash && has_trash_files(mount)) {
        ...
        g_signal_connect(dialog, "response", G_CALLBACK(empty_trash_prompt_cb),
                         data);
        gtk_widget_show_all(dialog);
        return;
    }
...
static void volume_mount_cb(GObject *source_object, GAsyncResult *res,
    ...
    if (!g_volume_mount_finish(G_VOLUME(source_object), res, &error)) {
        if (error->code != G_IO_ERROR_FAILED_HANDLED &&
            ...
            g_free(name);
            success = FALSE;
            show_dialog(primary, error->message, parent, GTK_MESSAGE_ERROR);
            g_free(primary);
        }
        g_error_free(error);
    }

    mount_callback = (NautilusMountCallback)g_object_get_data(G_OBJECT(mount_op),
                     "mount-callback");
    mount_callback_data_object =
        g_object_get_data(G_OBJECT(mount_op), "mount-callback-data");

    if (mount_callback != NULL) {
        (*mount_callback)(G_VOLUME(source_object), success,
                          mount_callback_data_object);

        if (mount_callback_data_object != NULL) {
            g_object_weak_unref(mount_callback_data_object,
                                mount_callback_data_notify, mount_op);
        }
    }
...
void nautilus_file_operations_mount_volume_full(
    ...
    if (mount_callback != NULL && mount_callback_data_object != NULL) {
        g_object_weak_ref(mount_callback_data_object, mount_callback_data_notify,
                          mount_op);
    }
    g_object_set_data(G_OBJECT(mount_op), "mount-callback-data",
                      mount_callback_data_object);

    g_volume_mount(volume, 0, mount_op, NULL, volume_mount_cb, mount_op);
}

static void report_preparing_count_progress(CommonJob *job,
        SourceInfo *source_info) {
    char *s;
    ...
    case OP_KIND_COPY: {
        ...
        s = g_strdup_printf(ngettext("Preparing to copy %'d file (%s)",
                                     ...
                                     source_info->num_files),
                            source_info->num_files, formatted_size);
    }
    break;
...
static void scan_dir(GFile *dir, SourceInfo *source_info, CommonJob *job,
                     GQueue *dirs) {
    GFileInfo *info;
    GError *error;
    GFile *subdir;
    ...
    if (!g_hash_table_contains(source_info->scanned_dirs_info, dir)) {
        ...
        g_hash_table_insert(source_info->scanned_dirs_info, g_object_ref(dir),
                            dir_info);
    }
...
retry:
    ...
    enumerator = g_file_enumerate_children(
                     ...
                     G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, job->cancellable, &error);
    if (enumerator) {
        error = NULL;
        ...
        if (error && IS_IO_ERROR(error, CANCELLED)) {
            ...
            if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
                secondary = g_strdup_printf(
                                _("Files in the folder “%s” cannot be handled "
                                  "because you do not have permissions to see them."),
                                basename);
            } else {
                secondary = g_strdup_printf(
                                _("There was an error getting information about the "
                                  "files in the folder “%s”."),
                                basename);
                details = error->message;
            }

            response = run_warning(job, primary, secondary, details, FALSE, CANCEL,
                                   RETRY, SKIP, NULL);
        ...
        if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
            secondary =
                g_strdup_printf(_("The folder “%s” cannot be handled because you "
                                  "do not have permissions to read it."),
                                basename);
        } else {
            secondary = g_strdup_printf(
                            _("There was an error reading the folder “%s”."), basename);
            details = error->message;
        }
        /* set show_all to TRUE here, as we don't know how many
         * files we'll end up processing yet.
         */
        response = run_warning(job, primary, secondary, details, TRUE, CANCEL,
                               SKIP_ALL, SKIP, RETRY, NULL);
...
static void scan_file(GFile *file, SourceInfo *source_info, CommonJob *job) {
    GFileInfo *info;
    GError *error;
    GQueue *dirs;
    GFile *dir;
...
retry:
    ...
    info = g_file_query_info(
               file, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_STANDARD_SIZE,
               G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, job->cancellable, &error);
    ...
    if (info) {
        ...
        if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
            secondary = g_strdup_printf(
                            _("The file “%s” cannot be handled because you do not have "
                              "permissions to read it."),
                            basename);
        } else {
            secondary = g_strdup_printf(
                            _("There was an error getting information about “%s”."), basename);
            details = error->message;
        }
        /* set show_all to TRUE here, as we don't know how many
         * files we'll end up processing yet.
...
static void scan_sources(GList *files, SourceInfo *source_info, CommonJob *job,
    ...
    source_info->op = kind;
    source_info->scanned_dirs_info = g_hash_table_new_full(
...
static void verify_destination(CommonJob *job, GFile *dest, char **dest_fs_id,
...
retry:
    ...
    error = NULL;
    info = g_file_query_info(
               dest, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_ID_FILESYSTEM,
    ...
    if (info == NULL) {
        ...
        if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
            secondary = g_strdup(
                            _("You do not have permissions to access the destination folder."));
        } else {
            secondary = g_strdup(
                            _("There was an error getting information about the destination."));
            details = error->message;
        }

        response =
            run_error(job, primary, secondary, details, FALSE, CANCEL, RETRY, NULL);
    ...
    if (dest_is_symlink) {
        /* We can't reliably statfs() destination if it's a symlink, thus not doing
         * any further checks. */
        return;
    }
    ...
    fsinfo = g_file_query_filesystem_info(
                 dest,
                 G_FILE_ATTRIBUTE_FILESYSTEM_FREE "," G_FILE_ATTRIBUTE_FILESYSTEM_READONLY,
                 job->cancellable, NULL);
    if (fsinfo == NULL) {
    ...
    if (!job_aborted(job) && g_file_info_get_attribute_boolean(
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
static void report_copy_progress(CopyMoveJob *copy_job, SourceInfo *source_info,
                                 TransferInfo *transfer_info) {
    int files_left;
    goffset total_size;
    double elapsed, transfer_rate;
        ...
        if (source_info->num_files == 1) {
            ...
            if (copy_job->destination != NULL) {
                ...
                if (copy_job->fake_display_source != NULL) {
                    ...
                    basename_fake_display_source =
                        get_basename(copy_job->fake_display_source);
                    tmp = g_strdup_printf(status, basename_fake_display_source,
                                          basename_dest);
                } else {
                    g_autofree gchar *basename_data = NULL;

                    basename_data = get_basename(G_FILE(copy_job->files->data));
                    tmp = g_strdup_printf(status, basename_data, basename_dest);
                }

                nautilus_progress_info_take_status(job->progress, tmp);
            } else {
                g_autofree gchar *basename = NULL;
            ...
            if (copy_job->destination != NULL) {
                if (files_left > 0) {
                    ...
                    if (is_move) {
                        status =
                            ngettext("Moving %'d file to “%s”", "Moving %'d files to “%s”",
                                     source_info->num_files);
                    } else {
                        status =
                            ngettext("Copying %'d file to “%s”",
                                     "Copying %'d files to “%s”", source_info->num_files);
                    }

                    basename = get_basename(G_FILE(copy_job->destination));
                    tmp = g_strdup_printf(status, source_info->num_files, basename);
                    ...
                    if (is_move) {
                        status =
                            ngettext("Moved %'d file to “%s”", "Moved %'d files to “%s”",
                                     source_info->num_files);
                    } else {
                        status =
                            ngettext("Copied %'d file to “%s”", "Copied %'d files to “%s”",
                                     source_info->num_files);
                    }
                ...
                if (files_left > 0) {
                    status =
                        ngettext("Duplicating %'d file in “%s”",
                                 "Duplicating %'d files in “%s”", source_info->num_files);
                    nautilus_progress_info_take_status(
                        job->progress,
                        g_strdup_printf(status, source_info->num_files, basename));
                } else {
                    status =
                        ngettext("Duplicated %'d file in “%s”",
                                 "Duplicated %'d files in “%s”", source_info->num_files);
                    nautilus_progress_info_take_status(
                        job->progress,
                        g_strdup_printf(status, source_info->num_files, basename));
                }
    ...
    if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE ||
            transfer_rate == 0 || !transfer_info->partial_progress) {
        if (source_info->num_files == 1) {
            ...
            details = g_strdup_printf(_("%s / %s"), formatted_size_num_bytes,
                                      formatted_size_total_size);
        } else {
            if (files_left > 0) {
                /* To translators: %'d is the number of files completed for the
                 * operation, so it will be something like 2/14. */
                details = g_strdup_printf(_("%'d / %'d"), transfer_info->num_files + 1,
                                          source_info->num_files);
            } else {
                /* To translators: %'d is the number of files completed for the
        ...
        if (source_info->num_files == 1) {
            if (files_left > 0) {
                ...
                /* To translators: %s will expand to a size like "2 bytes" or "3 MB", %s
                 * to a time duration like "2 minutes". So the whole thing will be
                 * something like "2 kb / 4 MB -- 2 hours left (4kb/sec)"
                 *
                 * The singular/plural form will be used depending on the remaining time
                 * (i.e. the %s argument).
                 */
                details = g_strdup_printf(
                              ngettext("%s / %s \xE2\x80\x94 %s left (%s/sec)",
                                       "%s / %s \xE2\x80\x94 %s left (%s/sec)",
                                       seconds_count_format_time_units(remaining_time)),
                              formatted_size_num_bytes, formatted_size_total_size, formatted_time,
    ...
    nautilus_progress_info_set_progress(job->progress, transfer_info->num_bytes,
                                        total_size);
}
#pragma GCC diagnostic pop
...
static gboolean make_file_name_valid_for_dest_fs(char *filename,
        const char *dest_fs_type) {
    if (dest_fs_type != NULL && filename != NULL) {
        if (!strcmp(dest_fs_type, "fat") || !strcmp(dest_fs_type, "vfat") ||
                ...
                !strcmp(dest_fs_type, "fuse") || !strcmp(dest_fs_type, "ntfs") ||
                !strcmp(dest_fs_type, "msdos") || !strcmp(dest_fs_type, "msdosfs")) {
            gboolean ret;
            int i, old_len;
...
static GFile *get_unique_target_file(GFile *src, GFile *dest_dir,
    ...
    dest = NULL;
    info = g_file_query_info(src, G_FILE_ATTRIBUTE_STANDARD_EDIT_NAME, 0, NULL,
                             NULL);
    if (info != NULL) {
        ...
        if (editname != NULL) {
            new_name =
                get_duplicate_name(editname, count, max_length, ignore_extension);
    ...
    if (dest == NULL) {
        ...
        if (g_utf8_validate(basename, -1, NULL)) {
            new_name =
                get_duplicate_name(basename, count, max_length, ignore_extension);
            make_file_name_valid_for_dest_fs(new_name, dest_fs_type);
            dest = g_file_get_child_for_display_name(dest_dir, new_name, NULL);
            g_free(new_name);
        }
...
static GFile *get_target_file_for_link(GFile *src, GFile *dest_dir,
                                       const char *dest_fs_type, int count) {
    const char *editname;
    char *basename, *new_name;
    GFileInfo *info;
    GFile *dest;
...
static GFile *get_target_file_with_custom_name(GFile *src, GFile *dest_dir,
        const char *dest_fs_type,
        gboolean same_fs,
    ...
    if (dest == NULL && !same_fs) {
        ...
        if (info) {
            ...
            /* if file is being restored from trash make sure it uses its original
             * name */
            if (g_file_has_uri_scheme(src, "trash")) {
                copyname = g_path_get_basename(g_file_info_get_attribute_byte_string(
                                                   info, G_FILE_ATTRIBUTE_TRASH_ORIG_PATH));
            }

            if (copyname == NULL) {
                copyname = g_strdup(g_file_info_get_attribute_string(
                                        info, G_FILE_ATTRIBUTE_STANDARD_COPY_NAME));
            }
...
static GFile *get_target_file(GFile *src, GFile *dest_dir,
                              const char *dest_fs_type, gboolean same_fs) {
    return get_target_file_with_custom_name(src, dest_dir, dest_fs_type, same_fs,
                                            NULL);
}
...
static gboolean has_fs_id(GFile *file, const char *fs_id) {
    ...
    res = FALSE;
    info = g_file_query_info(file, G_FILE_ATTRIBUTE_ID_FILESYSTEM,
...
static GFile *map_possibly_volatile_file_to_real(GFile *volatile_file,
        GCancellable *cancellable,
        GError **error) {
    ...
    if (info == NULL) {
        ...
        is_volatile = g_file_info_get_attribute_boolean(
                          info, G_FILE_ATTRIBUTE_STANDARD_IS_VOLATILE);
        if (is_volatile) {
            const gchar *target;
...
static GFile *map_possibly_volatile_file_to_real_on_write(
    GFile *volatile_file, GFileOutputStream *stream, GCancellable *cancellable,
    GError **error) {
    GFile *real_file = NULL;
    GFileInfo *info = NULL;

    info = g_file_output_stream_query_info(
               stream,
               G_FILE_ATTRIBUTE_STANDARD_IS_SYMLINK
               "," G_FILE_ATTRIBUTE_STANDARD_IS_VOLATILE
               "," G_FILE_ATTRIBUTE_STANDARD_SYMLINK_TARGET,
...
static void copy_move_file(CopyMoveJob *job, GFile *src, GFile *dest_dir,
                           gboolean same_fs, gboolean unique_names,
                           char **dest_fs_type, SourceInfo *source_info,
                           TransferInfo *transfer_info,
                           GHashTable *debuting_files, gboolean overwrite,
...
static CreateDestDirResult create_dest_dir(CommonJob *job, GFile *src,
        GFile **dest, gboolean same_fs,
        char **dest_fs_type) {
    GError *error;
...
retry:
    ...
    if (!res) {
        ...
        if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
            secondary = g_strdup_printf(
                            _("The folder “%s” cannot be copied because you do not "
                              "have permissions to create it in the destination."),
                            basename);
        } else {
            secondary = g_strdup_printf(
                            _("There was an error creating the folder “%s”."), basename);
            details = error->message;
        }
    ...
    if (job->undo_info != NULL) {
        nautilus_file_undo_info_ext_add_origin_target_pair(
            NAUTILUS_FILE_UNDO_INFO_EXT(job->undo_info), src, *dest);
    }
 ...
 */
static gboolean
copy_move_directory(CopyMoveJob *copy_job, GFile *src, GFile **dest,
                    gboolean same_fs, gboolean create_dest,
                    char **parent_dest_fs_type, SourceInfo *source_info,
                    TransferInfo *transfer_info, GHashTable *debuting_files,
                    gboolean *skipped_file, gboolean readonly_source_fs) {
    ...
    *skipped_file = FALSE;
    ...
    if (create_dest) {
        ...
        attrs_to_read = g_file_build_attribute_list_for_copy(
                            *dest, flags, job->cancellable, NULL);
        if (attrs_to_read != NULL) {
            src_info = g_file_query_info(src, attrs_to_read,
                                         G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                         job->cancellable, NULL);
        }
    }
...
retry:
    error = NULL;
    enumerator = g_file_enumerate_children(src, G_FILE_ATTRIBUTE_STANDARD_NAME,
                                           G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                           job->cancellable, &error);
    if (enumerator) {
        error = NULL;

        while (!job_aborted(job) && (info = g_file_enumerator_next_file(
                                                enumerator, job->cancellable,
            ...
            copy_move_file(copy_job, src_file, *dest, same_fs, FALSE, &dest_fs_type,
                           source_info, transfer_info, NULL, FALSE,
                           &local_skipped_file, readonly_source_fs);
        ...
        if (error && IS_IO_ERROR(error, CANCELLED)) {
            ...
            if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
                secondary = g_strdup_printf(
                                _("Files in the folder “%s” cannot be copied because you do "
                                  "not have permissions to see them."),
                                basename);
            } else {
                secondary =
                    g_strdup_printf(_("There was an error getting information about "
        ...
        info = g_file_query_info(src, G_FILE_ATTRIBUTE_STANDARD_SIZE,
                                 G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                 job->cancellable, NULL);
        ...
        if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
            secondary = g_strdup_printf(
                            _("The folder “%s” cannot be copied because you do not have "
                              "permissions to read it."),
                            basename);
        } else {
            secondary = g_strdup_printf(
    ...
    if (src_info != NULL) {
        /* Ignore errors here. Failure to copy metadata is not a hard error */
        g_file_set_attributes_from_info(*dest, src_info,
                                        G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                        job->cancellable, NULL);
    }
...
static void copy_file_progress_callback(goffset current_num_bytes,
    ...
    if (current_num_bytes != 0 && current_num_bytes != total_num_bytes) {
...
static char *query_fs_type(GFile *file, GCancellable *cancellable) {
    ...
    fsinfo = g_file_query_filesystem_info(file, G_FILE_ATTRIBUTE_FILESYSTEM_TYPE,
                                          cancellable, NULL);
    if (fsinfo != NULL) {
...
static FileConflictResponse *handle_copy_move_conflict(CommonJob *job,
        GFile *src, GFile *dest,
        GFile *dest_dir) {
    ...
    suggested_file =
        nautilus_generate_unique_file_in_directory(dest_dir, basename);
    suggestion = g_file_get_basename(suggested_file);

    response = copy_move_conflict_ask_user_action(job->parent_window,
               should_start_inactive, src,
               dest, dest_dir, suggestion);
 ...
 */
static GFile *get_target_file_from_source_display_name(CopyMoveJob *copy_job,
        GFile *src, GFile *dir) {
    CommonJob *job;
    g_autoptr(GError) error = NULL;
    ...
    info = g_file_query_info(src, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, 0, NULL,
                             &error);
    if (info == NULL) {
        if (copy_job->is_move) {
            primary = g_strdup(_("Error while moving."));
        ...
        run_error(job, primary, secondary, error->message, FALSE, CANCEL, NULL);

        abort_job(job);
    } else {
        dest = get_target_file_for_display_name(dir,
...
static void copy_move_file(CopyMoveJob *copy_job, GFile *src, GFile *dest_dir,
                           gboolean same_fs, gboolean unique_names,
                           char **dest_fs_type, SourceInfo *source_info,
                           TransferInfo *transfer_info,
                           GHashTable *debuting_files, gboolean overwrite,
    ...
    *skipped_file = FALSE;
    ...
    if (unique_names) {
        ...
        dest = get_target_file_with_custom_name(src, dest_dir, *dest_fs_type,
                                                same_fs, copy_job->target_name);
    ...
    if (test_dir_is_parent(dest_dir, src)) {
        ...
        primary = copy_job->is_move
                  ? g_strdup(_("You cannot move a folder into itself."))
                  : g_strdup(_("You cannot copy a folder into itself."));
        secondary =
            g_strdup(_("The destination folder is inside the source folder."));

        response = run_cancel_or_skip_warning(
                       job, primary, secondary, NULL, source_info->num_files,
                       source_info->num_files - transfer_info->num_files);
    ...
    if (test_dir_is_parent(src, dest)) {
        ...
        /*  the run_warning() frees all strings passed in automatically  */
        primary = copy_job->is_move
                  ? g_strdup(_("You cannot move a file over itself."))
                  : g_strdup(_("You cannot copy a file over itself."));
        secondary =
            g_strdup(_("The source file would be overwritten by the destination."));
...
retry:
    ...
    if (copy_job->is_move) {
        res = g_file_move(src, dest, flags, job->cancellable,
                          copy_file_progress_callback, &pdata, &error);
    } else {
        res = g_file_copy(src, dest, flags, job->cancellable,
                          copy_file_progress_callback, &pdata, &error);
    }
    ...
    /* Needs to recurse */
        ...
        if (overwrite && would_recurse) {
            ...
            if (!g_file_delete(dest, job->cancellable, &error) &&
                ...
                secondary =
                    g_strdup_printf(_("Could not remove the already existing file "
        ...
        if (!copy_move_directory(copy_job, src, &dest, same_fs, would_recurse,
    ...
    /* Other error */
        ...
        filename = get_truncated_parse_name(dest_dir);
        secondary = g_strdup_printf(
                        _("There was an error copying the file into %s."), filename);
        details = error->message;
...
static void copy_files(CopyMoveJob *job, const char *dest_fs_id,
                       SourceInfo *source_info, TransferInfo *transfer_info) {
    CommonJob *common;
    GList *l;
    GFile *src;
    ...
    /* Query the source dir, not the file because if it's a symlink we'll follow
     * it */
    source_dir = g_file_get_parent((GFile *)job->files->data);
    if (source_dir) {
        inf = g_file_query_filesystem_info(source_dir, "filesystem::readonly", NULL,
                                           NULL);
        if (inf != NULL) {
            readonly_source_fs =
                g_file_info_get_attribute_boolean(inf, "filesystem::readonly");
            g_object_unref(inf);
        }
    ...
    for (l = job->files; l != NULL && !job_aborted(common); l = l->next) {
        ...
        if (dest) {
            skipped_file = FALSE;
            copy_move_file(job, src, dest, same_fs, unique_names, &dest_fs_type,
                           source_info, transfer_info, job->debuting_files, FALSE,
                           &skipped_file, readonly_source_fs);
            g_object_unref(dest);
...
static void copy_task_done(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
    CopyMoveJob *job;

    job = user_data;
    if (job->done_callback) {
        job->done_callback(job->debuting_files, !job_aborted((CommonJob *)job),
                           job->done_callback_data);
    }
...
static CopyMoveJob *copy_job_setup(GList *files, GFile *target_dir,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data,
                                   NautilusCopyCallback done_callback,
                                   gpointer done_callback_data) {
     ...
     * button. */
    nautilus_progress_info_set_destination(((CommonJob *)job)->progress,
                                           target_dir);
...
static void nautilus_file_operations_copy(GTask *task, gpointer source_object,
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        ...
        /* In the case of duplicate, the undo_info is already set, so we don't want
         * to overwrite it wrongfully.
         */
        if (job->common.undo_info == NULL) {
            job->common.undo_info = nautilus_file_undo_info_ext_new(
                                        NAUTILUS_FILE_UNDO_OP_COPY, g_list_length(job->files), src_dir,
                                        job->destination);
        }
    }
...
void nautilus_file_operations_copy_sync(GList *files, GFile *target_dir) {
    GTask *task;
    CopyMoveJob *job;

    job = copy_job_setup(files, target_dir, NULL, NULL, NULL, NULL);
    ...
    g_object_unref(task);
    /* Since g_task_run_in_thread_sync doesn't work with callbacks (in this case
     * not reaching copy_task_done) we need to set up the undo information
     * ourselves.
     */
    copy_task_done(NULL, NULL, job);
}

void nautilus_file_operations_copy_async(
    GList *files, GFile *target_dir, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusCopyCallback done_callback, gpointer done_callback_data) {
    GTask *task;
    CopyMoveJob *job;

    job = copy_job_setup(files, target_dir, parent_window, dbus_data,
                         done_callback, done_callback_data);
...
static void report_preparing_move_progress(CopyMoveJob *move_job, int total,
        int left) {
    CommonJob *job;
    g_autofree gchar *basename = NULL;
    ...
    nautilus_progress_info_take_details(
        job->progress,
        g_strdup_printf(ngettext("Preparing to move %'d file",
                                 "Preparing to move %'d files", left),
                        left));
...
static MoveFileCopyFallback *move_copy_file_callback_new(GFile *file,
        gboolean overwrite) {
...
static void move_file_prepare(CopyMoveJob *move_job, GFile *src,
                              GFile *dest_dir, gboolean same_fs,
                              char **dest_fs_type, GHashTable *debuting_files,
                              GList **fallback_files, int files_left) {
    GFile *dest, *new_dest;
    g_autofree gchar *dest_uri = NULL;
    GError *error;
    CommonJob *job;
    ...
    if (test_dir_is_parent(dest_dir, src)) {
        ...
        /*  the run_warning() frees all strings passed in automatically  */
        primary = move_job->is_move
                  ? g_strdup(_("You cannot move a folder into itself."))
                  : g_strdup(_("You cannot copy a folder into itself."));
        secondary =
            g_strdup(_("The destination folder is inside the source folder."));

        response = run_warning(job, primary, secondary, NULL, files_left > 1,
                               CANCEL, SKIP_ALL, SKIP, NULL);
...
retry:
    ...
    /* Other error */
        ...
        filename = get_truncated_parse_name(dest_dir);
        secondary = g_strdup_printf(
                        _("There was an error moving the file into %s."), filename);

        details = error->message;

        response = run_warning(job, primary, secondary, details, files_left > 1,
                               CANCEL, SKIP_ALL, SKIP, NULL);
...
static void move_files_prepare(CopyMoveJob *job, const char *dest_fs_id,
                               char **dest_fs_type, GList **fallbacks) {
    CommonJob *common;
    GList *l;
    GFile *src;
    gboolean same_fs;
    ...
    for (l = job->files; l != NULL && !job_aborted(common); l = l->next) {
        ...
        move_file_prepare(job, src, job->destination, same_fs, dest_fs_type,
...
static void move_files(CopyMoveJob *job, GList *fallbacks,
                       const char *dest_fs_id, char **dest_fs_type,
                       SourceInfo *source_info, TransferInfo *transfer_info) {
    CommonJob *common;
    GList *l;
    GFile *src;
    ...
    for (l = fallbacks; l != NULL && !job_aborted(common); l = l->next) {
        ...
        skipped_file = FALSE;
        copy_move_file(job, src, job->destination, same_fs, FALSE, dest_fs_type,
...
static void move_task_done(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
    CopyMoveJob *job;
...
static CopyMoveJob *move_job_setup(GList *files, GFile *target_dir,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data,
                                   NautilusCopyCallback done_callback,
                                   gpointer done_callback_data) {
...
void nautilus_file_operations_move_sync(GList *files, GFile *target_dir) {
    GTask *task;
    CopyMoveJob *job;
    ...
    g_object_unref(task);
    /* Since g_task_run_in_thread_sync doesn't work with callbacks (in this case
     * not reaching move_task_done) we need to set up the undo information
     * ourselves.
     */
    move_task_done(NULL, NULL, job);
}

void nautilus_file_operations_move_async(
    GList *files, GFile *target_dir, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusCopyCallback done_callback, gpointer done_callback_data) {
    GTask *task;
    ...
    job = move_job_setup(files, target_dir, parent_window, dbus_data,
                         done_callback, done_callback_data);
...
static void nautilus_file_operations_move(GTask *task, gpointer source_object,
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        ...
        if (g_file_has_uri_scheme(g_list_first(job->files)->data, "trash")) {
            ...
            job->common.undo_info = nautilus_file_undo_info_ext_new(
                                        NAUTILUS_FILE_UNDO_OP_MOVE, g_list_length(job->files), src_dir,
                                        job->destination);
        }
    }
    ...
    verify_destination(&job->common, job->destination, NULL,
    ...
    memset(&transfer_info, 0, sizeof(transfer_info));
    move_files(job, fallbacks, dest_fs_id, &dest_fs_type, &source_info,
               &transfer_info);
...
static void report_preparing_link_progress(CopyMoveJob *link_job, int total,
        int left) {
    CommonJob *job;
    g_autofree gchar *basename = NULL;
    ...
    nautilus_progress_info_take_status(
        job->progress, g_strdup_printf(_("Creating links in “%s”"), basename));

    nautilus_progress_info_take_details(
        job->progress,
        g_strdup_printf(ngettext("Making link to %'d file",
                                 "Making links to %'d files", left),
                        left));
...
static void link_file(CopyMoveJob *job, GFile *src, GFile *dest_dir,
                      char **dest_fs_type, GHashTable *debuting_files,
                      int files_left) {
    GFile *src_dir;
    GFile *new_dest;
...
retry:
    ...
    if (path == NULL) {
        not_local = TRUE;
    } else if (g_file_make_symbolic_link(dest, path, common->cancellable,
                                         &error)) {
        if (common->undo_info != NULL) {
            nautilus_file_undo_info_ext_add_origin_target_pair(
                NAUTILUS_FILE_UNDO_INFO_EXT(common->undo_info), src, dest);
        }
    ...
    /* Other error */
        ...
        basename = get_basename(src);
        primary = g_strdup_printf(_("Error while creating link to %s."), basename);
        if (not_local) {
            ...
            filename = get_truncated_parse_name(dest_dir);
            secondary = g_strdup_printf(
                            _("There was an error creating the symlink in %s."), filename);
            details = error->message;
        }

        response = run_warning(common, primary, secondary, details, files_left > 1,
                               CANCEL, SKIP_ALL, SKIP, NULL);
...
static void link_task_done(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
    CopyMoveJob *job;
...
static void link_task_thread_func(GTask *task, gpointer source_object,
                                  gpointer task_data,
                                  GCancellable *cancellable) {
    CopyMoveJob *job;
    CommonJob *common;
    ...
    for (l = job->files; l != NULL && !job_aborted(common); l = l->next) {
        ...
        link_file(job, src, job->destination, &dest_fs_type, job->debuting_files,
                  left);
        report_preparing_link_progress(job, total, --left);
        i++;
    }
...
void nautilus_file_operations_link(GList *files, GFile *target_dir,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data,
                                   NautilusCopyCallback done_callback,
                                   gpointer done_callback_data) {
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        ...
        job->common.undo_info = nautilus_file_undo_info_ext_new(
                                    NAUTILUS_FILE_UNDO_OP_CREATE_LINK, g_list_length(files), src_dir,
                                    target_dir);
    }
...
void nautilus_file_operations_duplicate(
    GList *files, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusCopyCallback done_callback, gpointer done_callback_data) {
    g_autoptr(GTask) task = NULL;
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        ...
        job->common.undo_info =
            nautilus_file_undo_info_ext_new(NAUTILUS_FILE_UNDO_OP_DUPLICATE,
                                            g_list_length(files), src_dir, src_dir);
    }
...
static void set_permissions_task_done(GObject *source_object, GAsyncResult *res,
                                      gpointer user_data) {
...
static void set_permissions_file(SetPermissionsJob *job, GFile *file,
                                 GFileInfo *info);

static void set_permissions_contained_files(SetPermissionsJob *job,
        GFile *file) {
    CommonJob *common;
    GFileEnumerator *enumerator;
    ...
    enumerator = g_file_enumerate_children(
                     ...
                     G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, common->cancellable, NULL);
    if (enumerator) {
        GFileInfo *child_info;

        while (!job_aborted(common) &&
                (child_info = g_file_enumerator_next_file(
                                  enumerator, common->cancellable, NULL)) != NULL) {
            GFile *child;
    ...
    if (!job_aborted(common) &&
        ...
        if (common->undo_info != NULL) {
            nautilus_file_undo_info_rec_permissions_add_file(
                NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS(common->undo_info), file,
                current);
        }

        current = (current & ~mask) | value;

        g_file_set_attribute_uint32(file, G_FILE_ATTRIBUTE_UNIX_MODE, current,
                                    G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
...
static void set_permissions_thread_func(GTask *task, gpointer source_object,
                                        gpointer task_data,
                                        GCancellable *cancellable) {
...
void nautilus_file_set_permissions_recursive(
    const char *directory, guint32 file_permissions, guint32 file_mask,
    guint32 dir_permissions, guint32 dir_mask, NautilusOpCallback callback,
    gpointer callback_data) {
    g_autoptr(GTask) task = NULL;
    SetPermissionsJob *job;
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        job->common.undo_info = nautilus_file_undo_info_rec_permissions_new(
                                    job->file, file_permissions, file_mask, dir_permissions, dir_mask);
    }
...
static void callback_for_move_to_trash(GHashTable *debuting_uris,
...
void nautilus_file_operations_copy_move(
    const GList *item_uris, const char *target_dir, GdkDragAction copy_action,
    GtkWidget *parent_view, NautilusFileOperationsDBusData *dbus_data,
    NautilusCopyCallback done_callback, gpointer done_callback_data) {
    GList *locations;
    GList *p;
    GFile *dest, *src_dir;
    ...
    if (target_is_mapping && have_nonmapping_source &&
         ...
         * This is a workaround, because otherwise the source files would be
         * removed.
         */
    ...
    if (copy_action == GDK_ACTION_COPY) {
        ...
        if (target_dir == NULL ||
            ...
            nautilus_file_operations_duplicate(locations, parent_window, dbus_data,
                                               done_callback, done_callback_data);
        } else {
            nautilus_file_operations_copy_async(locations, dest, parent_window,
                                                dbus_data, done_callback,
                                                done_callback_data);
        }
        if (src_dir) {
            g_object_unref(src_dir);
        ...
        if (g_file_has_uri_scheme(dest, "trash")) {
            ...
            nautilus_file_operations_trash_or_delete_async(
                locations, parent_window, dbus_data,
                (NautilusDeleteCallback)callback_for_move_to_trash, cb_data);
        } else {
            nautilus_file_operations_move_async(locations, dest, parent_window,
                                                dbus_data, done_callback,
                                                done_callback_data);
        }
    } else {
        nautilus_file_operations_link(locations, dest, parent_window, dbus_data,
                                      done_callback, done_callback_data);
    }
...
static void create_task_done(GObject *source_object, GAsyncResult *res,
                             gpointer user_data) {
    CreateJob *job;

    job = user_data;
    if (job->done_callback) {
        job->done_callback(job->created_file, !job_aborted((CommonJob *)job),
                           job->done_callback_data);
    }
...
static void create_task_thread_func(GTask *task, gpointer source_object,
                                    gpointer task_data,
                                    GCancellable *cancellable) {
    CreateJob *job;
    CommonJob *common;
...
retry:
    ...
    if (job->make_dir) {
        ...
        if (res) {
            ...
            real =
                map_possibly_volatile_file_to_real(dest, common->cancellable, &error);
            if (real == NULL) {
                res = FALSE;
            } else {
                g_object_unref(dest);
        ...
        if (res && common->undo_info != NULL) {
            nautilus_file_undo_info_create_set_data(
                NAUTILUS_FILE_UNDO_INFO_CREATE(common->undo_info), dest, NULL, 0);
        }
    } else {
        if (job->src) {
            res = g_file_copy(job->src, dest, G_FILE_COPY_TARGET_DEFAULT_PERMS,
                              common->cancellable, NULL, NULL, &error);

            if (res) {
                GFile *real;

                real = map_possibly_volatile_file_to_real(dest, common->cancellable,
                        &error);
                if (real == NULL) {
                    res = FALSE;
                } else {
            ...
            if (res && common->undo_info != NULL) {
                ...
                nautilus_file_undo_info_create_set_data(
                    NAUTILUS_FILE_UNDO_INFO_CREATE(common->undo_info), dest, uri, 0);
            }
        } else {
            data = "";
            length = 0;
            ...
            out =
                g_file_create(dest, G_FILE_CREATE_NONE, common->cancellable, &error);
            if (out) {
                GFile *real;

                real = map_possibly_volatile_file_to_real_on_write(
                           dest, out, common->cancellable, &error);
                if (real == NULL) {
                    res = FALSE;
                    g_object_unref(out);
                } else {
                    ...
                    res = g_output_stream_write_all(G_OUTPUT_STREAM(out), data, length,
                                                    NULL, common->cancellable, &error);
                    if (res) {
                        ...
                        if (res && common->undo_info != NULL) {
                            nautilus_file_undo_info_create_set_data(
                                NAUTILUS_FILE_UNDO_INFO_CREATE(common->undo_info), dest, data,
                                length);
                        }
                    }
    ...
    if (res) {
        ...
        if (IS_IO_ERROR(error, INVALID_FILENAME) && !handled_invalid_filename) {
            ...
            if (count == 1) {
                ...
                if (max_length > 0 && strlen(filename2) > max_length) {
                    new_filename =
                        shorten_utf8_string(filename2, strlen(filename2) - max_length);
                }
            ...
            if (make_file_name_valid_for_dest_fs(new_filename, dest_fs_type)) {
                ...
                if (filename_is_utf8) {
                    dest = g_file_get_child_for_display_name(job->dest_dir, new_filename,
                            NULL);
                }
                if (dest == NULL) {
        ...
        if (IS_IO_ERROR(error, EXISTS)) {
            ...
            if (filename_is_utf8) {
                dest =
                    g_file_get_child_for_display_name(job->dest_dir, filename2, NULL);
            }
            if (dest == NULL) {
        ...
        /* Other error */
            ...
            if (job->make_dir) {
                primary =
                    g_strdup_printf(_("Error while creating directory %s."), basename);
            } else {
                primary = g_strdup_printf(_("Error while creating file %s."), basename);
            }
            parse_name = get_truncated_parse_name(job->dest_dir);
            secondary = g_strdup_printf(
                            _("There was an error creating the directory in %s."), parse_name);

            details = error->message;

            response = run_warning(common, primary, secondary, details, FALSE, CANCEL,
                                   SKIP, NULL);
...
void nautilus_file_operations_new_folder(
    GtkWidget *parent_view, NautilusFileOperationsDBusData *dbus_data,
    const char *parent_dir, const char *folder_name,
    NautilusCreateCallback done_callback, gpointer done_callback_data) {
    g_autoptr(GTask) task = NULL;
    CreateJob *job;
    GtkWindow *parent_window;
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        job->common.undo_info =
            nautilus_file_undo_info_create_new(NAUTILUS_FILE_UNDO_OP_CREATE_FOLDER);
    }
...
void nautilus_file_operations_new_file_from_template(
    GtkWidget *parent_view, const char *parent_dir, const char *target_filename,
    const char *template_uri, NautilusCreateCallback done_callback,
    gpointer done_callback_data) {
    g_autoptr(GTask) task = NULL;
    CreateJob *job;
    GtkWindow *parent_window;
...
static void delete_trash_file(CommonJob *job, GFile *file, gboolean del_file,
...
static void empty_trash_thread_func(GTask *task, gpointer source_object,
    ...
    if (confirmed) {
        for (l = job->trash_dirs; l != NULL && !job_aborted(common); l = l->next) {
            delete_trash_file(common, l->data, FALSE, TRUE);
        }
    }
}

void nautilus_file_operations_empty_trash(
    GtkWidget *parent_view, gboolean ask_confirmation,
    NautilusFileOperationsDBusData *dbus_data) {
    g_autoptr(GTask) task = NULL;
    EmptyTrashJob *job;
    GtkWindow *parent_window;
...
static void extract_task_done(GObject *source_object, GAsyncResult *res,
                              gpointer user_data) {
...
static GFile *extract_job_on_decide_destination(AutoarExtractor *extractor,
        GFile *destination,
        GList *files,
        gpointer user_data) {
    ...
    decided_destination = nautilus_generate_unique_file_in_directory(
                              extract_job->destination_directory, basename);
    ...
    extract_job->output_files =
        g_list_prepend(extract_job->output_files, decided_destination);
    extract_job->destination_decided = TRUE;

    return g_object_ref(decided_destination);
}

static void extract_job_on_progress(AutoarExtractor *extractor,
    ...
    job_progress =
        archive_decompress_progress * archive_weight + extract_job->base_progress;
    ...
    formatted_size_total_compressed_size =
        g_format_size(extract_job->total_compressed_size);
    if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE ||
            transfer_rate == 0) {
        ...
        details = g_strdup_printf(
                      ngettext("%s / %s \xE2\x80\x94 %s left (%s/sec)",
                               "%s / %s \xE2\x80\x94 %s left (%s/sec)",
                               seconds_count_format_time_units(remaining_time)),
                      formatted_size_job_completed_size, formatted_size_total_compressed_size,
                      formatted_time, formatted_size_transfer_rate);
    }
...
static void extract_job_on_error(AutoarExtractor *extractor, GError *error,
                                 gpointer user_data) {
    ExtractJob *extract_job = user_data;
    GFile *source_file;
    GFile *destination;
    ...
    basename = get_basename(source_file);
    nautilus_progress_info_take_status(
        extract_job->common.progress,
        g_strdup_printf(_("Error extracting “%s”"), basename));

    remaining_files =
        g_list_length(g_list_find_custom(extract_job->source_files, source_file,
                                         (GCompareFunc)g_file_equal)) -
        1;
    response_id = run_cancel_or_skip_warning(
                      (CommonJob *)extract_job,
                      g_strdup_printf(_("There was an error while extracting “%s”."), basename),
                      g_strdup(error->message), NULL, extract_job->total_files,
                      remaining_files);
...
static void extract_job_on_completed(AutoarExtractor *extractor,
                                     gpointer user_data) {
    ExtractJob *extract_job = user_data;
    GFile *output_file;
...
static gchar *extract_job_on_request_passphrase(AutoarExtractor *extractor,
        gpointer user_data) {
    ExtractJob *extract_job = user_data;
    GtkWindow *parent_window;
    GFile *source_file;
...
static void extract_job_on_scanned(AutoarExtractor *extractor,
                                   guint total_files, gpointer user_data) {
    guint64 total_size;
    ExtractJob *extract_job;
    GFile *source_file;
    g_autofree gchar *basename = NULL;
    ...
    fsinfo = g_file_query_filesystem_info(
                 source_file,
                 G_FILE_ATTRIBUTE_FILESYSTEM_FREE "," G_FILE_ATTRIBUTE_FILESYSTEM_READONLY,
                 extract_job->common.cancellable, NULL);
    free_size = g_file_info_get_attribute_uint64(
                    fsinfo, G_FILE_ATTRIBUTE_FILESYSTEM_FREE);
    ...
    if (total_size != G_MAXUINT64 && total_size > free_size) {
        ...
        run_error(
            ...
            NULL, NULL, FALSE, CANCEL, NULL);

        abort_job((CommonJob *)extract_job);
    }
}

static void report_extract_final_progress(ExtractJob *extract_job) {
    char *status;
    g_autofree gchar *basename_dest = NULL;
    g_autofree gchar *formatted_size = NULL;
    ...
    nautilus_progress_info_take_details(
        extract_job->common.progress,
        g_strdup_printf(_("%s / %s"), formatted_size, formatted_size));

    nautilus_progress_info_set_progress(extract_job->common.progress, 1, 1);
}

static void extract_task_thread_func(GTask *task, gpointer source_object,
                                     gpointer task_data,
                                     GCancellable *cancellable) {
    ...
    archive_compressed_sizes =
        g_malloc0_n(extract_job->total_files, sizeof(guint64));
    extract_job->total_compressed_size = 0;

    for (l = extract_job->source_files, i = 0;
            l != NULL && !job_aborted((CommonJob *)extract_job); l = l->next, i++) {
        GFile *source_file;
        g_autoptr(GFileInfo) info = NULL;

        source_file = G_FILE(l->data);
        info = g_file_query_info(source_file, G_FILE_ATTRIBUTE_STANDARD_SIZE,
                                 G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                 extract_job->common.cancellable, NULL);
    ...
    for (l = extract_job->source_files, i = 0;
        ...
        g_signal_connect(extractor, "scanned", G_CALLBACK(extract_job_on_scanned),
                         extract_job);
        g_signal_connect(extractor, "error", G_CALLBACK(extract_job_on_error),
                         extract_job);
        g_signal_connect(extractor, "decide-destination",
                         G_CALLBACK(extract_job_on_decide_destination),
                         extract_job);
        g_signal_connect(extractor, "progress", G_CALLBACK(extract_job_on_progress),
                         extract_job);
        g_signal_connect(extractor, "completed",
                         G_CALLBACK(extract_job_on_completed), extract_job);
        ...
        if (!extract_job->extraction_failed) {
            extract_job->base_progress +=
                (gdouble)extract_job->archive_compressed_size /
                (gdouble)extract_job->total_compressed_size;
        } else {
            extract_job->total_files--;
    ...
    if (extract_job->common.undo_info) {
        if (extract_job->output_files) {
            ...
            undo_info =
                NAUTILUS_FILE_UNDO_INFO_EXTRACT(extract_job->common.undo_info);
...
void nautilus_file_operations_extract_files(
    GList *files, GFile *destination_directory, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusExtractCallback done_callback, gpointer done_callback_data) {
    ExtractJob *extract_job;
    g_autoptr(GTask) task = NULL;

    extract_job = op_job_new(ExtractJob, parent_window, dbus_data);
    extract_job->source_files =
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        extract_job->common.undo_info =
            nautilus_file_undo_info_extract_new(files, destination_directory);
    }

    task = g_task_new(NULL, extract_job->common.cancellable, extract_task_done,
                      extract_job);
    g_task_set_task_data(task, extract_job, NULL);
    g_task_run_in_thread(task, extract_task_thread_func);
}

static void compress_task_done(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
...
static void compress_job_on_progress(AutoarCompressor *compressor,
    ...
    if (compress_job->total_files == 1) {
        ...
        status = g_strdup_printf(_("Compressing “%s” into “%s”"), basename_data,
        ...
        status = g_strdup_printf(ngettext("Compressing %'d file into “%s”",
                                 ...
                                 compress_job->total_files, basename_output_file);
    }
    ...
    if (elapsed > 0) {
        if (completed_size > 0) {
            ...
            remaining_time =
                (compress_job->total_size - completed_size) / transfer_rate;
        } else if (completed_files > 0) {
            transfer_rate = completed_files / elapsed;
            remaining_time =
                (compress_job->total_files - completed_files) / transfer_rate;
        }
    }
    ...
    if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE ||
        ...
        if (compress_job->total_files == 1) {
            ...
            details = g_strdup_printf(_("%'d / %'d"),
                                      files_left > 0 ? completed_files + 1
                                      : completed_files,
                                      compress_job->total_files);
        }
    } else {
            ...
            if (files_left > 0) {
                ...
                details = g_strdup_printf(
                              ngettext("%s / %s \xE2\x80\x94 %s left (%s/sec)",
                                       "%s / %s \xE2\x80\x94 %s left (%s/sec)",
                                       seconds_count_format_time_units(remaining_time)),
                              formatted_size_completed_size, formatted_size_total_size,
                              formatted_time, formatted_size_transfer_rate);
            } else {
                /* To translators: %s will expand to a size like "2 bytes" or "3 MB". */
                details = g_strdup_printf(_("%s / %s"), formatted_size_completed_size,
            ...
            if (files_left > 0) {
                ...
                details = g_strdup_printf(
                              ngettext("%'d / %'d \xE2\x80\x94 %s left (%s/sec)",
                                       "%'d / %'d \xE2\x80\x94 %s left (%s/sec)",
                                       seconds_count_format_time_units(remaining_time)),
                              completed_files + 1, compress_job->total_files, formatted_time,
                              formatted_size);
            } else {
                /* To translators: %'d is the number of files completed for the
                 * operation, so it will be something like 2/14. */
                details = g_strdup_printf(_("%'d / %'d"), completed_files,
                                          compress_job->total_files);
            }
        }
    }
    ...
    nautilus_progress_info_set_progress(common->progress, completed_size,
                                        compress_job->total_size);
}

static void compress_job_on_error(AutoarCompressor *compressor, GError *error,
                                  gpointer user_data) {
    CompressJob *compress_job = user_data;
    char *status;
    g_autofree gchar *basename_output_file = NULL;
    ...
    if (compress_job->total_files == 1) {
        ...
        status = g_strdup_printf(_("Error compressing “%s” into “%s”"),
                                 basename_data, basename_output_file);
    } else {
    ...
    run_error((CommonJob *)compress_job,
              ...
              g_strdup(error->message), NULL, FALSE, CANCEL, NULL);

    abort_job((CommonJob *)compress_job);
}

static void compress_job_on_completed(AutoarCompressor *compressor,
                                      gpointer user_data) {
    CompressJob *compress_job = user_data;
    ...
    if (compress_job->total_files == 1) {
        ...
        basename_data = get_basename(G_FILE(compress_job->source_files->data));
        status = g_strdup_printf(_("Compressed “%s” into “%s”"), basename_data,
                                 basename_output_file);
    } else {
...
static void compress_task_thread_func(GTask *task, gpointer source_object,
                                      gpointer task_data,
                                      GCancellable *cancellable) {
    ...
    scan_sources(compress_job->source_files, &source_info,
                 (CommonJob *)compress_job, OP_KIND_COMPRESS);
    ...
    compressor = autoar_compressor_new(
                     ...
                     compress_job->format, compress_job->filter, FALSE);
    ...
    g_signal_connect(compressor, "progress", G_CALLBACK(compress_job_on_progress),
                     compress_job);
    g_signal_connect(compressor, "error", G_CALLBACK(compress_job_on_error),
                     compress_job);
...
void nautilus_file_operations_compress(
    GList *files, GFile *output, AutoarFormat format, AutoarFilter filter,
    const gchar *passphrase, GtkWindow *parent_window,
    NautilusFileOperationsDBusData *dbus_data,
    NautilusCreateCallback done_callback, gpointer done_callback_data) {
    g_autoptr(GTask) task = NULL;
    ...
    if (!nautilus_file_undo_manager_is_operating()) {
        compress_job->common.undo_info = nautilus_file_undo_info_compress_new(
                                             files, output, format, filter, passphrase);
    }

    task = g_task_new(NULL, compress_job->common.cancellable, compress_task_done,
                      compress_job);
...
void nautilus_self_check_file_operations(void) {
    ...
    EEL_CHECK_STRING_RESULT(get_duplicate_name(" (copy)", 1, -1, FALSE),
                            " (another copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo", 1, -1, FALSE),
                            "foo (copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name(".bashrc", 1, -1, FALSE),
                            ".bashrc (copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name(".foo.txt", 1, -1, FALSE),
                            ".foo (copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo foo", 1, -1, FALSE),
                            "foo foo (copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo.txt", 1, -1, FALSE),
                            "foo (copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo foo.txt", 1, -1, FALSE),
                            "foo foo (copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo foo.txt txt", 1, -1, FALSE),
                            "foo foo (copy).txt txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo...txt", 1, -1, FALSE),
                            "foo.. (copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo...", 1, -1, FALSE),
                            "foo... (copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo. (copy)", 1, -1, FALSE),
                            "foo. (another copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (copy)", 1, -1, FALSE),
                            "foo (another copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (copy).txt", 1, -1, FALSE),
                            "foo (another copy).txt");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (another copy)", 1, -1, FALSE), "foo (3rd copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (another copy).txt", 1, -1, FALSE),
        "foo (3rd copy).txt");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo foo (another copy).txt", 1, -1, FALSE),
        "foo foo (3rd copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (13th copy)", 1, -1, FALSE),
                            "foo (14th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (13th copy).txt", 1, -1, FALSE),
        "foo (14th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (21st copy)", 1, -1, FALSE),
                            "foo (22nd copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (21st copy).txt", 1, -1, FALSE),
        "foo (22nd copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (22nd copy)", 1, -1, FALSE),
                            "foo (23rd copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (22nd copy).txt", 1, -1, FALSE),
        "foo (23rd copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (23rd copy)", 1, -1, FALSE),
                            "foo (24th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (23rd copy).txt", 1, -1, FALSE),
        "foo (24th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (24th copy)", 1, -1, FALSE),
                            "foo (25th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (24th copy).txt", 1, -1, FALSE),
        "foo (25th copy).txt");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo foo (24th copy)", 1, -1, FALSE),
        "foo foo (25th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo foo (24th copy).txt", 1, -1, FALSE),
        "foo foo (25th copy).txt");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo foo (100000000000000th copy).txt", 1, -1, FALSE),
        "foo foo (copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (10th copy)", 1, -1, FALSE),
                            "foo (11th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (10th copy).txt", 1, -1, FALSE),
        "foo (11th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (11th copy)", 1, -1, FALSE),
                            "foo (12th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (11th copy).txt", 1, -1, FALSE),
        "foo (12th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (12th copy)", 1, -1, FALSE),
                            "foo (13th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (12th copy).txt", 1, -1, FALSE),
        "foo (13th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (110th copy)", 1, -1, FALSE),
                            "foo (111th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (110th copy).txt", 1, -1, FALSE),
        "foo (111th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (122nd copy)", 1, -1, FALSE),
                            "foo (123rd copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (122nd copy).txt", 1, -1, FALSE),
        "foo (123rd copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (123rd copy)", 1, -1, FALSE),
                            "foo (124th copy)");
    EEL_CHECK_STRING_RESULT(
        get_duplicate_name("foo (123rd copy).txt", 1, -1, FALSE),
        "foo (124th copy).txt");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("dir.with.dots", 1, -1, TRUE),
                            "dir.with.dots (copy)");
    EEL_CHECK_STRING_RESULT(get_duplicate_name("dir (copy).dir", 1, -1, TRUE),
                            "dir (another copy).dir");
