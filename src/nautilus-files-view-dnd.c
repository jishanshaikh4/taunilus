...
void nautilus_files_view_handle_netscape_url_drop(NautilusFilesView *view,
  ...
  if (!g_file_is_native(f)) {
    show_dialog(_("Drag and drop is not supported."),
                ...
                GET_ANCESTOR(view), GTK_MESSAGE_WARNING);
    g_object_unref(f);
    return;
  }
  ...
  if ((action != GDK_ACTION_COPY) && (action != GDK_ACTION_MOVE)) {
    show_dialog(_("Drag and drop is not supported."),
                _("An invalid drag type was used."), GET_ANCESTOR(view),
                GTK_MESSAGE_WARNING);
    return;
  }
  ...
  nautilus_files_view_move_copy_items(view, uri_list, target_uri, action);
...
void nautilus_files_view_handle_uri_list_drop(NautilusFilesView *view,
  ...
  if (action == GDK_ACTION_ASK) {
    action = nautilus_drag_drop_action_ask(
        GTK_WIDGET(view), GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);
    if (action == 0) {
  ...
  nautilus_files_view_move_copy_items(view, real_uri_list, real_target_uri,
                                      action);
...
static char *get_drop_filename(const char *text) {
  char *filename;
  ...
  pango_get_log_attrs(trimmed, -1, -1, pango_language_get_default(), attrs,
                      num_attrs);

  /* since the end of the text will always match a word boundary don't include
   * it */
  ...
  if (i > MIN_LEN_FILENAME) {
    ...
    /* Translator: This is the filename used for when you dnd text to a
     * directory */
    filename = g_strdup(_("Dropped Text.txt"));
  }

  /* Remove any invalid characters */
  for (current_char = filename; *current_char;
...
void nautilus_files_view_handle_text_drop(NautilusFilesView *view,
  ...
  nautilus_files_view_new_file_with_initial_contents(
      view, target_uri != NULL ? target_uri : container_uri, filename, text,
      length);
  g_free(filename);
  g_free(container_uri);
}
...
void nautilus_files_view_handle_raw_drop(NautilusFilesView *view,
  ...
  nautilus_files_view_new_file_with_initial_contents(
      view, target_uri != NULL ? target_uri : container_uri, filename, raw_data,
      length);
...
void nautilus_files_view_drop_proxy_received_uris(NautilusFilesView *view,
  ...
  nautilus_files_view_move_copy_items(
      view, source_uri_list, target_uri != NULL ? target_uri : container_uri,
      action);
...
void nautilus_files_view_handle_hover(NautilusFilesView *view,
  ...
  target_is_dir =
      nautilus_file_get_file_type(target_file) == G_FILE_TYPE_DIRECTORY;
  current_location = nautilus_window_slot_get_location(slot);
  open_folder_on_hover = g_settings_get_boolean(
  ...
  if (target_is_dir && open_folder_on_hover &&
    ...
    nautilus_application_open_location_full(
        ...
        NAUTILUS_OPEN_FLAG_DONT_MAKE_ACTIVE, NULL, NULL, slot);
  }
