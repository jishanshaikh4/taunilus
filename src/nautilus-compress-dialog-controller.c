...
G_DEFINE_TYPE(NautilusCompressDialogController,
              nautilus_compress_dialog_controller,
              NAUTILUS_TYPE_FILE_NAME_WIDGET_CONTROLLER);

static gboolean nautilus_compress_dialog_controller_name_is_valid(
    NautilusFileNameWidgetController *self, gchar *name,
    gchar **error_message) {
  gboolean is_valid;
  ...
  if (strlen(name) == 0) {
    ...
    *error_message = _("An archive cannot be called “.”.");
    ...
    is_valid = FALSE;
    *error_message = _("An archive cannot be called “..”.");
  } else if (nautilus_file_name_widget_controller_is_name_too_long(self,
                                                                   name)) {
    is_valid = FALSE;
...
static gchar *nautilus_compress_dialog_controller_get_new_name(
  ...
  basename = NAUTILUS_FILE_NAME_WIDGET_CONTROLLER_CLASS(
                 ...
                 ->get_new_name(controller);
  /* Do not check or add the extension if the name is invalid */
  valid_name = nautilus_compress_dialog_controller_name_is_valid(
      controller, basename, &error_message);
...
static void compress_dialog_controller_on_response(GtkDialog *dialog,
                                                   gint response_id,
                                                   gpointer user_data) {
...
static void update_selected_format(NautilusCompressDialogController *self,
  ...
  if (!show_passphrase) {
    ...
    gtk_entry_set_icon_from_icon_name(GTK_ENTRY(self->passphrase_entry),
                                      GTK_ENTRY_ICON_SECONDARY, "view-conceal");
  }

  gtk_stack_set_visible_child(GTK_STACK(self->extension_stack), active_label);
  ...
  g_settings_set_enum(nautilus_compression_preferences,
                      NAUTILUS_PREFERENCES_DEFAULT_COMPRESSION_FORMAT, format);
...
static void zip_row_on_activated(HdyActionRow *row, gpointer user_data) {
  NautilusCompressDialogController *controller;

  controller = NAUTILUS_COMPRESS_DIALOG_CONTROLLER(user_data);

  gtk_popover_popdown(GTK_POPOVER(controller->extension_popover));
  update_selected_format(controller, NAUTILUS_COMPRESSION_ZIP);
}

static void encrypted_zip_row_on_activated(HdyActionRow *row,
                                           gpointer user_data) {
  NautilusCompressDialogController *controller;
  ...
  gtk_popover_popdown(GTK_POPOVER(controller->extension_popover));
  update_selected_format(controller, NAUTILUS_COMPRESSION_ENCRYPTED_ZIP);
}

static void tar_xz_row_on_activated(HdyActionRow *row, gpointer user_data) {
  NautilusCompressDialogController *controller;

  controller = NAUTILUS_COMPRESS_DIALOG_CONTROLLER(user_data);

  gtk_popover_popdown(GTK_POPOVER(controller->extension_popover));
  update_selected_format(controller, NAUTILUS_COMPRESSION_TAR_XZ);
}

static void seven_zip_row_on_activated(HdyActionRow *row, gpointer user_data) {
  NautilusCompressDialogController *controller;

  controller = NAUTILUS_COMPRESS_DIALOG_CONTROLLER(user_data);

  gtk_popover_popdown(GTK_POPOVER(controller->extension_popover));
  update_selected_format(controller, NAUTILUS_COMPRESSION_7ZIP);
}

static void passphrase_entry_on_changed(GtkEditable *editable,
                                        gpointer user_data) {
  NautilusCompressDialogController *self;
  ...
  self->passphrase =
      g_strdup(gtk_entry_get_text(GTK_ENTRY(self->passphrase_entry)));
...
static void passphrase_entry_on_icon_press(GtkEntry *entry,
  ...
  gtk_entry_set_icon_from_icon_name(
      GTK_ENTRY(self->passphrase_entry), GTK_ENTRY_ICON_SECONDARY,
      visibility ? "view-conceal" : "view-reveal");
  gtk_entry_set_visibility(GTK_ENTRY(self->passphrase_entry), !visibility);
}

static void activate_button_on_sensitive_notify(GObject *gobject,
                                                GParamSpec *pspec,
                                                gpointer user_data) {
  NautilusCompressDialogController *self;
  NautilusCompressionFormat format;
...
static void popover_on_show(GtkWidget *widget, gpointer user_data) {
  NautilusCompressDialogController *self;
  NautilusCompressionFormat format;
...
NautilusCompressDialogController *nautilus_compress_dialog_controller_new(
    GtkWindow *parent_window, NautilusDirectory *destination_directory,
    gchar *initial_name) {
  NautilusCompressDialogController *self;
  ...
  compress_dialog =
      GTK_WIDGET(gtk_builder_get_object(builder, "compress_dialog"));
  error_revealer =
      GTK_WIDGET(gtk_builder_get_object(builder, "error_revealer"));
  error_label = GTK_WIDGET(gtk_builder_get_object(builder, "error_label"));
  name_entry = GTK_WIDGET(gtk_builder_get_object(builder, "name_entry"));
  activate_button =
      GTK_WIDGET(gtk_builder_get_object(builder, "activate_button"));
  extension_stack =
      GTK_WIDGET(gtk_builder_get_object(builder, "extension_stack"));
  zip_label = GTK_WIDGET(gtk_builder_get_object(builder, "zip_label"));
  encrypted_zip_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "encrypted_zip_label"));
  tar_xz_label = GTK_WIDGET(gtk_builder_get_object(builder, "tar_xz_label"));
  seven_zip_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "seven_zip_label"));
  extension_popover =
      GTK_WIDGET(gtk_builder_get_object(builder, "extension_popover"));
  zip_checkmark = GTK_WIDGET(gtk_builder_get_object(builder, "zip_checkmark"));
  encrypted_zip_checkmark =
      GTK_WIDGET(gtk_builder_get_object(builder, "encrypted_zip_checkmark"));
  tar_xz_checkmark =
      GTK_WIDGET(gtk_builder_get_object(builder, "tar_xz_checkmark"));
  seven_zip_checkmark =
      GTK_WIDGET(gtk_builder_get_object(builder, "seven_zip_checkmark"));
  passphrase_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "passphrase_label"));
  passphrase_entry =
      GTK_WIDGET(gtk_builder_get_object(builder, "passphrase_entry"));
  zip_row = GTK_WIDGET(gtk_builder_get_object(builder, "zip_row"));
  encrypted_zip_row =
      GTK_WIDGET(gtk_builder_get_object(builder, "encrypted_zip_row"));
  ...
  gtk_window_set_transient_for(GTK_WINDOW(compress_dialog), parent_window);

  self =
      g_object_new(NAUTILUS_TYPE_COMPRESS_DIALOG_CONTROLLER, "error-revealer",
                   error_revealer, "error-label", error_label, "name-entry",
                   name_entry, "activate-button", activate_button,
  ...
  self->response_handler_id =
      g_signal_connect(compress_dialog, "response",
                       (GCallback)compress_dialog_controller_on_response, self);

  g_signal_connect(self->zip_row, "activated", G_CALLBACK(zip_row_on_activated),
                   self);
  ...
  g_signal_connect(self->extension_popover, "show", G_CALLBACK(popover_on_show),
                   self);
...
static void nautilus_compress_dialog_controller_init(
    NautilusCompressDialogController *self) {}

static void nautilus_compress_dialog_controller_finalize(GObject *object) {
  NautilusCompressDialogController *self;
  ...
  G_OBJECT_CLASS(nautilus_compress_dialog_controller_parent_class)
      ->finalize(object);
}

static void nautilus_compress_dialog_controller_class_init(
    NautilusCompressDialogControllerClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  NautilusFileNameWidgetControllerClass *parent_class =
      NAUTILUS_FILE_NAME_WIDGET_CONTROLLER_CLASS(klass);
  ...
  parent_class->name_is_valid =
      nautilus_compress_dialog_controller_name_is_valid;
}
