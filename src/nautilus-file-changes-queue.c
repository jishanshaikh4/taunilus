...
static NautilusFileChangesQueue *nautilus_file_changes_queue_new(void) {
...
static NautilusFileChangesQueue *nautilus_file_changes_queue_get(void) {
...
void nautilus_file_changes_queue_file_moved(GFile *from, GFile *to) {
    NautilusFileChange *new_item;
    NautilusFileChangesQueue *queue;
...
nautilus_file_changes_queue_get_change(NautilusFileChangesQueue *queue) {
    ...
    if (queue->tail == NULL) {
        ...
        queue->head = g_list_remove_link(queue->head, queue->tail);
...
enum { CONSUME_CHANGES_MAX_CHUNK = 20 };

static void pairs_list_free(GList *pairs) {
    GList *p;
    GFilePair *pair;
...
void nautilus_file_changes_consume_changes(gboolean consume_all) {
    ...
    for (chunk_count = 0;; chunk_count++) {
        ...
        /* figure out if we need to flush the pending changes that we collected
         * sofar */
        ...
        if (change == NULL) {
            ...
            flush_needed = additions != NULL && change->kind != CHANGE_FILE_ADDED;

            flush_needed |= changes != NULL && change->kind != CHANGE_FILE_CHANGED;

            flush_needed |= moves != NULL && change->kind != CHANGE_FILE_MOVED;

            flush_needed |= deletions != NULL && change->kind != CHANGE_FILE_REMOVED;
        ...
        case CHANGE_FILE_MOVED: {
            nautilus_tag_manager_update_moved_uris(tag_manager, change->from,
                                                   change->to);
