...
G_DEFINE_TYPE_WITH_CODE(NautilusPlacesView, nautilus_places_view, GTK_TYPE_BOX,
                        G_ADD_PRIVATE(NautilusPlacesView) G_IMPLEMENT_INTERFACE(
                            NAUTILUS_TYPE_VIEW,
                            nautilus_places_view_iface_init));
...
static void open_location_cb(NautilusPlacesView *view, GFile *location,
    ...
    if (slot) {
        ...
        nautilus_mime_activate_file(GTK_WINDOW(window), NAUTILUS_WINDOW_SLOT(slot),
                                    file, path, flags);
        nautilus_file_unref(file);
    }
}

static void loading_cb(NautilusView *view) {
    g_object_notify(G_OBJECT(view), "loading");
}

static void show_error_message_cb(NautilusGtkPlacesView *view,
                                  const gchar *primary,
    ...
    dialog = gtk_message_dialog_new(
                 ...
                 GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "%s", primary);
...
static void nautilus_places_view_finalize(GObject *object) {
    NautilusPlacesView *self = (NautilusPlacesView *)object;
    NautilusPlacesViewPrivate *priv =
        nautilus_places_view_get_instance_private(self);
...
static void nautilus_places_view_get_property(GObject *object, guint prop_id,
        GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_places_view_set_property(GObject *object, guint prop_id,
        const GValue *value,
        GParamSpec *pspec) {
    NautilusView *view = NAUTILUS_VIEW(object);
...
static GFile *nautilus_places_view_get_location(NautilusView *view) {
    NautilusPlacesViewPrivate *priv;
...
static void nautilus_places_view_set_location(NautilusView *view,
        GFile *location) {
    if (location) {
        NautilusPlacesViewPrivate *priv;
        gchar *uri;
        ...
        priv =
            nautilus_places_view_get_instance_private(NAUTILUS_PLACES_VIEW(view));
...
static GList *nautilus_places_view_get_selection(NautilusView *view) {
    /* STUB */
    return NULL;
}

static void nautilus_places_view_set_selection(NautilusView *view,
        GList *selection) {
    /* STUB */
}
...
static void nautilus_places_view_set_search_query(NautilusView *view,
        NautilusQuery *query) {
    NautilusPlacesViewPrivate *priv;
    gchar *text;
    ...
    nautilus_gtk_places_view_set_search_query(
        NAUTILUS_GTK_PLACES_VIEW(priv->places_view), text);
...
static gboolean nautilus_places_view_is_loading(NautilusView *view) {
    NautilusPlacesViewPrivate *priv;
...
static gboolean nautilus_places_view_is_searching(NautilusView *view) {
    NautilusPlacesViewPrivate *priv;
...
static guint nautilus_places_view_get_view_id(NautilusView *view) {
...
static void nautilus_places_view_iface_init(NautilusViewInterface *iface) {
    ...
    iface->get_toolbar_menu_sections =
        nautilus_places_view_get_toolbar_menu_sections;
...
static void nautilus_places_view_class_init(NautilusPlacesViewClass *klass) {
    ...
    g_object_class_override_property(object_class, PROP_SEARCH_QUERY,
                                     "search-query");
    g_object_class_override_property(object_class,
                                     PROP_EXTENSIONS_BACKGROUND_MENU,
                                     "extensions-background-menu");
    g_object_class_override_property(object_class, PROP_TEMPLATES_MENU,
                                     "templates-menu");
}

static void nautilus_places_view_init(NautilusPlacesView *self) {
    NautilusPlacesViewPrivate *priv;
    ...
    g_signal_connect_swapped(priv->places_view, "notify::loading",
                             G_CALLBACK(loading_cb), self);

    g_signal_connect_swapped(priv->places_view, "open-location",
                             G_CALLBACK(open_location_cb), self);

    g_signal_connect_swapped(priv->places_view, "show-error-message",
                             G_CALLBACK(show_error_message_cb), self);
