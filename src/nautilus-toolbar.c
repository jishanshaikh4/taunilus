...
/* Just design, context at
 * https://gitlab.gnome.org/GNOME/nautilus/issues/548#note_274131 */
...
static void toolbar_update_appearance(NautilusToolbar *self) {
    gboolean show_location_entry;

    show_location_entry =
        self->show_location_entry ||
    ...
    if (self->window_slot != NULL &&
        ...
        gtk_stack_set_visible_child_name(GTK_STACK(self->toolbar_switcher),
                                         "search");
...
static void activate_back_or_forward_menu_item(GtkMenuItem *menu_item,
        NautilusWindowSlot *window_slot,
        gboolean back) {
    int index;
...
static void activate_back_menu_item_callback(GtkMenuItem *menu_item,
        NautilusToolbar *self) {
    activate_back_or_forward_menu_item(menu_item, self->window_slot, TRUE);
}

static void activate_forward_menu_item_callback(GtkMenuItem *menu_item,
        NautilusToolbar *self) {
    activate_back_or_forward_menu_item(menu_item, self->window_slot, FALSE);
}

static void fill_menu(NautilusToolbar *self, GtkWidget *menu, gboolean back) {
    GtkWidget *menu_item;
    int index;
    GList *list;
    ...
    while (list != NULL) {
        ...
        g_signal_connect_object(
            menu_item, "activate",
            back ? G_CALLBACK(activate_back_menu_item_callback)
...
static void show_menu(NautilusToolbar *self, GtkWidget *widget,
    ...
    direction =
        GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(widget), "nav-direction"));
...
static void navigation_button_press_cb(GtkGestureMultiPress *gesture,
...
static void back_button_longpress_cb(GtkGestureLongPress *gesture, double x,
...
static void forward_button_longpress_cb(GtkGestureLongPress *gesture, double x,
                                        double y, gpointer user_data) {
    NautilusToolbar *self = user_data;
    GdkEventSequence *sequence;
    const GdkEvent *event;
...
static gboolean should_show_progress_info(NautilusProgressInfo *info) {
    return nautilus_progress_info_get_total_elapsed_time(info) +
           nautilus_progress_info_get_remaining_time(info) >
           OPERATION_MINIMUM_TIME;
}

static GList *get_filtered_progress_infos(NautilusToolbar *self) {
    GList *l;
    GList *filtered_progress_infos;
    GList *progress_infos;

    progress_infos =
        nautilus_progress_info_manager_get_all_infos(self->progress_manager);
...
static gboolean should_hide_operations_button(NautilusToolbar *self) {
    GList *progress_infos;
    GList *l;
    ...
    for (l = progress_infos; l != NULL; l = l->next) {
        if (nautilus_progress_info_get_total_elapsed_time(l->data) +
                nautilus_progress_info_get_remaining_time(l->data) >
                OPERATION_MINIMUM_TIME &&
...
static gboolean on_remove_finished_operations_timeout(NautilusToolbar *self) {
    nautilus_progress_info_manager_remove_finished_or_cancelled_infos(
...
static void unschedule_remove_finished_operations(NautilusToolbar *self) {
...
static void schedule_remove_finished_operations(NautilusToolbar *self) {
    if (self->remove_finished_operations_timeout_id == 0) {
        self->remove_finished_operations_timeout_id = g_timeout_add_seconds(
                    REMOVE_FINISHED_OPERATIONS_TIEMOUT,
                    (GSourceFunc)on_remove_finished_operations_timeout, self);
    }
}

static void remove_operations_button_attention_style(NautilusToolbar *self) {
...
static void add_operations_button_attention_style(NautilusToolbar *self) {
    GtkStyleContext *style_context;
    ...
    gtk_style_context_add_class(style_context,
                                "nautilus-operations-button-needs-attention");
    self->operations_button_attention_timeout_id = g_timeout_add(
                NEEDS_ATTENTION_ANIMATION_TIMEOUT,
                (GSourceFunc)on_remove_operations_button_attention_style_timeout, self);
}

static void on_progress_info_cancelled(NautilusToolbar *self) {
...
static void on_progress_info_progress_changed(NautilusToolbar *self) {
    /* Update the pie chart progress */
    gtk_widget_queue_draw(self->operations_icon);
}

static void on_progress_info_finished(NautilusToolbar *self,
    ...
    if (!gtk_toggle_button_get_active(
                GTK_TOGGLE_BUTTON(self->operations_button)) &&
            folder_to_open != NULL) {
        add_operations_button_attention_style(self);
        main_label = nautilus_progress_info_get_status(info);
        nautilus_window_show_operation_notification(self->window, main_label,
                folder_to_open);
        g_free(main_label);
    }
...
static void disconnect_progress_infos(NautilusToolbar *self) {
    GList *progress_infos;
    GList *l;
...
static void update_operations(NautilusToolbar *self) {
    ...
    for (l = progress_infos; l != NULL; l = l->next) {
        should_show_progress_button =
            should_show_progress_button || should_show_progress_info(l->data);
        ...
        g_signal_connect_swapped(l->data, "progress-changed",
                                 G_CALLBACK(on_progress_info_progress_changed),
                                 self);
    ...
    if (should_show_progress_button &&
        ...
        if (gtk_widget_is_visible(GTK_WIDGET(self))) {
            ...
            theatric = g_object_new(IDE_TYPE_BOX_THEATRIC, "alpha", 0.9, "background",
                                    "#fdfdfd", "target", self->operations_button,
                                    "height", rect.height, "width", rect.width, "x",
                                    rect.x, "y", rect.y, NULL);

            egg_object_animate_full(
                theatric, EGG_ANIMATION_EASE_IN_CUBIC, 250,
                gtk_widget_get_frame_clock(GTK_WIDGET(self->operations_button)),
                g_object_unref, theatric, "x", rect.x - ANIMATION_X_GROW, "width",
                rect.width + (ANIMATION_X_GROW * 2), "y", rect.y - ANIMATION_Y_GROW,
                "height", rect.height + (ANIMATION_Y_GROW * 2), "alpha", 0.0, NULL);
        }
    }
...
static gboolean on_progress_info_started_timeout(NautilusToolbar *self) {
    GList *progress_infos;
    GList *filtered_progress_infos;
...
static void schedule_operations_start(NautilusToolbar *self) {
    if (self->start_operations_timeout_id == 0) {
        ...
        self->start_operations_timeout_id =
            g_timeout_add(SECONDS_NEEDED_FOR_APROXIMATE_TRANSFER_RATE * 1000 + 500,
                          (GSourceFunc)on_progress_info_started_timeout, self);
    }
}

static void unschedule_operations_start(NautilusToolbar *self) {
    if (self->start_operations_timeout_id != 0) {
...
static void on_progress_info_started(NautilusProgressInfo *info,
                                     NautilusToolbar *self) {
    g_signal_handlers_disconnect_by_data(info, self);
    schedule_operations_start(self);
}

static void on_new_progress_info(NautilusProgressInfoManager *manager,
                                 NautilusProgressInfo *info,
                                 NautilusToolbar *self) {
    g_signal_connect(info, "started", G_CALLBACK(on_progress_info_started), self);
}

static void on_operations_icon_draw(GtkWidget *widget, cairo_t *cr,
                                    NautilusToolbar *self) {
    ...
    gtk_style_context_get_color(
        style_context, gtk_style_context_get_state(style_context), &foreground);
...
static void on_operations_button_toggled(NautilusToolbar *self,
...
static void update_menu_item(GtkWidget *menu_item, NautilusToolbar *self,
                             const char *action_name, gboolean enabled,
                             char *label) {
    GAction *action;
...
static void undo_manager_changed(NautilusToolbar *self) {
    ...
    /* Set the label of the undo and redo menu items, and activate them
     * appropriately
     */
...
static void on_location_entry_close(GtkWidget *close_button,
                                    NautilusToolbar *self) {
    nautilus_toolbar_set_show_location_entry(self, FALSE);
}

static gboolean on_location_entry_populate_popup(GtkEntry *entry,
        GtkWidget *widget,
        gpointer user_data) {
...
static void on_location_entry_focus_changed(GObject *object, GParamSpec *pspec,
        gpointer user_data) {
    NautilusToolbar *toolbar;
...
static GtkWidget *operations_list_create_widget(GObject *item,
        gpointer user_data) {
...
static void nautilus_toolbar_constructed(GObject *object) {
    NautilusToolbar *self = NAUTILUS_TOOLBAR(object);

    self->path_bar = GTK_WIDGET(g_object_new(NAUTILUS_TYPE_PATH_BAR, NULL));
    gtk_box_append(GTK_BOX(self->path_bar_container), self->path_bar);

    self->location_entry = nautilus_location_entry_new();
    gtk_box_append(GTK_BOX(self->location_entry_container), self->location_entry);
    self->location_entry_close_button = gtk_button_new_from_icon_name(
    ...
    gtk_list_box_bind_model(
        GTK_LIST_BOX(self->operations_list),
        G_LIST_MODEL(self->progress_infos_model),
        (GtkListBoxCreateWidgetFunc)operations_list_create_widget, NULL, NULL);
    update_operations(self);

    self->back_button_longpress_gesture =
    ...
    self->forward_button_longpress_gesture =
        gtk_gesture_long_press_new(self->forward_button);
    ...
    self->back_button_multi_press_gesture =
        gtk_gesture_multi_press_new(self->back_button);
    gtk_gesture_single_set_button(
        GTK_GESTURE_SINGLE(self->back_button_multi_press_gesture),
        GDK_BUTTON_SECONDARY);
    g_signal_connect(self->back_button_multi_press_gesture, "pressed",
                     G_CALLBACK(navigation_button_press_cb), self);

    self->forward_button_multi_press_gesture =
        gtk_gesture_multi_press_new(self->forward_button);
    gtk_gesture_single_set_button(
        GTK_GESTURE_SINGLE(self->forward_button_multi_press_gesture),
        GDK_BUTTON_SECONDARY);
...
static void nautilus_toolbar_init(NautilusToolbar *self) {
    gtk_widget_init_template(GTK_WIDGET(self));
}

void nautilus_toolbar_on_window_constructed(NautilusToolbar *self) {
    ...
    g_signal_connect_object(nautilus_file_undo_manager_get(), "undo-changed",
                            G_CALLBACK(undo_manager_changed), self,
                            G_CONNECT_SWAPPED);

    undo_manager_changed(self);
}

static void nautilus_toolbar_get_property(GObject *object, guint property_id,
    ...
    case PROP_SEARCHING: {
        g_value_set_boolean(value, gtk_toggle_button_get_active(
                                GTK_TOGGLE_BUTTON(self->search_button)));
    }
    break;
...
static void on_window_slot_destroyed(gpointer data,
...
static void on_window_focus_changed(GObject *object, GParamSpec *pspec,
                                    gpointer user_data) {
    GtkWidget *widget;
    NautilusToolbar *toolbar;
    ...
    /* The working assumption being made here is, if the location entry is
     * visible, the user must have switched windows while having keyboard focus on
     * the entry (because otherwise it would be invisible), so we focus the entry
     * explicitly to reset the “should auto-hide” flag.
     */
     ...
     * might want to copy a path from somewhere. This here prevents that from
     * happening.
     */
    else {
        toolbar->location_entry_should_auto_hide = FALSE;
...
static void nautilus_toolbar_set_property(GObject *object, guint property_id,
        const GValue *value,
        GParamSpec *pspec) {
    NautilusToolbar *self = NAUTILUS_TOOLBAR(object);
...
static void nautilus_toolbar_dispose(GObject *object) {
    NautilusToolbar *self;
...
static void nautilus_toolbar_finalize(GObject *obj) {
    ...
    if (self->window_slot != NULL) {
        ...
        g_object_weak_unref(G_OBJECT(self->window_slot), on_window_slot_destroyed,
                            self);
    ...
    g_signal_handlers_disconnect_by_func(self->window, on_window_focus_changed,
                                         self);
...
static void nautilus_toolbar_class_init(NautilusToolbarClass *klass) {
    GObjectClass *oclass;
    ...
    properties[PROP_WINDOW] = g_param_spec_object(
                                  "window", "The NautilusWindow",
                                  "The NautilusWindow this toolbar is part of", NAUTILUS_TYPE_WINDOW,
                                  G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS);
    properties[PROP_SHOW_LOCATION_ENTRY] = g_param_spec_boolean(
            "show-location-entry", "Whether to show the location entry",
            "Whether to show the location entry instead of the pathbar", FALSE,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    properties[PROP_WINDOW_SLOT] = g_param_spec_object(
                                       "window-slot", "Window slot currently active",
                                       "Window slot currently acive", NAUTILUS_TYPE_WINDOW_SLOT,
                                       (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    properties[PROP_SEARCHING] = g_param_spec_boolean(
                                     "searching", "Current view is searching",
                                     "Whether the current view is searching or not", FALSE, G_PARAM_READWRITE);

    g_object_class_install_properties(oclass, NUM_PROPERTIES, properties);

    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/gnome/nautilus/ui/nautilus-toolbar.ui");

    gtk_widget_class_bind_template_child(widget_class, NautilusToolbar,
                                         operations_button);
    gtk_widget_class_bind_template_child(widget_class, NautilusToolbar,
                                         operations_icon);
    gtk_widget_class_bind_template_child(widget_class, NautilusToolbar,
...
static void box_remove_all_children(GtkBox *box) {
    GtkWidget *child;
...
static void slot_on_extensions_background_menu_changed(
    NautilusToolbar *self, GParamSpec *param, NautilusWindowSlot *slot) {
    ...
    nautilus_path_bar_set_extensions_background_menu(
        NAUTILUS_PATH_BAR(self->path_bar), menu);
}

static void slot_on_templates_menu_changed(NautilusToolbar *self,
        GParamSpec *param,
        NautilusWindowSlot *slot) {
    g_autoptr(GMenuModel) menu = NULL;

    menu = nautilus_window_slot_get_templates_menu(slot);
    nautilus_path_bar_set_templates_menu(NAUTILUS_PATH_BAR(self->path_bar), menu);
}

static void on_slot_toolbar_menu_sections_changed(NautilusToolbar *self,
        GParamSpec *param,
        NautilusWindowSlot *slot) {
    ...
    gtk_widget_set_sensitive(self->view_button,
                             (new_sections->extended_section != NULL ||
...
disconnect_toolbar_menu_sections_change_handler(NautilusToolbar *self) {
    ...
    g_signal_handlers_disconnect_by_func(
        self->window_slot, G_CALLBACK(on_slot_toolbar_menu_sections_changed),
        self);
}

static gboolean nautilus_toolbar_view_toggle_icon_transform_to(
...
static gboolean nautilus_toolbar_view_toggle_tooltip_transform_to(
    GBinding *binding, const GValue *from_value, GValue *to_value,
    gpointer user_data) {
    const gchar *tooltip;
...
static void nautilus_toolbar_set_window_slot_real(NautilusToolbar *self,
    ...
    if (self->window_slot != NULL) {
        g_object_weak_ref(G_OBJECT(self->window_slot), on_window_slot_destroyed,
                          self);

        self->icon_binding = g_object_bind_property_full(
                                 self->window_slot, "icon", self->view_toggle_icon, "gicon",
                                 G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE,
                                 (GBindingTransformFunc)nautilus_toolbar_view_toggle_icon_transform_to,
                                 NULL, self, NULL);

        self->tooltip_binding = g_object_bind_property_full(
                                    self->window_slot, "tooltip", self->view_toggle_button, "tooltip-text",
                                    G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE,
                                    (GBindingTransformFunc)
                                    nautilus_toolbar_view_toggle_tooltip_transform_to,
                                    NULL, self, NULL);

        self->search_binding = g_object_bind_property(
                                   self->window_slot, "searching", self, "searching",
                                   G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
        ...
        g_signal_connect_swapped(
            self->window_slot, "notify::extensions-background-menu",
...
gboolean nautilus_toolbar_is_menu_visible(NautilusToolbar *self) {
    g_return_val_if_fail(NAUTILUS_IS_TOOLBAR(self), FALSE);

    return gtk_widget_is_visible(self->app_menu);
}

gboolean nautilus_toolbar_is_operations_button_active(NautilusToolbar *self) {
