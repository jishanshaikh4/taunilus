...
static gboolean
nautilus_window_slot_content_view_matches(NautilusWindowSlot *self, guint id);
static NautilusView *
...
static void trash_state_changed_cb(NautilusTrashMonitor *monitor,
                                   gboolean is_empty, gpointer user_data);
...
static void real_set_templates_menu(NautilusWindowSlot *self, GMenuModel *menu);
...
void nautilus_window_slot_restore_navigation_state(
  ...
  g_set_object(&self->current_location_bookmark,
               data->current_location_bookmark);
...
nautilus_window_slot_get_navigation_state(NautilusWindowSlot *self) {
  ...
  back_list = g_list_copy_deep(self->back_list, (GCopyFunc)g_object_ref, NULL);
  forward_list =
      g_list_copy_deep(self->forward_list, (GCopyFunc)g_object_ref, NULL);
  ...
  g_set_object(&data->current_location_bookmark,
...
nautilus_window_slot_get_view_for_location(NautilusWindowSlot *self,
  ...
  if (nautilus_file_is_other_locations(file)) {
    ...
    if (NAUTILUS_IS_FILES_VIEW(self->content_view)) {
      self->view_mode_before_places =
          nautilus_view_get_view_id(self->content_view);
    }
  ...
  if (nautilus_file_is_in_search(file)) {
    /* If it's already set, is because we already made the change to search
     * mode, so the view mode of the current view will be the one search is
     * using, which is not the one we are interested in */
    if (self->view_mode_before_search == NAUTILUS_VIEW_INVALID_ID &&
        NAUTILUS_IS_FILES_VIEW(self->content_view)) {
      self->view_mode_before_search =
          nautilus_view_get_view_id(self->content_view);
    }
    view_id = g_settings_get_enum(nautilus_preferences,
                                  NAUTILUS_PREFERENCES_SEARCH_VIEW);
  } else if (self->content_view != NULL) {
    /* If there is already a view, just use the view mode that it's currently
     * using, or if we were on search before, use what we were using before
...
static void update_search_visible(NautilusWindowSlot *self) {
  NautilusQuery *query;
  NautilusView *view;
...
static void nautilus_window_slot_sync_actions(NautilusWindowSlot *self) {
  ...
  /* Check if we need to close the search or show search after changing the
   * location. Needs to be done after the change has been done, if not, a loop
   * happens, because setting the search enabled or not actually opens a
   * location */
...
static void query_editor_cancel_callback(NautilusQueryEditor *editor,
                                         NautilusWindowSlot *self) {
  nautilus_window_slot_set_search_visible(self, FALSE);
}

static void query_editor_activated_callback(NautilusQueryEditor *editor,
                                            NautilusWindowSlot *self) {
  if (self->content_view != NULL) {
    if (NAUTILUS_IS_FILES_VIEW(self->content_view)) {
      nautilus_files_view_activate_selection(
          NAUTILUS_FILES_VIEW(self->content_view));
    }
  }
}

static void query_editor_focus_view_callback(NautilusQueryEditor *editor,
                                             NautilusWindowSlot *self) {
  if (self->content_view != NULL) {
...
static void query_editor_changed_callback(NautilusQueryEditor *editor,
                                          NautilusQuery *query, gboolean reload,
                                          NautilusWindowSlot *self) {
  NautilusView *view;
  ...
  nautilus_window_slot_open_location_full(
      self, nautilus_view_get_location(view), 0, NULL);
}
...
static void hide_query_editor(NautilusWindowSlot *self) {
  ...
  if (nautilus_view_is_searching(view)) {
    ...
    nautilus_window_slot_open_location_full(
        self, nautilus_view_get_location(view), 0, selection);
  }
...
static void show_query_editor(NautilusWindowSlot *self) {
  NautilusView *view;
...
void nautilus_window_slot_search(NautilusWindowSlot *self,
                                 NautilusQuery *query) {
  NautilusView *view;
...
gboolean nautilus_window_slot_handle_event(NautilusWindowSlot *self,
...
nautilus_window_slot_remove_extra_location_widgets(NautilusWindowSlot *self) {
  GtkWidget *widget;
  while ((widget = gtk_widget_get_first_child(self->extra_location_widgets)) !=
         NULL) {
...
nautilus_window_slot_add_extra_location_widget(NautilusWindowSlot *self,
                                               GtkWidget *widget) {
  gtk_box_pack_start(GTK_BOX(self->extra_location_widgets), widget, FALSE, TRUE,
                     0);
  gtk_widget_show(self->extra_location_widgets);
}

static void nautilus_window_slot_set_searching(NautilusWindowSlot *self,
                                               gboolean searching) {
  self->searching = searching;
  g_object_notify_by_pspec(G_OBJECT(self), properties[PROP_SEARCHING]);
}

static void nautilus_window_slot_set_selection(NautilusWindowSlot *self,
                                               GList *selection) {
...
static void nautilus_window_slot_set_property(GObject *object,
                                              guint property_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
...
static void nautilus_window_slot_get_property(GObject *object,
  ...
  case PROP_TOOLBAR_MENU_SECTIONS: {
    g_value_set_object(value,
                       nautilus_window_slot_get_toolbar_menu_sections(self));
  } break;
...
gboolean nautilus_window_slot_get_searching(NautilusWindowSlot *self) {
  return self->searching;
}

GList *nautilus_window_slot_get_selection(NautilusWindowSlot *self) {
  return self->selection;
}

static void nautilus_window_slot_constructed(GObject *object) {
  NautilusWindowSlot *self = NAUTILUS_WINDOW_SLOT(object);
  ...
  gtk_box_append(GTK_BOX(self), GTK_WIDGET(self->search_info_label_revealer));

  gtk_widget_show(GTK_WIDGET(self->search_info_label));
  gtk_widget_show(GTK_WIDGET(self->search_info_label_revealer));

  style_context =
      gtk_widget_get_style_context(GTK_WIDGET(self->search_info_label));
  gtk_style_context_add_class(style_context, "search-information");

  g_object_bind_property(self, "location", self->query_editor, "location",
                         G_BINDING_DEFAULT);

  self->title = g_strdup(_("Loading…"));
}

static void action_search_visible(GSimpleAction *action, GVariant *state,
                                  gpointer user_data) {
  NautilusWindowSlot *self;
...
static void change_files_view_mode(NautilusWindowSlot *self, guint view_id) {
  ...
  preferences_key =
      nautilus_view_is_searching(nautilus_window_slot_get_current_view(self))
          ? NAUTILUS_PREFERENCES_SEARCH_VIEW
          : NAUTILUS_PREFERENCES_DEFAULT_FOLDER_VIEWER;
...
static void action_files_view_mode_toggle(GSimpleAction *action,
...
static void action_files_view_mode(GSimpleAction *action, GVariant *value,
                                   gpointer user_data) {
  NautilusWindowSlot *self;
  guint view_id;
...
const GActionEntry slot_entries[] = {
    {"files-view-mode", NULL, "u",
     "uint32 " G_STRINGIFY(NAUTILUS_VIEW_INVALID_ID), action_files_view_mode},
...
static void recursive_search_preferences_changed(GSettings *settings,
                                                 gchar *key,
...
static void nautilus_window_slot_init(NautilusWindowSlot *self) {
  GApplication *app;
  const gchar *search_visible_accels[] = {"<control>f", "Search", NULL};

  app = g_application_get_default();

  g_signal_connect(nautilus_trash_monitor_get(), "trash-state-changed",
                   G_CALLBACK(trash_state_changed_cb), self);

  g_signal_connect_object(nautilus_preferences, "changed::recursive-search",
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(self->slot_action_group),
                                  slot_entries, G_N_ELEMENTS(slot_entries),
                                  self);
  gtk_widget_insert_action_group(GTK_WIDGET(self), "slot",
  ...
  nautilus_application_set_accelerator(
      ...
      "<control>2");
  nautilus_application_set_accelerators(app, "slot.search-visible",
                                        search_visible_accels);
...
static void begin_location_change(NautilusWindowSlot *slot, GFile *location,
                                  GFile *previous_location,
                                  GList *new_selection,
                                  NautilusLocationChangeType type,
                                  guint distance, const char *scroll_pos);
...
static void got_file_info_for_view_selection_callback(NautilusFile *file,
                                                      gpointer callback_data);
static gboolean setup_view(NautilusWindowSlot *self, NautilusView *view);
static void load_new_location(NautilusWindowSlot *slot, GFile *location,
                              GList *selection, NautilusFile *file_to_activate,
                              gboolean tell_current_content_view,
                              gboolean tell_new_content_view);

void nautilus_window_slot_open_location_full(NautilusWindowSlot *self,
                                             GFile *location,
...
static GList *check_select_old_location_containing_folder(
    GList *new_selection, GFile *location, GFile *previous_location) {
...
static void check_force_reload(GFile *location,
                               NautilusLocationChangeType type) {
...
static void save_scroll_position_for_history(NautilusWindowSlot *self) {
  /* Set current_bookmark scroll pos */
  if (self->current_location_bookmark != NULL && self->content_view != NULL &&
      NAUTILUS_IS_FILES_VIEW(self->content_view)) {
    char *current_pos;

    current_pos = nautilus_files_view_get_first_visible_file(
        NAUTILUS_FILES_VIEW(self->content_view));
    nautilus_bookmark_set_scroll_pos(self->current_location_bookmark,
                                     current_pos);
    g_free(current_pos);
  }
}
 ...
 * @previous_location: The url that was previously shown in the window that
 * initialized the change, if any
 * @new_selection: The initial selection to present after loading the location
 * @type: Which type of location change is this? Standard, back, forward, or
 * reload?
 * @distance: If type is back or forward, the index into the back or forward
 * chain. If type is standard or reload, this is ignored, and must be 0.
 * @scroll_pos: The file to scroll to when the location is loaded.
 *
 * This is the core function for changing the location of a window. Every change
 * to the location begins here.
 */
static void begin_location_change(NautilusWindowSlot *self, GFile *location,
                                  GFile *previous_location,
                                  GList *new_selection,
                                  NautilusLocationChangeType type,
                                  guint distance, const char *scroll_pos) {
  ...
  g_assert(type == NAUTILUS_LOCATION_CHANGE_BACK ||
           type == NAUTILUS_LOCATION_CHANGE_FORWARD || distance == 0);
  ...
  new_selection = check_select_old_location_containing_folder(
      new_selection, location, previous_location);
...
static void nautilus_window_slot_set_location(NautilusWindowSlot *self,
  ...
  if (self->location && g_file_equal(location, self->location)) {
...
static void viewed_file_changed_callback(NautilusFile *file,
                                         NautilusWindowSlot *self) {
  GFile *new_location;
...
static void nautilus_window_slot_go_home(NautilusWindowSlot *self,
                                         NautilusOpenFlags flags) {
  GFile *home;
...
static void nautilus_window_slot_set_viewed_file(NautilusWindowSlot *self,
  ...
  if (self->viewed_file != NULL) {
    g_signal_handlers_disconnect_by_func(
        self->viewed_file, G_CALLBACK(viewed_file_changed_callback), self);
    nautilus_file_monitor_remove(self->viewed_file, self);
  }
...
static void mount_not_mounted_callback(GObject *source_object,
...
static void nautilus_window_slot_display_view_selection_failure(
    NautilusWindow *window, NautilusFile *file, GFile *location,
    GError *error) {
  char *error_message;
  char *detail_message;
  char *scheme_string;
  ...
  if (error == NULL) {
    if (nautilus_file_is_directory(file)) {
      detail_message =
          g_strdup(_("Unable to display the contents of this folder."));
    } else {
      detail_message =
          g_strdup(_("This location doesn’t appear to be a folder."));
    }
    ...
    case G_IO_ERROR_NOT_FOUND: {
      ...
      if (file_path != NULL) {
        detail_message = g_strdup_printf(
            _("Unable to find “%s”. Please check the spelling and try again."),
            file_path);
      } else {
        detail_message = g_strdup(_("Unable to find the requested file. Please "
    ...
    case G_IO_ERROR_PERMISSION_DENIED: {
      detail_message = g_strdup(
          _("Don’t have permission to access the requested location."));
    } break;
    ...
    case G_IO_ERROR_HOST_NOT_FOUND: {
       ...
       */
      detail_message =
          g_strdup(_("Unable to find the requested location. Please check the "
    ...
    case G_IO_ERROR_CONNECTION_REFUSED: {
       ...
       */
      detail_message =
          g_strdup(_("The server has refused the connection. Typically this "
                     "means that the firewall is blocking access or that the "
  ...
  if (detail_message == NULL) {
    detail_message =
        g_strdup_printf(_("Unhandled error message: %s"), error->message);
  }
...
/* FIXME: This works in the folowwing way. begin_location_change tries to get
 * the information of the file directly. If the nautilus file finds that there
 * is an error trying to get its information and the error match that the file
 * is not mounted, it sets an internal attribute with the error then we try to
 * mount it here.
 *
 * However, files are cached, and if the file doesn't get finalized in a
 * location change, because needs to be in the navigation history or is a
 * bookmark, and the file is not the root of the mount point, which is tracked
 * by a volume monitor, and it gets unmounted aftwerwards, the file doesn't
 * realize it's unmounted, and therefore this trick to open an unmounted file
 * will fail the next time the user tries to open. For that, we need to always
 * invalidate the file attributes when a location is changed, which is done in
 * check_force_reload. A better way would be to make sure any children of the
 * mounted root gets akwnoledge by it either by adding a reference to its parent
 * volume monitor or with another solution. */
static gboolean handle_mount_if_needed(NautilusWindowSlot *self,
                                       NautilusFile *file) {
  NautilusWindow *window;
  ...
  if (error && error->domain == G_IO_ERROR &&
      error->code == G_IO_ERROR_NOT_MOUNTED && !self->tried_mount) {
    ...
    g_file_mount_enclosing_volume(location, 0, mount_op,
...
static gboolean handle_regular_file_if_needed(NautilusWindowSlot *self,
                                              NautilusFile *file) {
...
static void got_file_info_for_view_selection_callback(NautilusFile *file,
  ...
  /* desktop and other-locations GFile operations report
   * G_IO_ERROR_NOT_SUPPORTED, but it's not an actual error for Nautilus */
  if (!error || g_error_matches(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED)) {
    ...
    nautilus_window_slot_display_view_selection_failure(window, file, location,
                                                        error);
    ...
    if (!gtk_widget_get_visible(GTK_WIDGET(window))) {
       ...
       */
      /* if this is the only window, we don't want to quit, so we redirect it to
       * home */
      ...
      if (slot_location == NULL) {
         ...
         * can choose "Refresh" to get the original URI back in the location
         * bar.
         */
      }
    }
...
static gboolean setup_view(NautilusWindowSlot *self, NautilusView *view) {
  ...
  old_location = self->content_view
                     ? nautilus_view_get_location(self->content_view)
                     : NULL;

  /* Actually load the pending location and selection: */
  if (self->pending_location != NULL) {
    load_new_location(self, self->pending_location, self->pending_selection,
                      self->pending_file_to_activate, FALSE, TRUE);
    ...
    load_new_location(self, old_location, selection, NULL, FALSE, TRUE);
  } else {
    ret = FALSE;
    goto out;
  }
...
static void load_new_location(NautilusWindowSlot *self, GFile *location,
                              GList *selection, NautilusFile *file_to_activate,
                              gboolean tell_current_content_view,
                              gboolean tell_new_content_view) {
  NautilusView *view;
  ...
  if (view) {
    ...
    if (file_to_activate != NULL) {
      ...
      app_info =
          nautilus_mime_get_default_application_for_file(file_to_activate);
...
static void free_location_change(NautilusWindowSlot *self) {
  ...
  if (self->determine_view_file != NULL) {
    nautilus_file_cancel_call_when_ready(
        self->determine_view_file, got_file_info_for_view_selection_callback,
        self);
    self->determine_view_file = NULL;
  }
}
...
static void nautilus_window_slot_set_content_view(NautilusWindowSlot *self,
  ...
  if (g_list_length(selection) == 0 &&
    ...
    self->pending_scroll_to = nautilus_files_view_get_first_visible_file(
        NAUTILUS_FILES_VIEW(self->content_view));
  }
...
void nautilus_window_slot_back_or_forward(NautilusWindowSlot *self,
  ...
  begin_location_change(self, location, old_location, NULL,
                        back ? NAUTILUS_LOCATION_CHANGE_BACK
...
static void nautilus_window_slot_force_reload(NautilusWindowSlot *self) {
  ...
  begin_location_change(self, location, location, selection,
                        NAUTILUS_LOCATION_CHANGE_RELOAD, 0, current_pos);
  g_free(current_pos);
  g_object_unref(location);
}

void nautilus_window_slot_queue_reload(NautilusWindowSlot *self) {
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));
  ...
  if (self->pending_location != NULL || self->content_view == NULL ||
...
static void nautilus_window_slot_clear_forward_list(NautilusWindowSlot *self) {
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));
...
static void nautilus_window_slot_clear_back_list(NautilusWindowSlot *self) {
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));
...
static void nautilus_window_slot_update_bookmark(NautilusWindowSlot *self,
                                                 NautilusFile *file) {
  gboolean recreate;
  GFile *new_location;
  new_location = nautilus_file_get_location(file);
  ...
  if (self->current_location_bookmark == NULL) {
    ...
    bookmark_location =
        nautilus_bookmark_get_location(self->current_location_bookmark);
  ...
  if (recreate) {
    ...
    self->current_location_bookmark =
        nautilus_bookmark_new(new_location, display_name);
    g_free(display_name);
  }
...
static void check_bookmark_location_matches(NautilusBookmark *bookmark,
                                            GFile *location) {
  GFile *bookmark_location;
...
static void handle_go_direction(NautilusWindowSlot *self, GFile *location,
                                gboolean forward) {
  ...
  check_bookmark_location_matches(
      g_list_nth_data(list, self->location_change_distance), location);
...
static void handle_go_elsewhere(NautilusWindowSlot *self, GFile *location) {
  GFile *slot_location;
  ...
  if (slot_location != NULL) {
    /* If we're returning to the same uri somehow, don't put this uri on back
     * list. This also avoids a problem where set_displayed_location didn't
     * update last_location_bookmark since the uri didn't change.
     */
    if (!g_file_equal(slot_location, location)) {
      /* Store bookmark for current location in back list, unless there is no
       * current location */
      check_last_bookmark_location_matches_slot(self);
      /* Use the first bookmark in the history list rather than creating a new
       * one. */
      self->back_list =
          g_list_prepend(self->back_list, self->last_location_bookmark);
      g_object_ref(self->back_list->data);
    }
  }
}

static void update_history(NautilusWindowSlot *self,
                           NautilusLocationChangeType type,
                           GFile *new_location) {
  switch (type) {
...
static void
nautilus_window_slot_show_x_content_bar(NautilusWindowSlot *self, GMount *mount,
...
static void found_content_type_cb(const char **x_content_types,
                                  gpointer user_data) {
  NautilusWindowSlot *self;
  ...
  if (x_content_types != NULL && x_content_types[0] != NULL) {
    nautilus_window_slot_show_x_content_bar(
        self, data->mount, (const char *const *)x_content_types);
  }
...
static void found_mount_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  FindMountData *data = user_data;
  NautilusWindowSlot *self;
  GMount *mount;
  ...
  if (mount != NULL) {
    data->mount = mount;
    nautilus_get_x_content_types_for_mount_async(mount, found_content_type_cb,
...
static void nautilus_window_slot_show_trash_bar(NautilusWindowSlot *self) {
  GtkWidget *bar;
  NautilusView *view;
...
static void nautilus_window_slot_show_special_location_bar(
    NautilusWindowSlot *self, NautilusSpecialLocation special_location) {
  GtkWidget *bar;
...
static void slot_add_extension_extra_widgets(NautilusWindowSlot *self) {
  GList *providers, *l;
  GtkWidget *widget;
  char *uri;
  NautilusWindow *window;
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    widget = nautilus_location_widget_provider_get_widget(provider, uri,
                                                          GTK_WIDGET(window));
    if (widget != NULL) {
...
static void view_started_loading(NautilusWindowSlot *self, NautilusView *view) {
   ...
   */
  if (!nautilus_toolbar_is_menu_visible(
          NAUTILUS_TOOLBAR(nautilus_window_get_toolbar(self->window)))) {
    gtk_widget_grab_focus(GTK_WIDGET(self->window));
  }
...
static void view_ended_loading(NautilusWindowSlot *self, NautilusView *view) {
  if (view == self->content_view) {
    if (NAUTILUS_IS_FILES_VIEW(view) && self->pending_scroll_to != NULL) {
      nautilus_files_view_scroll_to_file(
          NAUTILUS_FILES_VIEW(self->content_view), self->pending_scroll_to);
    }
...
static void view_is_loading_changed_cb(GObject *object, GParamSpec *pspec,
                                       NautilusWindowSlot *self) {
  NautilusView *view;
...
nautilus_window_slot_setup_extra_location_widgets(NautilusWindowSlot *self) {
  ...
  if (nautilus_directory_is_in_trash(directory)) {
    ...
    if (nautilus_should_use_templates_directory() &&
      ...
      nautilus_window_slot_show_special_location_bar(
          self, NAUTILUS_SPECIAL_LOCATION_TEMPLATES);
    } else if (g_file_equal(location, scripts_file)) {
      nautilus_window_slot_show_special_location_bar(
          self, NAUTILUS_SPECIAL_LOCATION_SCRIPTS);
    }
  ...
  g_file_find_enclosing_mount_async(location, G_PRIORITY_DEFAULT,
                                    data->cancellable, found_mount_cb, data);
...
nautilus_window_slot_connect_new_content_view(NautilusWindowSlot *self) {
  if (self->new_content_view) {
    g_signal_connect(self->new_content_view, "notify::loading",
...
nautilus_window_slot_disconnect_content_view(NautilusWindowSlot *self) {
  if (self->content_view) {
    /* disconnect old view */
    g_signal_handlers_disconnect_by_func(
        self->content_view, G_CALLBACK(view_is_loading_changed_cb), self);
  }
}
...
nautilus_window_slot_switch_new_content_view(NautilusWindowSlot *self) {
  ...
  if (self->new_content_view != NULL) {
    ...
    self->searching_binding = g_object_bind_property(
        self->content_view, "searching", self, "searching",
        G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
    self->selection_binding = g_object_bind_property(
        self->content_view, "selection", self, "selection",
        G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
    self->extensions_background_menu_binding =
        g_object_bind_property(self->content_view, "extensions-background-menu",
                               self, "extensions-background-menu",
                               G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
    self->templates_menu_binding = g_object_bind_property(
        self->content_view, "templates-menu", self, "templates-menu",
        G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
...
/* This is called when we have decided we can actually change to the new
 * view/location situation. */
static void change_view(NautilusWindowSlot *self) {
...
static void nautilus_window_slot_grab_focus(GtkWidget *widget) {
  NautilusWindowSlot *self;
...
static void nautilus_window_slot_class_init(NautilusWindowSlotClass *klass) {
  ...
  properties[PROP_ACTIVE] =
      g_param_spec_boolean("active", "Whether the slot is active",
                           "Whether the slot is the active slot of the window",
                           FALSE, G_PARAM_READWRITE);

  properties[PROP_LOADING] = g_param_spec_boolean(
      "loading", "Whether the slot loading",
      "Whether the slot is loading a new location", FALSE, G_PARAM_READABLE);

  properties[PROP_SEARCHING] = g_param_spec_boolean(
      "searching", "Whether the current view of the slot is searching",
      "Whether the current view of the slot is searching. Proxy property from "
      "the view",
      FALSE, G_PARAM_READWRITE);

  properties[PROP_SELECTION] = g_param_spec_pointer(
      "selection", "Selection of the current view of the slot",
      "The selection of the current view of the slot. Proxy property from the "
      "view",
      G_PARAM_READWRITE);

  properties[PROP_WINDOW] = g_param_spec_object(
      "window", "The NautilusWindow", "The NautilusWindow this slot is part of",
      NAUTILUS_TYPE_WINDOW, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);

  properties[PROP_ICON] = g_param_spec_object(
      "icon", "Icon that represents the slot",
      "The icon that represents the slot", G_TYPE_ICON, G_PARAM_READABLE);

  properties[PROP_TOOLBAR_MENU_SECTIONS] = g_param_spec_pointer(
      "toolbar-menu-sections", "Menu sections for the toolbar menu",
      "The menu sections to add to the toolbar menu for this slot",
      G_PARAM_READABLE);

  properties[PROP_EXTENSIONS_BACKGROUND_MENU] = g_param_spec_object(
      "extensions-background-menu", "Background menu of extensions",
      "Proxy property from the view for the background menu for extensions",
      G_TYPE_MENU_MODEL, G_PARAM_READWRITE);

  properties[PROP_TEMPLATES_MENU] =
      g_param_spec_object("templates-menu", "Templates menu",
                          "Proxy property from the view for the templates menu",
                          G_TYPE_MENU_MODEL, G_PARAM_READWRITE);

  properties[PROP_LOCATION] = g_param_spec_object(
      "location", "Current location visible on the slot",
      "Either the location that is used currently, or the pending location. "
      "Clients will see the same value they set, and therefore it will be "
      "cosistent from clients point of view.",
      G_TYPE_FILE, G_PARAM_READWRITE);

  properties[PROP_TOOLTIP] = g_param_spec_string(
      "tooltip", "Tooltip that represents the slot",
      "The tooltip that represents the slot", NULL, G_PARAM_READWRITE);

  g_object_class_install_properties(oclass, NUM_PROPERTIES, properties);
}

GFile *nautilus_window_slot_get_location(NautilusWindowSlot *self) {
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), NULL);

  return self->location;
}

GFile *nautilus_window_slot_get_pending_location(NautilusWindowSlot *self) {
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), NULL);

  return self->pending_location;
}

const gchar *nautilus_window_slot_get_title(NautilusWindowSlot *self) {
  return self->title;
}
...
NautilusWindow *nautilus_window_slot_get_window(NautilusWindowSlot *self) {
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));

  return self->window;
}

void nautilus_window_slot_set_window(NautilusWindowSlot *self,
                                     NautilusWindow *window) {
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));
 ...
 *
 */
void nautilus_window_slot_update_title(NautilusWindowSlot *self) {
  NautilusWindow *window;
  char *title;
...
gboolean nautilus_window_slot_get_allow_stop(NautilusWindowSlot *self) {
  return self->allow_stop;
}

void nautilus_window_slot_set_allow_stop(NautilusWindowSlot *self,
                                         gboolean allow) {
  NautilusWindow *window;
  g_assert(NAUTILUS_IS_WINDOW_SLOT(self));
...
void nautilus_window_slot_stop_loading(NautilusWindowSlot *self) {
  GFile *location;
  NautilusDirectory *directory;
  location = nautilus_window_slot_get_location(self);
  ...
  if (self->pending_location != NULL && location != NULL &&
    ...
    selection = nautilus_view_get_selection(self->content_view);
    load_new_location(self, location, selection, NULL, TRUE, FALSE);
  }
...
NautilusView *nautilus_window_slot_get_current_view(NautilusWindowSlot *self) {
  if (self->content_view != NULL) {
...
NautilusBookmark *nautilus_window_slot_get_bookmark(NautilusWindowSlot *self) {
  return self->current_location_bookmark;
}

GList *nautilus_window_slot_get_back_history(NautilusWindowSlot *self) {
  return self->back_list;
}

GList *nautilus_window_slot_get_forward_history(NautilusWindowSlot *self) {
  return self->forward_list;
}

NautilusWindowSlot *nautilus_window_slot_new(NautilusWindow *window) {
  return g_object_new(NAUTILUS_TYPE_WINDOW_SLOT, "window", window, NULL);
}

GIcon *nautilus_window_slot_get_icon(NautilusWindowSlot *self) {
  guint current_view_id;
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), NULL);
  ...
  current_view_id =
      nautilus_view_get_view_id(NAUTILUS_VIEW(self->content_view));
...
const gchar *nautilus_window_slot_get_tooltip(NautilusWindowSlot *self) {
  guint current_view_id;
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), NULL);
...
gboolean nautilus_window_slot_get_active(NautilusWindowSlot *self) {
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), FALSE);

  return self->active;
}

void nautilus_window_slot_set_active(NautilusWindowSlot *self,
                                     gboolean active) {
  NautilusWindow *window;
  ...
  if (self->active != active) {
    ...
    if (active) {
      ...
      page_num = gtk_notebook_page_num(
          GTK_NOTEBOOK(nautilus_window_get_notebook(window)), GTK_WIDGET(self));
      g_assert(page_num >= 0);

      gtk_notebook_set_current_page(
          GTK_NOTEBOOK(nautilus_window_get_notebook(window)), page_num);
      ...
      gtk_widget_insert_action_group(GTK_WIDGET(window), "slot",
...
gboolean nautilus_window_slot_get_loading(NautilusWindowSlot *self) {
  g_return_val_if_fail(NAUTILUS_IS_WINDOW_SLOT(self), FALSE);
