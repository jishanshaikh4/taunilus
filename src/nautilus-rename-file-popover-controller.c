...
static void reset_state(NautilusRenameFilePopoverController *self) {
  g_assert(NAUTILUS_IS_RENAME_FILE_POPOVER_CONTROLLER(self));
...
static void rename_file_popover_controller_on_closed(GtkPopover *popover,
                                                     gpointer user_data) {
...
static gboolean nautilus_rename_file_popover_controller_name_is_valid(
    NautilusFileNameWidgetController *controller, gchar *name,
    gchar **error_message) {
  ...
  } else if (nautilus_file_name_widget_controller_is_name_too_long(controller,
                                                                   name)) {
    is_valid = FALSE;
    if (self->target_is_folder) {
...
static gboolean nautilus_rename_file_popover_controller_ignore_existing_file(
    NautilusFileNameWidgetController *controller, NautilusFile *existing_file) {
  NautilusRenameFilePopoverController *self;
  ...
  return nautilus_file_compare_display_name(self->target_file, display_name) ==
         0;
}
...
name_entry_on_f2_pressed(GtkWidget *widget,
  ...
  if (!all_selected || !nautilus_file_is_regular_file(self->target_file)) {
    ...
    gtk_editable_select_region(GTK_EDITABLE(widget), start_offset, end_offset);
  }

  return GDK_EVENT_STOP;
}

static gboolean name_entry_on_undo(GtkWidget *widget,
                                   NautilusRenameFilePopoverController *self) {
...
static void target_file_on_changed(NautilusFile *file, gpointer user_data) {
  NautilusRenameFilePopoverController *controller;
...
nautilus_rename_file_popover_controller_new(void) {
  ...
  rename_file_popover =
      GTK_WIDGET(gtk_builder_get_object(builder, "rename_file_popover"));
  error_revealer =
  ...
  self = g_object_new(NAUTILUS_TYPE_RENAME_FILE_POPOVER_CONTROLLER,
                      "error-revealer", error_revealer, "error-label",
                      error_label, "name-entry", name_entry, "activate-button",
...
NautilusFile *nautilus_rename_file_popover_controller_get_target_file(
    NautilusRenameFilePopoverController *self) {
...
void nautilus_rename_file_popover_controller_show_for_file(
    NautilusRenameFilePopoverController *self, NautilusFile *target_file,
    GdkRectangle *pointing_to, GtkWidget *relative_to) {
  ...
  nautilus_file_name_widget_controller_set_containing_directory(
      NAUTILUS_FILE_NAME_WIDGET_CONTROLLER(self), containing_directory);

  self->target_is_folder = nautilus_file_is_directory(self->target_file);

  self->closed_handler_id = g_signal_connect(
      self->rename_file_popover, "closed",
      G_CALLBACK(rename_file_popover_controller_on_closed), self);

  self->file_changed_handler_id = g_signal_connect(
      self->target_file, "changed", G_CALLBACK(target_file_on_changed), self);
  ...
  gtk_label_set_text(GTK_LABEL(self->title_label), self->target_is_folder
                                                       ? _("Rename Folder")
                                                       : _("Rename File"));
  ...
  gtk_popover_set_pointing_to(GTK_POPOVER(self->rename_file_popover),
                              pointing_to);
  gtk_popover_set_relative_to(GTK_POPOVER(self->rename_file_popover),
                              relative_to);
  ...
  if (nautilus_file_is_regular_file(self->target_file)) {
    ...
    /* Select the name part without the file extension */
    eel_filename_get_rename_region(edit_name, &start_offset, &end_offset);
    gtk_editable_select_region(GTK_EDITABLE(self->name_entry), start_offset,
                               end_offset);
  }

  n_chars = g_utf8_strlen(edit_name, -1);
  gtk_entry_set_width_chars(
      GTK_ENTRY(self->name_entry),
      MIN(MAX(n_chars, RENAME_ENTRY_MIN_CHARS), RENAME_ENTRY_MAX_CHARS));
}

static void on_name_accepted(NautilusFileNameWidgetController *controller) {
  NautilusRenameFilePopoverController *self;
...
static void nautilus_rename_file_popover_controller_init(
    NautilusRenameFilePopoverController *self) {
  g_signal_connect_after(self, "name-accepted", G_CALLBACK(on_name_accepted),
                         self);
}

static void nautilus_rename_file_popover_controller_finalize(GObject *object) {
  NautilusRenameFilePopoverController *self;
  ...
  G_OBJECT_CLASS(nautilus_rename_file_popover_controller_parent_class)
      ->finalize(object);
}

static void nautilus_rename_file_popover_controller_class_init(
    NautilusRenameFilePopoverControllerClass *klass) {
  ...
  parent_class->name_is_valid =
      nautilus_rename_file_popover_controller_name_is_valid;
