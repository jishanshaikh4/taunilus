...
enum { ACTIVATED, FOCUS_VIEW, CHANGED, CANCEL, LAST_SIGNAL };
...
static void update_fts_sensitivity(NautilusQueryEditor *editor) {
  ...
  if (editor->location) {
    ...
    fts_sensitive = !nautilus_file_is_other_locations(file) &&
                    ...
                    !(nautilus_file_is_remote(file) &&
                      location_settings_search_get_recursive_for_location(
                          editor->location) == NAUTILUS_QUERY_RECURSIVE_NEVER);
    nautilus_search_popover_set_fts_sensitive(
        NAUTILUS_SEARCH_POPOVER(editor->popover), fts_sensitive);
  }
}
...
static void nautilus_query_editor_dispose(GObject *object) {
  NautilusQueryEditor *editor;
  ...
  g_signal_handlers_disconnect_by_func(
      nautilus_preferences, recursive_search_preferences_changed, object);

  G_OBJECT_CLASS(nautilus_query_editor_parent_class)->dispose(object);
}

static void nautilus_query_editor_grab_focus(GtkWidget *widget) {
  NautilusQueryEditor *editor;
...
static void nautilus_query_editor_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec) {
  NautilusQueryEditor *editor;
...
static void nautilus_query_editor_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
...
static void nautilus_query_editor_finalize(GObject *object) {
#if 0 && TAGGED_ENTRY_NEEDS_GTK4_REIMPLEMENTATION
    NautilusQueryEditor *editor;
...
static void nautilus_query_editor_class_init(NautilusQueryEditorClass *class) {
  ...
  signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, 0,
                   ...
                   NAUTILUS_TYPE_QUERY, G_TYPE_BOOLEAN);
  ...
  signals[ACTIVATED] =
      g_signal_new("activated", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[FOCUS_VIEW] =
      g_signal_new("focus-view", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  gtk_binding_entry_add_signal(gtk_binding_set_by_class(class), GDK_KEY_Down, 0,
                               "focus-view", 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("location", "Location of the search",
                          "The current location of the editor", G_TYPE_FILE,
                          G_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_QUERY,
      g_param_spec_object("query", "Query of the search",
                          "The query that the editor is handling",
                          NAUTILUS_TYPE_QUERY, G_PARAM_READWRITE));
}
...
static void create_query(NautilusQueryEditor *editor) {
  NautilusQuery *query;
  g_autoptr(NautilusFile) file = NULL;
  ...
  fts_enabled = nautilus_search_popover_get_fts_enabled(
      NAUTILUS_SEARCH_POPOVER(editor->popover));
...
static void nautilus_query_editor_on_stop_search(GtkWidget *entry,
                                                 NautilusQueryEditor *editor) {
...
static void nautilus_query_editor_init(NautilusQueryEditor *editor) {
  g_signal_connect(nautilus_preferences, "changed::recursive-search",
                   G_CALLBACK(recursive_search_preferences_changed), editor);
}

static void search_popover_date_range_changed_cb(NautilusSearchPopover *popover,
...
static void search_popover_mime_type_changed_cb(NautilusSearchPopover *popover,
                                                gint mimetype_group,
...
static void search_popover_time_type_changed_cb(NautilusSearchPopover *popover,
...
static void search_popover_fts_changed_cb(GObject *popover, GParamSpec *pspec,
                                          gpointer user_data) {
  NautilusQueryEditor *editor;
  ...
  nautilus_query_set_search_content(editor->query,
                                    nautilus_search_popover_get_fts_enabled(
                                        NAUTILUS_SEARCH_POPOVER(popover)));
...
static void setup_widgets(NautilusQueryEditor *editor) {
  GtkWidget *hbox;
  GtkWidget *vbox;
  ...
  g_signal_connect(editor->popover, "show", G_CALLBACK(gtk_widget_grab_focus),
                   NULL);
  g_signal_connect_swapped(editor->popover, "closed",
                           G_CALLBACK(gtk_widget_grab_focus), editor);

  g_object_bind_property(editor, "query", editor->popover, "query",
                         G_BINDING_DEFAULT);

  /* setup the filter menu button */
  editor->dropdown_button = gtk_menu_button_new();
  gtk_menu_button_set_popover(GTK_MENU_BUTTON(editor->dropdown_button),
                              editor->popover);
  gtk_box_append(GTK_BOX(hbox), editor->dropdown_button);

  g_signal_connect(editor->entry, "activate", G_CALLBACK(entry_activate_cb),
                   editor);
...
void nautilus_query_editor_set_query(NautilusQueryEditor *self,
                                     NautilusQuery *query) {
...
void nautilus_query_editor_set_text(NautilusQueryEditor *self,
                                    const gchar *text) {
  g_return_if_fail(NAUTILUS_IS_QUERY_EDITOR(self));
...
static gboolean
nautilus_gtk_search_entry_is_keynav_event(guint keyval, GdkModifierType state) {
...
gboolean nautilus_query_editor_handle_event(NautilusQueryEditor *self,
  ...
  if (nautilus_gtk_search_entry_is_keynav_event(keyval, state) ||
      keyval == GDK_KEY_space || keyval == GDK_KEY_Menu) {
