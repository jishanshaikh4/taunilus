...
static void autorun_software_dialog_destroy(AutorunSoftwareDialogData *data) {
...
static gboolean _check_file(GFile *mount_root, const char *file_path,
                            gboolean must_be_executable) {
    g_autoptr(GFile) file = NULL;
    g_autoptr(GFileInfo) file_info = NULL;

    file = g_file_get_child(mount_root, file_path);
    file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE,
...
static void autorun(GMount *mount) {
    ...
    if (path_to_spawn != NULL && cwd_for_program != NULL) {
        ...
        error_string =
            g_strdup_printf(_("Unable to start the program:\n%s"), strerror(errno));
        goto out;
    }
...
static void autorun_software_dialog_response(GtkDialog *dialog,
        gint response_id, GMount *mount) {
...
static void present_autorun_for_software_dialog(GMount *mount) {
    GIcon *icon;
    ...
    dialog = gtk_message_dialog_new(
                 NULL, /* TODO: parent window? */
                 0, GTK_MESSAGE_OTHER, GTK_BUTTONS_CANCEL,
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(autorun_software_dialog_response), mount);
