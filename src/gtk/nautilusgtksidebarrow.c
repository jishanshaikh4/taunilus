...
G_DEFINE_TYPE(NautilusGtkSidebarRow, nautilus_gtk_sidebar_row,
...
static void cloud_row_update(NautilusGtkSidebarRow *self) {
  ...
  g_object_set(self, "label", cloud_providers_account_get_name(account), NULL);
  g_object_set(self, "tooltip",
               cloud_providers_account_get_status_details(account), NULL);
  g_object_set(self, "end-icon", end_icon, NULL);
...
static void nautilus_gtk_sidebar_row_get_property(GObject *object,
...
static void nautilus_gtk_sidebar_row_set_property(GObject *object,
                                                  guint prop_id,
                                                  const GValue *value,
                                                  GParamSpec *pspec) {
  NautilusGtkSidebarRow *self = NAUTILUS_GTK_SIDEBAR_ROW(object);
  ...
  case PROP_START_ICON: {
    ...
    if (object != NULL) {
      ...
      gtk_image_set_from_gicon(GTK_IMAGE(self->start_icon_widget),
                               self->start_icon, GTK_ICON_SIZE_MENU);
    } else {
  ...
  case PROP_CLOUD_PROVIDER_ACCOUNT:
    ...
    if (self->cloud_provider_account != NULL) {
      ...
      g_signal_connect_swapped(self->cloud_provider_account, "notify::status",
                               G_CALLBACK(cloud_row_update), self);
      g_signal_connect_swapped(self->cloud_provider_account,
...
static void on_child_revealed(GObject *self, GParamSpec *pspec,
                              gpointer user_data) {
...
void nautilus_gtk_sidebar_row_hide(NautilusGtkSidebarRow *self,
  ...
  transition_duration =
      gtk_revealer_get_transition_duration(GTK_REVEALER(self->revealer));
  if (inmediate)
...
void nautilus_gtk_sidebar_row_set_end_icon(NautilusGtkSidebarRow *self,
  ...
  if (self->end_icon != icon) {
    ...
    if (self->end_icon != NULL)
      gtk_image_set_from_gicon(GTK_IMAGE(self->end_icon_widget), self->end_icon,
                               GTK_ICON_SIZE_MENU);
    else if (self->end_icon_widget != NULL)
      gtk_image_clear(GTK_IMAGE(self->end_icon_widget));
...
static void nautilus_gtk_sidebar_row_finalize(GObject *object) {
  NautilusGtkSidebarRow *self = NAUTILUS_GTK_SIDEBAR_ROW(object);
...
static void nautilus_gtk_sidebar_row_init(NautilusGtkSidebarRow *self) {
...
nautilus_gtk_sidebar_row_class_init(NautilusGtkSidebarRowClass *klass) {
  ...
  properties[PROP_SIDEBAR] = g_param_spec_object(
      "sidebar", "Sidebar", "Sidebar", NAUTILUS_TYPE_GTK_PLACES_SIDEBAR,
      (G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  properties[PROP_START_ICON] = g_param_spec_object(
      "start-icon", "start-icon", "The start icon.", G_TYPE_ICON,
      (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_END_ICON] =
      g_param_spec_object("end-icon", "end-icon", "The end icon.", G_TYPE_ICON,
                          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_LABEL] =
      g_param_spec_string("label", "label", "The label text.", NULL,
                          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_TOOLTIP] =
      g_param_spec_string("tooltip", "Tooltip", "Tooltip", NULL,
                          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_EJECTABLE] =
      g_param_spec_boolean("ejectable", "Ejectable", "Ejectable", FALSE,
                           (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_ORDER_INDEX] =
      g_param_spec_int("order-index", "OrderIndex", "Order Index", 0, G_MAXINT,
                       0, (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_SECTION_TYPE] = g_param_spec_int(
      "section-type", "section type", "The section type.", SECTION_INVALID,
      N_SECTIONS, SECTION_INVALID,
      (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT_ONLY));

  properties[PROP_PLACE_TYPE] = g_param_spec_int(
      "place-type", "place type", "The place type.", PLACES_INVALID, N_PLACES,
      PLACES_INVALID,
      (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT_ONLY));
  ...
  properties[PROP_DRIVE] = g_param_spec_object(
      "drive", "Drive", "Drive", G_TYPE_DRIVE,
      (G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  properties[PROP_VOLUME] = g_param_spec_object(
      "volume", "Volume", "Volume", G_TYPE_VOLUME,
      (G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  properties[PROP_MOUNT] = g_param_spec_object(
      "mount", "Mount", "Mount", G_TYPE_MOUNT,
      (G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  properties[PROP_CLOUD_PROVIDER_ACCOUNT] =
      g_param_spec_object("cloud-provider-account", "CloudProvidersAccount",
                          "CloudProvidersAccount", G_TYPE_OBJECT,
                          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  properties[PROP_PLACEHOLDER] = g_param_spec_boolean(
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/gtk/ui/nautilusgtksidebarrow.ui");

  gtk_widget_class_bind_template_child(widget_class, NautilusGtkSidebarRow,
                                       start_icon_widget);
  gtk_widget_class_bind_template_child(widget_class, NautilusGtkSidebarRow,
                                       end_icon_widget);
  gtk_widget_class_bind_template_child(widget_class, NautilusGtkSidebarRow,
...
nautilus_gtk_sidebar_row_clone(NautilusGtkSidebarRow *self) {
  return g_object_new(
      NAUTILUS_TYPE_GTK_SIDEBAR_ROW, "sidebar", self->sidebar, "start-icon",
      self->start_icon, "end-icon", self->end_icon, "label", self->label,
      "tooltip", self->tooltip, "ejectable", self->ejectable, "order-index",
      self->order_index, "section-type", self->section_type, "place-type",
      self->place_type, "uri", self->uri, "drive", self->drive, "volume",
      self->volume, "mount", self->mount, "cloud-provider-account",
      self->cloud_provider_account, NULL);
}
