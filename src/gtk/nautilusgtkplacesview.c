 ...
 *
 * Copyright (C) 2015 Georges Basile Stavracas Neto
 ...
 * @Short_description: Widget that displays persistent drives and manages
 * mounted networks
 * @Title: NautilusGtkPlacesView
 * @See_also: #GtkFileChooser
 *
 * #NautilusGtkPlacesView is a stock widget that displays a list of persistent
 * drives such as harddisk partitions and networks.  #NautilusGtkPlacesView does
 * not monitor removable devices.
 *
 ...
 * to the #NautilusGtkPlacesView::open-location signal. This is emitted when the
 * user selects a location to open in the view.
 */
...
static void mount_volume(NautilusGtkPlacesView *view, GVolume *volume);
...
G_DEFINE_TYPE_WITH_PRIVATE(NautilusGtkPlacesView, nautilus_gtk_places_view,
                           GTK_TYPE_BOX)
...
enum { OPEN_LOCATION, SHOW_ERROR_MESSAGE, LAST_SIGNAL };

const gchar *unsupported_protocols[] = {
    "file",     "afc",     "obex",   "http",      "trash", "burn",
    "computer", "archive", "recent", "localtest", NULL};

static guint places_view_signals[LAST_SIGNAL] = {0};
static GParamSpec *properties[LAST_PROP];

static void emit_open_location(NautilusGtkPlacesView *view, GFile *location,
                               NautilusGtkPlacesOpenFlags open_flags) {
  ...
  g_signal_emit(view, places_view_signals[OPEN_LOCATION], 0, location,
                open_flags);
}
...
static void emit_show_error_message(NautilusGtkPlacesView *view,
  ...
  g_signal_emit(view, places_view_signals[SHOW_ERROR_MESSAGE], 0,
                primary_message, secondary_message);
}

static void server_file_changed_cb(NautilusGtkPlacesView *view) {
  populate_servers(view);
}

static GBookmarkFile *server_list_load(NautilusGtkPlacesView *view) {
  NautilusGtkPlacesViewPrivate *priv;
  ...
  if (!priv->server_list_monitor) {
    ...
    if (priv->server_list_file) {
      priv->server_list_monitor = g_file_monitor_file(
          priv->server_list_file, G_FILE_MONITOR_NONE, NULL, &error);

      if (error) {
        g_warning("Cannot monitor server file: %s", error->message);
        g_clear_error(&error);
      } else {
        g_signal_connect_swapped(priv->server_list_monitor, "changed",
                                 G_CALLBACK(server_file_changed_cb), view);
      }
    }
...
static void server_list_save(GBookmarkFile *bookmarks) {
  gchar *filename;

  filename =
      g_build_filename(g_get_user_config_dir(), "gtk-3.0", "servers", NULL);
  g_bookmark_file_to_file(bookmarks, filename, NULL);
  g_free(filename);
}

static void server_list_add_server(NautilusGtkPlacesView *view, GFile *file) {
  GBookmarkFile *bookmarks;
  GFileInfo *info;
  GError *error;
  gchar *title;
...
static void server_list_remove_server(NautilusGtkPlacesView *view,
                                      const gchar *uri) {
  GBookmarkFile *bookmarks;
...
static void set_busy_cursor(NautilusGtkPlacesView *view, gboolean busy) {
  GtkWidget *widget;
  GtkWindow *toplevel;
  GdkDisplay *display;
  GdkCursor *cursor;
...
static void activate_row(NautilusGtkPlacesView *view,
                         NautilusGtkPlacesViewRow *row,
...
static void nautilus_gtk_places_view_destroy(GtkWidget *widget) {
  NautilusGtkPlacesView *self = NAUTILUS_GTK_PLACES_VIEW(widget);
  NautilusGtkPlacesViewPrivate *priv =
      nautilus_gtk_places_view_get_instance_private(self);

  priv->destroyed = 1;

  g_signal_handlers_disconnect_by_func(priv->volume_monitor, update_places,
                                       widget);

  if (priv->network_monitor)
    g_signal_handlers_disconnect_by_func(priv->network_monitor, update_places,
                                         widget);

  if (priv->server_list_monitor)
    g_signal_handlers_disconnect_by_func(priv->server_list_monitor,
                                         server_file_changed_cb, widget);
...
static void nautilus_gtk_places_view_finalize(GObject *object) {
...
static void nautilus_gtk_places_view_get_property(GObject *object,
  ...
  case PROP_FETCHING_NETWORKS:
    g_value_set_boolean(value,
                        nautilus_gtk_places_view_get_fetching_networks(self));
    break;
...
static void nautilus_gtk_places_view_set_property(GObject *object,
                                                  guint prop_id,
                                                  const GValue *value,
                                                  GParamSpec *pspec) {
  NautilusGtkPlacesView *self = NAUTILUS_GTK_PLACES_VIEW(object);
...
static void on_remove_server_button_clicked(RemoveServerData *data) {
...
static void populate_servers(NautilusGtkPlacesView *view) {
  ...
  children =
      gtk_container_get_children(GTK_CONTAINER(priv->recent_servers_listbox));
  ...
  for (i = 0; i < num_uris; i++) {
    ...
    gtk_list_store_set(priv->completion_store, &iter, 0, name, 1, uris[i], -1);
    ...
    g_signal_connect_swapped(button, "clicked",
                             G_CALLBACK(on_remove_server_button_clicked), data);
...
static void update_view_mode(NautilusGtkPlacesView *view) {
  ...
  for (l = children; l; l = l->next) {
    /* GtkListBox filter rows by changing their GtkWidget::child-visible
     * property */
  ...
  if (!show_listbox && priv->search_query && priv->search_query[0] != '\0') {
...
static void insert_row(NautilusGtkPlacesView *view, GtkWidget *row,
                       gboolean is_network) {
  NautilusGtkPlacesViewPrivate *priv;
  ...
  g_signal_connect_swapped(nautilus_gtk_places_view_row_get_event_box(
                               NAUTILUS_GTK_PLACES_VIEW_ROW(row)),
                           "button-press-event",
                           G_CALLBACK(on_button_press_event), row);

  g_signal_connect(row, "popup-menu", G_CALLBACK(on_row_popup_menu), row);

  g_signal_connect(nautilus_gtk_places_view_row_get_eject_button(
                       NAUTILUS_GTK_PLACES_VIEW_ROW(row)),
                   "clicked", G_CALLBACK(on_eject_button_clicked), row);

  nautilus_gtk_places_view_row_set_path_size_group(
      NAUTILUS_GTK_PLACES_VIEW_ROW(row), priv->path_size_group);
  nautilus_gtk_places_view_row_set_space_size_group(
      NAUTILUS_GTK_PLACES_VIEW_ROW(row), priv->space_size_group);

  gtk_container_add(GTK_CONTAINER(priv->listbox), row);
}

static void add_volume(NautilusGtkPlacesView *view, GVolume *volume) {
  gboolean is_network;
  GMount *mount;
  GFile *root;
  GIcon *icon;
  ...
  path = !is_network ? g_volume_get_identifier(
  ...
  if (!mount || !g_mount_is_shadowed(mount)) {
    ...
    row =
        g_object_new(NAUTILUS_TYPE_GTK_PLACES_VIEW_ROW, "icon", icon, "name",
                     name, "path", path ? path : "", "volume", volume, "mount",
                     mount, "file", NULL, "is-network", is_network, NULL);
...
static void add_mount(NautilusGtkPlacesView *view, GMount *mount) {
  gboolean is_network;
  GFile *root;
  GIcon *icon;
  gchar *name;
  ...
  if (!g_mount_is_shadowed(mount)) {
    ...
    row = g_object_new(NAUTILUS_TYPE_GTK_PLACES_VIEW_ROW, "icon", icon, "name",
                       name, "path", path ? path : "", "volume", NULL, "mount",
                       mount, "file", NULL, "is-network", is_network, NULL);
...
static void add_drive(NautilusGtkPlacesView *view, GDrive *drive) {
  GList *volumes;
  GList *l;
...
static void add_file(NautilusGtkPlacesView *view, GFile *file, GIcon *icon,
                     const gchar *display_name, const gchar *path,
                     gboolean is_network) {
  GtkWidget *row;
  row = g_object_new(NAUTILUS_TYPE_GTK_PLACES_VIEW_ROW, "icon", icon, "name",
                     display_name, "path", path, "volume", NULL, "mount", NULL,
                     "file", file, "is_network", is_network, NULL);

  insert_row(view, row, is_network);
}

static gboolean has_networks(NautilusGtkPlacesView *view) {
  GList *l;
  NautilusGtkPlacesViewPrivate *priv;
  GList *children;
...
static void update_network_state(NautilusGtkPlacesView *view) {
  NautilusGtkPlacesViewPrivate *priv;
  ...
  if (priv->network_placeholder == NULL) {
    ...
    g_object_set_data(G_OBJECT(priv->network_placeholder), "is-network",
                      GINT_TO_POINTER(TRUE));
    /* mark the row as placeholder, so it always goes first */
    g_object_set_data(G_OBJECT(priv->network_placeholder), "is-placeholder",
                      GINT_TO_POINTER(TRUE));
...
static void monitor_network(NautilusGtkPlacesView *self) {
  NautilusGtkPlacesViewPrivate *priv;
  GFile *network_file;
  GError *error;
  ...
  priv->network_monitor =
      g_file_monitor(network_file, G_FILE_MONITOR_NONE, NULL, &error);
  ...
  g_signal_connect_swapped(priv->network_monitor, "changed",
                           G_CALLBACK(update_places), self);
}

static void populate_networks(NautilusGtkPlacesView *view,
  ...
  for (l = detected_networks; l != NULL; l = l->next) {
    ...
    if (type == G_FILE_TYPE_SHORTCUT || type == G_FILE_TYPE_MOUNTABLE)
      uri = g_file_info_get_attribute_as_string(
          l->data, G_FILE_ATTRIBUTE_STANDARD_TARGET_URI);
    else
      uri = g_file_get_uri(file);
    activatable_file = g_file_new_for_uri(uri);
    display_name = g_file_info_get_attribute_as_string(
        l->data, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME);
...
static void network_enumeration_next_files_finished(GObject *source_object,
  ...
  detected_networks = g_file_enumerator_next_files_finish(
      G_FILE_ENUMERATOR(source_object), res, &error);
...
static void network_enumeration_finished(GObject *source_object,
  ...
  if (error) {
    ...
    priv = nautilus_gtk_places_view_get_instance_private(
        NAUTILUS_GTK_PLACES_VIEW(user_data));
    g_file_enumerator_next_files_async(
        enumerator, G_MAXINT32, G_PRIORITY_DEFAULT,
        priv->networks_fetching_cancellable,
        network_enumeration_next_files_finished, user_data);
    g_object_unref(enumerator);
  }
}

static void fetch_networks(NautilusGtkPlacesView *view) {
  NautilusGtkPlacesViewPrivate *priv;
  GFile *network_file;
  ...
  for (found = FALSE; !found && supported_uris && supported_uris[0];
       supported_uris++)
  ...
  g_file_enumerate_children_async(
      network_file,
      "standard::type,standard::target-uri,standard::name,standard::display-"
      "name,standard::icon",
      G_FILE_QUERY_INFO_NONE, G_PRIORITY_DEFAULT,
      priv->networks_fetching_cancellable, network_enumeration_finished, view);
...
static void server_mount_ready_cb(GObject *source_file, GAsyncResult *res,
                                  gpointer user_data) {
  NautilusGtkPlacesViewPrivate *priv;
  NautilusGtkPlacesView *view;
  ...
  if (error) {
    ...
    if (error->code == G_IO_ERROR_ALREADY_MOUNTED) {
      ...
      emit_show_error_message(view, _("Unable to access location"),
                              error->message);
    }
...
static void volume_mount_ready_cb(GObject *source_volume, GAsyncResult *res,
                                  gpointer user_data) {
  NautilusGtkPlacesViewPrivate *priv;
  NautilusGtkPlacesView *view;
  gboolean should_show;
  ...
  if (error) {
    ...
    if (error->code == G_IO_ERROR_ALREADY_MOUNTED) {
      ...
      emit_show_error_message(NAUTILUS_GTK_PLACES_VIEW(user_data),
                              _("Unable to access location"), error->message);
      should_show = FALSE;
    }
  ...
  if (should_show) {
    ...
    if (priv->should_open_location)
      emit_open_location(NAUTILUS_GTK_PLACES_VIEW(user_data), root,
                         priv->open_flags);
...
static void unmount_ready_cb(GObject *source_mount, GAsyncResult *res,
                             gpointer user_data) {
  NautilusGtkPlacesView *view;
  NautilusGtkPlacesViewPrivate *priv;
  GMount *mount;
  ...
  if (error) {
    if (error->domain != G_IO_ERROR ||
        (error->code != G_IO_ERROR_CANCELLED &&
         error->code != G_IO_ERROR_FAILED_HANDLED)) {
      /* if it wasn't cancelled show a dialog */
      emit_show_error_message(view, _("Unable to unmount volume"),
                              error->message);
    }
...
static gboolean pulse_entry_cb(gpointer user_data) {
  NautilusGtkPlacesViewPrivate *priv;
...
static void unmount_mount(NautilusGtkPlacesView *view, GMount *mount) {
  NautilusGtkPlacesViewPrivate *priv;
  ...
  g_mount_unmount_with_operation(mount, 0, operation, priv->cancellable,
                                 unmount_ready_cb, view);
  g_object_unref(operation);
}

static void mount_server(NautilusGtkPlacesView *view, GFile *location) {
  NautilusGtkPlacesViewPrivate *priv;
  GMountOperation *operation;
  GtkWidget *toplevel;
  ...
  if (priv->entry_pulse_timeout_id == 0)
    priv->entry_pulse_timeout_id =
        g_timeout_add(100, (GSourceFunc)pulse_entry_cb, view);
  ...
  g_file_mount_enclosing_volume(location, 0, operation, priv->cancellable,
                                server_mount_ready_cb, view);
...
static void mount_volume(NautilusGtkPlacesView *view, GVolume *volume) {
  ...
  g_volume_mount(volume, 0, operation, priv->cancellable, volume_mount_ready_cb,
                 view);
...
static void popup_menu_detach_cb(GtkWidget *attach_widget, GtkMenu *menu) {
  ...
  priv = nautilus_gtk_places_view_get_instance_private(
      NAUTILUS_GTK_PLACES_VIEW(attach_widget));
  priv->popup_menu = NULL;
}

static void open_cb(GtkMenuItem *item, NautilusGtkPlacesViewRow *row) {
  NautilusGtkPlacesView *self;

  self = NAUTILUS_GTK_PLACES_VIEW(
      gtk_widget_get_ancestor(GTK_WIDGET(row), NAUTILUS_TYPE_GTK_PLACES_VIEW));
  activate_row(self, row, NAUTILUS_GTK_PLACES_OPEN_NORMAL);
}

static void open_in_new_tab_cb(GtkMenuItem *item,
                               NautilusGtkPlacesViewRow *row) {
  NautilusGtkPlacesView *self;
...
static void mount_cb(GtkMenuItem *item, NautilusGtkPlacesViewRow *row) {
  NautilusGtkPlacesViewPrivate *priv;
  GtkWidget *view;
  GVolume *volume;
...
static void unmount_cb(GtkMenuItem *item, NautilusGtkPlacesViewRow *row) {
  GtkWidget *view;
  GMount *mount;
...
static void attach_protocol_row_to_grid(GtkGrid *grid,
...
static void populate_available_protocols_grid(GtkGrid *grid) {
...
static void build_popup_menu(NautilusGtkPlacesView *view,
                             NautilusGtkPlacesViewRow *row) {
  NautilusGtkPlacesViewPrivate *priv;
  GtkWidget *item;
  GMount *mount;
  ...
  gtk_menu_attach_to_widget(GTK_MENU(priv->popup_menu), GTK_WIDGET(view),
                            popup_menu_detach_cb);

  /* Open item is always present */
  item = gtk_menu_item_new_with_mnemonic(_("_Open"));
  g_signal_connect(item, "activate", G_CALLBACK(open_cb), row);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);

  if (priv->open_flags & NAUTILUS_GTK_PLACES_OPEN_NEW_TAB) {
    item = gtk_menu_item_new_with_mnemonic(_("Open in New _Tab"));
    g_signal_connect(item, "activate", G_CALLBACK(open_in_new_tab_cb), row);
    gtk_widget_show(item);
    gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
  }
  ...
  if (priv->open_flags & NAUTILUS_GTK_PLACES_OPEN_NEW_WINDOW) {
    ...
    g_signal_connect(item, "activate", G_CALLBACK(open_in_new_window_cb), row);
    gtk_widget_show(item);
    gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
  }
  ...
  /* Mount/Unmount items */
  if (mount) {
    item = gtk_menu_item_new_with_mnemonic(is_network ? _("_Disconnect")
                                                      : _("_Unmount"));
    g_signal_connect(item, "activate", G_CALLBACK(unmount_cb), row);
    gtk_widget_show(item);
    gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
  } else {
    item = gtk_menu_item_new_with_mnemonic(is_network ? _("_Connect")
                                                      : _("_Mount"));
    g_signal_connect(item, "activate", G_CALLBACK(mount_cb), row);
    gtk_widget_show(item);
    gtk_menu_shell_append(GTK_MENU_SHELL(priv->popup_menu), item);
  }
}

static void popup_menu(NautilusGtkPlacesViewRow *row, GdkEventButton *event) {
  NautilusGtkPlacesViewPrivate *priv;
  GtkWidget *view;
...
static gboolean on_button_press_event(NautilusGtkPlacesViewRow *row,
                                      GdkEventButton *event) {
  if (row && gdk_event_triggers_context_menu((GdkEvent *)event) &&
...
static gboolean on_key_press_event(GtkWidget *widget, GdkEventKey *event,
  ...
  if (event) {
    ...
    if (event->keyval == GDK_KEY_Return || event->keyval == GDK_KEY_KP_Enter ||
        event->keyval == GDK_KEY_ISO_Enter || event->keyval == GDK_KEY_space) {
      ...
      activate_row(view, NAUTILUS_GTK_PLACES_VIEW_ROW(focus_widget),
                   priv->current_open_flags);
...
static void on_eject_button_clicked(GtkWidget *widget,
  ...
  if (row) {
    ...
    unmount_mount(NAUTILUS_GTK_PLACES_VIEW(view),
                  nautilus_gtk_places_view_row_get_mount(row));
  }
}

static void on_connect_button_clicked(NautilusGtkPlacesView *view) {
  NautilusGtkPlacesViewPrivate *priv;
  const gchar *uri;
  GFile *file;
  ...
  if (file) {
    ...
    emit_show_error_message(view, _("Unable to get remote server location"),
                            NULL);
  }
}
...
out:
  ...
  if (scheme && !supported)
    gtk_style_context_add_class(
        gtk_widget_get_style_context(priv->address_entry),
...
static void on_address_entry_show_help_pressed(NautilusGtkPlacesView *view,
  ...
  gtk_popover_set_pointing_to(GTK_POPOVER(priv->server_adresses_popover),
                              &rect);
...
static void on_listbox_row_activated(NautilusGtkPlacesView *view,
                                     NautilusGtkPlacesViewRow *row,
                                     GtkWidget *listbox) {
  NautilusGtkPlacesViewPrivate *priv;
  GdkEvent *event;
...
static gboolean is_mount_locally_accessible(GMount *mount) {
  GFile *base_file;
  gchar *path;
...
static gboolean listbox_filter_func(GtkListBoxRow *row, gpointer user_data) {
  NautilusGtkPlacesViewPrivate *priv;
  gboolean is_network;
  ...
  is_placeholder =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row), "is-placeholder"));
  ...
  g_object_get(row, "name", &name, "path", &path, NULL);
...
static void listbox_header_func(GtkListBoxRow *row, GtkListBoxRow *before,
                                gpointer user_data) {
  ...
  text = NULL;
  row_is_network =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row), "is-network"));

  if (!before) {
    text = g_strdup_printf("<b>%s</b>", row_is_network ? _("Networks")
                                                       : _("On This Computer"));
  } else {
    gboolean before_is_network;

    before_is_network =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(before), "is-network"));

    if (before_is_network != row_is_network)
      text = g_strdup_printf(
          "<b>%s</b>", row_is_network ? _("Networks") : _("On This Computer"));
  }
  ...
  if (text) {
    ...
    label = g_object_new(GTK_TYPE_LABEL, "use_markup", TRUE, "margin-start", 12,
                         "label", text, "xalign", 0.0f, NULL);
    if (row_is_network) {
      GtkWidget *header_name;
      GtkWidget *network_header_spinner;

      g_object_set(label, "margin-end", 6, NULL);

      header_name = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
      network_header_spinner = gtk_spinner_new();
      g_object_set(network_header_spinner, "margin-end", 12, NULL);
      g_object_bind_property(NAUTILUS_GTK_PLACES_VIEW(user_data),
                             "fetching-networks", network_header_spinner,
                             "active", G_BINDING_SYNC_CREATE);
      ...
      g_object_set(label, "hexpand", TRUE, "margin-end", 12, NULL);
...
static gint listbox_sort_func(GtkListBoxRow *row1, GtkListBoxRow *row2,
                              gpointer user_data) {
  ...
  row1_is_network =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row1), "is-network"));
  row2_is_network =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row2), "is-network"));
...
static void nautilus_gtk_places_view_constructed(GObject *object) {
  NautilusGtkPlacesViewPrivate *priv;

  priv = nautilus_gtk_places_view_get_instance_private(
      NAUTILUS_GTK_PLACES_VIEW(object));

  G_OBJECT_CLASS(nautilus_gtk_places_view_parent_class)->constructed(object);

  gtk_list_box_set_sort_func(GTK_LIST_BOX(priv->listbox), listbox_sort_func,
                             object, NULL);
  gtk_list_box_set_filter_func(GTK_LIST_BOX(priv->listbox), listbox_filter_func,
                               object, NULL);
  gtk_list_box_set_header_func(GTK_LIST_BOX(priv->listbox), listbox_header_func,
                               object, NULL);
  ...
  g_signal_connect_swapped(priv->volume_monitor, "mount-added",
                           G_CALLBACK(update_places), object);
...
static void nautilus_gtk_places_view_map(GtkWidget *widget) {
  NautilusGtkPlacesViewPrivate *priv;

  priv = nautilus_gtk_places_view_get_instance_private(
      NAUTILUS_GTK_PLACES_VIEW(widget));
...
nautilus_gtk_places_view_class_init(NautilusGtkPlacesViewClass *klass) {
   ...
   * Since: 3.18
   */
  places_view_signals[OPEN_LOCATION] = g_signal_new(
      "open-location", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesViewClass, open_location), NULL, NULL,
   ...
   * Since: 3.18
   */
  places_view_signals[SHOW_ERROR_MESSAGE] = g_signal_new(
      "show-error-message", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesViewClass, show_error_message), NULL,
  ...
  properties[PROP_LOADING] = g_param_spec_boolean(
      "loading", "Loading", "Whether the view is loading locations", FALSE,
      G_PARAM_READABLE);

  properties[PROP_FETCHING_NETWORKS] = g_param_spec_boolean(
      "fetching-networks", "Fetching networks",
      "Whether the view is fetching networks", FALSE, G_PARAM_READABLE);
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/gtk/ui/nautilusgtkplacesview.ui");

  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, actionbar);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, address_entry);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, address_entry_completion);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, completion_store);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, connect_button);
  gtk_widget_class_bind_template_child_private(widget_class,
                                               NautilusGtkPlacesView, listbox);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, recent_servers_listbox);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, recent_servers_popover);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, recent_servers_stack);
  gtk_widget_class_bind_template_child_private(widget_class,
                                               NautilusGtkPlacesView, stack);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, server_adresses_popover);
  gtk_widget_class_bind_template_child_private(
      widget_class, NautilusGtkPlacesView, available_protocols_grid);

  gtk_widget_class_bind_template_callback(widget_class,
                                          on_address_entry_text_changed);
  gtk_widget_class_bind_template_callback(widget_class,
                                          on_address_entry_show_help_pressed);
  gtk_widget_class_bind_template_callback(widget_class,
  ...
  gtk_widget_class_bind_template_callback(
      widget_class, on_recent_servers_listbox_row_activated);

  gtk_widget_class_set_css_name(widget_class, "placesview");
}

static void nautilus_gtk_places_view_init(NautilusGtkPlacesView *self) {
  NautilusGtkPlacesViewPrivate *priv;
