...
G_DEFINE_TYPE(NautilusGtkPlacesViewRow, nautilus_gtk_places_view_row,
...
static void measure_available_space_finished(GObject *object, GAsyncResult *res,
                                             gpointer user_data) {
  ...
  info = g_file_query_filesystem_info_finish(G_FILE(object), res, &error);
   ...
   * form should be based on the free space available. i.e. 1 GB / 24 GB
   * available.
   */
  label = g_strdup_printf(dngettext(GETTEXT_PACKAGE, "%s / %s available",
                                    "%s / %s available", plural_form),
...
static void measure_available_space(NautilusGtkPlacesViewRow *row) {
  gboolean should_measure;

  should_measure =
      (!row->is_network && (row->volume || row->mount || row->file));

  gtk_label_set_label(row->available_space_label, "");
  gtk_widget_set_visible(GTK_WIDGET(row->available_space_label),
                         should_measure);
  ...
  if (should_measure) {
    ...
    if (file) {
      ...
      g_file_query_filesystem_info_async(file,
                                         ...
                                         G_PRIORITY_DEFAULT, row->cancellable,
                                         measure_available_space_finished, row);
    }
  }
}

static void nautilus_gtk_places_view_row_finalize(GObject *object) {
...
static void nautilus_gtk_places_view_row_get_property(GObject *object,
                                                      guint prop_id,
                                                      GValue *value,
                                                      GParamSpec *pspec) {
...
static void nautilus_gtk_places_view_row_set_property(GObject *object,
                                                      guint prop_id,
                                                      const GValue *value,
                                                      GParamSpec *pspec) {
  NautilusGtkPlacesViewRow *self = NAUTILUS_GTK_PLACES_VIEW_ROW(object);
  ...
  case PROP_MOUNT:
    ...
    if (self->mount != NULL) {
      gtk_stack_set_visible_child(self->mount_stack,
  ...
  case PROP_IS_NETWORK:
    nautilus_gtk_places_view_row_set_is_network(self,
...
nautilus_gtk_places_view_row_class_init(NautilusGtkPlacesViewRowClass *klass) {
  ...
  properties[PROP_ICON] = g_param_spec_object(
      "icon", "Icon of the row", "The icon representing the volume",
      G_TYPE_ICON, G_PARAM_READWRITE);

  properties[PROP_NAME] =
      g_param_spec_string("name", "Name of the volume",
                          "The name of the volume", "", G_PARAM_READWRITE);

  properties[PROP_PATH] =
      g_param_spec_string("path", "Path of the volume",
                          "The path of the volume", "", G_PARAM_READWRITE);

  properties[PROP_VOLUME] =
      g_param_spec_object("volume", "Volume represented by the row",
                          "The volume represented by the row", G_TYPE_VOLUME,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  properties[PROP_MOUNT] = g_param_spec_object(
      "mount", "Mount represented by the row",
      "The mount point represented by the row, if any", G_TYPE_MOUNT,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  properties[PROP_FILE] = g_param_spec_object(
      "file", "File represented by the row",
      "The file represented by the row, if any", G_TYPE_FILE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  properties[PROP_IS_NETWORK] = g_param_spec_boolean(
      "is-network", "Whether the row represents a network location",
      "Whether the row represents a network location", FALSE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, LAST_PROP, properties);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/gtk/ui/nautilusgtkplacesviewrow.ui");

  gtk_widget_class_bind_template_child(widget_class, NautilusGtkPlacesViewRow,
...
static void nautilus_gtk_places_view_row_init(NautilusGtkPlacesViewRow *self) {
  gtk_widget_init_template(GTK_WIDGET(self));
}

GtkWidget *nautilus_gtk_places_view_row_new(GVolume *volume, GMount *mount) {
  return g_object_new(NAUTILUS_TYPE_GTK_PLACES_VIEW_ROW, "volume", volume,
                      "mount", mount, NULL);
}
...
void nautilus_gtk_places_view_row_set_busy(NautilusGtkPlacesViewRow *row,
  ...
  if (is_busy) {
    gtk_stack_set_visible_child(row->mount_stack,
                                GTK_WIDGET(row->busy_spinner));
...
void nautilus_gtk_places_view_row_set_is_network(NautilusGtkPlacesViewRow *row,
  ...
  if (row->is_network != is_network) {
    ...
    gtk_widget_set_tooltip_text(GTK_WIDGET(row->eject_button),
                                is_network ? _("Disconnect") : _("Unmount"));
  }
}

void nautilus_gtk_places_view_row_set_path_size_group(
    NautilusGtkPlacesViewRow *row, GtkSizeGroup *group) {
  if (group)
    gtk_size_group_add_widget(group, GTK_WIDGET(row->path_label));
}

void nautilus_gtk_places_view_row_set_space_size_group(
    NautilusGtkPlacesViewRow *row, GtkSizeGroup *group) {
  if (group)
