 ...
 * @Short_description: Sidebar that displays frequently-used places in the file
 * system
 * @Title: NautilusGtkPlacesSidebar
 * @See_also: #GtkFileChooser
 *
 * #NautilusGtkPlacesSidebar is a widget that displays a list of frequently-used
 * places in the file system:  the user’s home directory, the user’s bookmarks,
 * and volumes and drives. This widget is used as a sidebar in #GtkFileChooser
 ...
 * to the #NautilusGtkPlacesSidebar::open-location signal.  This is emitted when
 * the user selects in the sidebar a location to open.  The application should
 * also call nautilus_gtk_places_sidebar_set_location() when it changes the
 * currently-viewed location.
 *
 * # CSS nodes
 *
 * NautilusGtkPlacesSidebar uses a single CSS node with name placessidebar and
 * style class .sidebar.
 *
 ...
 * DROP_STATE_NEW_BOOKMARK_ARMED, however, if the client of
 * NautilusGtkPlacesSidebar wants to show hints about the valid targets, we sill
 * set it as DROP_STATE_NEW_BOOKMARK_ARMED_PERMANENT, so the sidebar will show
 * drop hints until the client says otherwise
 */
typedef enum {
  DROP_STATE_NORMAL,
...
struct _NautilusGtkPlacesSidebarClass {
  ...
  void (*open_location)(NautilusGtkPlacesSidebar *sidebar, GFile *location,
                        NautilusGtkPlacesOpenFlags open_flags);
  void (*populate_popup)(NautilusGtkPlacesSidebar *sidebar, GtkMenu *menu,
                         GFile *selected_item, GVolume *selected_volume);
  ...
  void (*drag_perform_drop)(NautilusGtkPlacesSidebar *sidebar, GFile *dest_file,
                            GList *source_file_list, GdkDragAction action);
  void (*show_enter_location)(NautilusGtkPlacesSidebar *sidebar);

  void (*show_other_locations)(NautilusGtkPlacesSidebar *sidebar);

  void (*show_other_locations_with_flags)(
      NautilusGtkPlacesSidebar *sidebar, NautilusGtkPlacesOpenFlags open_flags);
...
static void check_unmount_and_eject(GMount *mount, GVolume *volume,
                                    GDrive *drive, gboolean *show_unmount,
                                    gboolean *show_eject);
...
static void long_press_cb(GtkGesture *gesture, gdouble x, gdouble y,
...
enum { DND_UNKNOWN, DND_NAUTILUS_GTK_SIDEBAR_ROW, DND_TEXT_URI_LIST };

/* Target types for dragging from the shortcuts list */
static const GtkTargetEntry dnd_source_targets[] = {
    {"DND_NAUTILUS_GTK_SIDEBAR_ROW", GTK_TARGET_SAME_WIDGET,
...
G_DEFINE_TYPE(NautilusGtkPlacesSidebar, nautilus_gtk_places_sidebar,
              GTK_TYPE_SCROLLED_WINDOW);

static void emit_open_location(NautilusGtkPlacesSidebar *sidebar,
                               GFile *location,
                               NautilusGtkPlacesOpenFlags open_flags) {
  if ((open_flags & sidebar->open_flags) == 0)
    open_flags = NAUTILUS_GTK_PLACES_OPEN_NORMAL;

  g_signal_emit(sidebar, places_sidebar_signals[OPEN_LOCATION], 0, location,
                open_flags);
}

static void emit_show_error_message(NautilusGtkPlacesSidebar *sidebar,
                                    const gchar *primary,
                                    const gchar *secondary) {
  g_signal_emit(sidebar, places_sidebar_signals[SHOW_ERROR_MESSAGE], 0, primary,
                secondary);
}

static void emit_show_connect_to_server(NautilusGtkPlacesSidebar *sidebar) {
  g_signal_emit(sidebar, places_sidebar_signals[SHOW_CONNECT_TO_SERVER], 0);
}

static void emit_show_enter_location(NautilusGtkPlacesSidebar *sidebar) {
  g_signal_emit(sidebar, places_sidebar_signals[SHOW_ENTER_LOCATION], 0);
}

static void emit_show_other_locations(NautilusGtkPlacesSidebar *sidebar) {
...
emit_show_other_locations_with_flags(NautilusGtkPlacesSidebar *sidebar,
  ...
  g_signal_emit(sidebar,
                places_sidebar_signals[SHOW_OTHER_LOCATIONS_WITH_FLAGS], 0,
                open_flags);
}

static void emit_show_starred_location(NautilusGtkPlacesSidebar *sidebar,
                                       NautilusGtkPlacesOpenFlags open_flags) {
  g_signal_emit(sidebar, places_sidebar_signals[SHOW_STARRED_LOCATION], 0,
                open_flags);
}

static void emit_mount_operation(NautilusGtkPlacesSidebar *sidebar,
                                 GMountOperation *mount_op) {
  g_signal_emit(sidebar, places_sidebar_signals[MOUNT], 0, mount_op);
}

static void emit_unmount_operation(NautilusGtkPlacesSidebar *sidebar,
                                   GMountOperation *mount_op) {
...
emit_drag_action_requested(NautilusGtkPlacesSidebar *sidebar,
                           GdkDragContext *context, GFile *dest_file,
                           GList *source_file_list) {
...
static GdkDragAction emit_drag_action_ask(NautilusGtkPlacesSidebar *sidebar,
                                          GdkDragAction actions) {
  GdkDragAction ret_action = 0;

  g_signal_emit(sidebar, places_sidebar_signals[DRAG_ACTION_ASK], 0, actions,
                &ret_action);

  return ret_action;
}

static void emit_drag_perform_drop(NautilusGtkPlacesSidebar *sidebar,
                                   GFile *dest_file, GList *source_file_list,
                                   GdkDragAction action) {
  g_signal_emit(sidebar, places_sidebar_signals[DRAG_PERFORM_DROP], 0,
                dest_file, source_file_list, action);
}
static void list_box_header_func(GtkListBoxRow *row, GtkListBoxRow *before,
                                 gpointer user_data) {
...
static GtkWidget *add_place(NautilusGtkPlacesSidebar *sidebar,
                            NautilusGtkPlacesSidebarPlaceType place_type,
                            NautilusGtkPlacesSidebarSectionType section_type,
                            const gchar *name, GIcon *start_icon,
                            GIcon *end_icon, const gchar *uri, GDrive *drive,
...
#endif
                            const gint index, const gchar *tooltip) {
  ...
  row = g_object_new(NAUTILUS_TYPE_GTK_SIDEBAR_ROW, "sidebar", sidebar,
                     "start-icon", start_icon, "end-icon", end_icon, "label",
                     name, "tooltip", tooltip, "ejectable", show_eject_button,
                     "order-index", index, "section-type", section_type,
                     "place-type", place_type, "uri", uri, "drive", drive,
  ...
  eject_button =
      nautilus_gtk_sidebar_row_get_eject_button(NAUTILUS_GTK_SIDEBAR_ROW(row));
  event_box =
      nautilus_gtk_sidebar_row_get_event_box(NAUTILUS_GTK_SIDEBAR_ROW(row));
...
static GIcon *special_directory_get_gicon(GUserDirectory directory) {
#define ICON_CASE(x)                                                           \
...
static gboolean recent_scheme_is_supported(void) {
...
static gboolean should_show_recent(NautilusGtkPlacesSidebar *sidebar) {
...
static gboolean path_is_home_dir(const gchar *path) {
  GFile *home_dir;
  GFile *location;
...
static void open_home(NautilusGtkPlacesSidebar *sidebar) {
  const gchar *home_path;
  GFile *home_dir;
...
static void add_special_dirs(NautilusGtkPlacesSidebar *sidebar) {
  GList *dirs;
  gint index;
  ...
  for (index = 0; index < G_USER_N_DIRECTORIES; index++) {
    ...
    name = _nautilus_gtk_bookmarks_manager_get_bookmark_label(
    ...
    add_place(sidebar, PLACES_XDG_DIR, SECTION_COMPUTER, name, start_icon, NULL,
              mount_uri, NULL, NULL, NULL, NULL, 0, tooltip);
    g_free(name);
    g_object_unref(root);
...
static gchar *get_home_directory_uri(void) {
  const gchar *home;
...
static gchar *get_desktop_directory_uri(void) {
  const gchar *name;
...
static gboolean should_show_file(NautilusGtkPlacesSidebar *sidebar,
                                 GFile *file) {
  gchar *path;
...
static gboolean file_is_shown(NautilusGtkPlacesSidebar *sidebar, GFile *file) {
  gchar *uri;
  GList *rows;
  GList *l;
...
static void on_app_shortcuts_query_complete(GObject *source,
                                            GAsyncResult *result,
                                            gpointer data) {
  ...
  if (info) {
    ...
    add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, name, start_icon,
              NULL, uri, NULL, NULL, NULL, NULL, pos, tooltip);
...
static void add_application_shortcuts(NautilusGtkPlacesSidebar *sidebar) {
  GSList *l;
  ...
  for (l = sidebar->shortcuts; l; l = l->next) {
    ...
    g_file_query_info_async(
        file, "standard::display-name,standard::symbolic-icon",
        G_FILE_QUERY_INFO_NONE, G_PRIORITY_DEFAULT, sidebar->cancellable,
        on_app_shortcuts_query_complete, sidebar);
  }
}
...
static void on_bookmark_query_info_complete(GObject *source,
                                            GAsyncResult *result,
                                            gpointer data) {
  ...
  bookmark_name = _nautilus_gtk_bookmarks_manager_get_bookmark_label(
      sidebar->bookmarks_manager, root);
  ...
  if (info)
    ...
    start_icon = g_themed_icon_new_with_default_fallbacks(
        clos->is_native ? ICON_NAME_FOLDER : ICON_NAME_FOLDER_NETWORK);

  mount_uri = g_file_get_uri(root);
  tooltip = g_file_get_parse_name(root);

  add_place(sidebar, PLACES_BOOKMARK, SECTION_BOOKMARKS, bookmark_name,
            start_icon, NULL, mount_uri, NULL, NULL, NULL, NULL, clos->index,
            tooltip);
...
static gboolean is_external_volume(GVolume *volume) {
...
static void update_trash_icon(NautilusGtkPlacesSidebar *sidebar) {
  if (sidebar->trash_row) {
    GIcon *icon;

    icon = nautilus_trash_monitor_get_symbolic_icon();
    nautilus_gtk_sidebar_row_set_start_icon(
        NAUTILUS_GTK_SIDEBAR_ROW(sidebar->trash_row), icon);
    g_object_unref(icon);
  }
}
...
create_cloud_provider_account_row(NautilusGtkPlacesSidebar *sidebar,
  ...
  if (start_icon != NULL && name != NULL &&
    ...
    add_place(sidebar, PLACES_BUILT_IN, SECTION_CLOUD, name, start_icon,
              end_icon, mount_uri, NULL, NULL, NULL, account, 0, tooltip);
...
static void on_account_updated(GObject *object, GParamSpec *pspec,
                               gpointer user_data) {
  ...
  if (create_cloud_provider_account_row(sidebar, account)) {
    ...
    sidebar->unready_accounts =
        g_list_remove(sidebar->unready_accounts, account);
...
static void update_places(NautilusGtkPlacesSidebar *sidebar) {
  GList *mounts, *l, *ll;
  GMount *mount;
  GList *drives;
  GDrive *drive;
  ...
  if (should_show_recent(sidebar)) {
    ...
    add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, _("Recent"),
              start_icon, NULL, mount_uri, NULL, NULL, NULL, NULL, 0,
              _("Recent files"));
    g_object_unref(start_icon);
  }
  ...
  if (sidebar->show_starred_location) {
    ...
    add_place(sidebar, PLACES_STARRED_LOCATION, SECTION_COMPUTER, _("Starred"),
              start_icon, NULL, mount_uri, NULL, NULL, NULL, NULL, 0,
  ...
  add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, _("Home"), start_icon,
            NULL, home_uri, NULL, NULL, NULL, NULL, 0,
  ...
  if (sidebar->show_desktop) {
    ...
    if (mount_uri) {
      ...
      add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, _("Desktop"),
                start_icon, NULL, mount_uri, NULL, NULL, NULL, NULL, 0,
  ...
  if (sidebar->show_enter_location) {
    start_icon =
        g_themed_icon_new_with_default_fallbacks(ICON_NAME_NETWORK_SERVER);
    add_place(sidebar, PLACES_ENTER_LOCATION, SECTION_COMPUTER,
              _("Enter Location"), start_icon, NULL, NULL, NULL, NULL, NULL,
              NULL, 0, _("Manually enter a location"));
    g_object_unref(start_icon);
  }
  ...
  if (!sidebar->local_only && sidebar->show_trash) {
    ...
    sidebar->trash_row = add_place(
        sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, _("Trash"), start_icon,
        NULL, "trash:///", NULL, NULL, NULL, NULL, 0, _("Open the trash"));
...
#ifdef HAVE_CLOUDPROVIDERS
  cloud_providers =
      cloud_providers_collector_get_providers(sidebar->cloud_manager);
  ...
  for (l = cloud_providers; l != NULL; l = l->next) {
    ...
    cloud_providers_accounts =
        cloud_providers_provider_get_accounts(cloud_provider);
    for (ll = cloud_providers_accounts; ll != NULL; ll = ll->next) {
      ...
      if (!create_cloud_provider_account_row(sidebar, cloud_provider_account)) {
        ...
        g_signal_connect(cloud_provider_account, "notify::path",
                         G_CALLBACK(on_account_updated), sidebar);
        sidebar->unready_accounts = g_list_append(
            sidebar->unready_accounts, g_object_ref(cloud_provider_account));
        continue;
      }
    }
  }
    ...
    if (volumes != NULL) {
      for (ll = volumes; ll != NULL; ll = ll->next) {
        ...
        if (mount != NULL) {
          ...
          add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                    start_icon, NULL, mount_uri, drive, volume, mount, NULL, 0,
                    tooltip);
          g_object_unref(root);
          ...
          add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                    start_icon, NULL, NULL, drive, volume, NULL, NULL, 0,
                    tooltip);
          g_object_unref(start_icon);
          g_free(name);
          g_free(tooltip);
      ...
      if (g_drive_is_media_removable(drive) &&
        ...
        /* If the drive has no mountable volumes and we cannot detect media
         * change.. we display the drive in the sidebar so the user can manually
         * poll the drive by right clicking and selecting "Rescan..."
         *
         * This is mainly for drives like floppies where media detection doesn't
         * work.. but it's also for human beings who like to turn off media
         * detection in the OS to save battery juice.
         */
        start_icon = g_drive_get_symbolic_icon(drive);
        name = g_drive_get_name(drive);
        tooltip = g_strdup_printf(_("Mount and open “%s”"), name);

        add_place(sidebar, PLACES_BUILT_IN, SECTION_MOUNTS, name, start_icon,
                  NULL, NULL, drive, NULL, NULL, NULL, 0, tooltip);
        g_object_unref(start_icon);
        g_free(tooltip);
        g_free(name);
      }
  ...
  for (l = volumes; l != NULL; l = l->next) {
    ...
    if (mount != NULL) {
      ...
      name = g_mount_get_name(mount);
      add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                start_icon, NULL, mount_uri, NULL, volume, mount, NULL, 0,
                tooltip);
      g_object_unref(mount);
      g_object_unref(root);
      g_object_unref(start_icon);
      ...
      /* see comment above in why we add an icon for an unmounted mountable
       * volume */
      start_icon = g_volume_get_symbolic_icon(volume);
      name = g_volume_get_name(volume);
      add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                start_icon, NULL, NULL, NULL, volume, NULL, NULL, 0, name);
      g_object_unref(start_icon);
      g_free(name);
    }
    g_object_unref(volume);
  ...
  if (!sidebar->show_other_locations) {
    ...
    add_place(sidebar, PLACES_BUILT_IN, SECTION_MOUNTS, sidebar->hostname,
              start_icon, NULL, mount_uri, NULL, NULL, NULL, NULL, 0,
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    name = g_mount_get_name(mount);
    tooltip = g_file_get_parse_name(root);
    add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_COMPUTER, name,
              start_icon, NULL, mount_uri, NULL, NULL, mount, NULL, 0, tooltip);
    g_object_unref(root);
    g_object_unref(mount);
    g_object_unref(start_icon);
    g_free(name);
  ...
  /* add bookmarks */
  bookmarks = _nautilus_gtk_bookmarks_manager_list_bookmarks(
      sidebar->bookmarks_manager);
  ...
  for (sl = bookmarks, index = 0; sl; sl = sl->next, index++) {
    ...
    if (_nautilus_gtk_bookmarks_manager_get_is_builtin(
            sidebar->bookmarks_manager, root))
      continue;
    ...
    g_file_query_info_async(
        root, "standard::display-name,standard::symbolic-icon",
        G_FILE_QUERY_INFO_NONE, G_PRIORITY_DEFAULT, sidebar->cancellable,
        on_bookmark_query_info_complete, clos);
  }
  ...
  sidebar->new_bookmark_row =
      add_place(sidebar, PLACES_DROP_FEEDBACK, SECTION_BOOKMARKS,
                _("New bookmark"), new_bookmark_icon, NULL, NULL, NULL, NULL,
                NULL, NULL, 0, _("Add a new bookmark"));
  ...
  if (!sidebar->local_only) {
    if (sidebar->show_connect_to_server) {
      start_icon =
          g_themed_icon_new_with_default_fallbacks(ICON_NAME_NETWORK_SERVER);
      add_place(sidebar, PLACES_CONNECT_TO_SERVER, SECTION_MOUNTS,
                _("Connect to Server"), start_icon, NULL, NULL, NULL, NULL,
                NULL, NULL, 0, _("Connect to a network server address"));
      g_object_unref(start_icon);
    }
    ...
    for (l = network_volumes; l != NULL; l = l->next) {
      ...
      if (mount != NULL) {
        ...
        add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                  start_icon, NULL, NULL, NULL, volume, NULL, NULL, 0, tooltip);
        g_object_unref(start_icon);
        g_free(name);
        g_free(tooltip);
      }
    ...
    for (l = network_mounts; l != NULL; l = l->next) {
      ...
      tooltip = g_file_get_parse_name(root);
      add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_MOUNTS, name,
                start_icon, NULL, mount_uri, NULL, NULL, mount, NULL, 0,
                tooltip);
      g_object_unref(root);
      g_object_unref(start_icon);
      g_free(name);
  ...
  if (sidebar->show_other_locations) {
    start_icon =
        g_themed_icon_new_with_default_fallbacks(ICON_NAME_OTHER_LOCATIONS);

    add_place(sidebar, PLACES_OTHER_LOCATIONS, SECTION_OTHER_LOCATIONS,
   ...
   */
  nautilus_gtk_sidebar_row_hide(
      NAUTILUS_GTK_SIDEBAR_ROW(sidebar->new_bookmark_row), TRUE);
...
static gboolean check_valid_drop_target(NautilusGtkPlacesSidebar *sidebar,
  ...
  g_object_get(row, "place-type", &place_type, "section_type", &section_type,
               "uri", &uri, NULL);
  ...
  if (sidebar->drag_data_received &&
    ...
    if (context) {
      if (uri != NULL) {
        ...
        drag_action = emit_drag_action_requested(sidebar, context, dest_file,
                                                 sidebar->drag_list);
        valid = drag_action > 0;
...
static void update_possible_drop_targets(NautilusGtkPlacesSidebar *sidebar,
                                         gboolean dragging,
                                         GdkDragContext *context) {
  GList *rows;
  GList *l;
  ...
  for (l = rows; l != NULL; l = l->next) {
    sensitive =
        !dragging || check_valid_drop_target(
                         sidebar, NAUTILUS_GTK_SIDEBAR_ROW(l->data), context);
...
static gboolean get_drag_data(GtkWidget *list_box, GdkDragContext *context,
                              guint time) {
  GdkAtom target;
...
static void free_drag_data(NautilusGtkPlacesSidebar *sidebar) {
...
static void start_drop_feedback(NautilusGtkPlacesSidebar *sidebar,
                                NautilusGtkSidebarRow *row,
                                GdkDragContext *context) {
  if (sidebar->drag_data_info != DND_NAUTILUS_GTK_SIDEBAR_ROW) {
    nautilus_gtk_sidebar_row_reveal(
        NAUTILUS_GTK_SIDEBAR_ROW(sidebar->new_bookmark_row));
...
static void stop_drop_feedback(NautilusGtkPlacesSidebar *sidebar) {
  ...
  if (sidebar->drop_state != DROP_STATE_NEW_BOOKMARK_ARMED_PERMANENT &&
    ...
    nautilus_gtk_sidebar_row_hide(
        NAUTILUS_GTK_SIDEBAR_ROW(sidebar->new_bookmark_row), FALSE);
...
static gboolean on_motion_notify_event(GtkWidget *widget, GdkEventMotion *event,
  ...
  if (gtk_drag_check_threshold(widget, sidebar->drag_root_x,
                               sidebar->drag_root_y, event->x_root,
                               event->y_root)) {
    sidebar->dragging_over = TRUE;

    gtk_drag_begin_with_coordinates(widget, sidebar->source_targets,
                                    GDK_ACTION_MOVE, GDK_BUTTON_PRIMARY,
                                    (GdkEvent *)event, -1, -1);
  }
...
static void drag_begin_callback(GtkWidget *widget, GdkDragContext *context,
                                gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar = NAUTILUS_GTK_PLACES_SIDEBAR(user_data);
  ...
  drag_widget = GTK_WIDGET(nautilus_gtk_sidebar_row_clone(
      NAUTILUS_GTK_SIDEBAR_ROW(sidebar->drag_row)));
  ...
  gtk_drag_set_icon_widget(context, window, sidebar->drag_row_x,
                           sidebar->drag_row_y);
}

static GtkWidget *create_placeholder_row(NautilusGtkPlacesSidebar *sidebar) {
  return g_object_new(NAUTILUS_TYPE_GTK_SIDEBAR_ROW, "placeholder", TRUE, NULL);
}

static gboolean drag_motion_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  if (sidebar->drag_data_received &&
    ...
    if (row != NULL) {
      ...
      g_object_get(sidebar->row_placeholder, "order-index",
                   &row_placeholder_index, NULL);
       ...
       * For that, in the list box sort func we give priority to the placeholder
       * row, that means that if the index-order is the same as another bookmark
      ...
      gtk_widget_translate_coordinates(widget, GTK_WIDGET(row), x, y, &dest_x,
                                       &dest_y);
    ...
    g_object_set(sidebar->row_placeholder, "order-index", row_placeholder_index,
                 NULL);
    ...
    g_object_get(row, "place-type", &place_type, "uri", &drop_target_uri, NULL);
    ...
    if (sidebar->drag_list != NULL) {
      if (place_type == PLACES_DROP_FEEDBACK) {
        ...
        if (drop_target_uri != NULL) {
          ...
          action = emit_drag_action_requested(sidebar, context, dest_file,
                                              sidebar->drag_list);
...
static GList *build_file_list_from_uris(const gchar **uris) {
  GList *result;
  gint i;
...
static void reorder_bookmarks(NautilusGtkPlacesSidebar *sidebar,
                              NautilusGtkSidebarRow *row, gint new_position) {
  gchar *uri;
  GFile *file;

  g_object_get(row, "uri", &uri, NULL);
  file = g_file_new_for_uri(uri);
  _nautilus_gtk_bookmarks_manager_reorder_bookmark(sidebar->bookmarks_manager,
                                                   file, new_position, NULL);

  g_object_unref(file);
  g_free(uri);
}

/* Creates bookmarks for the specified files at the given position in the
 * bookmarks list */
static void drop_files_as_bookmarks(NautilusGtkPlacesSidebar *sidebar,
                                    GList *files, gint position) {
  GList *l;

  for (l = files; l; l = l->next) {
    GFile *f = G_FILE(l->data);
    GFileInfo *info =
        g_file_query_info(f, G_FILE_ATTRIBUTE_STANDARD_TYPE,
    ...
    if (info) {
      if ((g_file_info_get_file_type(info) == G_FILE_TYPE_DIRECTORY ||
        ...
        _nautilus_gtk_bookmarks_manager_insert_bookmark(
            sidebar->bookmarks_manager, f, position++, NULL);
...
static void drag_data_get_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  if (target == gdk_atom_intern_static_string("DND_NAUTILUS_GTK_SIDEBAR_ROW")) {
    gtk_selection_data_set(data, target, 8, (void *)&sidebar->drag_row,
                           sizeof(gpointer));
  }
}

static void drag_data_received_callback(GtkWidget *list_box,
  ...
  g_object_get(target_row, "place-type", &target_place_type, "section-type",
               &target_section_type, "order-index", &target_order_index, "uri",
               &target_uri, NULL);

  success = FALSE;

  if (!check_valid_drop_target(sidebar, NAUTILUS_GTK_SIDEBAR_ROW(target_row),
                               context))
    goto out;
  ...
  if (sidebar->drag_data_info == DND_NAUTILUS_GTK_SIDEBAR_ROW) {
    ...
    if (sidebar->row_placeholder != NULL)
      g_object_get(sidebar->row_placeholder, "order-index", &target_order_index,
                   NULL);

    reorder_bookmarks(sidebar, NAUTILUS_GTK_SIDEBAR_ROW(*source_row),
                      target_order_index);
    success = TRUE;
  } else {
    /* Dropping URIs! */
    ...
    if (real_action == GDK_ACTION_ASK)
      real_action =
          emit_drag_action_ask(sidebar, gdk_drag_context_get_actions(context));
    ...
    if (real_action > 0) {
      ...
      if (target_place_type == PLACES_DROP_FEEDBACK) {
        ...
        emit_drag_perform_drop(sidebar, dest_file, source_file_list,
                               real_action);
...
static void drag_end_callback(GtkWidget *widget, GdkDragContext *context,
                              gpointer user_data) {
 ...
 * nautilus_gtk_places_sidebar_show_drop_hints) and we will free the drag data
 * next time we build new drag data in drag_data_received.
 */
static void drag_leave_callback(GtkWidget *widget, GdkDragContext *context,
                                guint time, gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar = NAUTILUS_GTK_PLACES_SIDEBAR(user_data);
...
static gboolean drag_drop_callback(GtkWidget *list_box, GdkDragContext *context,
                                   gint x, gint y, guint time,
                                   gpointer user_data) {
...
static void check_visibility(GMount *mount, GVolume *volume, GDrive *drive,
                             gboolean *show_mount, gboolean *show_unmount,
                             gboolean *show_eject, gboolean *show_rescan,
                             gboolean *show_start, gboolean *show_stop) {
  *show_mount = FALSE;
  *show_rescan = FALSE;
  *show_start = FALSE;
  *show_stop = FALSE;
...
static void check_popover_sensitivity(NautilusGtkSidebarRow *row,
                                      PopoverData *data) {
  gboolean show_mount;
  ...
  g_object_get(row, "sidebar", &sidebar, "place-type", &type, "drive", &drive,
               "volume", &volume, "mount", &mount, NULL);

  gtk_widget_set_visible(data->add_shortcut_item,
                         (type == PLACES_MOUNTED_VOLUME));
  ...
  g_simple_action_set_enabled(
      ...
      (type == PLACES_BOOKMARK || type == PLACES_XDG_DIR));
  action = g_action_map_lookup_action(G_ACTION_MAP(actions), "open");
  g_simple_action_set_enabled(
      G_SIMPLE_ACTION(action),
      !gtk_list_box_row_is_selected(GTK_LIST_BOX_ROW(row)));

  check_visibility(mount, volume, drive, &show_mount, &show_unmount,
                   &show_eject, &show_rescan, &show_start, &show_stop);

  gtk_widget_set_visible(data->separator_item,
                         show_mount || show_unmount || show_eject);
  ...
  if ((show_start || show_stop) && drive != NULL) {
    ...
    case G_DRIVE_START_STOP_TYPE_MULTIDISK:
      g_object_set(data->start_item, "text", _("_Start Multi-disk Device"),
                   NULL);
...
static void volume_mount_cb(GObject *source_object, GAsyncResult *result,
                            gpointer user_data) {
  ...
  if (mount != NULL) {
    ...
    emit_open_location(sidebar, location,
                       sidebar->go_to_after_mount_open_flags);
...
static void mount_volume(NautilusGtkSidebarRow *row, GVolume *volume) {
  NautilusGtkPlacesSidebar *sidebar;
  GMountOperation *mount_op;
...
static void open_drive(NautilusGtkSidebarRow *row, GDrive *drive,
                       NautilusGtkPlacesOpenFlags open_flags) {
  NautilusGtkPlacesSidebar *sidebar;
  ...
  if (drive != NULL &&
    ...
    g_drive_start(drive, G_DRIVE_START_NONE, mount_op, NULL,
                  drive_start_from_bookmark_cb, NULL);
    g_object_unref(mount_op);
  }
}

static void open_volume(NautilusGtkSidebarRow *row, GVolume *volume,
                        NautilusGtkPlacesOpenFlags open_flags) {
  NautilusGtkPlacesSidebar *sidebar;
...
static void open_uri(NautilusGtkPlacesSidebar *sidebar, const gchar *uri,
                     NautilusGtkPlacesOpenFlags open_flags) {
  GFile *location;
...
static void open_row(NautilusGtkSidebarRow *row,
                     NautilusGtkPlacesOpenFlags open_flags) {
  gchar *uri;
  GDrive *drive;
  GVolume *volume;
  ...
  g_object_get(row, "sidebar", &sidebar, "uri", &uri, "place-type", &place_type,
               "drive", &drive, "volume", &volume, NULL);
...
static void open_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                             gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
...
static void add_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                            gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  gchar *uri;
  gchar *name;
  ...
  g_object_get(sidebar->context_row, "uri", &uri, "label", &name, NULL);

  if (uri != NULL) {
    location = g_file_new_for_uri(uri);
    if (_nautilus_gtk_bookmarks_manager_insert_bookmark(
            sidebar->bookmarks_manager, location, -1, NULL))
      _nautilus_gtk_bookmarks_manager_set_bookmark_label(
          sidebar->bookmarks_manager, location, name, NULL);
    g_object_unref(location);
  }
...
static void rename_entry_changed(GtkEntry *entry,
                                 NautilusGtkPlacesSidebar *sidebar) {
  NautilusGtkPlacesSidebarPlaceType type;
  gchar *name;
  gchar *uri;
  ...
  for (l = rows; l && !found; l = l->next) {
    g_object_get(l->data, "place-type", &type, "uri", &uri, "label", &name,
                 NULL);

    if ((type == PLACES_XDG_DIR || type == PLACES_BOOKMARK) &&
        strcmp(uri, sidebar->rename_uri) != 0 && strcmp(new_name, name) == 0)
      found = TRUE;
...
static void do_rename(GtkButton *button, NautilusGtkPlacesSidebar *sidebar) {
  gchar *new_text;
  GFile *file;
  ...
  if (!_nautilus_gtk_bookmarks_manager_has_bookmark(sidebar->bookmarks_manager,
                                                    file))
    _nautilus_gtk_bookmarks_manager_insert_bookmark(sidebar->bookmarks_manager,
                                                    file, -1, NULL);

  _nautilus_gtk_bookmarks_manager_set_bookmark_label(sidebar->bookmarks_manager,
                                                     file, new_text, NULL);
...
static void on_rename_popover_destroy(GtkWidget *rename_popover,
                                      NautilusGtkPlacesSidebar *sidebar) {
  if (sidebar) {
...
static void create_rename_popover(NautilusGtkPlacesSidebar *sidebar) {
  GtkWidget *popover;
  GtkWidget *grid;
  GtkWidget *label;
  GtkWidget *entry;
  ...
  g_signal_connect(popover, "destroy", G_CALLBACK(on_rename_popover_destroy),
                   sidebar);
  gtk_popover_set_position(GTK_POPOVER(popover), GTK_POS_RIGHT);
  grid = gtk_grid_new();
  gtk_container_add(GTK_CONTAINER(popover), grid);
  g_object_set(grid, "margin", 10, "row-spacing", 6, "column-spacing", 6, NULL);
  entry = gtk_entry_new();
 ...
 * use a style class.
 */
static void update_popover_shadowing(GtkWidget *row, gboolean shown) {
  GtkStyleContext *context;
  gint count;
...
static void show_rename_popover(NautilusGtkSidebarRow *row) {
  ...
  gtk_popover_set_relative_to(GTK_POPOVER(sidebar->rename_popover),
                              GTK_WIDGET(row));
...
static void rename_bookmark(NautilusGtkSidebarRow *row) {
  NautilusGtkPlacesSidebarPlaceType type;
...
static void rename_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                               gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
...
static void remove_bookmark(NautilusGtkSidebarRow *row) {
  NautilusGtkPlacesSidebarPlaceType type;
  gchar *uri;
  GFile *file;
  NautilusGtkPlacesSidebar *sidebar;

  g_object_get(row, "sidebar", &sidebar, "place-type", &type, "uri", &uri,
               NULL);

  if (type == PLACES_BOOKMARK) {
    file = g_file_new_for_uri(uri);
    _nautilus_gtk_bookmarks_manager_remove_bookmark(sidebar->bookmarks_manager,
                                                    file, NULL);
    g_object_unref(file);
  }
...
static void remove_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                               gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
...
static void mount_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                              gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GVolume *volume;
...
static void unmount_mount_cb(GObject *source_object, GAsyncResult *result,
                             gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar = NAUTILUS_GTK_PLACES_SIDEBAR(user_data);
  GMount *mount;
  GError *error;
 ...
 * same path
 */
static gboolean file_prefix_or_same(GFile *file1, GFile *file2) {
  return g_file_has_prefix(file1, file2) || g_file_equal(file1, file2);
}

static gboolean is_current_location_on_volume(NautilusGtkPlacesSidebar *sidebar,
                                              GMount *mount, GVolume *volume,
                                              GDrive *drive) {
  ...
  if (sidebar->current_location != NULL) {
    if (mount != NULL) {
      ...
      current_location_on_volume = file_prefix_or_same(
     ...
     */
      ...
      for (volume_for_drive = volumes_for_drive; volume_for_drive != NULL;
           volume_for_drive = volume_for_drive->next) {
        mount_for_volume = g_volume_get_mount(volume_for_drive->data);
        if (mount_for_volume != NULL) {
          mount_default_location =
              g_mount_get_default_location(mount_for_volume);
          current_location_on_volume = file_prefix_or_same(
              sidebar->current_location, mount_default_location);
...
static void do_unmount(GMount *mount, NautilusGtkPlacesSidebar *sidebar) {
  if (mount != NULL) {
    GMountOperation *mount_op;
    ...
    g_mount_unmount_with_operation(mount, 0, mount_op, NULL, unmount_mount_cb,
                                   g_object_ref(sidebar));
    g_object_unref(mount_op);
  }
}

static void unmount_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                                gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GMount *mount;

  g_object_get(sidebar->context_row, "mount", &mount, NULL);
...
static void drive_stop_cb(GObject *source_object, GAsyncResult *res,
                          gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar;
  GError *error;
  gchar *primary;
...
static void drive_eject_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar;
  GError *error;
  gchar *primary;
  ...
  error = NULL;
  if (!g_drive_eject_with_operation_finish(G_DRIVE(source_object), res,
                                           &error)) {
    if (error->code != G_IO_ERROR_FAILED_HANDLED) {
      name = g_drive_get_name(G_DRIVE(source_object));
...
static void volume_eject_cb(GObject *source_object, GAsyncResult *res,
                            gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar;
  GError *error;
  gchar *primary;
  ...
  error = NULL;
  if (!g_volume_eject_with_operation_finish(G_VOLUME(source_object), res,
                                            &error)) {
    if (error->code != G_IO_ERROR_FAILED_HANDLED) {
      name = g_volume_get_name(G_VOLUME(source_object));
      primary = g_strdup_printf(_("Unable to eject %s"), name);
...
static void mount_eject_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar;
  GError *error;
  gchar *primary;
  ...
  error = NULL;
  if (!g_mount_eject_with_operation_finish(G_MOUNT(source_object), res,
                                           &error)) {
    if (error->code != G_IO_ERROR_FAILED_HANDLED) {
...
static void do_eject(GMount *mount, GVolume *volume, GDrive *drive,
                     NautilusGtkPlacesSidebar *sidebar) {
  GMountOperation *mount_op;
...
static void eject_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                              gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GMount *mount;
  GVolume *volume;
  ...
  g_object_get(sidebar->context_row, "mount", &mount, "volume", &volume,
               "drive", &drive, NULL);
...
static gboolean eject_or_unmount_bookmark(NautilusGtkSidebarRow *row) {
  ...
  g_object_get(row, "sidebar", &sidebar, "mount", &mount, "volume", &volume,
               "drive", &drive, NULL);
  ret = FALSE;
...
static void rescan_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                               gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GDrive *drive;

  g_object_get(sidebar->context_row, "drive", &drive, NULL);
...
static void drive_start_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  NautilusGtkPlacesSidebar *sidebar;
  GError *error;
  gchar *primary;
...
static void start_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                              gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GDrive *drive;
  ...
  if (drive != NULL) {
    ...
    g_drive_start(drive, G_DRIVE_START_NONE, mount_op, NULL, drive_start_cb,
                  g_object_ref(sidebar));
...
static void stop_shortcut_cb(GSimpleAction *action, GVariant *parameter,
                             gpointer data) {
  NautilusGtkPlacesSidebar *sidebar = data;
  GDrive *drive;
...
static void add_actions(NautilusGtkPlacesSidebar *sidebar) {
  GActionGroup *actions;

  actions = G_ACTION_GROUP(g_simple_action_group_new());
  g_action_map_add_action_entries(G_ACTION_MAP(actions), entries,
                                  G_N_ELEMENTS(entries), sidebar);
  gtk_widget_insert_action_group(GTK_WIDGET(sidebar), "row", actions);
  g_object_unref(actions);
}

static GtkWidget *append_separator(GtkWidget *box) {
  GtkWidget *separator;

  separator = g_object_new(GTK_TYPE_SEPARATOR, "orientation",
                           GTK_ORIENTATION_HORIZONTAL, "visible", TRUE,
...
static GtkWidget *add_button(GtkWidget *box, const gchar *label,
                             const gchar *action) {
  GtkWidget *item;

  item = g_object_new(GTK_TYPE_MODEL_BUTTON, "visible", TRUE, "action-name",
                      action, "text", label, NULL);
  gtk_container_add(GTK_CONTAINER(box), item);

  return item;
}

static GtkWidget *add_open_button(GtkWidget *box, const gchar *label,
                                  NautilusGtkPlacesOpenFlags flags) {
  GtkWidget *item;

  item = g_object_new(
      GTK_TYPE_MODEL_BUTTON, "visible", TRUE, "action-name",
      flags == NAUTILUS_GTK_PLACES_OPEN_NORMAL ? "row.open" : "row.open-other",
      "action-target", g_variant_new_int32(flags), "text", label, NULL);
  gtk_container_add(GTK_CONTAINER(box), item);

  return item;
}

static void on_row_popover_destroy(GtkWidget *row_popover,
                                   NautilusGtkPlacesSidebar *sidebar) {
  if (sidebar)
...
#ifdef HAVE_CLOUDPROVIDERS
static void build_popup_menu_using_gmenu(NautilusGtkSidebarRow *row) {
  CloudProvidersAccount *cloud_provider_account;
  NautilusGtkPlacesSidebar *sidebar;
  GMenuModel *cloud_provider_menu;
  GActionGroup *cloud_provider_action_group;

  g_object_get(row, "sidebar", &sidebar, "cloud-provider-account",
  ...
  if (cloud_provider_account) {
    ...
    cloud_provider_menu =
        cloud_providers_account_get_menu_model(cloud_provider_account);
    cloud_provider_action_group =
        cloud_providers_account_get_action_group(cloud_provider_account);
    if (cloud_provider_menu != NULL && cloud_provider_action_group != NULL) {
      g_menu_append_section(menu, NULL, cloud_provider_menu);
      gtk_widget_insert_action_group(
          GTK_WIDGET(sidebar), "cloudprovider",
    ...
    sidebar->popover =
        gtk_popover_new_from_model(GTK_WIDGET(sidebar), G_MENU_MODEL(menu));
...
static void create_row_popover(NautilusGtkPlacesSidebar *sidebar,
                               NautilusGtkSidebarRow *row) {
  PopoverData data;
  GtkWidget *box;
  ...
  if (sidebar->open_flags & NAUTILUS_GTK_PLACES_OPEN_NEW_TAB)
    add_open_button(box, _("Open in New _Tab"),
                    NAUTILUS_GTK_PLACES_OPEN_NEW_TAB);

  if (sidebar->open_flags & NAUTILUS_GTK_PLACES_OPEN_NEW_WINDOW)
    add_open_button(box, _("Open in New _Window"),
  ...
  if (sidebar->populate_all) {
    ...
    g_object_get(row, "uri", &uri, "volume", &volume, NULL);
    ...
    g_signal_emit(sidebar, places_sidebar_signals[POPULATE_POPUP], 0, box, file,
                  volume);
...
static void show_row_popover(NautilusGtkSidebarRow *row) {
  NautilusGtkPlacesSidebar *sidebar;
...
static void on_row_activated(GtkListBox *list_box, GtkListBoxRow *row,
  ...
  selected_row =
      NAUTILUS_GTK_SIDEBAR_ROW(gtk_list_box_get_selected_row(list_box));
...
static gboolean on_button_press_event(GtkWidget *widget, GdkEventButton *event,
  ...
  g_object_get(NAUTILUS_GTK_SIDEBAR_ROW(row), "sidebar", &sidebar,
...
static void long_press_cb(GtkGesture *gesture, gdouble x, gdouble y,
  ...
  row =
      GTK_WIDGET(gtk_list_box_get_row_at_y(GTK_LIST_BOX(sidebar->list_box), y));
  if (NAUTILUS_IS_GTK_SIDEBAR_ROW(row))
    popup_menu_cb(NAUTILUS_GTK_SIDEBAR_ROW(row));
}

static gint list_box_sort_func(GtkListBoxRow *row1, GtkListBoxRow *row2,
                               gpointer user_data) {
  ...
  g_object_get(row1, "label", &label_1, "place-type", &place_type_1,
               "section-type", &section_type_1, "order-index", &index_1, NULL);
  g_object_get(row2, "label", &label_2, "place-type", &place_type_2,
               "section-type", &section_type_2, "order-index", &index_2, NULL);
  ...
  if (place_type_1 == PLACES_CONNECT_TO_SERVER) {
    ...
    if (section_type_1 == section_type_2) {
      ...
      } else if ((place_type_1 == PLACES_BOOKMARK ||
                  place_type_2 == PLACES_DROP_FEEDBACK) &&
                 (place_type_1 == PLACES_DROP_FEEDBACK ||
                  place_type_2 == PLACES_BOOKMARK)) {
       ...
       * of the row, we need to increase the order-index.
       */
      else if (place_type_1 == PLACES_BOOKMARK_PLACEHOLDER &&
               place_type_2 == PLACES_BOOKMARK) {
        if (index_1 == index_2)
          ...
          retval = index_1 - index_2;
                 ...
                 place_type_2 == PLACES_BOOKMARK_PLACEHOLDER) {
        if (index_1 == index_2)
          retval = index_1 - index_2 + 1;
        else
          retval = index_1 - index_2;
...
static void update_hostname(NautilusGtkPlacesSidebar *sidebar) {
  GVariant *variant;
  gsize len;
  ...
  if (len > 0 && g_strcmp0(sidebar->hostname, hostname) != 0) {
...
static void hostname_proxy_new_cb(GObject *source_object, GAsyncResult *res,
                                  gpointer user_data) {
  ...
  g_signal_connect_swapped(sidebar->hostnamed_proxy, "g-properties-changed",
                           G_CALLBACK(update_hostname), sidebar);
  update_hostname(sidebar);
}

static void create_volume_monitor(NautilusGtkPlacesSidebar *sidebar) {
...
static void shell_shows_desktop_changed(GtkSettings *settings,
...
nautilus_gtk_places_sidebar_init(NautilusGtkPlacesSidebar *sidebar) {
  ...
  sidebar->bookmarks_manager = _nautilus_gtk_bookmarks_manager_new(
      (GtkBookmarksChangedFunc)update_places, sidebar);

  sidebar->trash_monitor = nautilus_trash_monitor_get();
  sidebar->trash_monitor_changed_id =
      g_signal_connect_swapped(sidebar->trash_monitor, "trash-state-changed",
  ...
  gtk_gesture_single_set_touch_only(
      GTK_GESTURE_SINGLE(sidebar->long_press_gesture), TRUE);
  g_signal_connect(sidebar->long_press_gesture, "pressed",
                   G_CALLBACK(long_press_cb), sidebar);

  /* DND support */
  gtk_drag_dest_set(sidebar->list_box, 0, NULL, 0,
                    GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);
  target_list =
      gtk_target_list_new(dnd_drop_targets, G_N_ELEMENTS(dnd_drop_targets));
  ...
  sidebar->source_targets =
      gtk_target_list_new(dnd_source_targets, G_N_ELEMENTS(dnd_source_targets));
  ...
  g_signal_connect(sidebar->list_box, "drag-end", G_CALLBACK(drag_end_callback),
                   sidebar);
  ...
  g_dbus_proxy_new_for_bus(
      ...
      "org.freedesktop.hostname1", sidebar->hostnamed_cancellable,
      hostname_proxy_new_cb, sidebar);
  ...
  g_object_get(sidebar->gtk_settings, "gtk-shell-shows-desktop", &show_desktop,
               NULL);
  sidebar->show_desktop = show_desktop;
  ...
  g_signal_connect_swapped(sidebar->cloud_manager, "providers-changed",
                           G_CALLBACK(update_places), sidebar);
#endif
...
static void nautilus_gtk_places_sidebar_set_property(GObject *obj,
                                                     guint property_id,
                                                     const GValue *value,
                                                     GParamSpec *pspec) {
  ...
  switch (property_id) {
  case PROP_LOCATION:
    nautilus_gtk_places_sidebar_set_location(sidebar,
  ...
  case PROP_SHOW_RECENT:
    nautilus_gtk_places_sidebar_set_show_recent(sidebar,
                                                g_value_get_boolean(value));
    break;

  case PROP_SHOW_DESKTOP:
    nautilus_gtk_places_sidebar_set_show_desktop(sidebar,
                                                 g_value_get_boolean(value));
    break;
  ...
  case PROP_SHOW_CONNECT_TO_SERVER:
    ...
    nautilus_gtk_places_sidebar_set_show_connect_to_server(
        sidebar, g_value_get_boolean(value));
  ...
  case PROP_SHOW_TRASH:
    nautilus_gtk_places_sidebar_set_show_trash(sidebar,
                                               g_value_get_boolean(value));
    break;
  ...
  case PROP_LOCAL_ONLY:
    nautilus_gtk_places_sidebar_set_local_only(sidebar,
                                               g_value_get_boolean(value));
    break;
...
static void nautilus_gtk_places_sidebar_get_property(GObject *obj,
                                                     guint property_id,
                                                     GValue *value,
                                                     GParamSpec *pspec) {
  NautilusGtkPlacesSidebar *sidebar = NAUTILUS_GTK_PLACES_SIDEBAR(obj);
  ...
  case PROP_LOCATION:
    g_value_take_object(value,
                        nautilus_gtk_places_sidebar_get_location(sidebar));
    break;

  case PROP_OPEN_FLAGS:
    g_value_set_flags(value,
                      nautilus_gtk_places_sidebar_get_open_flags(sidebar));
    break;

  case PROP_SHOW_RECENT:
    g_value_set_boolean(value,
                        nautilus_gtk_places_sidebar_get_show_recent(sidebar));
    break;

  case PROP_SHOW_DESKTOP:
    g_value_set_boolean(value,
                        nautilus_gtk_places_sidebar_get_show_desktop(sidebar));
    break;

  case PROP_SHOW_CONNECT_TO_SERVER:
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    g_value_set_boolean(
        value, nautilus_gtk_places_sidebar_get_show_connect_to_server(sidebar));
    G_GNUC_END_IGNORE_DEPRECATIONS
    break;

  case PROP_SHOW_ENTER_LOCATION:
    g_value_set_boolean(
        value, nautilus_gtk_places_sidebar_get_show_enter_location(sidebar));
    break;

  case PROP_SHOW_OTHER_LOCATIONS:
    g_value_set_boolean(
        value, nautilus_gtk_places_sidebar_get_show_other_locations(sidebar));
    break;

  case PROP_SHOW_TRASH:
    g_value_set_boolean(value,
                        nautilus_gtk_places_sidebar_get_show_trash(sidebar));
    break;

  case PROP_SHOW_STARRED_LOCATION:
    g_value_set_boolean(
        value, nautilus_gtk_places_sidebar_get_show_starred_location(sidebar));
    break;

  case PROP_LOCAL_ONLY:
    g_value_set_boolean(value,
                        nautilus_gtk_places_sidebar_get_local_only(sidebar));
    break;
...
static void nautilus_gtk_places_sidebar_dispose(GObject *object) {
  NautilusGtkPlacesSidebar *sidebar;
#ifdef HAVE_CLOUDPROVIDERS
  GList *l;
#endif
  ...
  if (sidebar->trash_monitor) {
    g_signal_handler_disconnect(sidebar->trash_monitor,
                                sidebar->trash_monitor_changed_id);
  ...
  if (sidebar->volume_monitor != NULL) {
    g_signal_handlers_disconnect_by_func(sidebar->volume_monitor, update_places,
                                         sidebar);
  ...
  if (sidebar->gtk_settings) {
    g_signal_handlers_disconnect_by_func(sidebar->gtk_settings,
                                         shell_shows_desktop_changed, sidebar);
...
static void nautilus_gtk_places_sidebar_finalize(GObject *object) {
...
nautilus_gtk_places_sidebar_class_init(NautilusGtkPlacesSidebarClass *class) {
   ...
   * @open_flags: a single value from #NautilusGtkPlacesOpenFlags specifying how
   * the @location should be opened.
   *
   ...
   * Since: 3.10
   */
  places_sidebar_signals[OPEN_LOCATION] = g_signal_new(
      "open-location", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, open_location), NULL, NULL,
      NULL, G_TYPE_NONE, 2, G_TYPE_OBJECT, GTK_TYPE_PLACES_OPEN_FLAGS);
   ...
   * during their callbacks, the application can use
   * nautilus_gtk_places_sidebar_get_location() to get the file to which the
   * item refers.
   *
   ...
   * to add your items as #GtkMenuItems. Since 3.18, the popup may be
   * implemented as a #GtkPopover, in which case @container will be something
   * else, e.g. a #GtkBox, to which you may add #GtkModelButtons or other
   * widgets, such as #GtkEntries, #GtkSpinButtons, etc. If your application can
   * deal with this situation, you can set
   * #NautilusGtkPlacesSidebar::populate-all to %TRUE to request that this
   * signal is emitted for populating popovers as well.
   *
   * Since: 3.10
   */
  places_sidebar_signals[POPULATE_POPUP] = g_signal_new(
      "populate-popup", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, populate_popup), NULL,
      NULL, NULL, G_TYPE_NONE, 3, GTK_TYPE_WIDGET, G_TYPE_FILE, G_TYPE_VOLUME);
   ...
   * Since: 3.10
   */
  places_sidebar_signals[SHOW_ERROR_MESSAGE] = g_signal_new(
      "show-error-message", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, show_error_message), NULL,
   ...
   * the corresponding mount by using, for example,
   * g_file_mount_enclosing_volume().
   *
   * Deprecated: 3.18: use the #NautilusGtkPlacesSidebar::show-other-locations
   * signal to connect to network servers.
   */
  places_sidebar_signals[SHOW_CONNECT_TO_SERVER] = g_signal_new(
      "show-connect-to-server", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
   ...
   * Since: 3.14
   */
  places_sidebar_signals[SHOW_ENTER_LOCATION] = g_signal_new(
      "show-enter-location", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, show_enter_location), NULL,
      NULL, NULL, G_TYPE_NONE, 0);

  /**
   * NautilusGtkPlacesSidebar::drag-action-requested:
   * @sidebar: the object which received the signal.
   * @context: (type Gdk.DragContext): #GdkDragContext with information about
   * the drag operation
   * @dest_file: (type Gio.File): #GFile with the tentative location that is
   ...
   * it can check the type of the files indicated by @source_file_list against
   * the possible actions for the destination @dest_file.
   *
   ...
   * Since: 3.10
   */
  places_sidebar_signals[DRAG_ACTION_REQUESTED] = g_signal_new(
      "drag-action-requested", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, drag_action_requested),
      NULL, NULL, NULL, G_TYPE_INT, 3, GDK_TYPE_DRAG_CONTEXT, G_TYPE_OBJECT,
   ...
   *
   * Returns: the final drag action that the sidebar should pass to the drag
   * side of the drag-and-drop operation.
   *
   * Since: 3.10
   */
  places_sidebar_signals[DRAG_ACTION_ASK] = g_signal_new(
      "drag-action-ask", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, drag_action_ask), NULL,
      NULL, NULL, G_TYPE_INT, 1, G_TYPE_INT);
   ...
   * Since: 3.10
   */
  places_sidebar_signals[DRAG_PERFORM_DROP] = g_signal_new(
      "drag-perform-drop", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, drag_perform_drop), NULL,
      NULL, NULL, G_TYPE_NONE, 3, G_TYPE_OBJECT,
      G_TYPE_POINTER, /* GList of GFile */
      G_TYPE_INT);
   ...
   *
   * Deprecated: 3.20: use the
   * #NautilusGtkPlacesSidebar::show-other-locations-with-flags which includes
   * the open flags in order to allow the user to specify to open in a new tab
   * or window, in a similar way than #NautilusGtkPlacesSidebar::open-location
   *
   * Since: 3.18
   */
  places_sidebar_signals[SHOW_OTHER_LOCATIONS] = g_signal_new(
      "show-other-locations", G_OBJECT_CLASS_TYPE(gobject_class),
   ...
   * Since: 3.20
   */
  places_sidebar_signals[SHOW_OTHER_LOCATIONS_WITH_FLAGS] = g_signal_new(
      "show-other-locations-with-flags", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass,
                      show_other_locations_with_flags),
      NULL, NULL, NULL, G_TYPE_NONE, 1, GTK_TYPE_PLACES_OPEN_FLAGS);
   ...
   * In this way the application using the #NautilusGtkPlacesSidebar can track
   * the progress of the operation and, for example, show a notification.
   *
   * Since: 3.20
   */
  places_sidebar_signals[MOUNT] = g_signal_new(
      "mount", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, mount), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_MOUNT_OPERATION);
  /**
   ...
   * Since: 3.20
   */
  places_sidebar_signals[UNMOUNT] = g_signal_new(
      "unmount", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, unmount), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_MOUNT_OPERATION);
   ...
   * @sidebar: the object which received the signal.
   * @open_flags: a single value from #NautilusGtkPlacesOpenFlags specifying how
   * the starred file should be opened.
   *
   * The places sidebar emits this signal when it needs the calling
   ...
   * Since: 3.22.26
   */
  places_sidebar_signals[SHOW_STARRED_LOCATION] = g_signal_new(
      "show-starred-location", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(NautilusGtkPlacesSidebarClass, show_starred_location),
      NULL, NULL, NULL, G_TYPE_NONE, 1, GTK_TYPE_PLACES_OPEN_FLAGS);

  properties[PROP_LOCATION] =
      g_param_spec_object("location", "Location to Select",
                          "The location to highlight in the sidebar",
                          G_TYPE_FILE, G_PARAM_READWRITE);
  properties[PROP_OPEN_FLAGS] =
      g_param_spec_flags("open-flags", "Open Flags",
                         "Modes in which the calling application can open "
                         "locations selected in the sidebar",
                         GTK_TYPE_PLACES_OPEN_FLAGS,
                         NAUTILUS_GTK_PLACES_OPEN_NORMAL, G_PARAM_READWRITE);
  properties[PROP_SHOW_RECENT] = g_param_spec_boolean(
      "show-recent", "Show recent files",
      "Whether the sidebar includes a builtin shortcut for recent files", TRUE,
      G_PARAM_READWRITE);
  properties[PROP_SHOW_DESKTOP] = g_param_spec_boolean(
      "show-desktop", "Show 'Desktop'",
      "Whether the sidebar includes a builtin shortcut to the Desktop folder",
      TRUE, G_PARAM_READWRITE);
  properties[PROP_SHOW_CONNECT_TO_SERVER] =
      g_param_spec_boolean("show-connect-to-server", "Show 'Connect to Server'",
                           "Whether the sidebar includes a builtin shortcut to "
                           "a 'Connect to server' dialog",
                           FALSE, G_PARAM_READWRITE | G_PARAM_DEPRECATED);
  properties[PROP_SHOW_ENTER_LOCATION] =
      g_param_spec_boolean("show-enter-location", "Show 'Enter Location'",
                           "Whether the sidebar includes a builtin shortcut to "
  ...
  properties[PROP_LOCAL_ONLY] =
      g_param_spec_boolean("local-only", "Local Only",
                           ...
                           FALSE, G_PARAM_READWRITE);
  properties[PROP_SHOW_TRASH] = g_param_spec_boolean(
      "show-trash", "Show 'Trash'",
      "Whether the sidebar includes a builtin shortcut to the Trash location",
      TRUE, G_PARAM_READWRITE);
  properties[PROP_SHOW_OTHER_LOCATIONS] = g_param_spec_boolean(
      "show-other-locations", "Show 'Other locations'",
      "Whether the sidebar includes an item to show external locations", FALSE,
      G_PARAM_READWRITE);
  properties[PROP_SHOW_STARRED_LOCATION] = g_param_spec_boolean(
      "show-starred-location", "Show “Starred Location”",
      "Whether the sidebar includes an item to show starred files", FALSE,
      G_PARAM_READWRITE);

  /**
   * NautilusGtkPlacesSidebar:populate-all:
   *
   * If :populate-all is %TRUE, the #NautilusGtkPlacesSidebar::populate-popup
   * signal is also emitted for popovers.
   *
   * Since: 3.18
   */
  properties[PROP_POPULATE_ALL] = g_param_spec_boolean(
      "populate-all", "Populate all",
      "Whether to emit ::populate-popup for popups that are not menus", FALSE,
      G_PARAM_READWRITE);
 ...
 *
 * When the #NautilusGtkPlacesSidebar::open-location signal is emitted, its
 * flags argument will be set to one of the @flags that was passed in
 * nautilus_gtk_places_sidebar_set_open_flags().
 *
 * Passing 0 for @flags will cause #NAUTILUS_GTK_PLACES_OPEN_NORMAL to always be
 * sent to callbacks for the “open-location” signal.
 *
 * Since: 3.10
 */
void nautilus_gtk_places_sidebar_set_open_flags(
    NautilusGtkPlacesSidebar *sidebar, NautilusGtkPlacesOpenFlags flags) {
 ...
 * nothing is selected, for example, when
 * nautilus_gtk_places_sidebar_set_location() has been called with a location
 * that is not among the sidebar’s list of places to show.
 *
 * You can use this function to get the selection in the @sidebar.  Also, if you
 * connect to the #NautilusGtkPlacesSidebar::populate-popup signal, you can use
 ...
 * Since: 3.18
 */
void nautilus_gtk_places_sidebar_set_show_recent(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_recent) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * nautilus_gtk_places_sidebar_set_show_recent()
 *
 * Returns: %TRUE if the sidebar will display a builtin shortcut for recent
 * files
 *
 * Since: 3.18
 */
 ...
 * Since: 3.10
 */
void nautilus_gtk_places_sidebar_set_show_desktop(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_desktop) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * nautilus_gtk_places_sidebar_set_show_desktop()
 *
 * Returns: %TRUE if the sidebar will display a builtin shortcut to the desktop
 * folder.
 *
 * Since: 3.10
 */
gboolean nautilus_gtk_places_sidebar_get_show_desktop(
    NautilusGtkPlacesSidebar *sidebar) {
  g_return_val_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar), FALSE);
 ...
 * nautilus_gtk_places_sidebar_set_show_connect_to_server:
 * @sidebar: a places sidebar
 * @show_connect_to_server: whether to show an item for the Connect to Server
 * command
 *
 * Sets whether the @sidebar should show an item for connecting to a network
 * server; this is off by default. An application may want to turn this on if it
 * implements a way for the user to connect to network servers directly.
 *
 ...
 * Since: 3.10
 *
 * Deprecated: 3.18: It is recommended to group this functionality with the
...
void nautilus_gtk_places_sidebar_set_show_connect_to_server(
  ...
  if (sidebar->show_connect_to_server != show_connect_to_server) {
    ...
    update_places(sidebar);
    g_object_notify_by_pspec(G_OBJECT(sidebar),
                             properties[PROP_SHOW_CONNECT_TO_SERVER]);
  }
}
 ...
 * drives and network location under the new 'Other Location' item
 */
gboolean nautilus_gtk_places_sidebar_get_show_connect_to_server(
    NautilusGtkPlacesSidebar *sidebar) {
  g_return_val_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar), FALSE);
 ...
 * Since: 3.14
 */
void nautilus_gtk_places_sidebar_set_show_enter_location(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_enter_location) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * Since: 3.14
 */
gboolean nautilus_gtk_places_sidebar_get_show_enter_location(
    NautilusGtkPlacesSidebar *sidebar) {
  g_return_val_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar), FALSE);
 ...
 * persistent devices such as hard drives are hidden, otherwise they are shown
 * in the sidebar. An application may want to turn this on if it implements a
 * way for the user to see and interact with drives and network servers
 * directly.
 *
 * If you enable this, you should connect to the
...
void nautilus_gtk_places_sidebar_set_show_other_locations(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_other_locations) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * Since: 3.18
 */
gboolean nautilus_gtk_places_sidebar_get_show_other_locations(
    NautilusGtkPlacesSidebar *sidebar) {
  g_return_val_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar), FALSE);
 ...
 * Since: 3.18
 */
void nautilus_gtk_places_sidebar_set_show_trash(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_trash) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * Since: 3.12
 */
void nautilus_gtk_places_sidebar_set_local_only(
    NautilusGtkPlacesSidebar *sidebar, gboolean local_only) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
...
static GSList *find_shortcut_link(NautilusGtkPlacesSidebar *sidebar,
                                  GFile *location) {
  GSList *l;
 ...
 * inserted with nautilus_gtk_places_sidebar_add_shortcut().  If the @location
 * is not a shortcut in the sidebar, then nothing is done.
 *
 * Since: 3.10
 */
void nautilus_gtk_places_sidebar_remove_shortcut(
    NautilusGtkPlacesSidebar *sidebar, GFile *location) {
  GSList *link;
  GFile *shortcut;
 ...
 *     application-specific shortcuts with
 * nautilus_gtk_places_sidebar_add_shortcut(). To free this list, you can use
 * |[<!-- language="C" -->
 ...
 *
 * Returns: (nullable) (transfer full): The bookmark specified by the index @n,
 * or %NULL if no such index exist.  Note that the indices start at 0, even
 * though the file chooser starts them with the keyboard shortcut "Alt-1".
 *
 * Since: 3.10
 */
GFile *
nautilus_gtk_places_sidebar_get_nth_bookmark(NautilusGtkPlacesSidebar *sidebar,
  ...
  while (l != NULL) {
    ...
    g_object_get(l->data, "place-type", &place_type, "uri", &uri, NULL);
 ...
 *
 * Make the NautilusGtkPlacesSidebar show drop targets, so it can show the
 ...
 * automatically if the drag finishes in the NautilusGtkPlacesSidebar. You only
 * need to unset the state when the drag ends on some other widget on your
 * application.
 *
 * Since: 3.18
 */
void nautilus_gtk_places_sidebar_set_drop_targets_visible(
    NautilusGtkPlacesSidebar *sidebar, gboolean visible,
    GdkDragContext *context) {
  if (visible) {
 ...
 * Since: 3.22.26
 */
void nautilus_gtk_places_sidebar_set_show_starred_location(
    NautilusGtkPlacesSidebar *sidebar, gboolean show_starred_location) {
  g_return_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar));
 ...
 * Since: 3.22.26
 */
gboolean nautilus_gtk_places_sidebar_get_show_starred_location(
    NautilusGtkPlacesSidebar *sidebar) {
  g_return_val_if_fail(NAUTILUS_IS_GTK_PLACES_SIDEBAR(sidebar), FALSE);
