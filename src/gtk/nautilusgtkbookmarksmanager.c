 ...
 * nautilusgtkbookmarksmanager.c: Utilities to manage and monitor
 * ~/.gtk-bookmarks Copyright (C) 2003, Red Hat, Inc. Copyright (C) 2007-2008
 * Carlos Garnacho Copyright (C) 2011 Suse
 *
...
static void _gtk_bookmark_free(gpointer data) {
...
static void set_error_bookmark_doesnt_exist(GFile *file, GError **error) {
  gchar *uri = g_file_get_uri(file);

  g_set_error(error, GTK_FILE_CHOOSER_ERROR, GTK_FILE_CHOOSER_ERROR_NONEXISTENT,
              _("%s does not exist in the bookmarks list"), uri);

  g_free(uri);
}

static GFile *get_legacy_bookmarks_file(void) {
  GFile *file;
  gchar *filename;
...
static GFile *get_bookmarks_file(void) {
  GFile *file;
  gchar *filename;
...
static GSList *read_bookmarks(GFile *file) {
  gchar *contents;
  gchar **lines, *space;
...
static void save_bookmarks(GFile *bookmarks_file, GSList *bookmarks) {
  GError *error = NULL;
  GString *contents;
  GSList *l;
  GFile *parent = NULL;
  ...
  if (!g_file_replace_contents(bookmarks_file, contents->str, contents->len,
...
static void notify_changed(NautilusGtkBookmarksManager *manager) {
  if (manager->changed_func)
    manager->changed_func(manager->changed_func_data);
}

static void bookmarks_file_changed(GFileMonitor *monitor, GFile *file,
...
_nautilus_gtk_bookmarks_manager_new(GtkBookmarksChangedFunc changed_func,
  ...
  error = NULL;
  manager->bookmarks_monitor =
      g_file_monitor_file(bookmarks_file, G_FILE_MONITOR_NONE, NULL, &error);
  if (error) {
    g_warning("%s", error->message);
    g_error_free(error);
  } else
    manager->bookmarks_monitor_changed_id =
...
void _nautilus_gtk_bookmarks_manager_free(
    NautilusGtkBookmarksManager *manager) {
  g_return_if_fail(manager != NULL);

  if (manager->bookmarks_monitor) {
    g_file_monitor_cancel(manager->bookmarks_monitor);
    g_signal_handler_disconnect(manager->bookmarks_monitor,
                                manager->bookmarks_monitor_changed_id);
...
GSList *_nautilus_gtk_bookmarks_manager_list_bookmarks(
    NautilusGtkBookmarksManager *manager) {
...
static GSList *find_bookmark_link_for_file(GSList *bookmarks, GFile *file,
                                           int *position_ret) {
  int pos;
...
gboolean _nautilus_gtk_bookmarks_manager_has_bookmark(
    NautilusGtkBookmarksManager *manager, GFile *file) {
  GSList *link;
...
gboolean _nautilus_gtk_bookmarks_manager_insert_bookmark(
    NautilusGtkBookmarksManager *manager, GFile *file, gint position,
    GError **error) {
  GSList *link;
  GtkBookmark *bookmark;
  GFile *bookmarks_file;
  ...
  if (link) {
    ...
    g_set_error(error, GTK_FILE_CHOOSER_ERROR,
                GTK_FILE_CHOOSER_ERROR_ALREADY_EXISTS,
                _("%s already exists in the bookmarks list"), uri);
...
gboolean _nautilus_gtk_bookmarks_manager_remove_bookmark(
    NautilusGtkBookmarksManager *manager, GFile *file, GError **error) {
  GSList *link;
  GFile *bookmarks_file;
...
gboolean _nautilus_gtk_bookmarks_manager_reorder_bookmark(
    NautilusGtkBookmarksManager *manager, GFile *file, gint new_position,
    GError **error) {
  GSList *link;
  GFile *bookmarks_file;
  int old_position;
  ...
  if (link) {
    ...
    manager->bookmarks =
        g_slist_insert(manager->bookmarks, bookmark, new_position);
  } else {
    set_error_bookmark_doesnt_exist(file, error);
    return FALSE;
  }
...
gchar *_nautilus_gtk_bookmarks_manager_get_bookmark_label(
    NautilusGtkBookmarksManager *manager, GFile *file) {
  GSList *bookmarks;
  gchar *label = NULL;
...
gboolean _nautilus_gtk_bookmarks_manager_set_bookmark_label(
    NautilusGtkBookmarksManager *manager, GFile *file, const gchar *label,
    GError **error) {
  GFile *bookmarks_file;
  GSList *link;
...
gboolean _nautilus_gtk_bookmarks_manager_get_xdg_type(
    NautilusGtkBookmarksManager *manager, GFile *file,
...
gboolean _nautilus_gtk_bookmarks_manager_get_is_builtin(
    NautilusGtkBookmarksManager *manager, GFile *file) {
...
gboolean _nautilus_gtk_bookmarks_manager_get_is_xdg_dir_builtin(
    GUserDirectory xdg_type) {
