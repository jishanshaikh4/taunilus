...
void nautilus_search_hit_compute_scores(NautilusSearchHit *hit,
    ...
    DEBUG("Hit %s computed relevance %.2f (%.2f + %.2f + %.2f)", hit->uri,
          hit->relevance, proximity_bonus, recent_bonus, match_bonus);
...
static void nautilus_search_hit_set_uri(NautilusSearchHit *hit,
                                        const char *uri) {
    g_free(hit->uri);
    hit->uri = g_strdup(uri);
}

void nautilus_search_hit_set_fts_rank(NautilusSearchHit *hit, gdouble rank) {
    hit->fts_rank = rank;
}
...
static void nautilus_search_hit_set_property(GObject *object, guint arg_id,
        const GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_search_hit_get_property(GObject *object, guint arg_id,
...
static void nautilus_search_hit_finalize(GObject *object) {
...
static void nautilus_search_hit_class_init(NautilusSearchHitClass *class) {
    ...
    g_object_class_install_property(
        object_class, PROP_MODIFICATION_TIME,
        g_param_spec_boxed("modification-time", "Modification time",
                           "Modification time", G_TYPE_DATE_TIME,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
    g_object_class_install_property(
        object_class, PROP_ACCESS_TIME,
        g_param_spec_boxed("access-time", "acess time", "access time",
                           G_TYPE_DATE_TIME,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

    g_object_class_install_property(
        object_class, PROP_CREATION_TIME,
        g_param_spec_boxed("creation-time", "creation time", "creation time",
                           G_TYPE_DATE_TIME,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
    g_object_class_install_property(object_class, PROP_RELEVANCE,
                                    g_param_spec_double("relevance", NULL, NULL,
                                            -G_MAXDOUBLE, G_MAXDOUBLE,
                                            0, G_PARAM_READWRITE));
    g_object_class_install_property(object_class, PROP_FTS_RANK,
                                    g_param_spec_double("fts-rank", NULL, NULL,
                                            -G_MAXDOUBLE, G_MAXDOUBLE,
                                            0, G_PARAM_READWRITE));
    g_object_class_install_property(
        object_class, PROP_FTS_SNIPPET,
        g_param_spec_string("fts-snippet", "fts-snippet", "fts-snippet", NULL,
                            G_PARAM_READWRITE));
}

static void nautilus_search_hit_init(NautilusSearchHit *hit) {
    hit = G_TYPE_INSTANCE_GET_PRIVATE(hit, NAUTILUS_TYPE_SEARCH_HIT,
                                      NautilusSearchHit);
}
...
NautilusSearchHit *nautilus_search_hit_new(const char *uri) {
    ...
    hit = g_object_new(NAUTILUS_TYPE_SEARCH_HIT, "uri", uri, NULL);
