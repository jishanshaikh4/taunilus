...
#define NAUTILUS_FILE_ATTRIBUTES_FOR_ICON                                      \
  (NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_THUMBNAIL)
...
void nautilus_file_set_group(NautilusFile *file, const char *group_name_or_id,
                             NautilusFileOperationCallback callback,
                             gpointer callback_data);
void nautilus_file_set_permissions(NautilusFile *file, guint32 permissions,
                                   NautilusFileOperationCallback callback,
                                   gpointer callback_data);
 ...
 * existed. This is true when the NautilusFile object has been created, but no
 * I/O has yet confirmed the existence of a file by that name.
 */
...
gboolean nautilus_file_matches_uri(NautilusFile *file, const char *uri);
...
GIcon *nautilus_file_get_gicon(NautilusFile *file, NautilusFileIconFlags flags);
...
/* Convenience functions for dealing with a list of NautilusFile objects that
 * each have a ref. These are just convenient names for functions that work on
 * lists of GtkObject *.
 */
...
typedef struct {
    ...
    void (*monitor_add)(NautilusFile *file, gconstpointer client,
                        NautilusFileAttributes attributes);
    void (*monitor_remove)(NautilusFile *file, gconstpointer client);
    void (*call_when_ready)(NautilusFile *file, NautilusFileAttributes attributes,
                            NautilusFileCallback callback,
                            gpointer callback_data);
    ...
    gboolean (*check_if_ready)(NautilusFile *file,
                               NautilusFileAttributes attributes);
    gboolean (*get_item_count)(NautilusFile *file, guint *count,
                               gboolean *count_unreadable);
    NautilusRequestStatus (*get_deep_counts)(NautilusFile *file,
            ...
            goffset *total_size);
    gboolean (*get_date)(NautilusFile *file, NautilusDateType type, time_t *date);
    char *(*get_where_string)(NautilusFile *file);

    void (*set_metadata)(NautilusFile *file, const char *key, const char *value);
    void (*set_metadata_as_list)(NautilusFile *file, const char *key,
                                 char **value);

    void (*mount)(NautilusFile *file, GMountOperation *mount_op,
                  GCancellable *cancellable,
                  NautilusFileOperationCallback callback, gpointer callback_data);
    void (*unmount)(NautilusFile *file, GMountOperation *mount_op,
                    GCancellable *cancellable,
                    NautilusFileOperationCallback callback,
                    gpointer callback_data);
    void (*eject)(NautilusFile *file, GMountOperation *mount_op,
                  GCancellable *cancellable,
                  NautilusFileOperationCallback callback, gpointer callback_data);

    void (*start)(NautilusFile *file, GMountOperation *start_op,
                  GCancellable *cancellable,
                  NautilusFileOperationCallback callback, gpointer callback_data);
    void (*stop)(NautilusFile *file, GMountOperation *mount_op,
                 GCancellable *cancellable,
                 NautilusFileOperationCallback callback, gpointer callback_data);
