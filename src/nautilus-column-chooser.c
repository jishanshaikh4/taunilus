...
enum { PROP_FILE = 1, NUM_PROPERTIES };

enum { CHANGED, USE_DEFAULT, LAST_SIGNAL };
static guint signals[LAST_SIGNAL];

G_DEFINE_TYPE(NautilusColumnChooser, nautilus_column_chooser, GTK_TYPE_BOX);

static void nautilus_column_chooser_set_property(GObject *object,
                                                 guint param_id,
                                                 const GValue *value,
                                                 GParamSpec *pspec) {
...
static void update_buttons(NautilusColumnChooser *chooser) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(chooser->store), &iter, COLUMN_VISIBLE,
                       &visible, -1);
    ...
    bottom = !gtk_tree_model_iter_next(GTK_TREE_MODEL(chooser->store), &iter);

    gtk_widget_set_sensitive(chooser->move_up_button, !top);
    gtk_widget_set_sensitive(chooser->move_down_button, !bottom);
  } else {
...
static void list_changed(NautilusColumnChooser *chooser) {
  update_buttons(chooser);
  g_signal_emit(chooser, signals[CHANGED], 0);
}

static void toggle_path(NautilusColumnChooser *chooser, GtkTreePath *path) {
  GtkTreeIter iter;
  gboolean visible;

  gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->store), &iter, path);
  gtk_tree_model_get(GTK_TREE_MODEL(chooser->store), &iter, COLUMN_VISIBLE,
                     &visible, -1);
  gtk_list_store_set(chooser->store, &iter, COLUMN_VISIBLE, !visible, -1);
  list_changed(chooser);
}

static void visible_toggled_callback(GtkCellRendererToggle *cell,
                                     char *path_string, gpointer user_data) {
  GtkTreePath *path;
...
static void row_deleted_callback(GtkTreeModel *model, GtkTreePath *path,
                                 gpointer user_data) {
...
static void move_up_clicked_callback(GtkWidget *button, gpointer user_data) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->store), &prev, path)) {
      gtk_list_store_move_before(chooser->store, &iter, &prev);
    }
    gtk_tree_path_free(path);
  }
...
static void move_down_clicked_callback(GtkWidget *button, gpointer user_data) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    if (gtk_tree_model_iter_next(GTK_TREE_MODEL(chooser->store), &next)) {
      gtk_list_store_move_after(chooser->store, &iter, &next);
    }
  }
...
static void use_default_clicked_callback(GtkWidget *button,
                                         gpointer user_data) {
  g_signal_emit(NAUTILUS_COLUMN_CHOOSER(user_data), signals[USE_DEFAULT], 0);
}

static void populate_tree(NautilusColumnChooser *chooser) {
  GList *columns;
  GList *l;
  ...
  for (l = columns; l != NULL; l = l->next) {
    ...
    gtk_list_store_set(chooser->store, &iter, COLUMN_VISIBLE, visible,
                       COLUMN_LABEL, label, COLUMN_NAME, name, COLUMN_SENSITIVE,
                       sensitive, -1);
...
static void set_visible_columns(NautilusColumnChooser *chooser,
  ...
  for (i = 0; visible_columns[i] != NULL; ++i) {
    g_hash_table_insert(visible_columns_hash, visible_columns[i],
                        visible_columns[i]);
  }

  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(chooser->store), &iter)) {
    do {
      char *name;
      gboolean visible;

      gtk_tree_model_get(GTK_TREE_MODEL(chooser->store), &iter, COLUMN_NAME,
                         &name, -1);
...
static char **get_column_names(NautilusColumnChooser *chooser,
...
static gboolean get_column_iter(NautilusColumnChooser *chooser,
                                NautilusColumn *column, GtkTreeIter *iter) {
  char *column_name;
...
static void set_column_order(NautilusColumnChooser *chooser,
                             char **column_order) {
  GList *columns;
  GList *l;
  GtkTreePath *path;
  ...
  for (l = columns; l != NULL; l = l->next) {
    ...
    if (get_column_iter(chooser, NAUTILUS_COLUMN(l->data), &iter)) {
      GtkTreeIter before;
      if (path) {
        gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->store), &before, path);
        gtk_list_store_move_after(chooser->store, &iter, &before);
        gtk_tree_path_next(path);
      } else {
        gtk_list_store_move_after(chooser->store, &iter, NULL);
      }
    }
  }
  gtk_tree_path_free(path);
...
nautilus_column_chooser_class_init(NautilusColumnChooserClass *chooser_class) {
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/ui/nautilus-column-chooser.ui");
  gtk_widget_class_bind_template_child(widget_class, NautilusColumnChooser,
                                       view);
  gtk_widget_class_bind_template_child(widget_class, NautilusColumnChooser,
                                       store);
  gtk_widget_class_bind_template_child(widget_class, NautilusColumnChooser,
  ...
  gtk_widget_class_bind_template_callback(widget_class,
                                          view_row_activated_callback);
  gtk_widget_class_bind_template_callback(widget_class,
                                          selection_changed_callback);
  gtk_widget_class_bind_template_callback(widget_class,
                                          visible_toggled_callback);
  gtk_widget_class_bind_template_callback(widget_class,
                                          move_up_clicked_callback);
  gtk_widget_class_bind_template_callback(widget_class,
                                          move_down_clicked_callback);
  gtk_widget_class_bind_template_callback(widget_class,
                                          use_default_clicked_callback);

  signals[CHANGED] = g_signal_new(
      "changed", G_TYPE_FROM_CLASS(chooser_class), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[USE_DEFAULT] = g_signal_new(
      "use-default", G_TYPE_FROM_CLASS(chooser_class), G_SIGNAL_RUN_LAST, 0,
      NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  g_object_class_install_property(
      oclass, PROP_FILE,
      g_param_spec_object("file", "File", "The file this column chooser is for",
                          NAUTILUS_TYPE_FILE,
