...
enum { CHANGED, UPDATED_DEEP_COUNT_IN_PROGRESS, LAST_SIGNAL };
...
static GQuark attribute_name_q, attribute_size_q, attribute_type_q,
    attribute_detailed_type_q, attribute_modification_date_q,
    attribute_date_modified_q, attribute_date_modified_full_q,
    attribute_date_modified_with_time_q, attribute_accessed_date_q,
    attribute_date_accessed_q, attribute_date_accessed_full_q,
    attribute_date_created_q, attribute_date_created_full_q,
    attribute_mime_type_q, attribute_size_detail_q, attribute_deep_size_q,
    attribute_deep_file_count_q, attribute_deep_directory_count_q,
    attribute_deep_total_count_q, attribute_search_relevance_q,
    attribute_trashed_on_q, attribute_trashed_on_full_q,
    attribute_trash_orig_path_q, attribute_recency_q, attribute_permissions_q,
    attribute_selinux_context_q, attribute_octal_permissions_q,
    attribute_owner_q, attribute_group_q, attribute_uri_q, attribute_where_q,
    attribute_link_target_q, attribute_volume_q, attribute_free_space_q,
    attribute_starred_q;
...
static gboolean update_info_and_name(NautilusFile *file, GFileInfo *info);
...
static void nautilus_file_init(NautilusFile *file) {
  file->details = G_TYPE_INSTANCE_GET_PRIVATE((file), NAUTILUS_TYPE_FILE,
                                              NautilusFileDetails);
...
static GObject *
nautilus_file_constructor(GType type, guint n_construct_properties,
                          GObjectConstructParam *construct_params) {
  GObject *object;
  NautilusFile *file;

  object = (*G_OBJECT_CLASS(nautilus_file_parent_class)->constructor)(
...
gboolean nautilus_file_set_display_name(NautilusFile *file,
  ...
  if (g_strcmp0(file->details->display_name, display_name) != 0) {
    ...
    file->details->display_name_collation_key =
  ...
  if (g_strcmp0(file->details->edit_name, edit_name) != 0) {
    ...
    if (g_strcmp0(file->details->display_name, edit_name) == 0) {
      file->details->edit_name =
          g_ref_string_acquire(file->details->display_name);
    } else {
...
static void nautilus_file_clear_display_name(NautilusFile *file) {
  g_clear_pointer(&file->details->display_name, g_ref_string_release);
  g_free(file->details->display_name_collation_key);
...
static gboolean foreach_metadata_free(gpointer key, gpointer value,
                                      gpointer user_data) {
  guint id;
...
static void metadata_hash_free(GHashTable *hash) {
  g_hash_table_foreach_remove(hash, foreach_metadata_free, NULL);
  g_hash_table_destroy(hash);
}

static gboolean _g_strv_equal(GStrv a, GStrv b) {
...
static gboolean metadata_hash_equal(GHashTable *hash1, GHashTable *hash2) {
  GHashTableIter iter;
...
static void clear_metadata(NautilusFile *file) {
...
static GHashTable *get_metadata_from_info(GFileInfo *info) {
  GHashTable *metadata;
  char **attrs;
  guint id;
  int i;
...
gboolean nautilus_file_update_metadata_from_info(NautilusFile *file,
  ...
  if (g_file_info_has_namespace(info, "metadata")) {
    ...
    if (!metadata_hash_equal(metadata, file->details->metadata)) {
      changed = TRUE;
      clear_metadata(file);
...
void nautilus_file_set_directory(NautilusFile *file,
  ...
  file->details->directory_name_collation_key =
      g_utf8_collate_key_for_filename(parent_uri, -1);
  g_free(parent_uri);
}

static NautilusFile *
nautilus_file_new_from_filename(NautilusDirectory *directory,
                                const char *filename, gboolean self_owned) {
  NautilusFile *file;
  ...
  file = nautilus_directory_new_file_from_filename(directory, filename,
                                                   self_owned);
...
static void modify_link_hash_table(NautilusFile *file,
  ...
  found = g_hash_table_lookup_extended(symbolic_links, target_uri,
...
static void symbolic_link_weak_notify(gpointer data,
...
static void add_to_link_hash_table_list(GList **list, NautilusFile *file) {
...
static void add_to_link_hash_table(NautilusFile *file) {
  modify_link_hash_table(file, add_to_link_hash_table_list);
}

static void remove_from_link_hash_table_list(GList **list, NautilusFile *file) {
  if (g_list_find(*list, file) != NULL) {
...
static void remove_from_link_hash_table(NautilusFile *file) {
...
static NautilusFileInfo *nautilus_file_get_internal(GFile *location,
                                                    gboolean create) {
  gboolean self_owned;
  NautilusDirectory *directory;
  NautilusFile *file;
...
gboolean nautilus_file_can_eject(NautilusFile *file) {
  ...
  return file->details->can_eject || (file->details->mount != NULL &&
...
void nautilus_file_mount(NautilusFile *file, GMountOperation *mount_op,
  ...
  if (NAUTILUS_FILE_GET_CLASS(file)->mount == NULL) {
    ...
    NAUTILUS_FILE_GET_CLASS(file)->mount(file, mount_op, cancellable, callback,
                                         callback_data);
  }
}
...
static void unmount_done(void *callback_data) {
  UnmountData *data;
...
void nautilus_file_unmount(NautilusFile *file, GMountOperation *mount_op,
                           GCancellable *cancellable,
                           NautilusFileOperationCallback callback,
                           gpointer callback_data) {
  GError *error;
  ...
  if (file->details->can_unmount) {
    if (NAUTILUS_FILE_GET_CLASS(file)->unmount != NULL) {
      NAUTILUS_FILE_GET_CLASS(file)->unmount(file, mount_op, cancellable,
    ...
    nautilus_file_operations_unmount_mount_full(parent, file->details->mount,
                                                mount_op, FALSE, TRUE,
                                                unmount_done, data);
  } else if (callback) {
...
void nautilus_file_eject(NautilusFile *file, GMountOperation *mount_op,
                         GCancellable *cancellable,
                         NautilusFileOperationCallback callback,
                         gpointer callback_data) {
  GError *error;
  ...
  if (file->details->can_eject) {
    if (NAUTILUS_FILE_GET_CLASS(file)->eject != NULL) {
      NAUTILUS_FILE_GET_CLASS(file)->eject(file, mount_op, cancellable,
                                           callback, callback_data);
    } else {
      if (callback) {
        error = NULL;
    ...
    nautilus_file_operations_unmount_mount_full(
        parent, file->details->mount, mount_op, TRUE, TRUE, unmount_done, data);
  } else if (callback) {
    callback(file, NULL, NULL, callback_data);
  }
}

void nautilus_file_start(NautilusFile *file, GMountOperation *start_op,
                         GCancellable *cancellable,
                         NautilusFileOperationCallback callback,
                         gpointer callback_data) {
  GError *error;
  ...
  if ((file->details->can_start || file->details->can_start_degraded) &&
    ...
    NAUTILUS_FILE_GET_CLASS(file)->start(file, start_op, cancellable, callback,
                                         callback_data);
  } else {
    if (callback) {
      error = NULL;
...
static void file_stop_callback(GObject *source_object, GAsyncResult *res,
                               gpointer callback_data) {
  ...
  error = NULL;
  stopped = g_drive_stop_finish(G_DRIVE(source_object), res, &error);

  if (!stopped && error->domain == G_IO_ERROR &&
...
void nautilus_file_stop(NautilusFile *file, GMountOperation *mount_op,
                        GCancellable *cancellable,
                        NautilusFileOperationCallback callback,
                        gpointer callback_data) {
  GError *error;
  ...
  if (NAUTILUS_FILE_GET_CLASS(file)->stop != NULL) {
    ...
    if (drive != NULL && g_drive_can_stop(drive)) {
      ...
      g_drive_stop(drive, G_MOUNT_UNMOUNT_NONE, mount_op, op->cancellable,
                   file_stop_callback, op);
    } else {
      if (callback) {
        error = NULL;
        g_set_error_literal(&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
...
void nautilus_file_poll_for_media(NautilusFile *file) {
  if (file->details->can_poll_for_media) {
    ...
    drive = g_mount_get_drive(file->details->mount);
    if (drive != NULL) {
      g_drive_poll_for_media(drive, NULL, /* cancellable */
...
nautilus_file_operation_new(NautilusFile *file,
  ...
  op->file->details->operations_in_progress =
      g_list_prepend(op->file->details->operations_in_progress, op);

  return op;
}

static void nautilus_file_operation_remove(NautilusFileOperation *op) {
  GList *l;
  NautilusFile *file;

  op->file->details->operations_in_progress =
      g_list_remove(op->file->details->operations_in_progress, op);

  for (l = op->files; l != NULL; l = l->next) {
    file = NAUTILUS_FILE(l->data);
    file->details->operations_in_progress =
        g_list_remove(file->details->operations_in_progress, op);
  }
}
...
void nautilus_file_operation_complete(NautilusFileOperation *op,
                                      GFile *result_file, GError *error) {
...
static void rename_get_info_callback(GObject *source_object, GAsyncResult *res,
                                     gpointer callback_data) {
  NautilusFileOperation *op;
  NautilusDirectory *directory;
  ...
  if (new_info != NULL) {
    ...
    nautilus_tag_manager_update_moved_uris(tag_manager, old_location,
                                           new_location);
...
static void rename_callback(GObject *source_object, GAsyncResult *res,
                            gpointer callback_data) {
  NautilusFileOperation *op;
  GFile *new_file;
  GError *error;
  ...
  error = NULL;
  new_file = g_file_set_display_name_finish(G_FILE(source_object), res, &error);

  if (new_file != NULL) {
    if (op->undo_info != NULL) {
      nautilus_file_undo_info_rename_set_data_post(
          NAUTILUS_FILE_UNDO_INFO_RENAME(op->undo_info), new_file);
    }
    g_file_query_info_async(new_file, NAUTILUS_FILE_DEFAULT_ATTRIBUTES, 0,
                            G_PRIORITY_DEFAULT, op->cancellable,
...
static gboolean name_is(NautilusFile *file, const char *new_name) {
  const char *old_name;
...
static gchar *
nautilus_file_can_rename_file(NautilusFile *file, const char *new_name,
                              NautilusFileOperationCallback callback,
                              gpointer callback_data) {
  GError *error;
  gchar *new_file_name;
...
void nautilus_file_rename(NautilusFile *file, const char *new_name,
                          NautilusFileOperationCallback callback,
                          gpointer callback_data) {
  NautilusFileOperation *op;
  char *old_name;
  ...
  new_file_name =
      nautilus_file_can_rename_file(file, new_name, callback, callback_data);
  ...
  if (!nautilus_file_undo_manager_is_operating()) {
    ...
    nautilus_file_undo_info_rename_set_data_pre(
        NAUTILUS_FILE_UNDO_INFO_RENAME(op->undo_info), location, old_name,
        new_file_name);
    g_free(old_name);
  }

  /* Do the renaming. */
  g_file_set_display_name_async(location, new_file_name, G_PRIORITY_DEFAULT,
                                op->cancellable, rename_callback, op);
...
nautilus_file_rename_handle_file_gone(NautilusFile *file,
  ...
  if (nautilus_file_is_gone(file)) {
     ...
     */
    nautilus_file_changed(file);
    error = g_error_new(G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("File not found"));
    if (callback) {
      (*callback)(file, NULL, error, callback_data);
    }
    g_error_free(error);
...
static void batch_rename_get_info_callback(GObject *source_object,
                                           GAsyncResult *res,
                                           gpointer callback_data) {
  NautilusFileOperation *op;
  NautilusDirectory *directory;
...
static void real_batch_rename(GList *files, GList *new_names,
                              NautilusFileOperationCallback callback,
                              gpointer callback_data) {
  ...
  for (l1 = files->next; l1 != NULL; l1 = l1->next) {
    ...
    file->details->operations_in_progress =
        g_list_prepend(file->details->operations_in_progress, op);
  }
  ...
  for (l1 = files, l2 = new_names; l1 != NULL && l2 != NULL;
    ...
    new_file_name = nautilus_file_can_rename_file(file, new_name->str, callback,
                                                  callback_data);
    ...
    /* Do the renaming. */
    new_file = g_file_set_display_name(location, new_file_name, op->cancellable,
                                       &error);
    ...
    g_file_query_info_async(new_file, NAUTILUS_FILE_DEFAULT_ATTRIBUTES, 0,
                            G_PRIORITY_DEFAULT, op->cancellable,
                            batch_rename_get_info_callback, data);

    if (error != NULL) {
      g_warning("Batch rename for file \"%s\" failed",
  ...
  if (!nautilus_file_undo_manager_is_operating() &&
      op->skipped_files != g_list_length(files)) {
    op->undo_info =
        nautilus_file_undo_info_batch_rename_new(g_list_length(new_files));

    nautilus_file_undo_info_batch_rename_set_data_pre(
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(op->undo_info), old_files);

    nautilus_file_undo_info_batch_rename_set_data_post(
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(op->undo_info), new_files);
...
void nautilus_file_batch_rename(GList *files, GList *new_names,
                                NautilusFileOperationCallback callback,
                                gpointer callback_data) {
  real_batch_rename(files, new_names, callback, callback_data);
}
...
gboolean nautilus_file_rename_in_progress(NautilusFile *file) {
  ...
  for (node = file->details->operations_in_progress; node != NULL;
       node = node->next) {
    op = node->data;
    if (op->is_rename) {
      return TRUE;
...
gboolean nautilus_file_matches_uri(NautilusFile *file, const char *match_uri) {
...
int nautilus_file_compare_location(NautilusFile *file_1, NautilusFile *file_2) {
  GFile *loc_a, *loc_b;
  gboolean res;
...
static void update_link(NautilusFile *link_file, NautilusFile *target_file) {
...
static GList *get_link_files(NautilusFile *target_file) {
  char *uri;
  GList **link_files;
...
static void update_links_if_target(NautilusFile *target_file) {
  GList *link_files, *p;
...
static gboolean update_info_internal(NautilusFile *file, GFileInfo *info,
  ...
  guint32 permissions;
  gboolean can_read, can_write, can_execute, can_delete, can_trash, can_rename,
      can_mount, can_unmount, can_eject;
  gboolean can_start, can_start_degraded, can_stop, can_poll_for_media,
  ...
  changed |=
      nautilus_file_set_display_name(file, g_file_info_get_display_name(info),
                                     g_file_info_get_edit_name(info), FALSE);
  ...
  if (!file->details->got_custom_activation_uri &&
      (g_file_info_get_attribute_boolean(
       ...
       file_type == G_FILE_TYPE_SHORTCUT || nautilus_file_is_in_recent(file))) {
    activation_uri = g_file_info_get_attribute_string(
    ...
    if (activation_uri == NULL) {
      ...
      if (old_activation_uri) {
        if (strcmp(old_activation_uri, file->details->activation_uri) != 0) {
          changed = TRUE;
        }
  ...
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_READ)) {
    can_read = g_file_info_get_attribute_boolean(
  ...
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_DELETE)) {
    can_delete = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_ACCESS_CAN_DELETE);
  }
  ...
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_MOUNT)) {
    can_mount = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_MOUNT);
  }
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_UNMOUNT)) {
    can_unmount = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_UNMOUNT);
  }
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_EJECT)) {
    can_eject = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_EJECT);
  }
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_START)) {
    can_start = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_START);
  }
  if (g_file_info_has_attribute(
          info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_START_DEGRADED)) {
    can_start_degraded = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_START_DEGRADED);
  }
  if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_STOP)) {
    can_stop = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_CAN_STOP);
  }
  if (g_file_info_has_attribute(info,
                                G_FILE_ATTRIBUTE_MOUNTABLE_START_STOP_TYPE)) {
    start_stop_type = g_file_info_get_attribute_uint32(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_START_STOP_TYPE);
  }
  ...
  if (g_file_info_has_attribute(
          info, G_FILE_ATTRIBUTE_MOUNTABLE_IS_MEDIA_CHECK_AUTOMATIC)) {
    is_media_check_automatic = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_MOUNTABLE_IS_MEDIA_CHECK_AUTOMATIC);
  }
  ...
  owner_real =
      g_file_info_get_attribute_string(info, G_FILE_ATTRIBUTE_OWNER_USER_REAL);
  free_group = FALSE;
  ...
  if (file->details->uid != uid || file->details->gid != gid) {
    changed = TRUE;
  }
  ...
  if (file->details->atime != atime || file->details->mtime != mtime) {
  ...
  if (file->details->thumbnail != NULL && file->details->thumbnail_mtime != 0 &&
  ...
  thumbnailing_failed = g_file_info_get_attribute_boolean(
  ...
  if (mime_type == NULL) {
    mime_type = g_file_info_get_attribute_string(
  ...
  description = g_file_info_get_attribute_string(
  ...
  recency =
      g_file_info_get_attribute_int64(info, G_FILE_ATTRIBUTE_RECENT_MODIFIED);
  ...
  trash_orig_path =
      g_file_info_get_attribute_byte_string(info, "trash::orig-path");
  ...
  if (update_name) {
    ...
    if (file->details->name == NULL || strcmp(file->details->name, name) != 0) {
      changed = TRUE;

      node = nautilus_directory_begin_file_name_change(file->details->directory,
                                                       file);
      ...
      nautilus_directory_end_file_name_change(file->details->directory, file,
                                              node);
    }
  }
...
gboolean nautilus_file_update_info(NautilusFile *file, GFileInfo *info) {
  return update_info_internal(file, info, FALSE);
}

static gboolean update_name_internal(NautilusFile *file, const char *name,
...
gboolean nautilus_file_update_name(NautilusFile *file, const char *name) {
  gboolean ret;
...
gboolean
nautilus_file_update_name_and_directory(NautilusFile *file, const char *name,
...
static Knowledge get_item_count(NautilusFile *file, guint *count) {
  gboolean known, unreadable;

  known = nautilus_file_get_directory_item_count(file, count, &unreadable);
  if (!known) {
    return UNKNOWN;
  }
  if (unreadable) {
...
static Knowledge get_size(NautilusFile *file, goffset *size) {
...
static Knowledge get_time(NautilusFile *file, time_t *time_out,
...
static int compare_directories_by_count(NautilusFile *file_1,
                                        NautilusFile *file_2) {
  /* Sort order:
...
static int compare_files_by_size(NautilusFile *file_1, NautilusFile *file_2) {
  /* Sort order:
...
static int compare_by_size(NautilusFile *file_1, NautilusFile *file_2) {
  /* Sort order:
   *   Directories with n items
   *   Directories with 0 items
   *   Directories with "unknowable" # of items
...
static int compare_by_display_name(NautilusFile *file_1, NautilusFile *file_2) {
...
static int compare_by_directory_name(NautilusFile *file_1,
                                     NautilusFile *file_2) {
  return strcmp(file_1->details->directory_name_collation_key,
                file_2->details->directory_name_collation_key);
}

static GList *prepend_automatic_keywords(NautilusFile *file, GList *names) {
  ...
  if (!nautilus_file_can_write(file) && !nautilus_file_is_in_trash(file) &&
    ...
    names =
        g_list_prepend(names, g_strdup(NAUTILUS_FILE_EMBLEM_NAME_CANT_WRITE));
  }
  if (!nautilus_file_can_read(file)) {
    names =
        g_list_prepend(names, g_strdup(NAUTILUS_FILE_EMBLEM_NAME_CANT_READ));
  }
...
static int compare_by_type(NautilusFile *file_1, NautilusFile *file_2) {
  ...
  if (file_1->details->mime_type != NULL &&
      ...
      strcmp(file_1->details->mime_type, file_2->details->mime_type) == 0) {
    return 0;
  }
  ...
  if (result == 0) {
    ...
    result =
        g_utf8_collate(file_1->details->mime_type, file_2->details->mime_type);
  }
...
static int compare_by_starred(NautilusFile *file_1, NautilusFile *file_2) {
  g_autoptr(NautilusTagManager) tag_manager = nautilus_tag_manager_get();
  ...
  file_1_is_starred = nautilus_tag_manager_file_is_starred(tag_manager, uri_1);
  file_2_is_starred = nautilus_tag_manager_file_is_starred(tag_manager, uri_2);
...
static Knowledge get_search_relevance(NautilusFile *file,
...
static int compare_by_search_relevance(NautilusFile *file_1,
                                       NautilusFile *file_2) {
  gdouble r_1, r_2;
...
static int compare_by_time(NautilusFile *file_1, NautilusFile *file_2,
                           NautilusDateType type) {
  /* Sort order:
...
static int compare_by_full_path(NautilusFile *file_1, NautilusFile *file_2) {
  int compare;
 ...
 **/
int nautilus_file_compare_for_sort(NautilusFile *file_1, NautilusFile *file_2,
  ...
  result = nautilus_file_compare_for_sort_internal(file_1, file_2,
                                                   directories_first, reversed);
...
int nautilus_file_compare_for_sort_by_attribute_q(NautilusFile *file_1,
  ...
  if (attribute == 0 || attribute == attribute_name_q) {
    ...
    return nautilus_file_compare_for_sort(file_1, file_2,
                                          ...
                                          directories_first, reversed);
  } else if (attribute == attribute_date_created_q ||
    ...
    return nautilus_file_compare_for_sort(
        file_1, file_2, NAUTILUS_FILE_SORT_BY_SEARCH_RELEVANCE,
        directories_first, reversed);
  ...
  if (result == 0) {
    ...
    value_1 = nautilus_file_get_string_attribute_q(file_1, attribute);
    value_2 = nautilus_file_get_string_attribute_q(file_2, attribute);
...
int nautilus_file_compare_for_sort_by_attribute(NautilusFile *file_1,
                                                ...
                                                gboolean directories_first,
                                                gboolean reversed) {
  return nautilus_file_compare_for_sort_by_attribute_q(
      file_1, file_2, g_quark_from_string(attribute), directories_first,
      reversed);
}
 ...
 **/
int nautilus_file_compare_display_name(NautilusFile *file, const char *string) {
  const char *name;
  int result;
 ...
 */
gboolean nautilus_file_should_show(NautilusFile *file, gboolean show_hidden) {
...
static gboolean filter_hidden_partition_callback(NautilusFile *file,
                                                 gpointer callback_data) {
  FilterOptions options;

  options = GPOINTER_TO_INT(callback_data);

  return nautilus_file_should_show(file, options & SHOW_HIDDEN);
}

GList *nautilus_file_list_filter_hidden(GList *files, gboolean show_hidden) {
  ...
  filtered_files = nautilus_file_list_filter(
      files, &removed_files, filter_hidden_partition_callback,
 ...
 */
GList *nautilus_file_list_filter(GList *files, GList **failed,
...
char *nautilus_file_get_metadata(NautilusFile *file, const char *key,
  ...
  if (file == NULL || file->details->metadata == NULL) {
...
GList *nautilus_file_get_metadata_list(NautilusFile *file, const char *key) {
  GList *res;
  guint id;
  char **value;
  int i;
...
void nautilus_file_set_metadata(NautilusFile *file, const char *key,
                                const char *default_metadata,
                                const char *metadata) {
  const char *val;
...
void nautilus_file_set_metadata_list(NautilusFile *file, const char *key,
                                     GList *list) {
  char **val;
  int len, i;
  GList *l;
  ...
  NAUTILUS_FILE_CLASS(G_OBJECT_GET_CLASS(file))
      ->set_metadata_as_list(file, key, val);

  g_free(val);
}

gboolean nautilus_file_get_boolean_metadata(NautilusFile *file, const char *key,
...
int nautilus_file_get_integer_metadata(NautilusFile *file, const char *key,
  ...
  g_snprintf(default_as_string, sizeof(default_as_string), "%d",
             default_metadata);
...
static gboolean get_time_from_time_string(const char *time_string,
                                          time_t *time) {
  long scanned_time;
  char c;
...
time_t nautilus_file_get_time_metadata(NautilusFile *file, const char *key) {
  time_t time;
  char *time_string;
...
void nautilus_file_set_time_metadata(NautilusFile *file, const char *key,
                                     time_t time) {
  char time_str[21];
  char *metadata;
...
void nautilus_file_set_boolean_metadata(NautilusFile *file, const char *key,
                                        gboolean default_metadata,
                                        gboolean metadata) {
  g_return_if_fail(NAUTILUS_IS_FILE(file));
  g_return_if_fail(key != NULL);
  ...
  nautilus_file_set_metadata(file, key, default_metadata ? "true" : "false",
                             metadata ? "true" : "false");
}

void nautilus_file_set_integer_metadata(NautilusFile *file, const char *key,
                                        int default_metadata, int metadata) {
  ...
  nautilus_file_set_metadata(file, key, default_as_string, value_as_string);
}
...
static const char *nautilus_file_peek_display_name(NautilusFile *file) {
  ...
  if (file->details->display_name == NULL) {
    ...
    if (g_utf8_validate(name, -1, NULL)) {
      nautilus_file_set_display_name(file, name, NULL, FALSE);
    } else {
      escaped_name =
          g_uri_escape_string(name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
      nautilus_file_set_display_name(file, escaped_name, NULL, FALSE);
      g_free(escaped_name);
    }
  }
...
void nautilus_file_monitor_add(NautilusFile *file, gconstpointer client,
...
void nautilus_file_monitor_remove(NautilusFile *file, gconstpointer client) {
  g_return_if_fail(NAUTILUS_IS_FILE(file));
  g_return_if_fail(client != NULL);
...
static char *get_custom_icon_metadata_uri(NautilusFile *file) {
  ...
  if (uri != NULL && nautilus_file_is_directory(file) && is_uri_relative(uri)) {
...
static char *get_custom_icon_metadata_name(NautilusFile *file) {
  char *icon_name;

  icon_name = nautilus_file_get_metadata(
      file, NAUTILUS_METADATA_KEY_CUSTOM_ICON_NAME, NULL);

  return icon_name;
}

static GIcon *get_mount_icon(NautilusFile *file) {
  GMount *mount;
  GIcon *mount_icon;
...
static GIcon *get_custom_icon(NautilusFile *file) {
...
static GIcon *get_default_file_icon(void) {
...
static gboolean nautilus_is_video_file(NautilusFile *file) {
  const char *mime_type;
  guint i;
...
static GList *sort_keyword_list_and_remove_duplicates(GList *keywords) {
  GList *p;
...
static void clean_up_metadata_keywords(NautilusFile *file,
 ...
 *
 **/
static GList *nautilus_file_get_keywords(NautilusFile *file) {
  ...
  keywords = g_list_copy_deep(file->details->extension_emblems,
                              (GCopyFunc)g_strdup, NULL);
  keywords = g_list_concat(
      keywords, g_list_copy_deep(file->details->pending_extension_emblems,
                                 (GCopyFunc)g_strdup, NULL));

  metadata_keywords =
      nautilus_file_get_metadata_list(file, NAUTILUS_METADATA_KEY_EMBLEMS);
 ...
 *
 **/
static GList *nautilus_file_get_emblem_icons(NautilusFile *file) {
  GList *keywords, *l;
  GList *icons;
  char *icon_names[2];
  char *keyword;
...
static void prepend_icon_name(const char *name, GThemedIcon *icon) {
  g_themed_icon_prepend_name(icon, name);
}

static void apply_emblems_to_icon(NautilusFile *file, GIcon **icon,
...
nautilus_file_get_thumbnail_icon(NautilusFile *file, int size, int scale,
                                 NautilusFileIconFlags flags) {
  int modified_size;
  GdkPixbuf *pixbuf;
  int w, h, s;
  ...
  if (flags & NAUTILUS_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
    ...
    modified_size = size * scale * NAUTILUS_GRID_ICON_SIZE_STANDARD /
  ...
  if (file->details->thumbnail) {
    ...
    if (file->details->thumbnail_scale == thumb_scale &&
      ...
      pixbuf = gdk_pixbuf_scale_simple(
          file->details->thumbnail, MAX(w * thumb_scale, 1),
          MAX(h * thumb_scale, 1), GDK_INTERP_BILINEAR);

      /* We don't want frames around small icons */
      if (!gdk_pixbuf_get_has_alpha(file->details->thumbnail) ||
          s >= 128 * scale) {
    ...
    DEBUG("Returning thumbnailed image, at size %d %d", (int)(w * thumb_scale),
          (int)(h * thumb_scale));
  } else if (file->details->thumbnail_path == NULL && file->details->can_read &&
...
static gboolean nautilus_thumbnail_is_limited_by_zoom(int size, int scale) {
  int zoom_level;
...
NautilusIconInfo *nautilus_file_get_icon(NautilusFile *file, int size,
                                         int scale,
                                         NautilusFileIconFlags flags) {
  NautilusIconInfo *icon;
  GIcon *gicon;
...
GdkPixbuf *nautilus_file_get_icon_pixbuf(NautilusFile *file, int size,
                                         gboolean force_size, int scale,
                                         NautilusFileIconFlags flags) {
...
gboolean nautilus_file_get_date(NautilusFile *file, NautilusDateType date_type,
                                time_t *date) {
  if (date != NULL) {
    *date = 0;
  }
  ...
  return NAUTILUS_FILE_CLASS(G_OBJECT_GET_CLASS(file))
      ->get_date(file, date_type, date);
}

static char *nautilus_file_get_where_string(NautilusFile *file) {
  if (file == NULL) {
    return NULL;
  }
...
static char *nautilus_file_get_date_as_string(NautilusFile *file,
  ...
  if (date_format != NAUTILUS_DATE_FORMAT_FULL) {
    ...
    today_midnight = g_date_time_new_local(
        g_date_time_get_year(now), g_date_time_get_month(now),
        g_date_time_get_day_of_month(now), 0, 0, 0);

    file_date = g_date_time_new_local(
        g_date_time_get_year(file_date_time),
        g_date_time_get_month(file_date_time),
        g_date_time_get_day_of_month(file_date_time), 0, 0, 0);

    days_ago =
        g_date_time_difference(today_midnight, file_date) / G_TIME_SPAN_DAY;
  ...
  /* Replace ":" with ratio. Replacement is done afterward because
   * g_date_time_format may fail with utf8 chars in some locales */
...
static void show_directory_item_count_changed_callback(gpointer callback_data) {
  show_directory_item_count = g_settings_get_enum(
      nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}
...
gboolean nautilus_file_should_show_directory_item_count(NautilusFile *file) {
  ...
  if (!show_directory_item_count_callback_added) {
    g_signal_connect_swapped(
        ...
        G_CALLBACK(show_directory_item_count_changed_callback), NULL);
  ...
  return get_speed_tradeoff_preference_for_file(file,
 ...
 *
 **/
NautilusRequestStatus nautilus_file_get_deep_counts(
    NautilusFile *file, guint *directory_count, guint *file_count,
    guint *unreadable_directory_count, goffset *total_size, gboolean force) {
  ...
  return NAUTILUS_FILE_CLASS(G_OBJECT_GET_CLASS(file))
      ->get_deep_counts(file, directory_count, file_count,
...
static void set_attributes_get_info_callback(GObject *source_object,
                                             GAsyncResult *res,
                                             gpointer callback_data) {
  NautilusFileOperation *op;
  GFileInfo *new_info;
...
static void set_attributes_callback(GObject *source_object,
                                    GAsyncResult *result,
                                    gpointer callback_data) {
  NautilusFileOperation *op;
  GError *error;
  ...
  error = NULL;
  res =
      g_file_set_attributes_finish(G_FILE(source_object), result, NULL, &error);

  if (res) {
    g_file_query_info_async(G_FILE(source_object),
                            NAUTILUS_FILE_DEFAULT_ATTRIBUTES, 0,
                            G_PRIORITY_DEFAULT, op->cancellable,
...
void nautilus_file_set_attributes(NautilusFile *file, GFileInfo *attributes,
                                  NautilusFileOperationCallback callback,
                                  gpointer callback_data) {
  NautilusFileOperation *op;
  GFile *location;
  ...
  location = nautilus_file_get_location(file);
  g_file_set_attributes_async(location, attributes, 0, G_PRIORITY_DEFAULT,
                              op->cancellable, set_attributes_callback, op);
  g_object_unref(location);
}

void nautilus_file_set_search_relevance(NautilusFile *file, gdouble relevance) {
...
gboolean nautilus_file_can_set_permissions(NautilusFile *file) {
  ...
  if (file->details->uid != -1 && g_file_is_native(location)) {
    /* Check the user. */
    user_id = geteuid();
 ...
 **/
void nautilus_file_set_permissions(NautilusFile *file, guint32 new_permissions,
                                   NautilusFileOperationCallback callback,
                                   gpointer callback_data) {
  GFileInfo *info;
  GError *error;
  ...
  if (!nautilus_file_undo_manager_is_operating()) {
    ...
    undo_info = nautilus_file_undo_info_permissions_new(
        nautilus_file_get_location(file), file->details->permissions,
        new_permissions);
...
static char *get_real_name(const char *name, const char *gecos) {
  ...
  if (capitalized_login_name == NULL) {
    ...
    real_name = eel_str_replace_substring(part_before_comma, "&",
                                          capitalized_login_name);
    g_free(part_before_comma);
  }

  if (g_strcmp0(real_name, NULL) == 0 || g_strcmp0(name, real_name) == 0 ||
...
static gboolean get_group_id_from_group_name(const char *group_name,
                                             uid_t *gid) {
  struct group *group;
...
static gboolean get_ids_from_user_name(const char *user_name, uid_t *uid,
                                       uid_t *gid) {
...
static gboolean get_user_id_from_user_name(const char *user_name, uid_t *id) {
  return get_ids_from_user_name(user_name, id, NULL);
}

static gboolean get_id_from_digit_string(const char *digit_string, uid_t *id) {
  long scanned_id;
  char c;
 ...
 */
void nautilus_file_set_owner(NautilusFile *file, const char *user_name_or_id,
                             NautilusFileOperationCallback callback,
                             gpointer callback_data) {
  GError *error;
  GFileInfo *info;
  ...
  if (!nautilus_file_undo_manager_is_operating()) {
    ...
    undo_info = nautilus_file_undo_info_ownership_new(
        NAUTILUS_FILE_UNDO_OP_CHANGE_OWNER, nautilus_file_get_location(file),
        current_owner, user_name_or_id);
    nautilus_file_undo_manager_set_action(undo_info);
 ...
 */
static GList *nautilus_get_group_names_for_user(void) {
  GList *list;
  struct group *group;
  int count, i;
 ...
 * @callback_data: Parameter passed back with callback function.
 */
void nautilus_file_set_group(NautilusFile *file, const char *group_name_or_id,
                             NautilusFileOperationCallback callback,
                             gpointer callback_data) {
  GError *error;
  GFileInfo *info;
  ...
  if (!nautilus_file_undo_manager_is_operating()) {
    ...
    undo_info = nautilus_file_undo_info_ownership_new(
        NAUTILUS_FILE_UNDO_OP_CHANGE_GROUP, nautilus_file_get_location(file),
        current_group, group_name_or_id);
    nautilus_file_undo_manager_set_action(undo_info);
 ...
 *
 **/
static char *nautilus_file_get_octal_permissions_as_string(NautilusFile *file) {
  guint32 permissions;
...
static char *nautilus_file_get_permissions_as_string(NautilusFile *file) {
  ...
  return g_strdup_printf(
      ...
      : is_directory ? 'd'
                     : '-',
      permissions & S_IRUSR ? 'r' : '-', permissions & S_IWUSR ? 'w' : '-',
      permissions & S_IXUSR ? (suid ? 's' : 'x') : (suid ? 'S' : '-'),
      permissions & S_IRGRP ? 'r' : '-', permissions & S_IWGRP ? 'w' : '-',
      permissions & S_IXGRP ? (sgid ? 's' : 'x') : (sgid ? 'S' : '-'),
      permissions & S_IROTH ? 'r' : '-', permissions & S_IWOTH ? 'w' : '-',
      permissions & S_IXOTH ? (sticky ? 't' : 'x') : (sticky ? 'T' : '-'));
}
...
static char *nautilus_file_get_owner_as_string(NautilusFile *file,
  ...
  /* Before we have info on a file, the owner is unknown. */
  if (file->details->owner == NULL && file->details->owner_real == NULL) {
    return NULL;
  }

  if (include_real_name && file->details->uid == getuid()) {
...
static char *format_item_count_for_display(guint item_count,
  ...
  return g_strdup_printf(
      includes_directories
          ? (includes_files ? ngettext("%'u item", "%'u items", item_count)
                            : ngettext("%'u folder", "%'u folders", item_count))
          : ngettext("%'u file", "%'u files", item_count),
      item_count);
}
...
static char *nautilus_file_get_deep_count_as_string_internal(
    NautilusFile *file, gboolean report_size, gboolean report_directory_count,
  ...
  status = nautilus_file_get_deep_counts(file, &directory_count, &file_count,
                                         &unreadable_count, &total_size, FALSE);
  ...
  if (total_count == 0) {
    ...
    case NAUTILUS_REQUEST_DONE: {
      /* Don't return "zero" if we there were contents but we couldn't read
       * them. */
 ...
 *
 **/
static char *nautilus_file_get_deep_size_as_string(NautilusFile *file) {
  return nautilus_file_get_deep_count_as_string_internal(file, TRUE, FALSE,
                                                         FALSE);
}
 ...
 *
 **/
static char *nautilus_file_get_deep_total_count_as_string(NautilusFile *file) {
  return nautilus_file_get_deep_count_as_string_internal(file, FALSE, TRUE,
                                                         TRUE);
}
 ...
 *
 **/
static char *nautilus_file_get_deep_file_count_as_string(NautilusFile *file) {
  return nautilus_file_get_deep_count_as_string_internal(file, FALSE, FALSE,
                                                         TRUE);
}
 ...
 *"deep_size", "deep_directory_count", "deep_file_count", "deep_total_count",
 *"date_modified", "date_accessed", "date_created", "date_modified_full",
 *"date_accessed_full", "date_created_full", "owner", "group", "permissions",
 *"octal_permissions", "uri", "where", "link_target", "volume", "free_space",
 *"selinux_context", "trashed_on", "trashed_on_full", "trashed_orig_path",
 * "recency"
 *
...
char *nautilus_file_get_string_attribute_q(NautilusFile *file,
  ...
  if (attribute_q == attribute_date_modified_q) {
    return nautilus_file_get_date_as_string(file, NAUTILUS_DATE_TYPE_MODIFIED,
  ...
  if (attribute_q == attribute_date_modified_with_time_q) {
    return nautilus_file_get_date_as_string(
        file, NAUTILUS_DATE_TYPE_MODIFIED,
        NAUTILUS_DATE_FORMAT_REGULAR_WITH_TIME);
  }
  if (attribute_q == attribute_date_accessed_q) {
    return nautilus_file_get_date_as_string(file, NAUTILUS_DATE_TYPE_ACCESSED,
                                            NAUTILUS_DATE_FORMAT_REGULAR);
  }
  ...
  if (attribute_q == attribute_date_created_q) {
    return nautilus_file_get_date_as_string(file, NAUTILUS_DATE_TYPE_CREATED,
                                            NAUTILUS_DATE_FORMAT_REGULAR);
  }
  ...
  if (attribute_q == attribute_trashed_on_q) {
    return nautilus_file_get_date_as_string(file, NAUTILUS_DATE_TYPE_TRASHED,
                                            NAUTILUS_DATE_FORMAT_REGULAR);
  }
  ...
  if (attribute_q == attribute_recency_q) {
    return nautilus_file_get_date_as_string(file, NAUTILUS_DATE_TYPE_RECENCY,
                                            NAUTILUS_DATE_FORMAT_REGULAR);
  }
  ...
  if (file->details->pending_extension_attributes) {
    extension_attribute =
        g_hash_table_lookup(file->details->pending_extension_attributes,
                            GINT_TO_POINTER(attribute_q));
  }

  if (extension_attribute == NULL && file->details->extension_attributes) {
    extension_attribute = g_hash_table_lookup(
        file->details->extension_attributes, GINT_TO_POINTER(attribute_q));
  }
...
char *nautilus_file_get_string_attribute(NautilusFile *file,
  ...
  return nautilus_file_info_get_string_attribute(NAUTILUS_FILE_INFO(file),
                                                 attribute_name);
}
 ...
 * such as "unknown" if the value is unknown or @attribute_name is not
 *supported.
 *
 **/
char *nautilus_file_get_string_attribute_with_default_q(NautilusFile *file,
  ...
  if (attribute_q == attribute_size_q) {
    ...
    count_unreadable = FALSE;
    if (nautilus_file_is_directory(file)) {
      nautilus_file_get_directory_item_count(file, &item_count,
                                             &count_unreadable);
    }
...
nautilus_file_get_string_attribute_with_default(NautilusFile *file,
                                                const char *attribute_name) {
  return nautilus_file_get_string_attribute_with_default_q(
      file, g_quark_from_string(attribute_name));
}

gboolean nautilus_file_is_date_sort_attribute_q(GQuark attribute_q) {
...
static char *get_basic_type_for_mime_type(const char *mime_type) {
  char *icon_name;
...
static char *get_description(NautilusFile *file, gboolean detailed) {
  const char *mime_type;
...
static char *update_description_for_link(NautilusFile *file, char *string) {
  char *res;
 ...
 *
 **/
gboolean nautilus_file_is_mime_type(NautilusFile *file, const char *mime_type) {
  g_return_val_if_fail(NAUTILUS_IS_FILE(file), FALSE);
...
gboolean nautilus_file_is_launchable(NautilusFile *file) {
  ...
  return type_can_be_executable && nautilus_file_can_get_permissions(file) &&
         nautilus_file_can_execute(file) && nautilus_file_is_executable(file) &&
...
void nautilus_file_set_mount(NautilusFile *file, GMount *mount) {
  if (file->details->mount) {
    g_signal_handlers_disconnect_by_func(file->details->mount,
                                         file_mount_unmounted, file);
    g_object_unref(file->details->mount);
  ...
  if (mount) {
    ...
    g_signal_connect(mount, "unmounted", G_CALLBACK(file_mount_unmounted),
                     file);
  }
}
...
static void get_fs_free_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  NautilusFile *file;
  guint64 free_space;
  GFileInfo *info;
...
char *nautilus_file_get_volume_free_space(NautilusFile *file) {
  ...
  if (file->details->free_space_read == 0 ||
    ...
    g_file_query_filesystem_info_async(
        location, G_FILE_ATTRIBUTE_FILESYSTEM_FREE, 0, NULL, get_fs_free_cb,
        nautilus_file_ref(file));
    g_object_unref(location);
  }
 ...
 *
 * Returns: newly-allocated copy of the file path of the target of the symbolic
 * link.
 */
...
char *nautilus_file_get_symbolic_link_target_uri(NautilusFile *file) {
  ...
  if (file->details->symlink_name == NULL) {
    ...
    if (parent) {
      target =
          g_file_resolve_relative_path(parent, file->details->symlink_name);
      g_object_unref(parent);
    }
 ...
 * This is private and is used to decide whether or not to read the top left
 *text.
 * @file: NautilusFile representing the file in question.
 *
...
void nautilus_file_call_when_ready(NautilusFile *file,
  ...
  NAUTILUS_FILE_CLASS(G_OBJECT_GET_CLASS(file))
      ->call_when_ready(file, file_attributes, callback, callback_data);
}
...
static void invalidate_directory_count(NautilusFile *file) {
  file->details->directory_count_is_up_to_date = FALSE;
}

static void invalidate_deep_counts(NautilusFile *file) {
  file->details->deep_counts_status = NAUTILUS_REQUEST_NOT_STARTED;
}

static void invalidate_mime_list(NautilusFile *file) {
  file->details->mime_list_is_up_to_date = FALSE;
}

static void invalidate_file_info(NautilusFile *file) {
  file->details->file_info_is_up_to_date = FALSE;
}

static void invalidate_thumbnail(NautilusFile *file) {
  file->details->thumbnail_is_up_to_date = FALSE;
}

static void invalidate_mount(NautilusFile *file) {
...
void nautilus_file_invalidate_attributes(
    NautilusFile *file, NautilusFileAttributes file_attributes) {
...
NautilusFileAttributes nautilus_file_get_all_attributes(void) {
  return NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_DEEP_COUNTS |
         NAUTILUS_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
         NAUTILUS_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES |
         NAUTILUS_FILE_ATTRIBUTE_EXTENSION_INFO |
         NAUTILUS_FILE_ATTRIBUTE_THUMBNAIL | NAUTILUS_FILE_ATTRIBUTE_MOUNT;
}
...
void nautilus_file_dump(NautilusFile *file) {
  ...
  if (!file->details->got_file_info) {
    ...
    /* FIXME bugzilla.gnome.org 42431: add permissions and other useful stuff
     * here */
  }
  g_free(uri);
}
 ...
 * @list: GList of files.
 **/
GList *nautilus_file_list_ref(GList *list) {
 ...
 * @list: GList of files.
 **/
void nautilus_file_list_unref(GList *list) {
 ...
 * @list: GList of files.
 **/
void nautilus_file_list_free(GList *list) {
 ...
 * @list: GList of files.
 **/
GList *nautilus_file_list_copy(GList *list) {
  return g_list_copy(nautilus_file_list_ref(list));
}

static gboolean get_attributes_for_default_sort_type(NautilusFile *file,
                                                     gboolean *is_recent,
  ...
  *is_search = FALSE;
  ...
  if (file && nautilus_file_is_directory(file)) {
    is_recent_dir = nautilus_file_is_in_recent(file);
    is_download_dir = nautilus_file_is_user_special_directory(
        file, G_USER_DIRECTORY_DOWNLOAD);
    is_trash_dir = nautilus_file_is_in_trash(file);
    is_search_dir = nautilus_file_is_in_search(file);
...
NautilusFileSortType nautilus_file_get_default_sort_type(NautilusFile *file,
  ...
  is_trash = FALSE;
  is_search = FALSE;
  res = get_attributes_for_default_sort_type(file, &is_recent, &is_download,
                                             &is_trash, &is_search);
  ...
  if (res) {
    ...
    if (reversed != NULL) {
      *reversed = g_settings_get_boolean(
          nautilus_preferences,
...
static int compare_by_display_name_cover(gconstpointer a, gconstpointer b) {
 ...
 * @list: GList of files.
 **/
GList *nautilus_file_list_sort_by_display_name(GList *list) {
...
static void file_list_ready_data_free(FileListReadyData *data) {
  GList *l;
...
file_list_ready_data_new(GList *file_list, NautilusFileListCallback callback,
                         gpointer callback_data) {
...
static void file_list_file_ready_callback(NautilusFile *file,
                                          gpointer user_data) {
  FileListReadyData *data;
...
void nautilus_file_list_call_when_ready(GList *file_list,
  ...
  data = file_list_ready_data_new(file_list, callback, callback_data);
  ...
  while (l != NULL) {
    ...
    nautilus_file_call_when_ready(file, attributes,
                                  file_list_file_ready_callback, data);
  }
}
...
static void thumbnail_limit_changed_callback(gpointer user_data) {
  cached_thumbnail_limit = g_settings_get_uint64(
      nautilus_preferences, NAUTILUS_PREFERENCES_FILE_THUMBNAIL_LIMIT);

  /*Converts the obtained limit in MB to bytes */
  cached_thumbnail_limit *= MEGA_TO_BASE_RATE;

  /* Tell the world that icons might have changed. We could invent a
   * narrower-scope signal to mean only "thumbnails might have changed" if this
   * ends up being slow for some reason.
   */
  emit_change_signals_for_all_files_in_all_directories();
}

static void show_thumbnails_changed_callback(gpointer user_data) {
  show_file_thumbs = g_settings_get_enum(
      nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_FILE_THUMBNAILS);
...
static gboolean real_get_item_count(NautilusFile *file, guint *count,
                                    gboolean *count_unreadable) {
  if (count_unreadable != NULL) {
...
real_get_deep_counts(NautilusFile *file, guint *directory_count,
                     guint *file_count, guint *unreadable_directory_count,
                     goffset *total_size) {
  GFileType type;
...
static void real_set_metadata(NautilusFile *file, const char *key,
                              const char *value) {
  /* Dummy default impl */
}

static void real_set_metadata_as_list(NautilusFile *file, const char *key,
                                      char **value) {
  /* Dummy default impl */
}
...
static void nautilus_file_class_init(NautilusFileClass *class) {
  ...
  attribute_date_modified_full_q =
      g_quark_from_static_string("date_modified_full");
  attribute_date_modified_with_time_q =
      g_quark_from_static_string("date_modified_with_time");
  ...
  signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(NautilusFileClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[UPDATED_DEEP_COUNT_IN_PROGRESS] = g_signal_new(
      "updated-deep-count-in-progress", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusFileClass, updated_deep_count_in_progress), NULL,
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(thumbnail_limit_changed_callback), NULL);
  show_thumbnails_changed_callback(NULL);
  g_signal_connect_swapped(
      nautilus_preferences,
      "changed::" NAUTILUS_PREFERENCES_SHOW_FILE_THUMBNAILS,
      G_CALLBACK(show_thumbnails_changed_callback), NULL);

  g_signal_connect(nautilus_signaller_get_current(), "mime-data-changed",
...
static gboolean nautilus_drag_can_accept_files(NautilusFile *drop_target_item) {
...
gboolean nautilus_drag_can_accept_items(NautilusFile *drop_target_item,
  ...
  for (max = 100; items != NULL && max >= 0; items = items->next, max--) {
    if (!nautilus_drag_can_accept_item(
            drop_target_item,
...
gboolean nautilus_drag_can_accept_info(NautilusFile *drop_target_item,
  ...
  case NAUTILUS_ICON_DND_RAW: {
    return nautilus_drag_can_accept_files(
        drop_target_item); /* Check if we can accept files at this location */
  }
...
static gboolean is_gone(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static char *get_name(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static char *get_uri(NautilusFileInfo *file_info) {
  NautilusFile *file;
  g_autoptr(GFile) location = NULL;
...
static char *get_parent_uri(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static char *get_uri_scheme(NautilusFileInfo *file_info) {
  NautilusFile *file;
  g_autoptr(GFile) location = NULL;
...
static char *get_mime_type(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static gboolean is_mime_type(NautilusFileInfo *file_info,
                             const char *mime_type) {
  NautilusFile *file;
...
static gboolean is_directory(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static void add_emblem(NautilusFileInfo *file_info, const char *emblem_name) {
  NautilusFile *file;
  ...
  if (file->details->pending_info_providers) {
    file->details->pending_extension_emblems = g_list_prepend(
        file->details->pending_extension_emblems, g_strdup(emblem_name));
  } else {
    file->details->extension_emblems =
        g_list_prepend(file->details->extension_emblems, g_strdup(emblem_name));
  }

  nautilus_file_changed(file);
}

static char *get_string_attribute(NautilusFileInfo *file_info,
                                  const char *attribute_name) {
  NautilusFile *file;
...
static void add_string_attribute(NautilusFileInfo *file_info,
                                 const char *attribute_name,
                                 const char *value) {
  NautilusFile *file;
  ...
  if (file->details->pending_info_providers != NULL) {
    ...
    if (file->details->pending_extension_attributes == NULL) {
      file->details->pending_extension_attributes = g_hash_table_new_full(
    ...
    if (file->details->extension_attributes == NULL) {
      file->details->extension_attributes = g_hash_table_new_full(
          g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_free);
    }
...
static void invalidate_extension_info(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static char *get_activation_uri(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static GFileType get_file_type(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static GFile *get_location(NautilusFileInfo *file_info) {
  NautilusFile *file;
  g_autoptr(GFile) location = NULL;
...
static GFile *get_parent_location(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static NautilusFileInfo *get_parent_info(NautilusFileInfo *file_info) {
  NautilusFile *file;
  NautilusFile *parent_file;
  ...
  parent_file =
      nautilus_directory_get_corresponding_file(file->details->directory);

  return NAUTILUS_FILE_INFO(parent_file);
}

static GMount *get_mount(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
static gboolean can_write(NautilusFileInfo *file_info) {
  NautilusFile *file;
...
void nautilus_self_check_file(void) {
  NautilusFile *file_1;
  NautilusFile *file_2;
  GList *list;
  ...
  EEL_CHECK_BOOLEAN_RESULT(nautilus_file_get_by_uri("file:///home/") == file_1,
                           TRUE);
  nautilus_file_unref(file_1);
  ...
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(
          file_1, file_2, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) < 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(
          file_1, file_2, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) > 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(file_1, file_1,
                                     NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE,
                                     FALSE) == 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(
          file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, TRUE, FALSE) == 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(
          file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) == 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_file_compare_for_sort(
          file_1, file_1, NAUTILUS_FILE_SORT_BY_DISPLAY_NAME, TRUE, TRUE) == 0,
      TRUE);
