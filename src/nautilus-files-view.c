...
typedef struct {
     ...
     * This flag should need not be set inside subclasses. NautilusFilesView
     * automatically sets 'loading' to TRUE before it begins loading a directory's
     * contents and to FALSE after it finishes loading the directory and its view.
     */
    gboolean loading;
...
static void trash_or_delete_files(GtkWindow *parent_window, const GList *files,
                                  NautilusFilesView *view);
static void load_directory(NautilusFilesView *view,
                           NautilusDirectory *directory);
static void on_clipboard_owner_changed(GtkClipboard *clipboard, GdkEvent *event,
                                       gpointer user_data);
...
static void metadata_for_files_in_directory_ready_callback(
    NautilusDirectory *directory, GList *files, gpointer callback_data);
static void nautilus_files_view_trash_state_changed_callback(
    NautilusTrashMonitor *trash, gboolean state, gpointer callback_data);
...
static void extract_files(NautilusFilesView *view, GList *files,
...
static void set_wallpaper_fallback(NautilusFile *file, gpointer user_data);

G_DEFINE_TYPE_WITH_CODE(NautilusFilesView, nautilus_files_view, GTK_TYPE_GRID,
                        G_IMPLEMENT_INTERFACE(NAUTILUS_TYPE_VIEW,
                                nautilus_files_view_iface_init)
                        G_ADD_PRIVATE(NautilusFilesView));

/*
 * Floating Bar code
 */
static void remove_loading_floating_bar(NautilusFilesView *view) {
...
static void real_setup_loading_floating_bar(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;

    priv = nautilus_files_view_get_instance_private(view);

    nautilus_floating_bar_set_primary_label(
        NAUTILUS_FLOATING_BAR(priv->floating_bar),
        nautilus_view_is_searching(NAUTILUS_VIEW(view)) ? _("Searching…")
        : _("Loading…"));
    nautilus_floating_bar_set_details_label(
        NAUTILUS_FLOATING_BAR(priv->floating_bar), NULL);
    nautilus_floating_bar_set_show_spinner(
        NAUTILUS_FLOATING_BAR(priv->floating_bar), priv->loading);
    nautilus_floating_bar_set_show_stop(NAUTILUS_FLOATING_BAR(priv->floating_bar),
                                        priv->loading);
...
static gboolean setup_loading_floating_bar_timeout_cb(gpointer user_data) {
...
static void setup_loading_floating_bar(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    ...
    priv->floating_bar_loading_timeout_id = g_timeout_add(
            FLOATING_BAR_LOADING_DELAY, setup_loading_floating_bar_timeout_cb, view);
}

static void floating_bar_stop_cb(NautilusFloatingBar *floating_bar,
                                 NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void real_floating_bar_set_short_status(NautilusFilesView *view,
    ...
    nautilus_floating_bar_set_show_spinner(
        NAUTILUS_FLOATING_BAR(priv->floating_bar), FALSE);
    nautilus_floating_bar_set_show_stop(NAUTILUS_FLOATING_BAR(priv->floating_bar),
                                        FALSE);

    if (primary_status == NULL && detail_status == NULL) {
        gtk_widget_hide(priv->floating_bar);
        nautilus_floating_bar_remove_hover_timeout(
            NAUTILUS_FLOATING_BAR(priv->floating_bar));
        return;
    }
...
static void floating_bar_set_status_data_free(gpointer data) {
...
static gboolean floating_bar_set_status_timeout_cb(gpointer data) {
    NautilusFilesViewPrivate *priv;
...
static gboolean remove_floating_bar_passthrough(gpointer data) {
    NautilusFilesViewPrivate *priv;
...
static void set_floating_bar_status(NautilusFilesView *view,
                                    const gchar *primary_status,
                                    const gchar *detail_status) {
    ...
    settings =
        gtk_settings_get_for_screen(gtk_widget_get_screen(GTK_WIDGET(view)));
    ...
    priv->floating_bar_set_passthrough_timeout_id = g_timeout_add(
                (guint)double_click_time, remove_floating_bar_passthrough, view);
    ...
    priv->floating_bar_set_status_timeout_id =
        g_timeout_add_full(G_PRIORITY_DEFAULT, (guint)(double_click_time / 2),
                           floating_bar_set_status_timeout_cb, status_data,
 ...
 *
 **/
static void nautilus_files_view_select_all(NautilusFilesView *view) {
    g_return_if_fail(NAUTILUS_IS_FILES_VIEW(view));

    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))->select_all(view);
}

static void nautilus_files_view_select_first(NautilusFilesView *view) {
    g_return_if_fail(NAUTILUS_IS_FILES_VIEW(view));

    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))->select_first(view);
}

static void nautilus_files_view_call_set_selection(NautilusFilesView *view,
        GList *selection) {
    g_return_if_fail(NAUTILUS_IS_FILES_VIEW(view));

    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
...
static void nautilus_files_view_invert_selection(NautilusFilesView *view) {
    g_return_if_fail(NAUTILUS_IS_FILES_VIEW(view));
 ...
 **/
static void nautilus_files_view_reveal_selection(NautilusFilesView *view) {
    g_return_if_fail(NAUTILUS_IS_FILES_VIEW(view));
...
static GMenuModel *nautilus_files_view_get_templates_menu(NautilusView *self) {
    GMenuModel *menu;
...
static GMenuModel *real_get_extensions_background_menu(NautilusView *view) {
    NautilusFilesViewPrivate *priv;
...
static GMenuModel *real_get_templates_menu(NautilusView *view) {
    NautilusFilesViewPrivate *priv;
...
static void nautilus_files_view_set_templates_menu(NautilusView *self,
        GMenuModel *menu) {
...
static void real_set_extensions_background_menu(NautilusView *view,
        GMenuModel *menu) {
    NautilusFilesViewPrivate *priv;
...
static void real_set_templates_menu(NautilusView *view, GMenuModel *menu) {
    NautilusFilesViewPrivate *priv;
...
static gboolean showing_trash_directory(NautilusFilesView *view) {
    NautilusFile *file;
...
static gboolean showing_recent_directory(NautilusFilesView *view) {
    NautilusFile *file;
...
static gboolean showing_starred_directory(NautilusFilesView *view) {
    NautilusFile *file;
...
nautilus_files_view_supports_creating_files(NautilusFilesView *view) {
    ...
    return !nautilus_files_view_is_read_only(view) &&
           !showing_trash_directory(view) && !showing_recent_directory(view) &&
...
static gboolean nautilus_files_view_is_empty(NautilusFilesView *view) {
    g_return_val_if_fail(NAUTILUS_IS_FILES_VIEW(view), FALSE);
 ...
 *
 * bump the current zoom level by invoking the relevant subclass through the
 *slot
 *
 **/
...
nautilus_files_view_restore_standard_zoom_level(NautilusFilesView *view) {
    ...
    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
    ->restore_standard_zoom_level(view);
}
...
nautilus_files_view_get_zoom_level_percentage(NautilusFilesView *view) {
    ...
    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->get_zoom_level_percentage(view);
}
...
nautilus_files_view_is_zoom_level_default(NautilusFilesView *view) {
    ...
    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->is_zoom_level_default(view);
}

gboolean nautilus_files_view_is_searching(NautilusView *view) {
...
static guint nautilus_files_view_get_view_id(NautilusView *view) {
    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->get_view_id(NAUTILUS_FILES_VIEW(view));
}
 ...
 *
 **/
static GList *nautilus_files_view_get_selection(NautilusView *view) {
    g_return_val_if_fail(NAUTILUS_IS_FILES_VIEW(view), NULL);

    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->get_selection(NAUTILUS_FILES_VIEW(view));
}
...
static FileAndDirectory *file_and_directory_new(NautilusFile *file,
        NautilusDirectory *directory) {
...
static NautilusFile *file_and_directory_get_file(FileAndDirectory *fad) {
    g_return_val_if_fail(fad != NULL, NULL);

    return nautilus_file_ref(fad->file);
}

static void file_and_directory_free(gpointer data) {
...
static gboolean file_and_directory_equal(gconstpointer v1, gconstpointer v2) {
    const FileAndDirectory *fad1, *fad2;
    fad1 = v1;
    fad2 = v2;

    return (fad1->file == fad2->file && fad1->directory == fad2->directory);
}

static guint file_and_directory_hash(gconstpointer v) {
...
static void script_launch_parameters_free(ScriptLaunchParameters *parameters) {
...
static char *get_view_directory(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    char *uri, *path;
    GFile *f;
...
static void preview_export_data_free(gpointer _data) {
...
static void nautilus_files_view_preview_update(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv =
        nautilus_files_view_get_instance_private(view);
    GtkApplication *app;
    GtkWindow *window;
    g_autolist(NautilusFile) selection = NULL;
    PreviewExportData *data;

    if (!priv->active || !nautilus_previewer_is_visible()) {
        return;
    }
...
void nautilus_files_view_preview_selection_event(NautilusFilesView *view,
    ...
    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
    ->preview_selection_event(view, direction);
}
...
void nautilus_files_view_activate_selection(NautilusFilesView *view) {
    ...
    selection = nautilus_view_get_selection(NAUTILUS_VIEW(view));
    nautilus_files_view_activate_files(view, selection, 0, TRUE);
}

void nautilus_files_view_activate_files(NautilusFilesView *view, GList *files,
    ...
    files_to_extract = nautilus_file_list_filter(
                           files, &files_to_activate,
                           (NautilusFileFilterFunc)nautilus_mime_file_extracts, NULL);
    ...
    if (nautilus_files_view_supports_extract_here(view)) {
        ...
        location =
            nautilus_file_get_location(NAUTILUS_FILE(g_list_first(files)->data));
    ...
    nautilus_mime_activate_files(nautilus_files_view_get_containing_window(view),
                                 priv->slot, files_to_activate, path, flags,
                                 confirm_multiple);
...
static void action_open_with_default_application(GSimpleAction *action,
        GVariant *state,
        gpointer user_data) {
    NautilusFilesView *view;
...
static void action_open_item_location(GSimpleAction *action, GVariant *state,
                                      gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
    NautilusFile *item;
    ...
    nautilus_application_open_location_full(
        NAUTILUS_APPLICATION(g_application_get_default()), parent_location, 0,
        selection, NULL, nautilus_files_view_get_nautilus_window_slot(view));
...
static void action_open_item_new_tab(GSimpleAction *action, GVariant *state,
                                     gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
    ...
    nautilus_files_view_activate_files(
        view, selection,
        NAUTILUS_OPEN_FLAG_NEW_TAB | NAUTILUS_OPEN_FLAG_DONT_MAKE_ACTIVE, TRUE);
}

static void app_chooser_dialog_response_cb(GtkDialog *dialog, gint response_id,
        gpointer user_data) {
...
static void choose_program(NautilusFilesView *view, GList *files) {
    GtkWidget *dialog;
...
static void open_with_other_program(NautilusFilesView *view) {
    g_autolist(NautilusFile) selection = NULL;
...
static void action_open_with_other_application(GSimpleAction *action,
        GVariant *state,
        gpointer user_data) {
...
static void trash_or_delete_selected_files(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    priv = nautilus_files_view_get_instance_private(view);
...
static void action_move_to_trash(GSimpleAction *action, GVariant *state,
                                 gpointer user_data) {
    trash_or_delete_selected_files(NAUTILUS_FILES_VIEW(user_data));
}

static void action_remove_from_recent(GSimpleAction *action, GVariant *state,
                                      gpointer user_data) {
...
static void delete_selected_files(NautilusFilesView *view) {
    GList *selection;
    GList *node;
    GList *locations;
    ...
    for (node = selection; node != NULL; node = node->next) {
        locations = g_list_prepend(
                        locations, nautilus_file_get_location((NautilusFile *)node->data));
    }
    locations = g_list_reverse(locations);

    nautilus_file_operations_delete_async(
        locations, nautilus_files_view_get_containing_window(view), NULL, NULL,
        NULL);
...
static void action_delete(GSimpleAction *action, GVariant *state,
                          gpointer user_data) {
    delete_selected_files(NAUTILUS_FILES_VIEW(user_data));
}

static void action_star(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
    NautilusFilesViewPrivate *priv;
    ...
    nautilus_tag_manager_star_files(priv->tag_manager, G_OBJECT(view), selection,
                                    NULL, priv->starred_cancellable);
}

static void action_unstar(GSimpleAction *action, GVariant *state,
                          gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
    NautilusFilesViewPrivate *priv;
    ...
    nautilus_tag_manager_unstar_files(priv->tag_manager, G_OBJECT(view),
                                      selection, NULL, priv->starred_cancellable);
}

static void action_restore_from_trash(GSimpleAction *action, GVariant *state,
                                      gpointer user_data) {
    NautilusFilesView *view;
    GList *selection;
    ...
    selection = nautilus_files_view_get_selection_for_file_transfer(view);
    nautilus_restore_files_from_trash(
        selection, nautilus_files_view_get_containing_window(view));

    nautilus_file_list_free(selection);
}

static void action_select_all(GSimpleAction *action, GVariant *state,
                              gpointer user_data) {
    NautilusFilesView *view;
...
static void action_invert_selection(GSimpleAction *action, GVariant *state,
                                    gpointer user_data) {
    g_assert(NAUTILUS_IS_FILES_VIEW(user_data));
...
static void action_preview_selection(GSimpleAction *action, GVariant *state,
                                     gpointer user_data) {
...
static void pattern_select_response_cb(GtkWidget *dialog, int response,
                                       gpointer user_data) {
    NautilusFilesView *view;
    NautilusDirectory *directory;
    GtkWidget *entry;
    ...
    case GTK_RESPONSE_OK: {
        ...
        selection = nautilus_directory_match_pattern(
                        directory, gtk_entry_get_text(GTK_ENTRY(entry)));
...
static void select_pattern(NautilusFilesView *view) {
    ...
    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-files-view-select-items.ui");
    ...
    example_pattern = g_strdup_printf("%s<i>%s</i> ", _("Examples: "),
                                      "*.png, file\?\?.txt, pict*.\?\?\?");
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(pattern_select_response_cb),
                     view);
    gtk_widget_show(dialog);
}

static void action_select_pattern(GSimpleAction *action, GVariant *state,
                                  gpointer user_data) {
    g_assert(NAUTILUS_IS_FILES_VIEW(user_data));
...
static void track_newly_added_locations(NautilusFilesView *view,
                                        GList *new_files, gpointer user_data) {
    GHashTable *added_locations;
...
static void new_folder_done(GFile *new_folder, gboolean success,
                            gpointer user_data) {
    NautilusFilesView *directory_view;
    NautilusFilesViewPrivate *priv;
    NautilusFile *file;
    ...
    if (data->selection != NULL) {
        ...
        for (l = data->selection; l != NULL; l = l->next) {
            uris =
                g_list_prepend(uris, nautilus_file_get_uri((NautilusFile *)l->data));
        }
        uris = g_list_reverse(uris);

        target_uri = nautilus_file_get_uri(file);

        nautilus_files_view_move_copy_items(directory_view, uris, target_uri,
                                            GDK_ACTION_MOVE);
    ...
    if (g_hash_table_contains(data->added_locations, new_folder)) {
        ...
        g_hash_table_insert(priv->pending_reveal, file, GUINT_TO_POINTER(TRUE));
    }
...
static NewFolderData *new_folder_data_new(NautilusFilesView *directory_view,
        gboolean with_selection) {
    NewFolderData *data;

    data = g_new(NewFolderData, 1);
    data->directory_view = directory_view;
    data->added_locations = g_hash_table_new_full(
                                g_file_hash, (GEqualFunc)g_file_equal, g_object_unref, NULL);
    if (with_selection) {
        data->selection =
            nautilus_files_view_get_selection_for_file_transfer(directory_view);
    } else {
        data->selection = NULL;
    }
...
static GdkRectangle *nautilus_files_view_compute_rename_popover_pointing_to(
    NautilusFilesView *view) {
    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->compute_rename_popover_pointing_to(view);
}

static void disconnect_rename_controller_signals(NautilusFilesView *self) {
    NautilusFilesViewPrivate *priv;
    ...
    g_clear_signal_handler(&priv->name_accepted_handler_id,
                           priv->rename_file_controller);
    g_clear_signal_handler(&priv->cancelled_handler_id,
                           priv->rename_file_controller);
}

static void rename_file_popover_controller_on_name_accepted(
    NautilusFileNameWidgetController *controller, gpointer user_data) {
    NautilusFilesView *view;
    ...
    target_file = nautilus_rename_file_popover_controller_get_target_file(
                      priv->rename_file_controller);
...
static void rename_file_popover_controller_on_cancelled(
    NautilusFileNameWidgetController *controller, gpointer user_data) {
    NautilusFilesView *view;
...
nautilus_files_view_rename_file_popover_new(NautilusFilesView *view,
    ...
    nautilus_rename_file_popover_controller_show_for_file(
        priv->rename_file_controller, target_file, pointing_to, GTK_WIDGET(view));

    priv->name_accepted_handler_id = g_signal_connect(
                                         priv->rename_file_controller, "name-accepted",
                                         G_CALLBACK(rename_file_popover_controller_on_name_accepted), view);
    priv->cancelled_handler_id = g_signal_connect(
                                     priv->rename_file_controller, "cancelled",
                                     G_CALLBACK(rename_file_popover_controller_on_cancelled), view);
}

static void new_folder_dialog_controller_on_name_accepted(
    NautilusFileNameWidgetController *controller, gpointer user_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
    NewFolderData *data;
    ...
    with_selection = nautilus_new_folder_dialog_controller_get_with_selection(
    ...
    g_signal_connect_data(
        view, "add-files", G_CALLBACK(track_newly_added_locations),
        data->added_locations, (GClosureNotify)NULL, G_CONNECT_AFTER);

    parent_uri = nautilus_files_view_get_backing_uri(view);
    parent = nautilus_file_get_by_uri(parent_uri);
    nautilus_file_operations_new_folder(GTK_WIDGET(view), NULL, parent_uri, name,
...
static void new_folder_dialog_controller_on_cancelled(
    NautilusNewFolderDialogController *controller, gpointer user_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void nautilus_files_view_new_folder_dialog_new(NautilusFilesView *view,
        gboolean with_selection) {
    ...
    if (with_selection) {
        g_autolist(NautilusFile) selection = NULL;
        selection = nautilus_view_get_selection(NAUTILUS_VIEW(view));
        common_prefix = nautilus_get_common_filename_prefix(
                            selection, MIN_COMMON_FILENAME_PREFIX_LENGTH);
    }

    priv->new_folder_controller = nautilus_new_folder_dialog_controller_new(
                                      nautilus_files_view_get_containing_window(view), containing_directory,
                                      with_selection, common_prefix);

    g_signal_connect(priv->new_folder_controller, "name-accepted",
                     (GCallback)new_folder_dialog_controller_on_name_accepted,
                     view);
    g_signal_connect(priv->new_folder_controller, "cancelled",
                     (GCallback)new_folder_dialog_controller_on_cancelled, view);
}
...
static void compress_done(GFile *new_file, gboolean success,
                          gpointer user_data) {
    CompressData *data;
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
    ...
    g_signal_handlers_disconnect_by_func(
        view, G_CALLBACK(track_newly_added_locations), data->added_locations);
...
static void compress_dialog_controller_on_name_accepted(
    NautilusFileNameWidgetController *controller, gpointer user_data) {
    ...
    for (l = callback_data->selection; l != NULL; l = l->next) {
        source_files =
            g_list_prepend(source_files, nautilus_file_get_location(l->data));
    }
    ...
    data->added_locations = g_hash_table_new_full(
                                g_file_hash, (GEqualFunc)g_file_equal, g_object_unref, NULL);
    g_object_add_weak_pointer(G_OBJECT(data->view), (gpointer *)&data->view);

    g_signal_connect_data(view, "add-files",
                          G_CALLBACK(track_newly_added_locations),
                          data->added_locations, NULL, G_CONNECT_AFTER);
    ...
    case NAUTILUS_COMPRESSION_ENCRYPTED_ZIP: {
        format = AUTOAR_FORMAT_ZIP;
        filter = AUTOAR_FILTER_NONE;
        passphrase = nautilus_compress_dialog_controller_get_passphrase(
                         priv->compress_controller);
    }
    break;
    ...
    nautilus_file_operations_compress(
        source_files, output, format, filter, passphrase,
        nautilus_files_view_get_containing_window(view), NULL, compress_done,
        data);
...
static void compress_dialog_controller_on_cancelled(
    NautilusNewFolderDialogController *controller, gpointer user_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void compress_callback_data_free(CompressCallbackData *data) {
    nautilus_file_list_free(data->selection);
    g_free(data);
}

static void nautilus_files_view_compress_dialog_new(NautilusFilesView *view) {
    ...
    containing_directory =
        nautilus_directory_get_by_uri(nautilus_files_view_get_backing_uri(view));
    ...
    g_signal_connect_data(priv->compress_controller, "name-accepted",
                          (GCallback)compress_dialog_controller_on_name_accepted,
                          data, (GClosureNotify)compress_callback_data_free,
                          G_CONNECT_AFTER);

    g_signal_connect(priv->compress_controller, "cancelled",
                     (GCallback)compress_dialog_controller_on_cancelled, view);
}

static void nautilus_files_view_new_folder(NautilusFilesView *directory_view,
        gboolean with_selection) {
    nautilus_files_view_new_folder_dialog_new(directory_view, with_selection);
}

static NewFolderData *setup_new_folder_data(NautilusFilesView *directory_view) {
    NewFolderData *data;

    data = new_folder_data_new(directory_view, FALSE);

    g_signal_connect_data(
        directory_view, "add-files", G_CALLBACK(track_newly_added_locations),
        data->added_locations, (GClosureNotify)NULL, G_CONNECT_AFTER);

    return data;
}

void nautilus_files_view_new_file_with_initial_contents(
    NautilusFilesView *view, const char *parent_uri, const char *filename,
    const char *initial_contents, int length) {
    NewFolderData *data;
    ...
    nautilus_file_operations_new_file(GTK_WIDGET(view), parent_uri, filename,
...
static void nautilus_files_view_new_file(NautilusFilesView *directory_view,
    ...
    nautilus_file_operations_new_file_from_template(
        ...
        parent_uri != NULL ? parent_uri : container_uri, NULL, source_uri,
        new_folder_done, data);
...
static void action_new_folder(GSimpleAction *action, GVariant *state,
                              gpointer user_data) {
    g_assert(NAUTILUS_IS_FILES_VIEW(user_data));
...
static void action_new_folder_with_selection(GSimpleAction *action,
        GVariant *state,
        gpointer user_data) {
    g_assert(NAUTILUS_IS_FILES_VIEW(user_data));
...
static void action_properties(GSimpleAction *action, GVariant *state,
                              gpointer user_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
    g_autolist(NautilusFile) selection = NULL;
    ...
    if (g_list_length(selection) == 0) {
        if (priv->directory_as_file != NULL) {
            ...
            nautilus_properties_window_present(files, GTK_WIDGET(view), NULL, NULL,
                                               NULL);

            nautilus_file_list_free(files);
        }
    } else {
        nautilus_properties_window_present(selection, GTK_WIDGET(view), NULL, NULL,
                                           NULL);
    }
}

static void action_current_dir_properties(GSimpleAction *action,
...
static void nautilus_files_view_set_show_hidden_files(NautilusFilesView *view,
    ...
    if (show_hidden != priv->show_hidden_files) {
        ...
        g_settings_set_boolean(gtk_filechooser_preferences,
                               NAUTILUS_PREFERENCES_SHOW_HIDDEN_FILES, show_hidden);
...
static void action_show_hidden_files(GSimpleAction *action, GVariant *state,
                                     gpointer user_data) {
    gboolean show_hidden;
    NautilusFilesView *view;
...
static void action_zoom_in(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
    NautilusFilesView *view;
...
static void action_zoom_out(GSimpleAction *action, GVariant *state,
                            gpointer user_data) {
    NautilusFilesView *view;
...
static void action_zoom_standard(GSimpleAction *action, GVariant *state,
                                 gpointer user_data) {
    nautilus_files_view_restore_standard_zoom_level(user_data);
}

static void action_open_item_new_window(GSimpleAction *action, GVariant *state,
                                        gpointer user_data) {
    NautilusFilesView *view;
    GList *selection;
    ...
    nautilus_files_view_activate_files(view, selection,
                                       NAUTILUS_OPEN_FLAG_NEW_WINDOW, TRUE);

    nautilus_file_list_free(selection);
}

static void handle_clipboard_data(NautilusFilesView *view,
                                  GtkSelectionData *selection_data,
                                  char *destination_uri, GdkDragAction action) {
    GList *item_uris;

    item_uris =
        nautilus_clipboard_get_uri_list_from_selection_data(selection_data);
...
static void paste_clipboard_data(NautilusFilesView *view,
                                 GtkSelectionData *selection_data,
...
static void paste_clipboard_received_callback(GtkClipboard *clipboard,
        GtkSelectionData *selection_data,
        gpointer data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void paste_files(NautilusFilesView *view) {
     ...
     */
    g_object_ref(view);
    gtk_clipboard_request_contents(clipboard, nautilus_clipboard_get_atom(),
                                   paste_clipboard_received_callback, view);
}

static void action_paste_files(GSimpleAction *action, GVariant *state,
                               gpointer user_data) {
    NautilusFilesView *view;
...
static void action_paste_files_accel(GSimpleAction *action, GVariant *state,
                                     gpointer user_data) {
    NautilusFilesView *view;
...
static void create_links_clipboard_received_callback(
...
static void action_create_links(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFilesView *view;
    ...
    gtk_clipboard_request_contents(
        nautilus_clipboard_get(GTK_WIDGET(view)), nautilus_clipboard_get_atom(),
        create_links_clipboard_received_callback, view);
}

static void click_policy_changed_callback(gpointer callback_data) {
    NautilusFilesView *view;
...
static void sort_directories_first_changed_callback(gpointer callback_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
    ...
    preference_value = g_settings_get_boolean(
...
static void show_hidden_files_changed_callback(gpointer callback_data) {
    ...
    preference_value = g_settings_get_boolean(
                           gtk_filechooser_preferences, NAUTILUS_PREFERENCES_SHOW_HIDDEN_FILES);
...
static gboolean set_up_scripts_directory_global(void) {
    ...
    if (override) {
        old_scripts_directory_path =
            g_build_filename(override, "nautilus-scripts", NULL);
    } else {
        old_scripts_directory_path =
            g_build_filename(g_get_home_dir(), ".gnome2", "nautilus-scripts", NULL);
    }
    ...
    if (file_type == G_FILE_TYPE_DIRECTORY &&
        ...
        if (!g_file_query_exists(updated, NULL)) {
            ...
            if (error == NULL ||
                ...
                g_file_set_attribute_uint32(parent, G_FILE_ATTRIBUTE_UNIX_MODE, S_IRWXU,
                                            G_FILE_QUERY_INFO_NONE, NULL, NULL);

                g_file_move(old_scripts_directory, scripts_directory, G_FILE_COPY_NONE,
                            NULL, NULL, NULL, &error);

                if (error == NULL) {
                    g_file_replace_contents(updated, message, strlen(message), NULL,
                                            FALSE, G_FILE_CREATE_PRIVATE, NULL, NULL,
                                            NULL);
                }
            }
    ...
    if (error == NULL || g_error_matches(error, G_IO_ERROR, G_IO_ERROR_EXISTS)) {
        g_file_set_attribute_uint32(scripts_directory, G_FILE_ATTRIBUTE_UNIX_MODE,
                                    S_IRWXU, G_FILE_QUERY_INFO_NONE, NULL, NULL);
...
static void scripts_added_or_changed_callback(NautilusDirectory *directory,
        GList *files,
        gpointer callback_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void templates_added_or_changed_callback(NautilusDirectory *directory,
        GList *files,
        gpointer callback_data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void add_directory_to_directory_list(NautilusFilesView *view,
        NautilusDirectory *directory,
...
static void remove_directory_from_directory_list(NautilusFilesView *view,
        NautilusDirectory *directory,
        GList **directory_list,
        GCallback changed_callback) {
    *directory_list = g_list_remove(*directory_list, directory);

    g_signal_handlers_disconnect_by_func(directory, G_CALLBACK(changed_callback),
                                         view);
...
add_directory_to_scripts_directory_list(NautilusFilesView *view,
    ...
    add_directory_to_directory_list(
        view, directory, &priv->scripts_directory_list,
...
remove_directory_from_scripts_directory_list(NautilusFilesView *view,
    ...
    remove_directory_from_directory_list(
        view, directory, &priv->scripts_directory_list,
        G_CALLBACK(scripts_added_or_changed_callback));
}
...
add_directory_to_templates_directory_list(NautilusFilesView *view,
    ...
    add_directory_to_directory_list(
        view, directory, &priv->templates_directory_list,
...
static void slot_active_changed(NautilusWindowSlot *slot, GParamSpec *pspec,
                                NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    ...
    if (priv->active) {
        ...
        gtk_widget_insert_action_group(
            GTK_WIDGET(nautilus_files_view_get_window(view)), "view",
            G_ACTION_GROUP(priv->view_action_group));
    } else {
        remove_update_context_menus_timeout_callback(view);
        gtk_widget_insert_action_group(
            GTK_WIDGET(nautilus_files_view_get_window(view)), "view", NULL);
    }
}

static void nautilus_files_view_grab_focus(GtkWidget *widget) {
...
static void nautilus_files_view_dispose(GObject *object) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
    GtkClipboard *clipboard;
    GList *node, *next;
    ...
    g_signal_handlers_disconnect_by_func(gtk_filechooser_preferences,
                                         ...
                                         view);
    g_signal_handlers_disconnect_by_func(
        gtk_filechooser_preferences, show_hidden_files_changed_callback, view);
    g_signal_handlers_disconnect_by_func(
        nautilus_window_state, nautilus_files_view_display_selection_info, view);
    ...
    g_signal_handlers_disconnect_by_func(clipboard, on_clipboard_owner_changed,
                                         view);
...
static void nautilus_files_view_finalize(GObject *object) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
 ...
 *
 * Display information about the current selection, and notify the view frame of
...
void nautilus_files_view_display_selection_info(NautilusFilesView *view) {
    ...
    for (p = selection; p != NULL; p = p->next) {
        ...
        if (nautilus_file_is_directory(file)) {
            folder_count++;
            if (nautilus_file_get_directory_item_count(file, &file_item_count,
                    NULL)) {
    ...
    /* Break out cases for localization's sake. But note that there are still
     * pieces being assembled in a particular order, which may be a problem for
     * some localizers.
     */
    ...
    if (folder_count != 0) {
        if (folder_count == 1 && non_folder_count == 0) {
            ...
            folder_count_str = g_strdup_printf(
                                   ngettext("%'d folder selected", "%'d folders selected", folder_count),
                                   folder_count);
        }
        ...
        if (folder_count == 1) {
            if (!folder_item_count_known) {
                ...
                folder_item_count_str = g_strdup_printf(
                                            ngettext("(containing %'d item)", "(containing %'d items)",
                                                     folder_item_count),
                                            folder_item_count);
            }
        } else {
            if (!folder_item_count_known) {
                 ...
                 * more than 1) */
                folder_item_count_str = g_strdup_printf(
                                            ngettext("(containing a total of %'d item)",
                                                     "(containing a total of %'d items)", folder_item_count),
                                            folder_item_count);
            }
        }
    }
        ...
        if (folder_count == 0) {
            if (non_folder_count == 1) {
                non_folder_count_str =
                    g_strdup_printf(_("“%s” selected"), first_item_name);
            } else {
                non_folder_count_str =
            ...
            non_folder_count_str = g_strdup_printf(
                                       ngettext("%'d other item selected", "%'d other items selected",
                                                non_folder_count),
                                       non_folder_count);
        }
    ...
    if (folder_count == 0 && non_folder_count == 0) {
        ...
        if (folder_item_count_known) {
            folder_counts_str =
                g_strconcat(folder_count_str, " ", folder_item_count_str, NULL);
        } else {
            folder_counts_str = g_strdup(folder_count_str);
        }

        if (non_folder_size_known) {
            non_folder_counts_str = g_strconcat(non_folder_count_str, " ",
                                                non_folder_item_count_str, NULL);
        } else {
         ...
         */
        primary_status =
            g_strdup_printf(_("%s, %s"), folder_counts_str, non_folder_counts_str);
        detail_status = NULL;
    }
...
static void nautilus_files_view_send_selection_change(NautilusFilesView *view) {
    g_signal_emit(view, signals[SELECTION_CHANGED], 0);
    g_object_notify(G_OBJECT(view), "selection");
}

static void nautilus_files_view_set_location(NautilusView *view,
        GFile *location) {
    NautilusDirectory *directory;
    NautilusFilesView *files_view;
    ...
    if (NAUTILUS_IS_SEARCH_DIRECTORY(directory)) {
        ...
        base_model = nautilus_search_directory_get_base_model(
                         NAUTILUS_SEARCH_DIRECTORY(directory));
        previous_query = nautilus_search_directory_get_query(
                             NAUTILUS_SEARCH_DIRECTORY(directory));
...
static gboolean reveal_selection_idle_callback(gpointer data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void real_check_empty_states(NautilusFilesView *view) {
    ...
    if (!priv->loading && nautilus_files_view_is_empty(view)) {
...
static void done_loading(NautilusFilesView *view, gboolean all_files_seen) {
    NautilusFilesViewPrivate *priv;
    g_autolist(NautilusFile) selection = NULL;
    ...
    if (!priv->in_destruction) {
        ...
        if (nautilus_view_is_searching(NAUTILUS_VIEW(view)) && all_files_seen &&
                selection == NULL && priv->pending_selection == NULL) {
        ...
        if (do_reveal) {
            if (NAUTILUS_IS_LIST_VIEW(view) ||
                    NAUTILUS_IS_VIEW_ICON_CONTROLLER(view)) {
...
static void debuting_files_data_free(DebutingFilesData *data) {
 ...
 */
static void debuting_files_add_files_callback(NautilusFilesView *view,
        GList *new_files,
    ...
    for (l = new_files; l != NULL; l = l->next) {
        ...
        if (g_hash_table_remove(data->debuting_files, location)) {
            ...
            data->added_files =
                g_list_prepend(data->added_files, NAUTILUS_FILE(l->data));
        }
...
static void copy_move_done_data_free(CopyMoveDoneData *data) {
    g_assert(data != NULL);
...
static void pre_copy_move_add_files_callback(NautilusFilesView *view,
        GList *new_files,
 ...
 */
static gboolean copy_move_done_partition_func(NautilusFile *file,
        gpointer callback_data) {
    GFile *location;
    gboolean result;
...
static gboolean remove_not_really_moved_files(gpointer key, gpointer value,
        gpointer callback_data) {
    GList **added_files;
    GFile *loc;
    ...
    *added_files = g_list_prepend(*added_files, nautilus_file_get(loc));
    return TRUE;
}
...
static void copy_move_done_callback(GHashTable *debuting_files,
    ...
    if (directory_view != NULL) {
        ...
        debuting_files_data->added_files = nautilus_file_list_filter(
                                               ...
                                               copy_move_done_partition_func, debuting_files);
        nautilus_file_list_free(copy_move_done_data->added_files);
        copy_move_done_data->added_files = failed_files;

        /* We're passed the same data used by pre_copy_move_add_files_callback, so
         * disconnecting it will free data. We've already siphoned off the
         * added_files we need, and stashed the directory_view pointer.
         */
        g_signal_handlers_disconnect_by_func(
            directory_view, G_CALLBACK(pre_copy_move_add_files_callback), data);
         ...
         */
        g_hash_table_foreach_remove(debuting_files, remove_not_really_moved_files,
        ...
        if (g_hash_table_size(debuting_files) == 0) {
            ...
            if (debuting_files_data->added_files != NULL) {
                nautilus_files_view_call_set_selection(
                    directory_view, debuting_files_data->added_files);
                nautilus_files_view_reveal_selection(directory_view);
            }
            debuting_files_data_free(debuting_files_data);
        } else {
            /* We need to run after the default handler adds the folder we want to
             * operate on. The ADD_FILES signal is registered as G_SIGNAL_RUN_LAST, so
             * we must use connect_after.
             */
            g_signal_connect_data(
                directory_view, "add-files",
                G_CALLBACK(debuting_files_add_files_callback), debuting_files_data,
                (GClosureNotify)debuting_files_data_free, G_CONNECT_AFTER);
        }
...
static gboolean view_file_still_belongs(NautilusFilesView *view,
                                        FileAndDirectory *fad) {
    NautilusFilesViewPrivate *priv;
...
static gboolean still_should_show_file(NautilusFilesView *view,
                                       FileAndDirectory *fad) {
    return nautilus_files_view_should_show_file(view, fad->file) &&
           view_file_still_belongs(view, fad);
}

static gboolean ready_to_load(NautilusFile *file) {
    return nautilus_file_check_if_ready(file, NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);
}

static int compare_files_cover(gconstpointer a, gconstpointer b,
                               gpointer callback_data) {
    const FileAndDirectory *fad1, *fad2;
    NautilusFilesView *view;
...
static void sort_files(NautilusFilesView *view, GList **list) {
 ...
 */
static void process_new_files(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void on_end_file_changes(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    ...
    if (g_hash_table_size(priv->pending_reveal) > 0) {
        ...
        for (l = keys; l && all_files_acknowledged; l = l->next) {
            all_files_acknowledged =
                GPOINTER_TO_UINT(g_hash_table_lookup(priv->pending_reveal, l->data));
        }
...
static int compare_pointers(gconstpointer pointer_1, gconstpointer pointer_2) {
...
static gboolean _g_lists_sort_and_check_for_intersection(GList **list_1,
        GList **list_2) {
    GList *node_1;
    GList *node_2;
    int compare_result;
...
static void process_old_files(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    ...
    if (files_added != NULL || files_changed != NULL) {
        ...
        for (GList *node = files_added; node != NULL; node = node->next) {
            ...
            if (g_hash_table_contains(priv->pending_reveal, pending->file)) {
                g_hash_table_insert(priv->pending_reveal, pending->file,
                                    GUINT_TO_POINTER(TRUE));
            }
        }

        if (files_added != NULL) {
            g_signal_emit(view, signals[ADD_FILES], 0, pending_additions);
        }
        ...
        for (GList *node = files_changed; node != NULL; node = node->next) {
            ...
            if (g_hash_table_contains(priv->pending_reveal, pending->file)) {
                if (should_show_file) {
                    ...
                    g_hash_table_remove(priv->pending_reveal, pending->file);
                }
            }
        }
        ...
        if (files_changed != NULL) {
            g_autolist(NautilusFile) selection = NULL;
            selection = nautilus_view_get_selection(NAUTILUS_VIEW(view));
            files = g_list_copy_deep(files_changed,
                                     (GCopyFunc)file_and_directory_get_file, NULL);
            send_selection_change =
                _g_lists_sort_and_check_for_intersection(&files, &selection);
            nautilus_file_list_free(files);
        }
...
static void display_pending_files(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    g_autolist(NautilusFile) selection = NULL;
    ...
    if (selection == NULL && !priv->pending_selection &&
            nautilus_view_is_searching(NAUTILUS_VIEW(view))) {
        nautilus_files_view_select_first(view);
    }
...
static void update_context_menus_if_pending(NautilusFilesView *view) {
    remove_update_context_menus_timeout_callback(view);

    nautilus_files_view_update_context_menus(view);
}

static gboolean update_context_menus_timeout_callback(gpointer data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static gboolean display_pending_callback(gpointer data) {
    NautilusFilesView *view;
    NautilusFilesViewPrivate *priv;
...
static void schedule_idle_display_of_pending_files(NautilusFilesView *view) {
    ...
    priv->display_pending_source_id = g_idle_add_full(
                                          G_PRIORITY_DEFAULT_IDLE - 20, display_pending_callback, view, NULL);
}

static void schedule_timeout_display_of_pending_files(NautilusFilesView *view,
        guint interval) {
    NautilusFilesViewPrivate *priv;
...
static void queue_pending_files(NautilusFilesView *view,
                                NautilusDirectory *directory, GList *files,
                                GList **pending_list) {
    NautilusFilesViewPrivate *priv;
    GList *fad_list;
    ...
    fad_list =
        g_list_copy_deep(files, (GCopyFunc)file_and_directory_new, directory);
    *pending_list = g_list_concat(fad_list, *pending_list);
     ...
     * seen */
    if (!priv->loading || (nautilus_directory_are_all_files_seen(directory) ||
                           nautilus_view_is_searching(NAUTILUS_VIEW(view)))) {
        schedule_timeout_display_of_pending_files(view, priv->update_interval);
    }
}

static void remove_changes_timeout_callback(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static gboolean changes_timeout_callback(gpointer data) {
    gint64 now;
    gint64 time_delta;
    gboolean ret;
    NautilusFilesView *view;
    ...
    if (time_delta < UPDATE_INTERVAL_RESET * 1000) {
        if (priv->update_interval < UPDATE_INTERVAL_MAX && priv->loading) {
            /* Increase */
...
static void schedule_changes(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
     ...
     */
    if (priv->changes_timeout_id != 0 || priv->loading) {
        return;
    }

    priv->changes_timeout_id = g_timeout_add(UPDATE_INTERVAL_TIMEOUT_INTERVAL,
                               changes_timeout_callback, view);
}

static void files_added_callback(NautilusDirectory *directory, GList *files,
                                 gpointer callback_data) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *view;
    GtkWindow *window;
    ...
    DEBUG_FILES(files, "Files added in window %p: %s", window,
...
static void files_changed_callback(NautilusDirectory *directory, GList *files,
                                   gpointer callback_data) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *view;
    GtkWindow *window;
    ...
    window = nautilus_files_view_get_containing_window(view);
    uri = nautilus_files_view_get_uri(view);
    DEBUG_FILES(files, "Files changed in window %p: %s", window,
                uri ? uri : "(no directory)");
    g_free(uri);
...
static void load_error_callback(NautilusDirectory *directory, GError *error,
                                gpointer callback_data) {
    NautilusFilesView *view;
    ...
    nautilus_report_error_loading_directory(
        nautilus_files_view_get_directory_as_file(view), error,
        nautilus_files_view_get_containing_window(view));
}
...
void nautilus_files_view_add_subdirectory(NautilusFilesView *view,
    ...
    nautilus_directory_file_monitor_add(directory, &priv->model,
                                        priv->show_hidden_files, attributes,
                                        files_added_callback, view);

    g_signal_connect(directory, "files-added", G_CALLBACK(files_added_callback),
                     view);
...
void nautilus_files_view_remove_subdirectory(NautilusFilesView *view,
    ...
    g_signal_handlers_disconnect_by_func(
        directory, G_CALLBACK(files_changed_callback), view);
...
static gboolean home_dir_in_selection(GList *selection) {
...
static void trash_or_delete_done_cb(GHashTable *debuting_uris,
...
static void trash_or_delete_files(GtkWindow *parent_window, const GList *files,
    ...
    nautilus_file_operations_trash_or_delete_async(
        locations, parent_window, NULL,
        (NautilusDeleteCallback)trash_or_delete_done_cb, view);
    g_list_free_full(locations, g_object_unref);
}
...
static GdkPixbuf *get_menu_icon_for_file(NautilusFile *file,
        GtkWidget *widget) {
    ...
    pixbuf = nautilus_icon_info_get_pixbuf_nodefault_at_size(
...
static GList *get_extension_selection_menu_items(NautilusFilesView *view) {
    NautilusWindow *window;
    GList *items;
    GList *providers;
    GList *l;
    ...
    for (l = providers; l != NULL; l = l->next) {
        ...
        file_items = nautilus_menu_provider_get_file_items(
                         provider, GTK_WIDGET(window), selection);
...
static GList *get_extension_background_menu_items(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    NautilusWindow *window;
    GList *items;
    GList *providers;
    ...
    for (l = providers; l != NULL; l = l->next) {
        ...
        file_items = nautilus_menu_provider_get_background_items(
                         provider, GTK_WIDGET(window), file_info);
        items = g_list_concat(items, file_items);
    }
...
static void extension_action_callback(GSimpleAction *action, GVariant *state,
                                      gpointer user_data) {
    NautilusMenuItem *item = user_data;
    nautilus_menu_item_activate(item);
}

static void add_extension_action(NautilusFilesView *view,
    ...
    g_signal_connect_data(action, "activate",
                          ...
                          g_object_ref(item), (GClosureNotify)g_object_unref, 0);
...
static GMenuModel *build_menu_for_extension_menu_items(
    NautilusFilesView *view, const gchar *extension_prefix, GList *menu_items) {
    GList *l;
    GMenu *gmenu;
    gint idx = 0;
    ...
    for (l = menu_items; l; l = l->next) {
        ...
        g_object_get(item, "label", &label, "menu", &menu, "name", &name, NULL);

        escaped_name = g_uri_escape_string(name, NULL, TRUE);
        extension_id = g_strdup_printf("extension_%s_%d_%s", extension_prefix, idx,
                                       escaped_name);
        ...
        if (menu != NULL) {
            ...
            children_menu =
                build_menu_for_extension_menu_items(view, extension_id, children);
...
static void update_extensions_menus(NautilusFilesView *view,
                                    GtkBuilder *builder) {
    ...
    nautilus_view_set_extensions_background_menu(NAUTILUS_VIEW(view),
            background_menu);
}

static char *change_to_view_directory(NautilusFilesView *view) {
    char *path;
    char *old_path;
...
static char **get_file_names_as_parameter_array(GList *selection,
        NautilusDirectory *model) {
    char **parameters;
...
static char *get_file_paths_as_newline_delimited_string(GList *selection) {
    return get_file_paths_or_uris_as_newline_delimited_string(selection, TRUE);
}

static char *get_file_uris_as_newline_delimited_string(GList *selection) {
 ...
 */
static void set_script_environment_variables(NautilusFilesView *view,
...
static void run_script(GSimpleAction *action, GVariant *state,
                       gpointer user_data) {
    ...
    priv = nautilus_files_view_get_instance_private(
    ...
    selection = nautilus_view_get_selection(
                    NAUTILUS_VIEW(launch_parameters->directory_view));
    set_script_environment_variables(launch_parameters->directory_view,
                                     selection);
    ...
    nautilus_launch_application_from_command_array(
        screen, quoted_path, FALSE, (const char *const *)parameters);

    unset_script_environment_variables();
    g_chdir(old_working_dir);
}

static void add_script_to_scripts_menus(NautilusFilesView *view,
                                        NautilusFile *file, GMenu *menu) {
    NautilusFilesViewPrivate *priv;
    gchar *name;
    g_autofree gchar *uri = NULL;
    ...
    g_signal_connect_data(action, "activate", G_CALLBACK(run_script),
                          launch_parameters,
...
static gboolean directory_belongs_in_scripts_menu(const char *uri) {
    int num_levels;
    int i;
...
static void nautilus_load_custom_accel_for_scripts(void) {
    gchar *path, *contents;
    gchar **lines, **result;
    GError *error = NULL;
    ...
    if (g_file_get_contents(path, &contents, NULL, &error)) {
        ...
        for (i = 0; lines[i] && (strstr(lines[i], " ") > 0); i++) {
            ...
            g_hash_table_insert(script_accels, g_strndup(result[1], max_len),
...
static GMenu *update_directory_in_scripts_menu(NautilusFilesView *view,
        NautilusDirectory *directory) {
    ...
    num = 0;
    any_scripts = FALSE;
    for (node = filtered; num < TEMPLATE_LIMIT && node != NULL;
            node = node->next, num++) {
        file = node->data;
        if (nautilus_file_is_directory(file)) {
            uri = nautilus_file_get_uri(file);
            if (directory_belongs_in_scripts_menu(uri)) {
                ...
                if (children_menu != NULL) {
                    ...
                    menu_item =
                        g_menu_item_new_submenu(file_name, G_MENU_MODEL(children_menu));
                    g_menu_append_item(menu, menu_item);
                    any_scripts = TRUE;
                    g_object_unref(menu_item);
...
static void update_scripts_menu(NautilusFilesView *view, GtkBuilder *builder) {
    NautilusFilesViewPrivate *priv;
...
static void create_template(GSimpleAction *action, GVariant *state,
                            gpointer user_data) {
    CreateTemplateParameters *parameters;

    parameters = user_data;

    nautilus_files_view_new_file(parameters->directory_view, NULL,
                                 parameters->file);
}

static void add_template_to_templates_menus(NautilusFilesView *view,
        NautilusFile *file, GMenu *menu) {
    NautilusFilesViewPrivate *priv;
    char *tmp, *uri, *name;
    g_autofree gchar *escaped_uri = NULL;
    ...
    g_signal_connect_data(action, "activate", G_CALLBACK(create_template),
                          parameters,
...
static gboolean filter_templates_callback(NautilusFile *file,
        gpointer callback_data) {
...
static GList *filter_templates(GList *files, gboolean show_hidden) {
    GList *filtered_files;
    GList *removed_files;

    filtered_files = nautilus_file_list_filter(files, &removed_files,
...
static void update_templates_menu(NautilusFilesView *view,
                                  GtkBuilder *builder) {
    NautilusFilesViewPrivate *priv;
    g_autolist(NautilusDirectory) sorted_copy = NULL;
    g_autoptr(NautilusDirectory) directory = NULL;
...
static void action_open_scripts_folder(GSimpleAction *action, GVariant *state,
                                       gpointer user_data) {
    ...
    nautilus_application_open_location_full(
        NAUTILUS_APPLICATION(g_application_get_default()), location, 0, NULL,
        NULL, NULL);
}
...
static void copy_data_free(CopyCallbackData *data) {
    nautilus_file_list_free(data->selection);
    g_free(data);
}

static void on_destination_dialog_response(GtkDialog *dialog, gint response_id,
        gpointer user_data) {
    ...
    if (response_id == GTK_RESPONSE_OK) {
        ...
        nautilus_files_view_move_copy_items(copy_data->view, uris, target_uri,
                                            copy_data->is_move ? GDK_ACTION_MOVE
                                            : GDK_ACTION_COPY);
...
static void copy_or_move_selection(NautilusFilesView *view, gboolean is_move) {
    NautilusFilesViewPrivate *priv;
    GtkWidget *dialog;
    g_autoptr(GFile) location = NULL;
    ...
    dialog = gtk_file_chooser_dialog_new(
                 title, GTK_WINDOW(nautilus_files_view_get_window(view)),
                 GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("_Cancel"), GTK_RESPONSE_CANCEL,
                 _("_Select"), GTK_RESPONSE_OK, NULL);
    ...
    if (nautilus_view_is_searching(NAUTILUS_VIEW(view))) {
        directory = nautilus_search_directory_get_base_model(
                        NAUTILUS_SEARCH_DIRECTORY(priv->model));
        location = nautilus_directory_get_location(directory);
    } else if (showing_starred_directory(view)) {
        location =
            nautilus_file_get_parent_location(NAUTILUS_FILE(selection->data));
    } else {
        location = nautilus_directory_get_location(priv->model);
    }

    gtk_file_chooser_set_current_folder_file(GTK_FILE_CHOOSER(dialog), location,
            NULL);
    g_signal_connect(dialog, "response",
                     G_CALLBACK(on_destination_dialog_response), copy_data);

    gtk_widget_show_all(dialog);
}

static void action_copy(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
    NautilusFilesView *view;
    GtkClipboard *clipboard;
    GList *selection;
...
static void action_cut(GSimpleAction *action, GVariant *state,
                       gpointer user_data) {
    NautilusFilesView *view;
    GList *selection;
    GtkClipboard *clipboard;
...
static void action_create_links_in_place(GSimpleAction *action, GVariant *state,
        gpointer user_data) {
    NautilusFilesView *view;
    GList *selection;
    GList *item_uris;
...
static void action_copy_to(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
    NautilusFilesView *view;
...
static void action_move_to(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
    NautilusFilesView *view;
...
static void paste_into(NautilusFilesView *view, NautilusFile *target) {
    PasteIntoData *data;
    ...
    gtk_clipboard_request_contents(nautilus_clipboard_get(GTK_WIDGET(view)),
                                   ...
                                   paste_into_clipboard_received_callback, data);
}

static void action_paste_files_into(GSimpleAction *action, GVariant *state,
                                    gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
...
static void real_action_rename(NautilusFilesView *view) {
    ...
    if (selection != NULL) {
        ...
        if (selection->next != NULL) {
            ...
            dialog = nautilus_batch_rename_dialog_new(
                         selection, nautilus_files_view_get_model(view),
                         nautilus_files_view_get_window(view));
...
static void action_rename(GSimpleAction *action, GVariant *state,
                          gpointer user_data) {
...
static void extract_done(GList *outputs, gpointer user_data) {
    NautilusFilesViewPrivate *priv;
    ExtractData *data;
    GList *l;
    ...
    for (l = outputs; l && all_files_acknowledged; l = l->next) {
        all_files_acknowledged =
            g_hash_table_contains(data->added_locations, l->data);
    }
    ...
    if (all_files_acknowledged) {
        ...
        nautilus_files_view_set_selection(NAUTILUS_VIEW(data->view), selection);
        ...
        for (l = outputs; l != NULL; l = l->next) {
            ...
            acknowledged = g_hash_table_contains(data->added_locations, l->data);

            g_hash_table_insert(priv->pending_reveal, nautilus_file_get(l->data),
...
static void extract_files(NautilusFilesView *view, GList *files,
    ...
    for (l = files; l != NULL; l = l->next) {
        locations = g_list_prepend(locations, nautilus_file_get_location(l->data));
    }

    locations = g_list_reverse(locations);

    extracting_to_current_directory = g_file_equal(
                                          destination_directory, nautilus_view_get_location(NAUTILUS_VIEW(view)));
    ...
    if (extracting_to_current_directory) {
        ...
        nautilus_file_operations_extract_files(
            locations, destination_directory,
            nautilus_files_view_get_containing_window(view), NULL, extract_done,
            data);
    } else {
        nautilus_file_operations_extract_files(
            locations, destination_directory,
            nautilus_files_view_get_containing_window(view), NULL, NULL, NULL);
    }
...
static void on_extract_destination_dialog_response(GtkDialog *dialog,
        gint response_id,
        gpointer user_data) {
    ExtractToData *data;
...
static void extract_files_to_chosen_location(NautilusFilesView *view,
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(on_extract_destination_dialog_response), data);

    gtk_widget_show_all(dialog);
}

static void action_extract_here(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
    g_autoptr(GFile) location = NULL;
    ...
    selection = nautilus_view_get_selection(NAUTILUS_VIEW(view));
    location =
        nautilus_file_get_location(NAUTILUS_FILE(g_list_first(selection)->data));
    /* Get a parent from a random file. We assume all files has a common parent.
     * But don't assume the parent is the view location, since that's not the
     * case in list view when expand-folder setting is set
     */
...
static void action_extract_to(GSimpleAction *action, GVariant *state,
                              gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
...
static void action_compress(GSimpleAction *action, GVariant *state,
                            gpointer user_data) {
    NautilusFilesView *view = user_data;
...
static gboolean can_run_in_terminal(GList *selection) {
    NautilusFile *file;
    ...
    if (nautilus_file_is_launchable(file) && nautilus_file_contains_text(file)) {
...
static void action_run_in_terminal(GSimpleAction *action, GVariant *state,
                                   gpointer user_data) {
    NautilusFilesView *view;
    g_autolist(NautilusFile) selection = NULL;
...
static void set_uri_as_wallpaper(const char *uri) {
    ...
    g_settings_set_enum(settings, BG_KEY_COLOR_TYPE,
                        G_DESKTOP_BACKGROUND_SHADING_SOLID);
    g_settings_set_enum(settings, BG_KEY_PICTURE_PLACEMENT,
...
static void wallpaper_copy_done_callback(GHashTable *debuting_files,
        gboolean success, gpointer data) {
    GHashTableIter iter;
    gpointer key, value;
...
static gboolean can_set_wallpaper(GList *selection) {
    NautilusFile *file;
...
static void set_wallpaper_with_portal_cb(GObject *source, GAsyncResult *result,
        gpointer user_data) {
...
static void set_wallpaper_with_portal(NautilusFile *file, gpointer user_data) {
    ...
    xdp_portal_set_wallpaper(portal, parent, uri,
                             XDP_WALLPAPER_FLAG_BACKGROUND |
                             XDP_WALLPAPER_FLAG_PREVIEW,
                             NULL, set_wallpaper_with_portal_cb, data);
...
static void set_wallpaper_fallback(NautilusFile *file, gpointer user_data) {
    ...
    target = g_file_new_build_filename(
                 g_get_user_special_dir(G_USER_DIRECTORY_PICTURES), _("Wallpapers"), NULL);
    ...
    if (!g_file_equal(file_parent, target)) {
        ...
        nautilus_file_operations_copy_move(uris, target_uri, GDK_ACTION_COPY,
                                           GTK_WIDGET(user_data), NULL,
                                           wallpaper_copy_done_callback, NULL);
    } else {
        set_uri_as_wallpaper(file_uri);
    }
}

static void action_set_as_wallpaper(GSimpleAction *action, GVariant *state,
                                    gpointer user_data) {
    g_autolist(NautilusFile) selection = NULL;
...
static void file_mount_callback(NautilusFile *file, GFile *result_location,
    ...
    if (error != NULL && (error->domain != G_IO_ERROR ||
        ...
        show_dialog(text, error->message,
                    GTK_WINDOW(nautilus_files_view_get_window(view)),
                    GTK_MESSAGE_ERROR);
        g_free(text);
        g_free(name);
...
static void file_unmount_callback(NautilusFile *file, GFile *result_location,
                                  GError *error, gpointer callback_data) {
    NautilusFilesView *view;
...
static void file_eject_callback(NautilusFile *file, GFile *result_location,
                                GError *error, gpointer callback_data) {
    NautilusFilesView *view;
...
static void file_stop_callback(NautilusFile *file, GFile *result_location,
                               GError *error, gpointer callback_data) {
    NautilusFilesView *view;
    ...
    if (error != NULL && (error->domain != G_IO_ERROR ||
                          (error->code != G_IO_ERROR_CANCELLED &&
                           error->code != G_IO_ERROR_FAILED_HANDLED))) {
        show_dialog(_("Unable to stop drive"), error->message,
                    GTK_WINDOW(nautilus_files_view_get_window(view)),
                    GTK_MESSAGE_ERROR);
    }
}

static void action_mount_volume(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFile *file;
    GList *selection, *l;
    NautilusFilesView *view;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_mount(file)) {
            mount_op = gtk_mount_operation_new(
                           nautilus_files_view_get_containing_window(view));
...
static void action_unmount_volume(GSimpleAction *action, GVariant *state,
                                  gpointer user_data) {
    NautilusFile *file;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_unmount(file)) {
            ...
            mount_op = gtk_mount_operation_new(
                           nautilus_files_view_get_containing_window(view));
            nautilus_file_unmount(file, mount_op, NULL, file_unmount_callback,
                                  g_object_ref(view));
            g_object_unref(mount_op);
        }
    }
...
static void action_eject_volume(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFile *file;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_eject(file)) {
            ...
            mount_op = gtk_mount_operation_new(
                           nautilus_files_view_get_containing_window(view));
            nautilus_file_eject(file, mount_op, NULL, file_eject_callback,
                                g_object_ref(view));
            g_object_unref(mount_op);
        }
    }
...
static void file_start_callback(NautilusFile *file, GFile *result_location,
                                GError *error, gpointer callback_data) {
    NautilusFilesView *view;
...
static void action_start_volume(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFile *file;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_start(file) ||
                nautilus_file_can_start_degraded(file)) {
            mount_op = gtk_mount_operation_new(
                           nautilus_files_view_get_containing_window(view));
...
static void action_stop_volume(GSimpleAction *action, GVariant *state,
                               gpointer user_data) {
    NautilusFile *file;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_stop(file)) {
            ...
            mount_op = gtk_mount_operation_new(
                           nautilus_files_view_get_containing_window(view));
            nautilus_file_stop(file, mount_op, NULL, file_stop_callback, view);
            g_object_unref(mount_op);
        }
    }
}

static void action_detect_media(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
    NautilusFile *file;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    for (l = selection; l != NULL; l = l->next) {
        ...
        if (nautilus_file_can_poll_for_media(file) &&
                !nautilus_file_is_media_check_automatic(file)) {
...
static gboolean can_paste_into_file(NautilusFile *file) {
    ...
    if (nautilus_file_has_activation_uri(file)) {
         ...
         *  case as can-write */
        res =
            (nautilus_file_get_file_type(activation_file) == G_FILE_TYPE_UNKNOWN) ||
            (nautilus_file_get_file_type(activation_file) ==
...
static void on_clipboard_contents_received(GtkClipboard *clipboard,
        GtkSelectionData *selection_data,
        gpointer user_data) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *view;
    ...
    if (priv->in_destruction || !priv->active) {
        /* We've been destroyed or became inactive since call */
        g_object_unref(view);
        return;
    }

    settings_show_create_link = g_settings_get_boolean(
                                    nautilus_preferences, NAUTILUS_PREFERENCES_SHOW_CREATE_LINK);
    ...
    can_link_from_copied_files =
        !nautilus_clipboard_is_cut_from_selection_data(selection_data) &&
...
static void on_clipboard_targets_received(GtkClipboard *clipboard,
...
static void file_should_show_foreach(NautilusFile *file, gboolean *show_mount,
                                     gboolean *show_unmount,
                                     gboolean *show_eject, gboolean *show_start,
                                     gboolean *show_stop, gboolean *show_poll,
...
static gboolean can_restore_from_trash(GList *files) {
    ...
    if (files != NULL) {
        if (g_list_length(files) == 1) {
            ...
            original_dirs_hash =
                nautilus_trashed_files_get_original_directories(files, NULL);
...
static gboolean can_delete_all(GList *files) {
    NautilusFile *file;
    GList *l;
...
static gboolean can_trash_all(GList *files) {
    NautilusFile *file;
    GList *l;
...
static gboolean all_in_trash(GList *files) {
    NautilusFile *file;
    GList *l;
...
static gboolean can_extract_all(GList *files) {
    NautilusFile *file;
    GList *l;
...
static gboolean nautilus_handles_all_files_to_extract(GList *files) {
    NautilusFile *file;
    GList *l;
...
static void real_update_actions_state(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    g_autolist(NautilusFile) selection = NULL;
    GList *l;
    ...
    can_delete_files = can_delete_all(selection) && selection_count != 0 &&
                       !selection_contains_home_dir;
    can_trash_files = can_trash_all(selection) && selection_count != 0 &&
                      !selection_contains_home_dir;
    can_copy_files = selection_count != 0;
    can_move_files = can_delete_files && !selection_contains_recent &&
                     !selection_contains_starred;
    can_paste_files_into = (!selection_contains_recent &&
                            !selection_contains_starred && selection_count == 1 &&
                            can_paste_into_file(NAUTILUS_FILE(selection->data)));
    can_extract_files = selection_count != 0 && can_extract_all(selection);
    can_extract_here = nautilus_files_view_supports_extract_here(view);
    handles_all_files_to_extract =
        nautilus_handles_all_files_to_extract(selection);
    settings_show_delete_permanently = g_settings_get_boolean(
    ...
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "rename");
    ...
    g_simple_action_set_enabled(
        G_SIMPLE_ACTION(action),
        can_extract_files && !handles_all_files_to_extract && can_extract_here);

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "extract-to");
    g_simple_action_set_enabled(
        G_SIMPLE_ACTION(action),
        can_extract_files && (!handles_all_files_to_extract || can_extract_here));

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "compress");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
    ...
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "new-folder");
    ...
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), can_delete_files);

    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "delete-permanently-menu-item");
    g_simple_action_set_enabled(
        G_SIMPLE_ACTION(action),
        can_delete_files && !can_trash_files && !selection_all_in_trash &&
        !selection_contains_recent && !selection_contains_starred);
    ...
    g_simple_action_set_enabled(
        G_SIMPLE_ACTION(action),
        can_delete_files && can_trash_files && settings_show_delete_permanently &&
    ...
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "cut");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                can_move_files && !selection_contains_recent &&
                                !selection_contains_starred);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "copy");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), can_copy_files);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "create-link-in-place");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                can_copy_files && can_create_files &&
                                settings_show_create_link);
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "copy-to");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), can_copy_files);
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "move-to");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                can_move_files && !selection_contains_recent &&
                                !selection_contains_starred);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
    ...
    for (l = selection;
            l != NULL && (show_mount || show_unmount || show_eject || show_start ||
                          show_stop || show_detect_media);
            l = l->next) {
        NautilusFile *file;
        ...
        file = NAUTILUS_FILE(l->data);
        file_should_show_foreach(file, &show_mount_one, &show_unmount_one,
                                 &show_eject_one, &show_start_one, &show_stop_one,
                                 &show_detect_media_one, &start_stop_type);
    ...
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "mount-volume");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_mount);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "unmount-volume");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_unmount);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "eject-volume");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_eject);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "start-volume");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_start);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "stop-volume");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_stop);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "detect-media");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_detect_media);

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "scripts");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
    ...
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "new-folder");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), can_create_files);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "paste");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                !is_read_only && !selection_contains_recent &&
                                !selection_contains_starred);

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "paste-into");
    g_simple_action_set_enabled(
        G_SIMPLE_ACTION(action),
    ...
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "properties");
    ...
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "new-document");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                can_create_files && !selection_contains_recent &&
                                !selection_contains_starred &&
    ...
    gtk_clipboard_request_targets(nautilus_clipboard_get(GTK_WIDGET(view)),
                                  on_clipboard_targets_received, view);

    g_object_ref(view); /* Need to keep the object alive until we get the reply */
    gtk_clipboard_request_contents(nautilus_clipboard_get(GTK_WIDGET(view)),
                                   nautilus_clipboard_get_atom(),
                                   on_clipboard_contents_received, view);

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "select-all");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
    ...
    g_action_group_change_action_state(
        view_action_group, "show-hidden-files",
        g_variant_new_boolean(priv->show_hidden_files));

    /* Zoom */
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "zoom-in");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                nautilus_files_view_can_zoom_in(view));
    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "zoom-out");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
    ...
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                ...
                                !zoom_level_is_default);
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group),
                                        "zoom-to-level");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
                                !nautilus_files_view_is_empty(view));

    current_location = nautilus_file_get_location(
                           nautilus_files_view_get_directory_as_file(view));
    current_uri = g_file_get_uri(current_location);
    can_star_current_directory = nautilus_tag_manager_can_star_contents(
                                     priv->tag_manager, current_location);
    ...
    action = g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "star");
    g_simple_action_set_enabled(G_SIMPLE_ACTION(action), show_star);

    action =
        g_action_map_lookup_action(G_ACTION_MAP(view_action_group), "unstar");
...
void nautilus_files_view_update_actions_state(NautilusFilesView *view) {
    ...
    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
    ->update_actions_state(view);
}

static void update_selection_menu(NautilusFilesView *view,
                                  GtkBuilder *builder) {
    NautilusFilesViewPrivate *priv =
        nautilus_files_view_get_instance_private(view);
    g_autolist(NautilusFile) selection = NULL;
...
static void update_background_menu(NautilusFilesView *view,
    ...
    if (nautilus_files_view_supports_creating_files(view) &&
            !showing_recent_directory(view) && !showing_starred_directory(view)) {
 ...
 * actions state if the children subclass
 * nautilus_files_view_update_actions_state
 */
void nautilus_files_view_update_context_menus(NautilusFilesView *view) {
    g_assert(NAUTILUS_IS_FILES_VIEW(view));

    NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
    ->update_context_menus(view);
}

static void nautilus_files_view_reset_view_menu(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    GActionGroup *view_action_group;
    ...
    zoom_level_percent = g_strdup_printf(
                             "%.0f%%", nautilus_files_view_get_zoom_level_percentage(view) * 100.0);
...
nautilus_files_view_reveal_for_selection_context_menu(NautilusFilesView *view) {
    return NAUTILUS_FILES_VIEW_CLASS(G_OBJECT_GET_CLASS(view))
           ->reveal_for_selection_context_menu(view);
}
...
void nautilus_files_view_pop_up_selection_context_menu(NautilusFilesView *view,
    ...
    gtk_menu_shell_bind_model(GTK_MENU_SHELL(priv->selection_menu),
                              G_MENU_MODEL(priv->selection_menu_model), NULL,
                              TRUE);
    ...
    if (event != NULL) {
        ...
        gtk_menu_popup_at_rect(
            GTK_MENU(priv->selection_menu), gtk_widget_get_window(GTK_WIDGET(view)),
            rectangle, GDK_GRAVITY_SOUTH_WEST, GDK_GRAVITY_NORTH_WEST, NULL);
    }
}
...
void nautilus_files_view_pop_up_background_context_menu(NautilusFilesView *view,
    ...
    gtk_menu_shell_bind_model(GTK_MENU_SHELL(priv->background_menu),
                              G_MENU_MODEL(priv->background_menu_model), NULL,
                              TRUE);
    if (event != NULL) {
         ...
         */
        gtk_menu_popup_at_widget(GTK_MENU(priv->background_menu), GTK_WIDGET(view),
                                 GDK_GRAVITY_CENTER, GDK_GRAVITY_CENTER, NULL);
    }
}

static gboolean popup_menu_callback(NautilusFilesView *view) {
    g_autolist(NautilusFile) selection = NULL;
...
static void schedule_update_context_menus(NautilusFilesView *view) {
    ...
    if (priv->update_context_menus_timeout_id == 0) {
        priv->update_context_menus_timeout_id = g_timeout_add(
                priv->update_interval, update_context_menus_timeout_callback, view);
    }
}
...
static gboolean update_status_idle_callback(gpointer data) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *view;
...
static void schedule_update_status(NautilusFilesView *view) {
    ...
    if (priv->update_status_idle_id == 0) {
        priv->update_status_idle_id = g_idle_add_full(
                                          G_PRIORITY_DEFAULT_IDLE - 20, update_status_idle_callback, view, NULL);
    }
}
...
void nautilus_files_view_notify_selection_changed(NautilusFilesView *view) {
    ...
    if (priv->display_selection_idle_id == 0) {
        priv->display_selection_idle_id =
            g_idle_add(display_selection_info_idle_callback, view);
    }
...
static void file_changed_callback(NautilusFile *file, gpointer callback_data) {
...
static void load_directory(NautilusFilesView *view,
    ...
    /* FIXME bugzilla.gnome.org 45062: In theory, we also need to monitor metadata
     * here (as well as doing a call when ready), in case external forces change
     * the directory's file metadata.
     */
    attributes = NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_MOUNT |
                 NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO;
    priv->metadata_for_directory_as_file_pending = TRUE;
    priv->metadata_for_files_in_directory_pending = TRUE;
    nautilus_file_call_when_ready(priv->directory_as_file, attributes,
                                  metadata_for_directory_as_file_ready_callback,
                                  view);
    nautilus_directory_call_when_ready(
        priv->model, attributes, FALSE,
     ...
     */
    attributes =
        NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO;
    nautilus_file_monitor_add(priv->directory_as_file, &priv->directory_as_file,
                              attributes);

    priv->file_changed_handler_id =
        g_signal_connect(priv->directory_as_file, "changed",
                         G_CALLBACK(file_changed_callback), view);

    nautilus_profile_end(NULL);
}

static void finish_loading(NautilusFilesView *view) {
    NautilusFileAttributes attributes;
    NautilusFilesViewPrivate *priv;
    ...
    priv->load_error_handler_id = g_signal_connect(
                                      priv->model, "load-error", G_CALLBACK(load_error_callback), view);
    ...
    priv->files_added_handler_id = g_signal_connect(
                                       priv->model, "files-added", G_CALLBACK(files_added_callback), view);
    priv->files_changed_handler_id = g_signal_connect(
                                         priv->model, "files-changed", G_CALLBACK(files_changed_callback), view);

    nautilus_directory_file_monitor_add(priv->model, &priv->model,
                                        priv->show_hidden_files, attributes,
                                        files_added_callback, view);
...
static void finish_loading_if_all_metadata_loaded(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void disconnect_model_handlers(NautilusFilesView *view) {
    ...
    g_clear_signal_handler(&priv->file_changed_handler_id,
                           priv->directory_as_file);
    nautilus_file_cancel_call_when_ready(
        priv->directory_as_file, metadata_for_directory_as_file_ready_callback,
        view);
    nautilus_directory_cancel_callback(
        priv->model, metadata_for_files_in_directory_ready_callback, view);
    nautilus_directory_file_monitor_remove(priv->model, &priv->model);
...
gboolean nautilus_files_view_should_show_file(NautilusFilesView *view,
    ...
    return nautilus_file_should_show(file, priv->show_hidden_files);
}
...
void nautilus_files_view_move_copy_items(NautilusFilesView *view,
    ...
    if (copy_action == GDK_ACTION_COPY && nautilus_is_file_roller_installed() &&
            target_file != NULL && nautilus_file_is_archive(target_file)) {
        char *command, *quoted_uri, *tmp;
        const GList *l;
        GdkScreen *screen;

        /* Handle dropping onto a file-roller archiver file, instead of starting a
         * move/copy */
    ...
    nautilus_file_operations_copy_move(
        item_uris, target_uri, copy_action, GTK_WIDGET(view), NULL,
        copy_move_done_callback, pre_copy_move(view));
}

static void nautilus_files_view_trash_state_changed_callback(
    NautilusTrashMonitor *trash_monitor, gboolean state,
    gpointer callback_data) {
    NautilusFilesView *view;
...
void nautilus_files_view_start_batching_selection_changes(
    NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
void nautilus_files_view_stop_batching_selection_changes(
    NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void nautilus_files_view_get_property(GObject *object, guint prop_id,
    ...
    case PROP_SELECTION: {
        g_value_set_pointer(value,
                            nautilus_view_get_selection(NAUTILUS_VIEW(view)));
    }
    break;
    ...
    case PROP_EXTENSIONS_BACKGROUND_MENU: {
        g_value_set_object(
            value, real_get_extensions_background_menu(NAUTILUS_VIEW(view)));
    }
    break;

    case PROP_TEMPLATES_MENU: {
        g_value_set_object(value, real_get_templates_menu(NAUTILUS_VIEW(view)));
    }
    break;
...
static void nautilus_files_view_set_property(GObject *object, guint prop_id,
        const GValue *value,
        GParamSpec *pspec) {
    NautilusFilesView *directory_view;
    NautilusFilesViewPrivate *priv;
    ...
    case PROP_WINDOW_SLOT: {
        ...
        g_signal_connect_object(priv->slot, "notify::active",
                                G_CALLBACK(slot_active_changed), directory_view, 0);
    }
    break;
    ...
    case PROP_LOCATION: {
        nautilus_view_set_location(NAUTILUS_VIEW(directory_view),
                                   g_value_get_object(value));
    }
    break;

    case PROP_SEARCH_QUERY: {
        nautilus_view_set_search_query(NAUTILUS_VIEW(directory_view),
                                       g_value_get_object(value));
    }
    break;

    case PROP_SELECTION: {
        nautilus_view_set_selection(NAUTILUS_VIEW(directory_view),
...
static gboolean on_event(GtkWidget *widget, GdkEvent *event,
                         gpointer user_data) {
    NautilusFilesView *directory_view;
...
static void action_reload_enabled_changed(GActionGroup *action_group,
        gchar *action_name, gboolean enabled,
        NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void action_stop_enabled_changed(GActionGroup *action_group,
                                        gchar *action_name, gboolean enabled,
                                        NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
...
static void on_parent_changed(GObject *object, GParamSpec *pspec,
    ...
    if (parent != NULL) {
        ...
        priv->stop_signal_handler =
            g_signal_connect(window, "action-enabled-changed::stop",
                             G_CALLBACK(action_stop_enabled_changed), view);
        priv->reload_signal_handler =
            g_signal_connect(window, "action-enabled-changed::reload",
                             G_CALLBACK(action_reload_enabled_changed), view);
    } else {
        remove_update_context_menus_timeout_callback(view);
...
static NautilusQuery *nautilus_files_view_get_search_query(NautilusView *view) {
    NautilusFilesViewPrivate *priv;
    ...
    if (!nautilus_query_is_empty(query)) {
        if (nautilus_view_is_searching(NAUTILUS_VIEW(files_view))) {
            ...
            nautilus_search_directory_set_query(
                NAUTILUS_SEARCH_DIRECTORY(priv->model), query);
            ...
            nautilus_search_directory_set_base_model(
                NAUTILUS_SEARCH_DIRECTORY(directory), base_model);
...
static void nautilus_files_view_set_search_query(NautilusView *view,
        NautilusQuery *query) {
    NautilusDirectory *base_model;
    NautilusFilesView *files_view;
    NautilusFilesViewPrivate *priv;
...
static GFile *nautilus_files_view_get_location(NautilusView *view) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *files_view;
...
static gboolean nautilus_files_view_is_loading(NautilusView *view) {
    NautilusFilesViewPrivate *priv;
    NautilusFilesView *files_view;
...
static void nautilus_files_view_iface_init(NautilusViewInterface *iface) {
    ...
    iface->get_toolbar_menu_sections =
        nautilus_files_view_get_toolbar_menu_sections;
    ...
    iface->get_extensions_background_menu =
        nautilus_files_view_get_extensions_background_menu;
    iface->set_extensions_background_menu =
        nautilus_files_view_set_extensions_background_menu;
}

static void nautilus_files_view_class_init(NautilusFilesViewClass *klass) {
    GObjectClass *oclass;
    ...
    signals[ADD_FILES] = g_signal_new(
                             "add-files", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                             G_STRUCT_OFFSET(NautilusFilesViewClass, add_files), NULL, NULL,
                             g_cclosure_marshal_generic, G_TYPE_NONE, 1, G_TYPE_POINTER);
    signals[BEGIN_FILE_CHANGES] = g_signal_new(
                                      "begin-file-changes", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                      G_STRUCT_OFFSET(NautilusFilesViewClass, begin_file_changes), NULL, NULL,
                                      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    signals[BEGIN_LOADING] =
        g_signal_new("begin-loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(NautilusFilesViewClass, begin_loading), NULL,
                     NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    signals[CLEAR] =
        g_signal_new("clear", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(NautilusFilesViewClass, clear), NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    signals[END_FILE_CHANGES] = g_signal_new(
                                    "end-file-changes", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                                    G_STRUCT_OFFSET(NautilusFilesViewClass, end_file_changes), NULL, NULL,
                                    g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    signals[END_LOADING] = g_signal_new(
                               "end-loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                               G_STRUCT_OFFSET(NautilusFilesViewClass, end_loading), NULL, NULL,
                               g_cclosure_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
    signals[FILE_CHANGED] =
        g_signal_new("file-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(NautilusFilesViewClass, file_changed), NULL,
                     NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 2,
                     NAUTILUS_TYPE_FILE, NAUTILUS_TYPE_DIRECTORY);
    signals[REMOVE_FILE] =
        g_signal_new("remove-file", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(NautilusFilesViewClass, remove_file), NULL,
                     NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 2,
                     NAUTILUS_TYPE_FILE, NAUTILUS_TYPE_DIRECTORY);
    signals[SELECTION_CHANGED] = g_signal_new(
                                     "selection-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL,
                                     NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
    ...
    g_object_class_install_property(
        oclass, PROP_WINDOW_SLOT,
        g_param_spec_object("window-slot", "Window Slot",
                            "The parent window slot reference",
                            NAUTILUS_TYPE_WINDOW_SLOT,
                            G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property(
        oclass, PROP_SUPPORTS_ZOOMING,
        g_param_spec_boolean("supports-zooming", "Supports zooming",
                             "Whether the view supports zooming", TRUE,
                             G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
    ...
    g_object_class_override_property(oclass, PROP_EXTENSIONS_BACKGROUND_MENU,
                                     "extensions-background-menu");
    g_object_class_override_property(oclass, PROP_TEMPLATES_MENU,
                                     "templates-menu");
}

static void nautilus_files_view_init(NautilusFilesView *view) {
    NautilusFilesViewPrivate *priv;
    GtkBuilder *builder;
    ...
    GtkClipboard *clipboard;
    GApplication *app;
    const gchar *open_accels[] = {"Return", "KP_Enter", "<control>o", "<alt>Down",
                                  NULL
                                 };
    const gchar *open_properties[] = {"<control>i", "<alt>Return", NULL};
    const gchar *zoom_in_accels[] = {"<control>equal", "<control>plus",
                                     "<control>KP_Add", "ZoomIn", NULL
                                    };
    const gchar *zoom_out_accels[] = {"<control>minus", "<control>KP_Subtract",
                                      "ZoomOut", NULL
                                     };
    const gchar *zoom_standard_accels[] = {"<control>0", "<control>KP_0", NULL};
    const gchar *move_to_trash_accels[] = {"Delete", "KP_Delete", NULL};
    const gchar *delete_permanently_accels[] = {"<shift>Delete",
                                                "<shift>KP_Delete", NULL
                                               };
    ...
    /* Toolbar menu */
    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-toolbar-view-menu.ui");
    priv->toolbar_menu_sections = g_new0(NautilusToolbarMenuSections, 1);
    priv->toolbar_menu_sections->supports_undo_redo = TRUE;
    priv->toolbar_menu_sections->zoom_section = GTK_WIDGET(
                g_object_ref_sink(gtk_builder_get_object(builder, "zoom_section")));
    priv->toolbar_menu_sections->extended_section = GTK_WIDGET(
                g_object_ref_sink(gtk_builder_get_object(builder, "extended_section")));
    priv->zoom_controls_box =
        GTK_WIDGET(gtk_builder_get_object(builder, "zoom_controls_box"));
    priv->zoom_level_label =
        GTK_WIDGET(gtk_builder_get_object(builder, "zoom_level_label"));

    priv->sort_menu = GTK_WIDGET(gtk_builder_get_object(builder, "sort_menu"));
    priv->sort_trash_time =
        GTK_WIDGET(gtk_builder_get_object(builder, "sort_trash_time"));
    priv->visible_columns =
        GTK_WIDGET(gtk_builder_get_object(builder, "visible_columns"));
    priv->reload = GTK_WIDGET(gtk_builder_get_object(builder, "reload"));
    priv->stop = GTK_WIDGET(gtk_builder_get_object(builder, "stop"));

    g_signal_connect(view, "end-file-changes", G_CALLBACK(on_end_file_changes),
                     view);
    g_signal_connect(view, "notify::selection",
                     G_CALLBACK(nautilus_files_view_preview_update), view);
    g_signal_connect(view, "notify::parent", G_CALLBACK(on_parent_changed), NULL);
    ...
    gtk_grid_attach_next_to(GTK_GRID(view), priv->overlay, NULL, GTK_POS_BOTTOM,
                            1, 1);
    ...
    g_signal_connect_swapped(priv->scrolled_window, "event", G_CALLBACK(on_event),
                             view);
    g_signal_connect_swapped(priv->scrolled_window, "popup-menu",
                             G_CALLBACK(popup_menu_callback), view);

    gtk_overlay_set_child(GTK_OVERLAY(priv->overlay), priv->scrolled_window);

    /* Empty states */
    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-no-search-results.ui");
    ...
    gtk_overlay_add_overlay(GTK_OVERLAY(priv->overlay),
                            priv->no_search_results_widget);
    gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(priv->overlay),
                                         priv->no_search_results_widget, TRUE);
    g_object_unref(builder);

    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-folder-is-empty.ui");
    priv->folder_is_empty_widget =
        GTK_WIDGET(gtk_builder_get_object(builder, "folder_is_empty"));
    gtk_overlay_add_overlay(GTK_OVERLAY(priv->overlay),
                            priv->folder_is_empty_widget);
    gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(priv->overlay),
                                         priv->folder_is_empty_widget, TRUE);
    g_object_unref(builder);

    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-starred-is-empty.ui");
    priv->starred_is_empty_widget =
        GTK_WIDGET(gtk_builder_get_object(builder, "starred_is_empty"));
    gtk_overlay_add_overlay(GTK_OVERLAY(priv->overlay),
                            priv->starred_is_empty_widget);
    gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(priv->overlay),
                                         priv->starred_is_empty_widget, TRUE);
    g_object_unref(builder);

    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/nautilus-trash-is-empty.ui");
    priv->trash_is_empty_widget =
        GTK_WIDGET(gtk_builder_get_object(builder, "trash_is_empty"));
    gtk_overlay_add_overlay(GTK_OVERLAY(priv->overlay),
                            priv->trash_is_empty_widget);
    gtk_overlay_set_overlay_pass_through(GTK_OVERLAY(priv->overlay),
                                         priv->trash_is_empty_widget, TRUE);
    g_object_unref(builder);
    ...
    g_signal_connect(priv->floating_bar, "stop", G_CALLBACK(floating_bar_stop_cb),
                     view);

    priv->non_ready_files =
        g_hash_table_new_full(file_and_directory_hash, file_and_directory_equal,
                              file_and_directory_free, NULL);
    ...
    priv->sort_directories_first = g_settings_get_boolean(
                                       gtk_filechooser_preferences, NAUTILUS_PREFERENCES_SORT_DIRECTORIES_FIRST);
    priv->show_hidden_files = g_settings_get_boolean(
                                  gtk_filechooser_preferences, NAUTILUS_PREFERENCES_SHOW_HIDDEN_FILES);

    g_signal_connect_object(
        nautilus_trash_monitor_get(), "trash-state-changed",
    ...
    g_signal_connect_object(
        nautilus_signaller_get_current(), "popup-menu-changed",
    ...
    g_action_map_add_action_entries(G_ACTION_MAP(priv->view_action_group),
                                    view_entries, G_N_ELEMENTS(view_entries),
                                    view);
    gtk_widget_insert_action_group(GTK_WIDGET(view), "view",
                                   G_ACTION_GROUP(priv->view_action_group));
    ...
    nautilus_application_set_accelerator(app, "view.show-hidden-files",
                                         "<control>h");
    /* Background menu */
    ...
    /* Selection menu */
    nautilus_application_set_accelerators(
        app, "view.open-with-default-application", open_accels);
    nautilus_application_set_accelerator(app, "view.open-item-new-tab",
                                         "<control>Return");
    nautilus_application_set_accelerator(app, "view.open-item-new-window",
                                         "<Shift>Return");
    nautilus_application_set_accelerators(app, "view.move-to-trash",
                                          move_to_trash_accels);
    nautilus_application_set_accelerators(app, "view.delete-from-trash",
                                          move_to_trash_accels);
    nautilus_application_set_accelerators(app, "view.delete-permanently-shortcut",
                                          delete_permanently_accels);
    /* When trash is not available, allow the "Delete" keys to delete permanently,
     * that is, when the menu item is available, since we never make both the
     * trash and delete-permanently-menu-item actions active */
    nautilus_application_set_accelerators(
        app, "view.delete-permanently-menu-item", move_to_trash_accels);
    nautilus_application_set_accelerators(
        app, "view.permanent-delete-permanently-menu-item",
        delete_permanently_accels);
    nautilus_application_set_accelerators(app, "view.properties",
                                          open_properties);
    nautilus_application_set_accelerator(app, "view.open-item-location",
                                         "<control><alt>o");
    ...
    nautilus_application_set_accelerator(app, "view.create-link-in-place",
                                         "<control><shift>m");
    nautilus_application_set_accelerator(app, "view.new-folder",
                                         "<control><shift>n");
    /* Only accesible by shorcuts */
    nautilus_application_set_accelerator(app, "view.select-pattern",
                                         "<control>s");
    nautilus_application_set_accelerators(app, "view.zoom-standard",
                                          zoom_standard_accels);
    nautilus_application_set_accelerator(app, "view.invert-selection",
                                         "<shift><control>i");
...
NautilusFilesView *nautilus_files_view_new(guint id, NautilusWindowSlot *slot) {
