...
enum { PROP_0, PROP_RUNNING, NUM_PROPERTIES };
...
G_DEFINE_TYPE_WITH_CODE(NautilusSearchEngineSimple,
                        nautilus_search_engine_simple, G_TYPE_OBJECT,
...
static void search_thread_data_free(SearchThreadData *data) {
    GList *hits;
...
static gboolean search_thread_done(SearchThreadData *data) {
...
static void search_thread_process_hits_idle(SearchThreadData *data,
        GList *hits) {
    if (!g_cancellable_is_cancelled(data->cancellable)) {
...
static gboolean search_thread_process_idle(gpointer user_data) {
...
static void finish_search_thread(SearchThreadData *thread_data) {
    g_mutex_lock(&thread_data->idle_mutex);
...
static void process_batch_in_idle(SearchThreadData *thread_data, GList *hits) {
    ...
    if (thread_data->processing_id == 0) {
        thread_data->processing_id =
            g_idle_add(search_thread_process_idle, thread_data);
    }
}

static void send_batch_in_idle(SearchThreadData *thread_data) {
...
static void visit_directory(GFile *dir, SearchThreadData *data) {
    ...
    enumerator = g_file_enumerate_children(
                     dir,
                     data->mime_types->len > 0 ? STD_ATTRIBUTES
                     "," G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE
                     : STD_ATTRIBUTES,
                     G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, data->cancellable, NULL);
    ...
    while ((info = g_file_enumerator_next_file(enumerator, data->cancellable,
                   NULL)) != NULL) {
        ...
        if (found && data->mime_types->len > 0) {
            ...
            for (gint i = 0; i < data->mime_types->len; i++) {
                if (g_content_type_is_a(mime_type,
                                        g_ptr_array_index(data->mime_types, i))) {
                    found = TRUE;
                    break;
                }
            }
        ...
        if (found && date_range != NULL) {
            ...
            found = nautilus_file_date_in_between(current_file_time, initial_date,
                                                  end_date);
        }
        ...
        if (recursive != NAUTILUS_QUERY_RECURSIVE_NEVER &&
                ...
                is_recursive_search(NAUTILUS_SEARCH_ENGINE_TYPE_NON_INDEXED, recursive,
                                    child)) {
            id = g_file_info_get_attribute_string(info, G_FILE_ATTRIBUTE_ID_FILE);
            visited = FALSE;
            if (id) {
                if (g_hash_table_lookup_extended(data->visited, id, NULL, NULL)) {
                    visited = TRUE;
                } else {
...
static gpointer search_thread_func(gpointer user_data) {
    ...
    info = g_file_query_info(dir, G_FILE_ATTRIBUTE_ID_FILE, 0, data->cancellable,
                             NULL);
    if (info) {
        id = g_file_info_get_attribute_string(info, G_FILE_ATTRIBUTE_ID_FILE);
        if (id) {
...
static void nautilus_search_engine_simple_get_property(GObject *object,
        guint arg_id,
        GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_search_engine_simple_class_init(
    NautilusSearchEngineSimpleClass *class) {
