...
static gboolean invoke_main_context_source_func_wrapper(gpointer user_data) {
...
static void invoke_main_context_completed(gpointer user_data) {
    ContextInvokeData *data = (ContextInvokeData *)user_data;
 ...
 */
static void invoke_main_context_sync(GMainContext *main_context,
                                     GSourceFunc source_func,
                                     gpointer user_data) {
    ContextInvokeData *data = (ContextInvokeData *)user_data;
    ...
    g_main_context_invoke(main_context, invoke_main_context_source_func_wrapper,
                          user_data);
...
void file_conflict_response_free(FileConflictResponse *response) {
    g_free(response->new_name);
    g_slice_free(FileConflictResponse, response);
}

static void set_copy_move_dialog_text(FileConflictDialogData *data) {
    ...
    destination_directory_name =
        nautilus_file_get_display_name(data->destination_directory_file);
    ...
    if (destination_is_directory) {
        if (nautilus_file_is_symbolic_link(data->source) &&
            ...
            primary_text =
                g_strdup_printf(_("You are trying to replace the destination folder "
                                  "“%s” with a symbolic link."),
                                destination_name);
            message =
                g_strdup_printf(_("This is not allowed in order to avoid the "
                                  "deletion of the destination folder’s contents."));
            message_extra =
                _("Please rename the symbolic link or press the skip button.");
        } else if (source_is_directory) {
            primary_text = g_strdup_printf(_("Merge folder “%s”?"), destination_name);
            ...
            primary_text =
                g_strdup_printf(_("Replace folder “%s”?"), destination_name);
        ...
        primary_text = g_strdup_printf(_("Replace file “%s”?"), destination_name);
    ...
    nautilus_file_conflict_dialog_set_text(data->dialog, primary_text,
                                           secondary_text);
}

static void set_images(FileConflictDialogData *data) {
    GdkPixbuf *source_pixbuf;
    GdkPixbuf *destination_pixbuf;

    destination_pixbuf = nautilus_file_get_icon_pixbuf(
                             data->destination, NAUTILUS_GRID_ICON_SIZE_SMALL, TRUE, 1,
                             NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS);

    source_pixbuf = nautilus_file_get_icon_pixbuf(
                        data->source, NAUTILUS_GRID_ICON_SIZE_SMALL, TRUE, 1,
                        NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS);

    nautilus_file_conflict_dialog_set_images(data->dialog, destination_pixbuf,
            source_pixbuf);
...
static void set_file_labels(FileConflictDialogData *data) {
    ...
    should_show_type =
        !nautilus_file_is_mime_type(data->source, destination_mime_type);

    destination_date = nautilus_file_get_string_attribute_with_default(
                           data->destination, "date_modified");
    destination_size = nautilus_file_get_string_attribute_with_default(
                           data->destination, "size");

    if (should_show_type) {
        destination_type = nautilus_file_get_string_attribute_with_default(
    ...
    g_string_append_printf(destination_label, "%s %s", _("Last modified:"),
                           destination_date);

    source_date = nautilus_file_get_string_attribute_with_default(
                      data->source, "date_modified");
    source_size =
        nautilus_file_get_string_attribute_with_default(data->source, "size");

    if (should_show_type) {
        source_type =
            nautilus_file_get_string_attribute_with_default(data->source, "type");
    }

    source_label = g_string_new(NULL);
    if (source_is_directory) {
        g_string_append_printf(source_label, "<b>%s</b>\n",
                               destination_is_directory ? _("Merge with")
                               : _("Replace with"));
    ...
    g_string_append_printf(source_label, "%s %s", _("Last modified:"),
                           source_date);

    nautilus_file_conflict_dialog_set_file_labels(
        data->dialog, destination_label->str, source_label->str);

    g_string_free(destination_label, TRUE);
    g_string_free(source_label, TRUE);
}

static void set_conflict_and_suggested_names(FileConflictDialogData *data) {
    g_autofree gchar *conflict_name = NULL;

    conflict_name = nautilus_file_get_edit_name(data->destination);

    nautilus_file_conflict_dialog_set_conflict_name(data->dialog, conflict_name);

    nautilus_file_conflict_dialog_set_suggested_name(data->dialog,
            data->suggestion);
}

static void set_replace_button_label(FileConflictDialogData *data) {
...
static void file_icons_changed(NautilusFile *file,
                               FileConflictDialogData *data) {
    set_images(data);
}

static void copy_move_conflict_on_file_list_ready(GList *files,
        gpointer user_data) {
    ...
    nautilus_file_monitor_add(data->source, data,
                              NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);
    nautilus_file_monitor_add(data->destination, data,
                              NAUTILUS_FILE_ATTRIBUTES_FOR_ICON);

    data->source_handler_id = g_signal_connect(
                                  data->source, "changed", G_CALLBACK(file_icons_changed), data);
    data->destination_handler_id = g_signal_connect(
                                       data->destination, "changed", G_CALLBACK(file_icons_changed), data);
}

static void on_conflict_dialog_response(GtkDialog *dialog, gint response_id,
                                        gpointer user_data) {
    FileConflictDialogData *data = user_data;
    ...
    if (data->destination_handler_id) {
        g_signal_handler_disconnect(data->destination,
                                    data->destination_handler_id);
...
static gboolean run_file_conflict_dialog(gpointer user_data) {
    FileConflictDialogData *data = user_data;
    GList *files = NULL;

    data->source = nautilus_file_get(data->source_name);
    data->destination = nautilus_file_get(data->destination_name);
    data->destination_directory_file =
        nautilus_file_get(data->destination_directory_name);
    ...
    nautilus_file_list_call_when_ready(
        ...
        &data->handle, data->on_file_list_ready, data);

    g_signal_connect(data->dialog, "response",
                     G_CALLBACK(on_conflict_dialog_response), data);
...
FileConflictResponse *copy_move_conflict_ask_user_action(
    GtkWindow *parent_window, gboolean should_start_inactive,
    GFile *source_name, GFile *destination_name,
    GFile *destination_directory_name, gchar *suggestion) {
    FileConflictDialogData *data;
    FileConflictResponse *response;
    ...
    invoke_main_context_sync(NULL, run_file_conflict_dialog, data);
...
static void on_app_chooser_response(GtkDialog *dialog, gint response_id,
                                    gpointer user_data) {
...
static gboolean open_file_in_application(gpointer user_data) {
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(on_app_chooser_response),
                     data);
...
static gboolean run_passphrase_dialog(gpointer user_data) {
    PassphraseRequestData *data = user_data;
    ...
    builder = gtk_builder_new_from_resource(
                  "/org/gnome/nautilus/ui/"
                  "nautilus-operations-ui-manager-request-passphrase.ui");
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(on_request_passphrase_cb),
                     data);
