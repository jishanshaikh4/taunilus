...
G_DEFINE_TYPE_WITH_CODE(
    NautilusSearchEngine, nautilus_search_engine, G_TYPE_OBJECT,
    G_ADD_PRIVATE(NautilusSearchEngine)
    G_IMPLEMENT_INTERFACE(NAUTILUS_TYPE_SEARCH_PROVIDER,
                          nautilus_search_provider_init))

static void nautilus_search_engine_set_query(NautilusSearchProvider *provider,
        NautilusQuery *query) {
    ...
    nautilus_search_provider_set_query(NAUTILUS_SEARCH_PROVIDER(priv->tracker),
                                       query);
    nautilus_search_provider_set_query(NAUTILUS_SEARCH_PROVIDER(priv->recent),
                                       query);
    nautilus_search_provider_set_query(NAUTILUS_SEARCH_PROVIDER(priv->model),
                                       query);
    nautilus_search_provider_set_query(NAUTILUS_SEARCH_PROVIDER(priv->simple),
                                       query);
}

static void search_engine_start_real_setup(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
...
static void search_engine_start_real_tracker(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
...
static void search_engine_start_real_recent(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
...
static void search_engine_start_real_model(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
...
static void search_engine_start_real_simple(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
...
static void search_engine_start_real(NautilusSearchEngine *engine,
...
void nautilus_search_engine_start_by_target(
    ...
    if (priv->running) {
        if (num_finished == priv->providers_running && priv->restart) {
...
static void nautilus_search_engine_stop(NautilusSearchProvider *provider) {
    NautilusSearchEngine *engine;
    NautilusSearchEnginePrivate *priv;
...
static void search_provider_hits_added(NautilusSearchProvider *provider,
                                       GList *hits,
                                       NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
    GList *added = NULL;
    ...
    if (added != NULL) {
        ...
        nautilus_search_provider_hits_added(NAUTILUS_SEARCH_PROVIDER(engine),
                                            added);
        g_list_free(added);
    }
}

static void check_providers_status(NautilusSearchEngine *engine) {
    NautilusSearchEnginePrivate *priv;
    gint num_finished;
    ...
    if (num_finished == priv->providers_error) {
        ...
        nautilus_search_provider_error(
        ...
        nautilus_search_provider_finished(
            NAUTILUS_SEARCH_PROVIDER(engine),
...
static void search_provider_error(NautilusSearchProvider *provider,
...
static void search_provider_finished(NautilusSearchProvider *provider,
...
static void connect_provider_signals(NautilusSearchEngine *engine,
    ...
    g_signal_connect(provider, "hits-added",
                     G_CALLBACK(search_provider_hits_added), engine);
    g_signal_connect(provider, "finished", G_CALLBACK(search_provider_finished),
                     engine);
    g_signal_connect(provider, "error", G_CALLBACK(search_provider_error),
                     engine);
}
...
static void nautilus_search_engine_finalize(GObject *object) {
    NautilusSearchEngine *engine;
    NautilusSearchEnginePrivate *priv;
...
static void nautilus_search_engine_get_property(GObject *object, guint prop_id,
        GValue *value,
        GParamSpec *pspec) {
...
gboolean is_recursive_search(NautilusSearchEngineType engine_type,
    ...
    case NAUTILUS_QUERY_RECURSIVE_LOCAL_ONLY: {
        ...
        file_system_info = g_file_query_filesystem_info(
                               location, G_FILE_ATTRIBUTE_FILESYSTEM_REMOTE, NULL, NULL);
        if (file_system_info != NULL) {
            return !g_file_info_get_attribute_boolean(
                       file_system_info, G_FILE_ATTRIBUTE_FILESYSTEM_REMOTE);
        }
    }
    }
