...
enum { PROP_0, PROP_FILE, PROP_ICON_SIZE, PROP_ITEM_UI, N_PROPS };

enum { FILE_CHANGED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL];

static void nautilus_view_item_model_dispose(GObject *object) {
...
static void nautilus_view_item_model_finalize(GObject *object) {
  NautilusViewItemModel *self = NAUTILUS_VIEW_ITEM_MODEL(object);
...
static void nautilus_view_item_model_get_property(GObject *object,
...
static void nautilus_view_item_model_set_property(GObject *object,
                                                  guint prop_id,
                                                  const GValue *value,
                                                  GParamSpec *pspec) {
  NautilusViewItemModel *self = NAUTILUS_VIEW_ITEM_MODEL(object);
...
static void nautilus_view_item_model_init(NautilusViewItemModel *self) {}
...
nautilus_view_item_model_class_init(NautilusViewItemModelClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int(
          "icon-size", "Icon size", "The size in pixels of the icon",
          NAUTILUS_GRID_ICON_SIZE_SMALL, NAUTILUS_GRID_ICON_SIZE_LARGEST,
          NAUTILUS_GRID_ICON_SIZE_LARGE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_FILE,
      g_param_spec_object("file", "File", "The file the icon item represents",
                          NAUTILUS_TYPE_FILE,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_ITEM_UI,
      g_param_spec_object("item-ui", "Item ui",
                          "The UI that reprensents the item model",
                          GTK_TYPE_WIDGET, G_PARAM_READWRITE));

  signals[FILE_CHANGED] = g_signal_new(
      "file-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

NautilusViewItemModel *nautilus_view_item_model_new(NautilusFile *file,
                                                    guint icon_size) {
  return g_object_new(NAUTILUS_TYPE_VIEW_ITEM_MODEL, "file", file, "icon-size",
                      icon_size, NULL);
}
