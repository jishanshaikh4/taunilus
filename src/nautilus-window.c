...
struct _NautilusWindow {
  ...
  GVolume
      *selected_volume; /* the selected volume in the sidebar popup callback */
...
enum { SLOT_ADDED, SLOT_REMOVED, LAST_SIGNAL };
...
static const GtkPadActionEntry pad_actions[] = {
    ...
    {GTK_PAD_ACTION_BUTTON, 3, -1, N_("Close current view"),
     "close-current-view"},
    {GTK_PAD_ACTION_BUTTON, 4, -1, N_("Back"), "back"},
    {GTK_PAD_ACTION_BUTTON, 5, -1, N_("Forward"), "forward"},
};

static void action_close_current_view(GSimpleAction *action, GVariant *state,
                                      gpointer user_data) {
...
static void action_go_home(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
  NautilusWindow *window;
  GFile *home;
...
static void action_go_starred(GSimpleAction *action, GVariant *state,
                              gpointer user_data) {
  NautilusWindow *window;
...
static void action_reload(GSimpleAction *action, GVariant *state,
                          gpointer user_data) {
  NautilusWindowSlot *slot;
...
static void action_stop(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
  NautilusWindow *window;
  NautilusWindowSlot *slot;
...
static void action_up(GSimpleAction *action, GVariant *state,
                      gpointer user_data) {
  NautilusWindowSlot *slot;
  ...
  if (location != NULL) {
    ...
    if (parent != NULL) {
      nautilus_window_open_location_full(NAUTILUS_WINDOW(user_data), parent, 0,
                                         NULL, NULL);
    }
...
static void action_back(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
  nautilus_window_back_or_forward(NAUTILUS_WINDOW(user_data), TRUE, 0);
}

static void action_forward(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
  nautilus_window_back_or_forward(NAUTILUS_WINDOW(user_data), FALSE, 0);
}

static void action_bookmark_current_location(GSimpleAction *action,
                                             GVariant *state,
                                             gpointer user_data) {
...
static void action_new_tab(GSimpleAction *action, GVariant *state,
                           gpointer user_data) {
  nautilus_window_new_tab(NAUTILUS_WINDOW(user_data));
}

static void action_enter_location(GSimpleAction *action, GVariant *state,
                                  gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_tab_previous(GSimpleAction *action, GVariant *state,
                                gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_tab_next(GSimpleAction *action, GVariant *state,
                            gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_tab_move_left(GSimpleAction *action, GVariant *state,
                                 gpointer user_data) {
  NautilusWindow *window = user_data;

  nautilus_notebook_reorder_current_child_relative(
      NAUTILUS_NOTEBOOK(window->notebook), -1);
}

static void action_tab_move_right(GSimpleAction *action, GVariant *state,
                                  gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_go_to_tab(GSimpleAction *action, GVariant *value,
                             gpointer user_data) {
...
static void action_prompt_for_location_root(GSimpleAction *action,
                                            GVariant *state,
                                            gpointer user_data) {
  NautilusWindow *window = user_data;
  GFile *location;
  ...
  nautilus_location_entry_set_location(NAUTILUS_LOCATION_ENTRY(entry),
                                       location);

  g_object_unref(location);
}

static void action_prompt_for_location_home(GSimpleAction *action,
                                            GVariant *state,
                                            gpointer user_data) {
  GtkWidget *entry;
...
static void action_redo(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_undo(GSimpleAction *action, GVariant *state,
                        gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void action_toggle_state_view_button(GSimpleAction *action,
                                            GVariant *state,
                                            gpointer user_data) {
...
static void action_show_current_location_menu(GSimpleAction *action,
                                              GVariant *state,
                                              gpointer user_data) {
  NautilusWindow *window = user_data;
  GtkWidget *path_bar;
...
static void on_location_changed(NautilusWindow *window) {
  nautilus_gtk_places_sidebar_set_location(
      NAUTILUS_GTK_PLACES_SIDEBAR(window->places_sidebar),
      nautilus_window_slot_get_location(
          nautilus_window_get_active_slot(window)));
}

static void on_slot_location_changed(NautilusWindowSlot *slot,
                                     GParamSpec *pspec,
                                     NautilusWindow *window) {
...
static void notebook_switch_page_cb(GtkNotebook *notebook, GtkWidget *page,
...
static void connect_slot(NautilusWindow *window, NautilusWindowSlot *slot) {
  g_signal_connect(slot, "notify::location",
                   G_CALLBACK(on_slot_location_changed), window);
}

static void disconnect_slot(NautilusWindow *window, NautilusWindowSlot *slot) {
...
void nautilus_window_initialize_slot(NautilusWindow *window,
  ...
  nautilus_notebook_add_tab(
      NAUTILUS_NOTEBOOK(window->notebook), slot,
      (flags & NAUTILUS_OPEN_FLAG_SLOT_APPEND) != 0
          ? -1
          : gtk_notebook_get_current_page(GTK_NOTEBOOK(window->notebook)) + 1,
      FALSE);
  g_signal_handlers_unblock_by_func(
      window->notebook, G_CALLBACK(notebook_switch_page_cb), window);
...
void nautilus_window_open_location_full(NautilusWindow *window, GFile *location,
  ...
  if ((flags & NAUTILUS_OPEN_FLAG_NEW_TAB) != 0) {
    new_tab_at_end =
        g_settings_get_enum(nautilus_preferences,
  ...
  nautilus_window_slot_open_location_full(target_slot, location, flags,
                                          selection);
}

static void unset_focus_widget(NautilusWindow *window) {
...
static void remember_focus_widget(NautilusWindow *window) {
...
static void nautilus_window_grab_focus(GtkWidget *widget) {
  NautilusWindowSlot *slot;
...
static void restore_focus_widget(NautilusWindow *window) {
  if (window->last_focus_widget != NULL) {
...
static void location_entry_cancel_callback(GtkWidget *widget,
                                           NautilusWindow *window) {
  nautilus_toolbar_set_show_location_entry(NAUTILUS_TOOLBAR(window->toolbar),
                                           FALSE);

  restore_focus_widget(window);
}

static void location_entry_location_changed_callback(GtkWidget *widget,
                                                     GFile *location,
                                                     NautilusWindow *window) {
  nautilus_toolbar_set_show_location_entry(NAUTILUS_TOOLBAR(window->toolbar),
                                           FALSE);
...
static void close_slot(NautilusWindow *window, NautilusWindowSlot *slot,
...
void nautilus_window_new_tab(NautilusWindow *window) {
  ...
  if (location != NULL) {
    ...
    nautilus_window_open_location_full(window, location,
                                       NAUTILUS_OPEN_FLAG_NEW_TAB, NULL, NULL);
    g_object_unref(location);
  }
}
...
static void update_cursor(NautilusWindow *window) {
  ...
  if (slot != NULL && nautilus_window_slot_get_allow_stop(slot)) {
    GdkDisplay *display;
...
void nautilus_window_sync_allow_stop(NautilusWindow *window,
  ...
  stop_action = g_action_map_lookup_action(G_ACTION_MAP(window), "stop");
  reload_action = g_action_map_lookup_action(G_ACTION_MAP(window), "reload");
  ...
  if (!slot_is_active || allow_stop != slot_allow_stop) {
    if (slot_is_active) {
      g_simple_action_set_enabled(G_SIMPLE_ACTION(stop_action),
                                  slot_allow_stop);
      g_simple_action_set_enabled(G_SIMPLE_ACTION(reload_action),
                                  !slot_allow_stop);
    }
    ...
    if (nautilus_notebook_contains_slot(NAUTILUS_NOTEBOOK(window->notebook),
                                        slot)) {
...
static gboolean save_sidebar_width_cb(gpointer user_data) {
  NautilusWindow *window = user_data;
  ...
  g_settings_set_int(nautilus_window_state, NAUTILUS_WINDOW_STATE_SIDEBAR_WIDTH,
...
/* side pane helpers */
static void side_pane_size_allocate_callback(GtkWidget *widget,
  ...
  if (allocation->width != window->side_pane_width && allocation->width > 1) {
...
static void setup_side_pane_width(NautilusWindow *window) {
  g_return_if_fail(window->sidebar != NULL);

  window->side_pane_width = g_settings_get_int(
      nautilus_window_state, NAUTILUS_WINDOW_STATE_SIDEBAR_WIDTH);

  gtk_paned_set_position(GTK_PANED(window->content_paned),
                         window->side_pane_width);
}

/* Callback used when the places sidebar changes location; we need to change the
 * displayed folder */
static void open_location_cb(NautilusWindow *window, GFile *location,
  ...
  /* FIXME: We shouldn't need to provide the window, but seems
   * gtk_application_get_active_window is not working properly in
   * GtkApplication, so we cannot rely on that...
   */
  nautilus_application_open_location_full(application, location, flags, NULL,
                                          window, NULL);
}
...
static void places_sidebar_show_other_locations_with_flags(
    NautilusWindow *window, NautilusGtkPlacesOpenFlags open_flags) {
  GFile *location;
...
static GList *build_selection_list_from_gfile_list(GList *gfile_list) {
  GList *result;
  GList *l;
...
void nautilus_window_start_dnd(NautilusWindow *window,
  ...
  nautilus_gtk_places_sidebar_set_drop_targets_visible(
      NAUTILUS_GTK_PLACES_SIDEBAR(window->places_sidebar), TRUE, context);
}
...
void nautilus_window_end_dnd(NautilusWindow *window, GdkDragContext *context) {
  ...
  nautilus_gtk_places_sidebar_set_drop_targets_visible(
      NAUTILUS_GTK_PLACES_SIDEBAR(window->places_sidebar), FALSE, context);
}

/* Callback used when the places sidebar needs to know the drag action to
 * suggest */
static GdkDragAction places_sidebar_drag_action_requested_cb(
    NautilusGtkPlacesSidebar *sidebar, GdkDragContext *context,
    GFile *dest_file, GList *source_file_list, gpointer user_data) {
  GList *items;
  char *uri;
  int action = 0;
  ...
  nautilus_drag_default_drop_action_for_icons(context, uri, items,
...
/* Callback used when the places sidebar needs us to pop up a menu with possible
 * drag actions */
static GdkDragAction
places_sidebar_drag_action_ask_cb(NautilusGtkPlacesSidebar *sidebar,
                                  GdkDragAction actions, gpointer user_data) {
  return nautilus_drag_drop_action_ask(GTK_WIDGET(sidebar), actions);
}

static GList *build_uri_list_from_gfile_list(GList *file_list) {
  GList *result;
  GList *l;
...
/* Callback used when the places sidebar has URIs dropped into it.  We do a
 * normal file operation for them. */
static void
places_sidebar_drag_perform_drop_cb(NautilusGtkPlacesSidebar *sidebar,
                                    GFile *dest_file, GList *source_file_list,
                                    GdkDragAction action, gpointer user_data) {
  char *dest_uri;
  ...
  nautilus_file_operations_copy_move(source_uri_list, dest_uri, action,
                                     GTK_WIDGET(sidebar), NULL, NULL, NULL);
...
static void action_empty_trash(GSimpleAction *action, GVariant *variant,
                               gpointer user_data) {
  NautilusWindow *window = NAUTILUS_WINDOW(user_data);
...
static void action_properties(GSimpleAction *action, GVariant *variant,
                              gpointer user_data) {
  NautilusWindow *window = NAUTILUS_WINDOW(user_data);
  GList *list;
  NautilusFile *file;
...
static gboolean check_have_gnome_disks(void) {
  gchar *disks_path;
  gboolean res;
...
static gboolean should_show_format_command(GVolume *volume) {
...
static void action_restore_tab(GSimpleAction *action, GVariant *state,
                               gpointer user_data) {
  NautilusWindow *window = NAUTILUS_WINDOW(user_data);
  NautilusOpenFlags flags;
...
static guint get_window_xid(NautilusWindow *window) {
#ifdef GDK_WINDOWING_X11
...
static void action_format(GSimpleAction *action, GVariant *variant,
                          gpointer user_data) {
  NautilusWindow *window = NAUTILUS_WINDOW(user_data);
  GAppInfo *app_info;
  gchar *cmdline, *device_identifier, *xid_string;

  device_identifier = g_volume_get_identifier(
      window->selected_volume, G_VOLUME_IDENTIFIER_KIND_UNIX_DEVICE);
  xid_string = g_strdup_printf("%x", get_window_xid(window));

  cmdline = g_strconcat("gnome-disks ", "--block-device ", device_identifier,
                        " ", "--format-device ", "--xid ", xid_string, NULL);
...
static void places_sidebar_populate_popup_cb(NautilusGtkPlacesSidebar *sidebar,
                                             GtkWidget *menu,
                                             GFile *selected_file,
  ...
  if (selected_file) {
    ...
    if (g_file_equal(trash, selected_file)) {
      ...
      action = g_action_map_lookup_action(G_ACTION_MAP(window), "empty-trash");
  ...
  if (selected_volume) {
    if (should_show_format_command(selected_volume)) {
      menu_item = gtk_model_button_new();
      gtk_actionable_set_action_name(GTK_ACTIONABLE(menu_item), "win.format");
      ...
      action = g_action_map_lookup_action(G_ACTION_MAP(window), "format");
      g_simple_action_set_enabled(G_SIMPLE_ACTION(action),
...
static void nautilus_window_set_up_sidebar(NautilusWindow *window) {
  setup_side_pane_width(window);
  g_signal_connect(window->sidebar, "size-allocate",
                   G_CALLBACK(side_pane_size_allocate_callback), window);

  nautilus_gtk_places_sidebar_set_open_flags(
      NAUTILUS_GTK_PLACES_SIDEBAR(window->places_sidebar),
      (NAUTILUS_GTK_PLACES_OPEN_NORMAL | NAUTILUS_GTK_PLACES_OPEN_NEW_TAB |
...
static void nautilus_window_sync_bookmarks(NautilusWindow *window) {
  ...
  if (location != NULL) {
    ...
    can_bookmark =
        nautilus_bookmark_list_can_bookmark_location(bookmarks, location);
  }
...
void nautilus_window_sync_location_widgets(NautilusWindow *window) {
  ...
  if (location != NULL) {
    ...
    location_entry =
        nautilus_toolbar_get_location_entry(NAUTILUS_TOOLBAR(window->toolbar));
    nautilus_location_entry_set_location(
        NAUTILUS_LOCATION_ENTRY(location_entry), location);
...
static void remove_notifications(NautilusWindow *window) {
  GtkRevealerTransitionType transition_type;

  /* Hide it inmediatily so we can animate the new notification. */
  transition_type = gtk_revealer_get_transition_type(
      GTK_REVEALER(window->in_app_notification_undo));
  ...
  gtk_revealer_set_reveal_child(GTK_REVEALER(window->in_app_notification_undo),
                                FALSE);
  gtk_revealer_set_transition_type(
      GTK_REVEALER(window->in_app_notification_undo), transition_type);
  ...
  transition_type = gtk_revealer_get_transition_type(
      GTK_REVEALER(window->notification_operation));
...
static void hide_in_app_notification_undo(NautilusWindow *window) {
  if (window->in_app_notification_undo_timeout_id != 0) {
    g_source_remove(window->in_app_notification_undo_timeout_id);
    window->in_app_notification_undo_timeout_id = 0;
  }
...
static gboolean on_in_app_notification_undo_timeout(NautilusWindow *window) {
  hide_in_app_notification_undo(window);
...
in_app_notification_undo_deleted_get_label(NautilusFileUndoInfo *undo_info) {
  ...
  files = nautilus_file_undo_info_trash_get_files(
      NAUTILUS_FILE_UNDO_INFO_TRASH(undo_info));
  ...
  if (length == 1) {
    ...
    label = g_markup_printf_escaped(
        ngettext("%d file deleted", "%d files deleted", length), length);
  }
...
in_app_notification_undo_unstar_get_label(NautilusFileUndoInfo *undo_info) {
  ...
  files = nautilus_file_undo_info_starred_get_files(
      NAUTILUS_FILE_UNDO_INFO_STARRED(undo_info));
  length = g_list_length(files);
  if (length == 1) {
     ...
     * is the count. */
    label = g_markup_printf_escaped(
        ngettext("%d file unstarred", "%d files unstarred", length), length);
  }

  return label;
}

static void nautilus_window_on_undo_changed(NautilusFileUndoManager *manager,
                                            NautilusWindow *window) {
  ...
  if (undo_info != NULL && state == NAUTILUS_FILE_UNDO_MANAGER_STATE_UNDO) {
    gboolean popup_notification = FALSE;
    g_autofree gchar *label = NULL;

    if (nautilus_file_undo_info_get_op_type(undo_info) ==
       ...
       * is not in the this window. This is an easy way to know from which
       * window was the delete operation made */
    ...
    } else if (nautilus_file_undo_info_get_op_type(undo_info) ==
    ...
    if (popup_notification) {
      ...
      gtk_revealer_set_reveal_child(
          GTK_REVEALER(window->in_app_notification_undo), TRUE);
      window->in_app_notification_undo_timeout_id = g_timeout_add_seconds(
          NOTIFICATION_TIMEOUT,
          (GSourceFunc)on_in_app_notification_undo_timeout, window);
    }
  } else {
    hide_in_app_notification_undo(window);
  }
...
static void hide_notification_operation(NautilusWindow *window) {
  if (window->notification_operation_timeout_id != 0) {
    g_source_remove(window->notification_operation_timeout_id);
    window->notification_operation_timeout_id = 0;
  }
...
static void on_notification_operation_open_clicked(GtkWidget *notification,
                                                   NautilusWindow *window) {
  nautilus_window_open_location_full(window, window->folder_to_open, 0, NULL,
                                     NULL);
  hide_notification_operation(window);
}

static void on_notification_operation_close_clicked(GtkWidget *notification,
                                                    NautilusWindow *window) {
  hide_notification_operation(window);
}

static gboolean on_notification_operation_timeout(NautilusWindow *window) {
  hide_notification_operation(window);
...
void nautilus_window_show_operation_notification(NautilusWindow *window,
  ...
  if (gtk_window_is_active(GTK_WINDOW(window))) {
    ...
    gtk_revealer_set_reveal_child(GTK_REVEALER(window->notification_operation),
                                  TRUE);
    window->notification_operation_timeout_id = g_timeout_add_seconds(
        NOTIFICATION_TIMEOUT, (GSourceFunc)on_notification_operation_timeout,
        window);
  }
}

static void on_path_bar_open_location(NautilusWindow *window, GFile *location,
                                      NautilusOpenFlags open_flags) {
  nautilus_window_open_location_full(window, location, open_flags, NULL, NULL);
}

static void notebook_popup_menu_new_tab_cb(GtkMenuItem *menuitem,
                                           gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void notebook_popup_menu_move_left_cb(GtkMenuItem *menuitem,
                                             gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void notebook_popup_menu_move_right_cb(GtkMenuItem *menuitem,
                                              gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void notebook_popup_menu_close_cb(GtkMenuItem *menuitem,
                                         gpointer user_data) {
  NautilusWindow *window = user_data;
  NautilusWindowSlot *slot;
...
static void notebook_popup_menu_show(NautilusWindow *window,
  ...
  can_move_left =
      nautilus_notebook_can_reorder_current_child_relative(notebook, -1);
  can_move_right =
      nautilus_notebook_can_reorder_current_child_relative(notebook, 1);
  ...
  g_signal_connect(item, "activate", G_CALLBACK(notebook_popup_menu_new_tab_cb),
                   window);
  ...
  g_signal_connect(item, "activate",
                   G_CALLBACK(notebook_popup_menu_move_left_cb), window);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), item);
  gtk_widget_set_sensitive(item, can_move_left);

  item = gtk_menu_item_new_with_mnemonic(_("Move Tab _Right"));
  g_signal_connect(item, "activate",
                   G_CALLBACK(notebook_popup_menu_move_right_cb), window);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), item);
  ...
  g_signal_connect(item, "activate", G_CALLBACK(notebook_popup_menu_close_cb),
                   window);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), item);
...
static void notebook_tab_close_requested(NautilusNotebook *notebook,
                                         NautilusWindowSlot *slot,
                                         NautilusWindow *window) {
  nautilus_window_slot_close(window, slot);
}

static void notebook_button_press_cb(GtkGestureMultiPress *gesture,
  ...
  if (nautilus_notebook_content_area_hit(NAUTILUS_NOTEBOOK(window->notebook), x,
                                         y)) {
    return;
  }
...
static gboolean notebook_popup_menu_cb(GtkWidget *widget, gpointer user_data) {
  NautilusWindow *window = user_data;
...
static void setup_toolbar(NautilusWindow *window) {
  GtkWidget *path_bar;
  GtkWidget *location_entry;
  ...
  g_signal_connect_object(location_entry, "location-changed",
                          G_CALLBACK(location_entry_location_changed_callback),
                          window, 0);
  g_signal_connect_object(location_entry, "cancel",
                          G_CALLBACK(location_entry_cancel_callback), window,
                          0);
}

static void notebook_page_removed_cb(GtkNotebook *notebook, GtkWidget *page,
  ...
  dnd_slot =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(slot), "dnd-window-slot"));
  if (!dnd_slot) {
    return;
  }
...
static void notebook_page_added_cb(GtkNotebook *notebook, GtkWidget *page,
                                   guint page_num, gpointer user_data) {
  NautilusWindow *window = user_data;
  NautilusWindowSlot *slot = NAUTILUS_WINDOW_SLOT(page);
  ...
  g_object_set_data(G_OBJECT(page), "dnd-window-slot", GINT_TO_POINTER(FALSE));
...
static GtkNotebook *notebook_create_window_cb(GtkNotebook *notebook,
  ...
  g_object_set_data(G_OBJECT(slot), "dnd-window-slot", GINT_TO_POINTER(TRUE));
...
static void setup_notebook(NautilusWindow *window) {
  g_signal_connect(window->notebook, "tab-close-request",
                   G_CALLBACK(notebook_tab_close_requested), window);
...
#define ACCELS(...) ((const char *[]){__VA_ARGS__, NULL})
  ...
  nautilus_application_set_accelerators(app, "win.back",
                                        ACCELS("<alt>Left", "Back"));
  nautilus_application_set_accelerators(app, "win.forward",
                                        ACCELS("<alt>Right", "Forward"));
  nautilus_application_set_accelerators(app, "win.enter-location",
                                        ACCELS("<control>l", "Go", "OpenURL"));
  nautilus_application_set_accelerator(app, "win.new-tab", "<control>t");
  nautilus_application_set_accelerator(app, "win.close-current-view",
                                       "<control>w");

  /* Special case reload, since users are used to use two shortcuts instead of
   * one */
  nautilus_application_set_accelerators(
      app, "win.reload", ACCELS("F5", "<ctrl>r", "Refresh", "Reload"));
  ...
  nautilus_application_set_accelerators(app, "win.bookmark-current-location",
                                        ACCELS("<control>d", "AddFavorite"));
  nautilus_application_set_accelerator(app, "win.up", "<alt>Up");
  nautilus_application_set_accelerators(
      app, "win.go-home", ACCELS("<alt>Home", "HomePage", "Start"));
  nautilus_application_set_accelerator(app, "win.go-starred", "Favorites");
  nautilus_application_set_accelerator(app, "win.tab-previous",
                                       "<control>Page_Up");
  nautilus_application_set_accelerator(app, "win.tab-next",
                                       "<control>Page_Down");
  nautilus_application_set_accelerator(app, "win.tab-move-left",
                                       "<shift><control>Page_Up");
  nautilus_application_set_accelerator(app, "win.tab-move-right",
                                       "<shift><control>Page_Down");
  nautilus_application_set_accelerators(app, "win.prompt-root-location",
                                        ACCELS("slash", "KP_Divide"));
  /* Support keyboard layouts which have a dead tilde key but not a tilde key.
   */
  nautilus_application_set_accelerators(app, "win.prompt-home-location",
                                        ACCELS("asciitilde", "dead_tilde"));
  nautilus_application_set_accelerator(app, "win.current-location-menu", "F10");
  nautilus_application_set_accelerator(app, "win.restore-tab",
                                       "<shift><control>t");

  /* Alt+N for the first 9 tabs */
  for (i = 0; i < 9; ++i) {
    g_snprintf(detailed_action, sizeof(detailed_action), "win.go-to-tab(%i)",
               i);
...
static void nautilus_window_constructed(GObject *self) {
  ...
  gtk_window_set_default_size(GTK_WINDOW(window), NAUTILUS_WINDOW_DEFAULT_WIDTH,
  ...
  /* Is required that the UI is constructed before initializating the actions,
   * since some actions trigger UI widgets to show/hide. */
  nautilus_window_initialize_actions(window);

  slot = nautilus_window_create_and_init_slot(window, 0);
  nautilus_window_set_active_slot(window, slot);

  window->bookmarks_id = g_signal_connect_swapped(
      nautilus_application_get_bookmarks(application), "changed",
...
static gint sort_slots_active_last(NautilusWindowSlot *a, NautilusWindowSlot *b,
                                   NautilusWindow *window) {
...
static void destroy_slots_foreach(gpointer data, gpointer user_data) {
...
static void nautilus_window_dispose(GObject *object) {
  NautilusWindow *window;
  ...
  if (window->active_slot != NULL) {
    /* Make sure active slot is last one to be closed, to avoid default
     * activation of others slots when closing the active one, see bug #741952
     */
    slots_copy = g_list_sort_with_data(
        slots_copy, (GCompareDataFunc)sort_slots_active_last, window);
  }
...
static void nautilus_window_finalize(GObject *object) {
  NautilusWindow *window;
  ...
  g_signal_handlers_disconnect_by_func(
      ...
      G_CALLBACK(nautilus_window_on_undo_changed), window);
...
static void nautilus_window_save_geometry(NautilusWindow *window) {
...
void nautilus_window_set_active_slot(NautilusWindow *window,
  ...
  DEBUG("Setting new slot %p as active, old slot inactive %p", new_slot,
        old_slot);
  ...
  if (new_slot) {
    ...
    nautilus_toolbar_set_window_slot(NAUTILUS_TOOLBAR(window->toolbar),
                                     new_slot);
...
static gboolean nautilus_window_key_capture(GtkEventControllerKey *controller,
                                            unsigned int keyval,
...
static gboolean nautilus_window_key_bubble(GtkEventControllerKey *controller,
                                           unsigned int keyval,
                                           unsigned int keycode,
                                           GdkModifierType state,
                                           gpointer user_data) {
...
void nautilus_window_sync_title(NautilusWindow *window,
  ...
  if (slot == nautilus_window_get_active_slot(window)) {
    gtk_window_set_title(GTK_WINDOW(window),
                         nautilus_window_slot_get_title(slot));
  }
...
static void wayland_window_handle_exported(GdkWindow *window,
  ...
  data->window->export_handle =
      g_strdup_printf("wayland:%s", wayland_handle_str);
...
static void on_is_maximized_changed(GObject *object, GParamSpec *pspec,
                                    gpointer user_data) {
  gboolean is_maximized;
...
static gboolean nautilus_window_delete_event(GtkWidget *widget,
                                             GdkEventAny *event) {
...
static void on_multi_press_gesture_pressed(GtkGestureMultiPress *gesture,
                                           gint n_press, gdouble x, gdouble y,
                                           gpointer user_data) {
  GtkWidget *widget;
  NautilusWindow *window;
...
static void mouse_back_button_changed(gpointer callback_data) {
  ...
  new_back_button = g_settings_get_int(nautilus_preferences,
...
static void mouse_forward_button_changed(gpointer callback_data) {
  ...
  new_forward_button = g_settings_get_int(
      nautilus_preferences, NAUTILUS_PREFERENCES_MOUSE_FORWARD_BUTTON);
...
static void use_extra_mouse_buttons_changed(gpointer callback_data) {
  mouse_extra_buttons = g_settings_get_boolean(
      nautilus_preferences, NAUTILUS_PREFERENCES_MOUSE_USE_EXTRA_BUTTONS);
}
...
static void nautilus_window_init(NautilusWindow *window) {
  ...
  g_object_set(window->places_sidebar, "vexpand", TRUE, "visible", TRUE,
               "populate-all", TRUE, "show-other-locations", TRUE,
               "show-starred-location", TRUE, NULL);
  gtk_box_append(GTK_BOX(window->sidebar), window->places_sidebar);

  g_signal_connect_object(
      window->places_sidebar, "show-other-locations-with-flags",
      G_CALLBACK(places_sidebar_show_other_locations_with_flags), window,
      G_CONNECT_SWAPPED);
  ...
  g_signal_connect_object(
      window->in_app_notification_undo_close_button, "clicked",
      G_CALLBACK(on_in_app_notification_undo_close_button_clicked), window, 0);
  g_signal_connect_object(
      window->in_app_notification_undo_undo_button, "clicked",
  ...
  window->pad_controller =
      gtk_pad_controller_new(GTK_WINDOW(window), G_ACTION_GROUP(window), NULL);
  gtk_pad_controller_set_action_entries(window->pad_controller, pad_actions,
                                        G_N_ELEMENTS(pad_actions));

  window->multi_press_gesture = gtk_gesture_multi_press_new(GTK_WIDGET(window));

  gtk_event_controller_set_propagation_phase(
      GTK_EVENT_CONTROLLER(window->multi_press_gesture), GTK_PHASE_CAPTURE);
  gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(window->multi_press_gesture),
                                0);

  g_signal_connect(window->multi_press_gesture, "pressed",
                   G_CALLBACK(on_multi_press_gesture_pressed), NULL);

  window->notebook_multi_press_gesture =
      gtk_gesture_multi_press_new(window->notebook);

  gtk_event_controller_set_propagation_phase(
      GTK_EVENT_CONTROLLER(window->notebook_multi_press_gesture),
      GTK_PHASE_CAPTURE);
  gtk_gesture_single_set_button(
      GTK_GESTURE_SINGLE(window->notebook_multi_press_gesture),
      GDK_BUTTON_SECONDARY);

  window->key_capture_controller =
      gtk_event_controller_key_new(GTK_WIDGET(window));
  gtk_event_controller_set_propagation_phase(window->key_capture_controller,
                                             GTK_PHASE_CAPTURE);
  g_signal_connect(window->key_capture_controller, "key-pressed",
                   G_CALLBACK(nautilus_window_key_capture), NULL);

  window->key_bubble_controller =
      gtk_event_controller_key_new(GTK_WIDGET(window));
  gtk_event_controller_set_propagation_phase(window->key_bubble_controller,
                                             GTK_PHASE_BUBBLE);
  g_signal_connect(window->key_bubble_controller, "key-pressed",
...
static void nautilus_window_class_init(NautilusWindowClass *class) {
  ...
  gtk_widget_class_set_template_from_resource(
      wclass, "/org/gnome/nautilus/ui/nautilus-window.ui");
  ...
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
                                       in_app_notification_undo);
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
                                       in_app_notification_undo_label);
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
  ...
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
                                       notification_operation_label);
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
                                       notification_operation_open);
  gtk_widget_class_bind_template_child(wclass, NautilusWindow,
                                       notification_operation_close);

  signals[SLOT_ADDED] =
      g_signal_new("slot-added", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL,
                   g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
                   NAUTILUS_TYPE_WINDOW_SLOT);
  signals[SLOT_REMOVED] =
      g_signal_new("slot-removed", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL,
                   g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
                   NAUTILUS_TYPE_WINDOW_SLOT);
  ...
  g_signal_connect_swapped(nautilus_preferences,
                           ...
                           G_CALLBACK(mouse_back_button_changed), NULL);

  g_signal_connect_swapped(
      nautilus_preferences,
      "changed::" NAUTILUS_PREFERENCES_MOUSE_FORWARD_BUTTON,
      G_CALLBACK(mouse_forward_button_changed), NULL);

  g_signal_connect_swapped(
      nautilus_preferences,
      "changed::" NAUTILUS_PREFERENCES_MOUSE_USE_EXTRA_BUTTONS,
      G_CALLBACK(use_extra_mouse_buttons_changed), NULL);

  gtk_widget_class_bind_template_callback(
      wclass, on_notification_operation_open_clicked);
  gtk_widget_class_bind_template_callback(
      wclass, on_notification_operation_close_clicked);
}

NautilusWindow *nautilus_window_new(GdkScreen *screen) {
  return g_object_new(NAUTILUS_TYPE_WINDOW, "icon-name", APPLICATION_ID,
                      "screen", screen, NULL);
}

void nautilus_window_show_about_dialog(NautilusWindow *window) {
  const gchar *artists[] = {"The GNOME Project", NULL};
  const gchar *authors[] = {"The contributors to the Nautilus project", NULL};
  const gchar *documenters[] = {"GNOME Documentation Team", "Sun Microsystems",
                                NULL};
  ...
  gtk_show_about_dialog(
      window ? GTK_WINDOW(window) : NULL, "program-name", program_name,
      "version", VERSION, "comments", _("Access and organize your files"),
      "website", "https://wiki.gnome.org/action/show/Apps/Files", "copyright",
      "© 1999 The Files Authors", "license-type", GTK_LICENSE_GPL_3_0,
      "artists", artists, "authors", authors, "documenters", documenters,
