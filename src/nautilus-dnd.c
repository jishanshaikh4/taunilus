/* nautilus-dnd.c - Common Drag & drop handling code shared by the icon
 * container and the list view.
 *
...
void nautilus_drag_init(NautilusDragInfo *drag_info,
    ...
    drag_info->target_list = gtk_target_list_new(drag_types, drag_type_count);
...
static void drag_selection_item_destroy(NautilusDragSelectionItem *item) {
...
GList *nautilus_drag_build_selection_list(GtkSelectionData *data) {
    ...
    while (size > 0) {
        ...
        item->got_icon_position =
            sscanf((const gchar *)p, "%d:%d:%d:%d%*s", &item->icon_x, &item->icon_y,
...
gboolean nautilus_drag_items_local(const char *target_uri_string,
                                   const GList *selection_list) {
    /* must have at least one item */
    g_assert(selection_list);

    return nautilus_drag_file_local_internal(
               target_uri_string,
...
nautilus_drag_default_drop_action_for_netscape_url(GdkDragContext *context) {
    ...
    } else if (gdk_drag_context_get_suggested_action(context) ==
               GDK_ACTION_MOVE) {
        /* Don't support move */
...
static gboolean check_same_fs(NautilusFile *file1, NautilusFile *file2) {
    char *id1, *id2;
    gboolean result;
...
static gboolean source_is_deletable(GFile *file) {
...
NautilusDragInfo *nautilus_drag_get_source_data(GdkDragContext *context) {
    ...
    if (GTK_IS_TREE_VIEW(source_widget)) {
        ...
        if (NAUTILUS_IS_LIST_VIEW(view)) {
            source_data = nautilus_list_view_dnd_get_drag_source_data(
                              NAUTILUS_LIST_VIEW(view), context);
        } else {
            g_warning("Got a drag context with a tree view source widget, but "
                      "current view is not list view");
            source_data = NULL;
        }
    } else {
        /* it's a slot or something else */
        g_warning(
            "Requested drag source data from a widget that doesn't support it");
        source_data = NULL;
    }
 ...
 */
static void add_one_gnome_icon(const char *uri, int x, int y, int w, int h,
                               gpointer data) {
    GString *result;
...
static void add_one_uri(const char *uri, int x, int y, int w, int h,
                        gpointer data) {
    GString *result;
...
static void cache_one_item(const char *uri, int x, int y, int w, int h,
                           gpointer data) {
    GList **cache = data;
    NautilusDragSelectionItem *item;
...
GList *nautilus_drag_create_selection_cache(
...
gboolean
nautilus_drag_drag_data_get_from_cache(GList *cache, GdkDragContext *context,
    ...
    for (l = cache; l != NULL; l = l->next) {
        ...
        (*func)(item->uri, item->icon_x, item->icon_y, item->icon_width,
                item->icon_height, result);
    }

    gtk_selection_data_set(selection_data,
                           gtk_selection_data_get_target(selection_data), 8,
                           (guchar *)result->str, result->len);
...
static void menu_deactivate_callback(GtkWidget *menu, gpointer data) {
...
static void drop_action_activated_callback(GtkWidget *menu_item,
        gpointer data) {
    DropActionMenuData *damd;
    ...
    damd->chosen =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(menu_item), "action"));
...
static void append_drop_action_menu_item(GtkWidget *menu, const char *text,
    ...
    g_signal_connect(menu_item, "activate",
                     G_CALLBACK(drop_action_activated_callback), damd);
...
GdkDragAction nautilus_drag_drop_action_ask(GtkWidget *widget,
    ...
    append_drop_action_menu_item(menu, _("_Move Here"), GDK_ACTION_MOVE,
                                 (actions & GDK_ACTION_MOVE) != 0, &damd);

    append_drop_action_menu_item(menu, _("_Copy Here"), GDK_ACTION_COPY,
                                 (actions & GDK_ACTION_COPY) != 0, &damd);

    append_drop_action_menu_item(menu, _("_Link Here"), GDK_ACTION_LINK,
                                 (actions & GDK_ACTION_LINK) != 0, &damd);
    ...
    g_signal_connect(menu, "deactivate", G_CALLBACK(menu_deactivate_callback),
                     &damd);
...
gboolean nautilus_drag_autoscroll_in_scroll_region(GtkWidget *widget) {
    ...
    nautilus_drag_autoscroll_calculate_delta(widget, &x_scroll_delta,
            &y_scroll_delta);
...
void nautilus_drag_autoscroll_calculate_delta(GtkWidget *widget,
    ...
    gdk_window_get_device_position(gtk_widget_get_window(widget), pointer, &x, &y,
                                   NULL);
    ...
    *y_scroll_delta = 0;
    ...
    /* Adjust the scroll delta to the proper acceleration values depending on how
     * far into the sroll margins we are.
...
void nautilus_drag_autoscroll_start(NautilusDragInfo *drag_info,
                                    GtkWidget *widget, GSourceFunc callback,
                                    gpointer user_data) {
    if (nautilus_drag_autoscroll_in_scroll_region(widget)) {
        if (drag_info->auto_scroll_timeout_id == 0) {
            ...
            drag_info->start_auto_scroll_in =
                g_get_monotonic_time() + AUTOSCROLL_INITIAL_DELAY;

            drag_info->auto_scroll_timeout_id =
                g_timeout_add(AUTOSCROLL_TIMEOUT_INTERVAL, callback, user_data);
        }
    } else {
        if (drag_info->auto_scroll_timeout_id != 0) {
