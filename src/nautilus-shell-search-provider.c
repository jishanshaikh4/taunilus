...
G_DEFINE_TYPE(NautilusShellSearchProvider, nautilus_shell_search_provider,
              G_TYPE_OBJECT)

static gchar *get_display_name(NautilusShellSearchProvider *self,
                               NautilusFile *file) {
    GFile *location;
    ...
    bookmark =
        nautilus_bookmark_list_item_with_location(bookmarks, location, NULL);
...
static GIcon *get_gicon(NautilusShellSearchProvider *self, NautilusFile *file) {
    GFile *location;
    NautilusBookmark *bookmark;
    NautilusBookmarkList *bookmarks;
...
static void pending_search_free(PendingSearch *search) {
...
static void pending_search_finish(PendingSearch *search,
                                  GDBusMethodInvocation *invocation,
                                  GVariant *result) {
...
static void cancel_current_search(NautilusShellSearchProvider *self) {
    if (self->current_search != NULL) {
        g_debug("*** Cancel current search");
        nautilus_search_provider_stop(
            NAUTILUS_SEARCH_PROVIDER(self->current_search->engine));
    }
}

static void cancel_current_search_ignoring_partial_results(
    NautilusShellSearchProvider *self) {
    if (self->current_search != NULL) {
        cancel_current_search(self);
        g_signal_handlers_disconnect_by_data(G_OBJECT(self->current_search->engine),
                                             self->current_search);
        pending_search_finish(self->current_search,
                              self->current_search->invocation,
                              g_variant_new("(as)", NULL));
    }
}

static void search_hits_added_cb(NautilusSearchEngine *engine, GList *hits,
                                 gpointer user_data) {
...
static gint search_hit_compare_relevance(gconstpointer a, gconstpointer b) {
...
static void search_finished_cb(NautilusSearchEngine *engine,
...
static void search_error_cb(NautilusSearchEngine *engine,
...
static void search_hit_candidate_free(SearchHitCandidate *candidate) {
...
static SearchHitCandidate *search_hit_candidate_new(const gchar *uri,
        const gchar *name) {
...
static void search_add_volumes_and_bookmarks(PendingSearch *search) {
    NautilusSearchHit *hit;
    NautilusBookmark *bookmark;
    const gchar *name;
    gchar *string, *uri;
...
static NautilusQuery *shell_query_new(gchar **terms) {
    NautilusQuery *query;
...
static void execute_search(NautilusShellSearchProvider *self,
    ...
    g_signal_connect(pending_search->engine, "error", G_CALLBACK(search_error_cb),
                     pending_search);
    ...
    nautilus_search_provider_set_query(
        NAUTILUS_SEARCH_PROVIDER(pending_search->engine), query);
    nautilus_search_provider_start(
        NAUTILUS_SEARCH_PROVIDER(pending_search->engine));
}
...
static gboolean handle_get_subsearch_result_set(
    NautilusShellSearchProvider2 *skeleton, GDBusMethodInvocation *invocation,
    gchar **previous_results, gchar **terms, gpointer user_data) {
    NautilusShellSearchProvider *self = user_data;
...
static void result_metas_data_free(ResultMetasData *data) {
...
static void result_metas_return_from_cache(ResultMetasData *data) {
    GVariantBuilder builder;
    GVariant *meta;
    gint64 current_time;
    gint idx;
    ...
    if (data->uris) {
        for (idx = 0; data->uris[idx] != NULL; idx++) {
            meta = g_hash_table_lookup(data->self->metas_cache, data->uris[idx]);
...
static void result_metas_return_empty(ResultMetasData *data) {
    g_clear_pointer(&data->uris, g_strfreev);
    result_metas_return_from_cache(data);
    result_metas_data_free(data);
}

static void cancel_result_meta_requests(NautilusShellSearchProvider *self) {
...
static void result_list_attributes_ready_cb(GList *file_list,
        gpointer user_data) {
    ...
    icon_scale = gdk_monitor_get_scale_factor(
    ...
    for (l = file_list; l != NULL; l = l->next) {
        ...
        g_variant_builder_add(&meta, "{sv}", "id", g_variant_new_string(uri));
        g_variant_builder_add(&meta, "{sv}", "name",
                              g_variant_new_string(display_name));
        /* Some backends like trash:/// don't have a path, so we show the uri
         * itself. */
        g_variant_builder_add(
            &meta, "{sv}", "description",
            g_variant_new_string(description ? description : uri));
        ...
        if (gicon == NULL) {
            gicon = G_ICON(nautilus_file_get_icon_pixbuf(
                               file, 128, TRUE, icon_scale,
                               NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS));
        }

        g_variant_builder_add(&meta, "{sv}", "icon", g_icon_serialize(gicon));
        g_object_unref(gicon);

        meta_variant = g_variant_builder_end(&meta);
        g_hash_table_insert(data->self->metas_cache, g_strdup(uri),
...
static gboolean handle_get_result_metas(NautilusShellSearchProvider2 *skeleton,
                                        GDBusMethodInvocation *invocation,
                                        gchar **results, gpointer user_data) {
    NautilusShellSearchProvider *self = user_data;
    ...
    for (idx = 0; results[idx] != NULL; idx++) {
        ...
        if (!g_hash_table_lookup(self->metas_cache, uri)) {
            missing_files =
                g_list_prepend(missing_files, nautilus_file_get_by_uri(uri));
        }
    }
    ...
    nautilus_file_list_call_when_ready(
        missing_files, NAUTILUS_FILE_ATTRIBUTES_FOR_ICON, &data->handle,
        result_list_attributes_ready_cb, data);
...
static void show_uri_callback(gboolean res, gpointer user_data) {
    ...
    nautilus_shell_search_provider2_complete_activate_result(data->skeleton,
            data->invocation);
...
static gboolean handle_activate_result(NautilusShellSearchProvider2 *skeleton,
                                       GDBusMethodInvocation *invocation,
                                       gchar *result, gchar **terms,
...
static gboolean handle_launch_search(NautilusShellSearchProvider2 *skeleton,
                                     GDBusMethodInvocation *invocation,
                                     gchar **terms, guint32 timestamp,
                                     gpointer user_data) {
...
nautilus_shell_search_provider_init(NautilusShellSearchProvider *self) {
    self->metas_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
...
static void nautilus_shell_search_provider_class_init(
    NautilusShellSearchProviderClass *klass) {
...
NautilusShellSearchProvider *nautilus_shell_search_provider_new(void) {
    return g_object_new(nautilus_shell_search_provider_get_type(), NULL);
}
...
void nautilus_shell_search_provider_unregister(
    NautilusShellSearchProvider *self) {
