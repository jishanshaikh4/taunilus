...
enum { OPEN_LOCATION, LAST_SIGNAL };
...
static void action_pathbar_open_item_new_tab(GSimpleAction *action,
                                             GVariant *state,
                                             gpointer user_data);
static void action_pathbar_properties(GSimpleAction *action, GVariant *state,
                                      gpointer user_data);
...
static void action_pathbar_properties(GSimpleAction *action, GVariant *state,
  ...
  nautilus_properties_window_present(files, GTK_WIDGET(self), NULL, NULL, NULL);

  nautilus_file_list_free(files);
}

static void on_adjustment_changed(GtkAdjustment *adjustment) {
  /* Automatically scroll to the end, to keep the current folder visible */
  gtk_adjustment_set_value(adjustment, gtk_adjustment_get_upper(adjustment));
}

static gboolean bind_current_view_menu_model_to_popover(NautilusPathBar *self) {
  gtk_popover_bind_model(self->current_view_menu_popover,
                         G_MENU_MODEL(self->current_view_menu), NULL);
  return G_SOURCE_REMOVE;
}

static void nautilus_path_bar_init(NautilusPathBar *self) {
  ...
  g_signal_connect(adjustment, "changed", (GCallback)on_adjustment_changed,
                   NULL);
  ...
  gtk_menu_button_set_child(
      GTK_MENU_BUTTON(self->current_view_menu_button),
      gtk_image_new_from_icon_name("view-more-symbolic", GTK_ICON_SIZE_MENU));
  ...
  gtk_builder_add_from_resource(
      builder, "/org/gnome/nautilus/ui/nautilus-pathbar-context-menu.ui",
      &error);
  if (error != NULL) {
    g_error("Failed to add pathbar-context-menu.ui: %s", error->message);
  }
  self->button_menu =
      g_object_ref_sink(G_MENU(gtk_builder_get_object(builder, "button-menu")));
  self->button_menu_popover = g_object_ref_sink(GTK_POPOVER(
      gtk_popover_new_from_model(NULL, G_MENU_MODEL(self->button_menu))));

  /* Add current location menu, which matches the view's background context menu
   */
  gtk_builder_add_from_resource(
      builder, "/org/gnome/nautilus/ui/nautilus-files-view-context-menus.ui",
      &error);
  if (error != NULL) {
    g_error("failed to add files-view-context-menus.ui: %s", error->message);
  }
  self->current_view_menu = g_object_ref_sink(
      G_MENU(gtk_builder_get_object(builder, "background-menu")));
  self->extensions_section = g_object_ref(
      G_MENU(gtk_builder_get_object(builder, "background-extensions-section")));
  self->templates_submenu = g_object_ref(
      G_MENU(gtk_builder_get_object(builder, "templates-submenu")));
  self->current_view_menu_popover =
      g_object_ref_sink(GTK_POPOVER(gtk_popover_new(NULL)));
  ...
  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(self->buttons_box)),
      "path-buttons-box");
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(self->action_group),
                                  path_bar_actions,
                                  G_N_ELEMENTS(path_bar_actions), self);
  gtk_widget_insert_action_group(GTK_WIDGET(self), "pathbar",
...
static void remove_settings_signal(NautilusPathBar *self, GdkScreen *screen) {
...
static void nautilus_path_bar_style_updated(GtkWidget *widget) {
...
static void nautilus_path_bar_screen_changed(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(nautilus_path_bar_parent_class)->screen_changed) {
    GTK_WIDGET_CLASS(nautilus_path_bar_parent_class)
...
static void nautilus_path_bar_class_init(NautilusPathBarClass *path_bar_class) {
  ...
  path_bar_signals[OPEN_LOCATION] =
      g_signal_new("open-location", G_OBJECT_CLASS_TYPE(path_bar_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
                   G_TYPE_NONE, 2, G_TYPE_FILE, NAUTILUS_TYPE_OPEN_FLAGS);
}
...
static void reload_icons(NautilusPathBar *self) {
  GList *list;
...
static void nautilus_path_bar_check_icon_theme(NautilusPathBar *self) {
  ...
  settings =
      gtk_settings_get_for_screen(gtk_widget_get_screen(GTK_WIDGET(self)));
  self->settings_signal_id = g_signal_connect(
      settings, "notify", G_CALLBACK(settings_notify_cb), self);
...
void nautilus_path_bar_show_current_location_menu(NautilusPathBar *self) {
  g_return_if_fail(NAUTILUS_IS_PATH_BAR(self));
...
static void button_clicked_cb(GtkButton *button, gpointer data) {
  ...
  if (g_file_equal(button_data->path, self->current_path)) {
    ...
    g_signal_emit(self, path_bar_signals[OPEN_LOCATION], 0, button_data->path,
                  0);
  }
}

static void real_pop_up_pathbar_context_menu(NautilusPathBar *self) {
  gtk_popover_popup(self->button_menu_popover);
}

static void pathbar_popup_file_attributes_ready(NautilusFile *file,
                                                gpointer data) {
  NautilusPathBar *self;
...
static void unschedule_pop_up_context_menu(NautilusPathBar *self) {
  if (self->context_menu_file != NULL) {
    ...
    nautilus_file_cancel_call_when_ready(
        self->context_menu_file, pathbar_popup_file_attributes_ready, self);
    g_clear_pointer(&self->context_menu_file, nautilus_file_unref);
  }
}

static void schedule_pop_up_context_menu(NautilusPathBar *self,
                                         NautilusFile *file) {
  g_return_if_fail(NAUTILUS_IS_FILE(file));

  if (file == self->context_menu_file) {
    if (nautilus_file_check_if_ready(
            file, NAUTILUS_FILE_ATTRIBUTE_INFO | NAUTILUS_FILE_ATTRIBUTE_MOUNT |
    ...
    nautilus_file_call_when_ready(self->context_menu_file,
                                  NAUTILUS_FILE_ATTRIBUTE_INFO |
                                      ...
                                      NAUTILUS_FILE_ATTRIBUTE_FILESYSTEM_INFO,
                                  pathbar_popup_file_attributes_ready, self);
  }
}
  ...
  case GDK_BUTTON_PRIMARY: {
    if ((state & GDK_CONTROL_MASK) != 0) {
      g_signal_emit(button_data->path_bar, path_bar_signals[OPEN_LOCATION], 0,
                    button_data->path, NAUTILUS_OPEN_FLAG_NEW_WINDOW);
    } else {
...
static GIcon *get_gicon_for_mount(ButtonData *button_data) {
  GIcon *icon;
  GMount *mount;
...
static GIcon *get_gicon(ButtonData *button_data) {
  switch (button_data->type) {
  case ROOT_BUTTON:
  case ADMIN_ROOT_BUTTON: {
...
static void nautilus_path_bar_update_button_appearance(ButtonData *button_data,
  ...
  if (button_data->label != NULL) {
    ...
    if (g_utf8_strlen(dir_name, -1) > min_chars + 1) {
      ...
      gtk_label_set_ellipsize(GTK_LABEL(button_data->label),
...
static void setup_button_type(ButtonData *button_data, NautilusPathBar *self,
                              GFile *location) {
...
static void button_data_file_changed(NautilusFile *file,
                                     ButtonData *button_data) {
  GtkWidget *ancestor;
  GFile *location;
  ...
  ancestor =
      gtk_widget_get_ancestor(button_data->button, NAUTILUS_TYPE_PATH_BAR);
  if (ancestor == NULL) {
    return;
  }
  ...
  if (!g_file_equal(button_data->path, location)) {
    ...
    if (renamed) {
       ...
       * If it was not below the currently displayed location, update the path
       * bar
       */
      child = g_file_has_prefix(button_data->path, self->current_path);
      ...
      if (child) {
         ...
         */
        current_location =
            nautilus_file_get_location(current_button_data->file);
      }
...
static ButtonData *make_button_data(NautilusPathBar *self, NautilusFile *file,
                                    gboolean current_dir) {
  GFile *path;
  ...
  default: {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(separator_label),
                                "dim-label");
    button_data->label = gtk_label_new(NULL);
  ...
  if (current_dir) {
    gtk_style_context_add_class(
        gtk_widget_get_style_context(button_data->button), "current-dir");
  ...
  if (button_data->label != NULL) {
    ...
    if (!current_dir) {
      gtk_style_context_add_class(
          gtk_widget_get_style_context(button_data->label), "dim-label");
      gtk_style_context_add_class(
          gtk_widget_get_style_context(button_data->image), "dim-label");
    }
  }
  ...
  if (button_data->file == NULL) {
    ...
    button_data->file_changed_signal_id =
        g_signal_connect(button_data->file, "changed",
                         G_CALLBACK(button_data_file_changed), button_data);
  }
   ...
   * secondary-clicking.
   */
  button_data->multi_press_gesture =
      gtk_gesture_multi_press_new(button_data->button);

  gtk_gesture_single_set_button(
      GTK_GESTURE_SINGLE(button_data->multi_press_gesture), 0);
...
void nautilus_path_bar_set_path(NautilusPathBar *self, GFile *file_path) {
  ButtonData *button_data;
