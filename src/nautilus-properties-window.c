...
enum { COLUMN_NAME, COLUMN_VALUE, COLUMN_USE_ORIGINAL, COLUMN_ID, NUM_COLUMNS };
...
static void file_changed_callback(NautilusFile *file, gpointer user_data);
static void permission_button_update(GtkToggleButton *button,
                                     NautilusPropertiesWindow *self);
static void permission_combo_update(GtkComboBox *combo,
                                    NautilusPropertiesWindow *self);
static void value_field_update(GtkLabel *field, NautilusPropertiesWindow *self);
static void properties_window_update(NautilusPropertiesWindow *self,
                                     GList *files);
static void is_directory_ready_callback(NautilusFile *file, gpointer data);
...
static void select_image_button_callback(GtkWidget *widget,
                                         NautilusPropertiesWindow *self);
static void set_icon(const char *icon_path, NautilusPropertiesWindow *self);
static void remove_pending(StartupData *data, gboolean cancel_call_when_ready,
                           gboolean cancel_timed_wait);
static void append_extension_pages(NautilusPropertiesWindow *self);

static void name_field_focus_changed(GObject *object, GParamSpec *pspec,
                                     gpointer user_data);
static void name_field_activate(GtkWidget *name_field, gpointer user_data);
static void setup_pie_widget(NautilusPropertiesWindow *self);

G_DEFINE_TYPE(NautilusPropertiesWindow, nautilus_properties_window,
              HDY_TYPE_WINDOW);

static gboolean is_multi_file_window(NautilusPropertiesWindow *self) {
  GList *l;
  int count;
...
static int get_not_gone_original_file_count(NautilusPropertiesWindow *self) {
  GList *l;
  int count;
...
static NautilusFile *get_original_file(NautilusPropertiesWindow *self) {
...
static NautilusFile *get_target_file_for_original_file(NautilusFile *file) {
...
static NautilusFile *get_target_file(NautilusPropertiesWindow *self) {
  return NAUTILUS_FILE(self->target_files->data);
}

static void get_image_for_properties_window(NautilusPropertiesWindow *self,
                                            char **icon_name,
  ...
  for (l = self->original_files; l != NULL; l = l->next) {
    ...
    if (!icon) {
      icon = nautilus_file_get_icon(
          file, NAUTILUS_GRID_ICON_SIZE_STANDARD, icon_scale,
          NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS |
              NAUTILUS_FILE_ICON_FLAGS_IGNORE_VISITING);
    } else {
      new_icon = nautilus_file_get_icon(
          file, NAUTILUS_GRID_ICON_SIZE_STANDARD, icon_scale,
          NAUTILUS_FILE_ICON_FLAGS_USE_THUMBNAILS |
              NAUTILUS_FILE_ICON_FLAGS_IGNORE_VISITING);
  ...
  if (!icon) {
    icon = nautilus_icon_info_lookup_from_name(
        "text-x-generic", NAUTILUS_GRID_ICON_SIZE_STANDARD, icon_scale);
  }
  ...
  if (icon_pixbuf != NULL) {
    *icon_pixbuf = nautilus_icon_info_get_pixbuf_at_size(
        icon, NAUTILUS_GRID_ICON_SIZE_STANDARD);
  }
}

static void update_properties_window_icon(NautilusPropertiesWindow *self) {
  ...
  surface = gdk_cairo_surface_create_from_pixbuf(
      pixbuf, gtk_widget_get_scale_factor(GTK_WIDGET(self)),
...
static gboolean uri_is_local_image(const char *uri) {
  g_autoptr(GdkPixbuf) pixbuf = NULL;
...
static void reset_icon(NautilusPropertiesWindow *self) {
  GList *l;
...
static void nautilus_properties_window_drag_data_received(
  ...
  if (!exactly_one) {
    show_dialog(_("You cannot assign more than one custom icon at a time!"),
                _("Please drop just one image to set a custom icon."), window,
                GTK_MESSAGE_ERROR);
  } else {
    if (uri_is_local_image(uris[0])) {
      ...
      if (!g_file_is_native(f)) {
        show_dialog(_("The file that you dropped is not local."),
                    _("You can only use local images as custom icons."), window,
                    GTK_MESSAGE_ERROR);
      } else {
...
static void setup_image_widget(NautilusPropertiesWindow *self,
  ...
  if (is_customizable) {
    ...
    g_signal_connect(self->icon_button_image, "drag-data-received",
                     G_CALLBACK(nautilus_properties_window_drag_data_received),
                     NULL);
...
static void set_name_field(NautilusPropertiesWindow *self,
                           const gchar *original_name, const gchar *name) {
  GtkWidget *stack_child_label;
  GtkWidget *stack_child_entry;
  gboolean use_label;

  stack_child_label =
      gtk_stack_get_child_by_name(self->name_stack, "name_value_label");
  stack_child_entry =
      gtk_stack_get_child_by_name(self->name_stack, "name_value_entry");

  use_label = is_multi_file_window(self) ||
              !nautilus_file_can_rename(get_original_file(self));
  ...
  if (original_name == NULL || strcmp(original_name, name) != 0) {
    if (!use_label) {
       ...
       * selection change).
       */
      g_autofree gchar *displayed_name =
          gtk_editable_get_chars(GTK_EDITABLE(self->name_field), 0, -1);
...
static void update_name_field(NautilusPropertiesWindow *self) {
  NautilusFile *file;

  gtk_label_set_text_with_mnemonic(
      self->name_title_label,
      ngettext("_Name", "_Names", get_not_gone_original_file_count(self)));
  ...
  if (is_multi_file_window(self)) {
    ...
    if (original_name == NULL || g_strcmp0(original_name, current_name) != 0) {
      g_object_set_data_full(G_OBJECT(self->name_field), "original_name",
                             g_steal_pointer(&current_name), g_free);
    }
  }
}

static void name_field_restore_original_name(GtkWidget *name_field) {
  const char *original_name;
  g_autofree char *displayed_name = NULL;

  original_name =
      (const char *)g_object_get_data(G_OBJECT(name_field), "original_name");
...
static void rename_callback(NautilusFile *file, GFile *res_loc, GError *error,
                            gpointer callback_data) {
  g_autoptr(NautilusPropertiesWindow) self =
      NAUTILUS_PROPERTIES_WINDOW(callback_data);

  /* Complain to user if rename failed. */
  if (error != NULL) {
    nautilus_report_error_renaming_file(file, self->pending_name, error,
                                        GTK_WINDOW(self));
    name_field_restore_original_name(self->name_field);
  }
}

static void set_pending_name(NautilusPropertiesWindow *self, const char *name) {
  g_free(self->pending_name);
  self->pending_name = g_strdup(name);
}

static void name_field_done_editing(GtkWidget *name_field,
                                    NautilusPropertiesWindow *self) {
  NautilusFile *file;
  ...
  if (strlen(new_name) == 0) {
    ...
    if (strcmp(new_name, original_name) != 0) {
      ...
      g_object_ref(self);
      nautilus_file_rename(file, new_name, rename_callback, self);
    }
  }
}
...
static gboolean select_all_at_idle(gpointer user_data) {
  NautilusPropertiesWindow *self;

  self = NAUTILUS_PROPERTIES_WINDOW(user_data);

  gtk_editable_select_region(GTK_EDITABLE(self->name_field), 0, -1);
...
static void name_field_activate(GtkWidget *name_field, gpointer user_data) {
  ...
  if (self->select_idle_id == 0) {
    self->select_idle_id = g_idle_add(select_all_at_idle, self);
  }
}

static void update_properties_window_title(NautilusPropertiesWindow *self) {
  ...
  if (!is_multi_file_window(self)) {
    ...
    if (file != NULL) {
      ...
      if (nautilus_file_is_directory(file)) {
        ...
        title = g_strdup_printf(C_("folder", "%s Properties"), truncated_name);
      } else {
        /* To translators: %s is the name of the file. */
        title = g_strdup_printf(C_("file", "%s Properties"), truncated_name);
      }
    }
  }
...
static void clear_extension_pages(NautilusPropertiesWindow *self) {
  int i;
  int num_pages;
  GtkWidget *page;
  ...
  for (i = 0; i < num_pages; i++) {
    page = gtk_notebook_get_nth_page(GTK_NOTEBOOK(self->notebook), i);

    if (GPOINTER_TO_INT(
            g_object_get_data(G_OBJECT(page), "is-extension-page"))) {
      gtk_notebook_remove_page(GTK_NOTEBOOK(self->notebook), i);
      num_pages--;
      i--;
    }
  }
...
static void remove_from_dialog(NautilusPropertiesWindow *self,
                               NautilusFile *file) {
  int index;
  GList *original_link;
  GList *target_link;
  ...
  self->original_files =
      g_list_delete_link(self->original_files, original_link);
  ...
  g_signal_handlers_disconnect_by_func(original_file,
                                       G_CALLBACK(file_changed_callback), self);
...
static gboolean mime_list_equal(GList *a, GList *b) {
  while (a && b) {
...
static GList *get_mime_list(NautilusPropertiesWindow *self) {
  return g_list_copy_deep(self->target_files,
                          (GCopyFunc)nautilus_file_get_mime_type, NULL);
}

static gboolean start_spinner_callback(NautilusPropertiesWindow *self) {
...
static void schedule_start_spinner(NautilusPropertiesWindow *self) {
  if (self->deep_count_spinner_timeout_id == 0) {
    self->deep_count_spinner_timeout_id =
        g_timeout_add_seconds(1, (GSourceFunc)start_spinner_callback, self);
  }
}

static void stop_spinner(NautilusPropertiesWindow *self) {
...
static void stop_deep_count_for_file(NautilusPropertiesWindow *self,
                                     NautilusFile *file) {
  if (g_list_find(self->deep_count_files, file)) {
    g_signal_handlers_disconnect_by_func(
        file, G_CALLBACK(schedule_directory_contents_update), self);
    nautilus_file_unref(file);
    self->deep_count_files = g_list_remove(self->deep_count_files, file);
  }
}

static void start_deep_count_for_file(NautilusFile *file,
                                      NautilusPropertiesWindow *self) {
  if (!nautilus_file_is_directory(file)) {
    return;
  }
  ...
  if (!g_list_find(self->deep_count_files, file)) {
    ...
    if (!self->deep_count_finished) {
      g_signal_connect_object(file, "updated-deep-count-in-progress",
...
static void properties_window_update(NautilusPropertiesWindow *self,
  ...
  for (GList *tmp = files; tmp != NULL; tmp = tmp->next) {
    ...
    if (changed_file == NULL || g_list_find(self->target_files, changed_file)) {
      dirty_target = TRUE;
    }
  }
  ...
  if (dirty_target) {
    g_list_foreach(self->permission_buttons, (GFunc)permission_button_update,
                   self);
    g_list_foreach(self->permission_combos, (GFunc)permission_combo_update,
                   self);
    g_list_foreach(self->value_fields, (GFunc)value_field_update, self);
  }
...
static gboolean update_files_callback(gpointer data) {
  NautilusPropertiesWindow *self;
...
static void schedule_files_update(NautilusPropertiesWindow *self) {
  g_assert(NAUTILUS_IS_PROPERTIES_WINDOW(self));

  if (self->update_files_timeout_id == 0) {
    self->update_files_timeout_id =
        g_timeout_add(FILES_UPDATE_INTERVAL, update_files_callback, self);
  }
}

static gboolean file_list_attributes_identical(GList *file_list,
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    if (first_attr == NULL) {
      first_attr =
          nautilus_file_get_string_attribute_with_default(file, attribute_name);
    } else {
...
static char *file_list_get_string_attribute(GList *file_list,
                                            const char *attribute_name,
  ...
  if (file_list_attributes_identical(file_list, attribute_name)) {
    ...
    for (l = file_list; l != NULL; l = l->next) {
      ...
      file = NAUTILUS_FILE(l->data);
      if (!nautilus_file_is_gone(file)) {
        return nautilus_file_get_string_attribute_with_default(file,
                                                               attribute_name);
      }
    }
...
#define INCONSISTENT_STATE_STRING "\xE2\x80\x92"

static gboolean location_show_original(NautilusPropertiesWindow *self) {
  NautilusFile *file;
...
static void value_field_update(GtkLabel *label,
                               NautilusPropertiesWindow *self) {
  GList *file_list;
  const char *attribute_name;
  ...
  attribute_value = file_list_get_string_attribute(file_list, attribute_name,
                                                   inconsistent_string);
  if (!strcmp(attribute_name, "detailed_type") &&
      strcmp(attribute_value, inconsistent_string)) {
    g_autofree char *mime_type = file_list_get_string_attribute(
        file_list, "mime_type", inconsistent_string);
    if (strcmp(mime_type, inconsistent_string) &&
        strcmp(mime_type, "inode/directory")) {
      g_autofree char *tmp = g_steal_pointer(&attribute_value);
      attribute_value = g_strdup_printf(
          C_("MIME type description (MIME type)", "%s (%s)"), tmp, mime_type);
    }
  }
...
static void group_change_free(GroupChange *change) {
...
static void group_change_callback(NautilusFile *file, GFile *res_loc,
                                  GError *error, GroupChange *change) {
  NautilusPropertiesWindow *self;
  ...
  if (!change->cancelled) {
    ...
    eel_timed_wait_stop((EelCancelCallback)cancel_group_change_callback,
                        change);
...
static void cancel_group_change_callback(GroupChange *change) {
  ...
  nautilus_file_cancel(change->file,
                       (NautilusFileOperationCallback)group_change_callback,
                       change);
}

static gboolean schedule_group_change_timeout(GroupChange *change) {
  g_assert(NAUTILUS_IS_PROPERTIES_WINDOW(change->window));
  g_assert(NAUTILUS_IS_FILE(change->file));
  g_assert(change->group != NULL);
  ...
  eel_timed_wait_start((EelCancelCallback)cancel_group_change_callback, change,
                       _("Cancel Group Change?"), change->window);

  nautilus_file_set_group(change->file, change->group,
                          (NautilusFileOperationCallback)group_change_callback,
                          change);
...
static void schedule_group_change(NautilusPropertiesWindow *self,
                                  NautilusFile *file, const char *group) {
  GroupChange *change;
  ...
  change->timeout = g_timeout_add(
      CHOWN_CHGRP_TIMEOUT, (GSourceFunc)schedule_group_change_timeout, change);

  self->group_change = change;
}

static void unschedule_or_cancel_group_change(NautilusPropertiesWindow *self) {
  GroupChange *change;
  ...
  if (change != NULL) {
    if (change->timeout == 0) {
      /* The operation was started, cancel it and let the operation callback
       * free the change */
...
static void changed_group_callback(GtkComboBox *combo_box, NautilusFile *file) {
  NautilusPropertiesWindow *self;
  ...
  if (group != NULL && strcmp(group, cur_group) != 0) {
    ...
    self = NAUTILUS_PROPERTIES_WINDOW(
        gtk_widget_get_ancestor(GTK_WIDGET(combo_box), GTK_TYPE_WINDOW));
...
static gboolean tree_model_entries_equal(GtkTreeModel *model,
                                         unsigned int column, GList *entries) {
  GtkTreeIter iter;
...
static char *combo_box_get_active_entry(GtkComboBox *combo_box,
                                        unsigned int column) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  char *val;
 ...
 * or entry is NULL.
 * */
static int tree_model_get_entry_index(GtkTreeModel *model, unsigned int column,
                                      const char *entry) {
  GtkTreeIter iter;
  int index;
  gboolean empty_model;
...
static void synch_groups_combo_box(GtkComboBox *combo_box, NautilusFile *file) {
  GList *groups = NULL;
  GList *node;
  GtkTreeModel *model;
  GtkListStore *store;
  ...
  if (!tree_model_entries_equal(model, 0, groups)) {
    ...
    for (node = groups, group_index = 0; node != NULL;
         node = node->next, ++group_index) {
  ...
  current_group_index =
      tree_model_get_entry_index(model, 0, current_group_name);
...
static void setup_group_combo_box(GtkWidget *combo_box, NautilusFile *file) {
  gtk_combo_box_set_row_separator_func(
      GTK_COMBO_BOX(combo_box), combo_box_row_separator_func, NULL, NULL);

  synch_groups_combo_box(GTK_COMBO_BOX(combo_box), file);

  /* Connect to signal to update menu when file changes. */
  g_signal_connect_object(file, "changed", G_CALLBACK(synch_groups_combo_box),
                          combo_box, G_CONNECT_SWAPPED);
  g_signal_connect_data(
      combo_box, "changed", G_CALLBACK(changed_group_callback),
      nautilus_file_ref(file), (GClosureNotify)nautilus_file_unref, 0);
}

static void owner_change_free(OwnerChange *change) {
  nautilus_file_unref(change->file);
  g_free(change->owner);
  g_object_unref(change->window);
...
static void owner_change_callback(NautilusFile *file, GFile *result_location,
                                  GError *error, OwnerChange *change) {
  NautilusPropertiesWindow *self;
  ...
  if (!change->cancelled) {
    /* Report the error if it's an error. */
    eel_timed_wait_stop((EelCancelCallback)cancel_owner_change_callback,
                        change);
...
static void cancel_owner_change_callback(OwnerChange *change) {
  ...
  change->cancelled = TRUE;
  nautilus_file_cancel(change->file,
                       (NautilusFileOperationCallback)owner_change_callback,
                       change);
}

static gboolean schedule_owner_change_timeout(OwnerChange *change) {
  g_assert(NAUTILUS_IS_PROPERTIES_WINDOW(change->window));
  g_assert(NAUTILUS_IS_FILE(change->file));
  g_assert(change->owner != NULL);
  ...
  eel_timed_wait_start((EelCancelCallback)cancel_owner_change_callback, change,
                       _("Cancel Owner Change?"), change->window);

  nautilus_file_set_owner(change->file, change->owner,
                          (NautilusFileOperationCallback)owner_change_callback,
                          change);
...
static void schedule_owner_change(NautilusPropertiesWindow *self,
                                  NautilusFile *file, const char *owner) {
  OwnerChange *change;
  ...
  change->window = GTK_WINDOW(g_object_ref(self));
  change->timeout = g_timeout_add(
      CHOWN_CHGRP_TIMEOUT, (GSourceFunc)schedule_owner_change_timeout, change);

  self->owner_change = change;
}

static void unschedule_or_cancel_owner_change(NautilusPropertiesWindow *self) {
  OwnerChange *change;
...
static void changed_owner_callback(GtkComboBox *combo_box, NautilusFile *file) {
  NautilusPropertiesWindow *self;
...
static void synch_user_menu(GtkComboBox *combo_box, NautilusFile *file) {
  GList *users = NULL;
  GList *node;
  GtkTreeModel *model;
  GtkListStore *store;
  ...
  if (!tree_model_entries_equal(model, 1, users)) {
    ...
    for (node = users, user_index = 0; node != NULL;
         node = node->next, ++user_index) {
      ...
      gtk_list_store_set(store, &iter, 0, combo_text, 1, user_name, 2,
                         name_array[0], -1);
    }
  }
  ...
  if (owner_index < 0 && owner_name != NULL) {
    if (users != NULL) {
      ...
      gtk_list_store_set(store, &iter, 0, "-", 1, NULL, 2, NULL, -1);
    }

    owner_index = 0;

    gtk_list_store_prepend(store, &iter);
    gtk_list_store_set(store, &iter, 0, nice_owner_name, 1, owner_name, 2,
                       owner_name, -1);
  }
...
static void setup_owner_combo_box(GtkWidget *combo_box, NautilusFile *file) {
  ...
  /* Connect to signal to update menu when file changes. */
  g_signal_connect_object(file, "changed", G_CALLBACK(synch_user_menu),
                          combo_box, G_CONNECT_SWAPPED);
  g_signal_connect_data(
      combo_box, "changed", G_CALLBACK(changed_owner_callback),
      nautilus_file_ref(file), (GClosureNotify)nautilus_file_unref, 0);
}

static gboolean file_has_prefix(NautilusFile *file, GList *prefix_candidates) {
  GList *p;
  g_autoptr(GFile) location = NULL;
...
directory_contents_value_field_update(NautilusPropertiesWindow *self) {
  ...
  for (l = self->target_files; l; l = l->next) {
    ...
    if (nautilus_file_is_directory(file)) {
      file_status =
          nautilus_file_get_deep_counts(file, &directory_count, &file_count,
                                        &file_unreadable, &file_size, TRUE);
  ...
  if (total_count == 0) {
    ...
    text = g_strdup_printf(ngettext("%'d item, with size %s",
                                    "%'d items, totalling %s", total_count),
  ...
  gtk_label_set_text(GTK_LABEL(self->contents_value_label), text);
...
static gboolean update_directory_contents_callback(gpointer data) {
  NautilusPropertiesWindow *self;
  ...
  if (self->update_directory_contents_timeout_id == 0) {
    self->update_directory_contents_timeout_id =
        g_timeout_add(DIRECTORY_CONTENTS_UPDATE_INTERVAL,
                      update_directory_contents_callback, self);
  }
}

static void setup_contents_field(NautilusPropertiesWindow *self,
                                 GtkGrid *grid) {
  g_list_foreach(self->target_files, (GFunc)start_deep_count_for_file, self);
...
static gboolean is_network_directory(NautilusFile *file) {
...
static gboolean is_burn_directory(NautilusFile *file) {
  g_autofree char *file_uri = NULL;
...
static gboolean should_show_file_type(NautilusPropertiesWindow *self) {
  if (!is_multi_file_window(self) &&
...
static gboolean should_show_location_info(NautilusPropertiesWindow *self) {
  GList *l;
...
static gboolean should_show_trash_orig_path(NautilusPropertiesWindow *self) {
  GList *l;
...
static gboolean should_show_accessed_date(NautilusPropertiesWindow *self) {
...
static gboolean should_show_modified_date(NautilusPropertiesWindow *self) {
  return !is_multi_file_window(self);
}

static gboolean should_show_created_date(NautilusPropertiesWindow *self) {
  return !is_multi_file_window(self);
}

static gboolean should_show_trashed_on(NautilusPropertiesWindow *self) {
  GList *l;
...
static gboolean should_show_link_target(NautilusPropertiesWindow *self) {
  if (!is_multi_file_window(self) &&
...
static gboolean should_show_free_space(NautilusPropertiesWindow *self) {
  if (!is_multi_file_window(self) &&
      (nautilus_file_is_in_trash(get_target_file(self)) ||
       is_network_directory(get_target_file(self)) ||
...
static gboolean should_show_volume_info(NautilusPropertiesWindow *self) {
  NautilusFile *file;
...
static gboolean should_show_volume_usage(NautilusPropertiesWindow *self) {
  NautilusFile *file;
...
static void paint_legend(GtkWidget *widget, cairo_t *cr, gpointer data) {
...
static void paint_slice(GtkWidget *widget, cairo_t *cr, double percent_start,
                        double percent_width, const gchar *style_class) {
  double angle1;
  double angle2;
  gboolean full;
...
static void paint_pie_chart(GtkWidget *widget, cairo_t *cr, gpointer data) {
  NautilusPropertiesWindow *self;
...
static void setup_pie_widget(NautilusPropertiesWindow *self) {
  ...
  info = g_file_query_filesystem_info(
      location, G_FILE_ATTRIBUTE_FILESYSTEM_TYPE, NULL, NULL);
  if (info) {
  ...
  g_signal_connect(self->pie_chart, "draw", G_CALLBACK(paint_pie_chart), self);
  g_signal_connect(self->used_color, "draw", G_CALLBACK(paint_legend), self);
  g_signal_connect(self->free_color, "draw", G_CALLBACK(paint_legend), self);
}

static void setup_volume_usage_widget(NautilusPropertiesWindow *self) {
  NautilusFile *file;
  g_autofree gchar *uri = NULL;
  g_autoptr(GFile) location = NULL;
  g_autoptr(GFileInfo) info = NULL;
  ...
  if (info) {
    self->volume_capacity = g_file_info_get_attribute_uint64(
    ...
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_FILESYSTEM_USED)) {
      self->volume_used = g_file_info_get_attribute_uint64(
...
static void open_in_disks(GtkButton *button, NautilusPropertiesWindow *self) {
  g_autofree char *message = NULL;
  g_autoptr(GError) error = NULL;
  g_autoptr(GAppInfo) app_info = NULL;

  app_info = g_app_info_create_from_commandline("gnome-disks", NULL,
  ...
  if (error != NULL) {
    ...
    show_dialog(_("There was an error launching the application."), message,
                GTK_WINDOW(self), GTK_MESSAGE_ERROR);
  }
}

static void setup_basic_page(NautilusPropertiesWindow *self) {
  GtkGrid *grid;
  ...
  if (should_show_file_type(self)) {
    ...
    self->value_fields =
        g_list_prepend(self->value_fields, self->type_value_label);
  }

  if (should_show_link_target(self)) {
    gtk_widget_show(self->link_target_title_label);
    gtk_widget_show(self->link_target_value_label);
    g_object_set_data_full(G_OBJECT(self->link_target_value_label),
                           "file_attribute", g_strdup("link_target"), g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->link_target_value_label);
  }
  ...
  if (is_multi_file_window(self) ||
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->size_value_label), "file_attribute",
                           g_strdup("size_detail"), g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->size_value_label);
  }
  ...
  if (should_show_location_info(self)) {
    ...
    g_object_set_data_full(G_OBJECT(self->parent_folder_value_label),
                           "file_attribute", g_strdup("where"), g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->parent_folder_value_label);
  }
  ...
  if (should_show_trash_orig_path(self)) {
    ...
    g_object_set_data_full(G_OBJECT(self->original_folder_value_label),
                           "file_attribute", g_strdup("trash_orig_path"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->original_folder_value_label);
  }
  ...
  if (should_show_volume_info(self)) {
    ...
    self->value_fields =
        g_list_prepend(self->value_fields, self->volume_value_label);
  }

  if (should_show_trashed_on(self)) {
    gtk_widget_show(self->trashed_on_title_label);
    gtk_widget_show(self->trashed_on_value_label);
    g_object_set_data_full(G_OBJECT(self->trashed_on_value_label),
                           "file_attribute", g_strdup("trashed_on_full"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->trashed_on_value_label);
  }

  if (should_show_accessed_date(self) || should_show_modified_date(self) ||
  ...
  if (should_show_accessed_date(self)) {
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->accessed_value_label),
                           "file_attribute", g_strdup("date_accessed_full"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->accessed_value_label);
  }
  ...
  if (should_show_modified_date(self)) {
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->modified_value_label),
                           "file_attribute", g_strdup("date_modified_full"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->modified_value_label);
  }
  ...
  if (should_show_created_date(self)) {
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->created_value_label),
                           "file_attribute", g_strdup("date_created_full"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->created_value_label);
  }

  if (should_show_free_space(self) && !should_show_volume_usage(self)) {
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->free_space_value_label),
                           "file_attribute", g_strdup("free_space"), g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->free_space_value_label);
  }
  ...
  if (should_show_volume_usage(self)) {
    ...
    g_signal_connect(self->open_in_disks_button, "clicked",
                     G_CALLBACK(open_in_disks), NULL);
  }
}

static gboolean files_has_directory(NautilusPropertiesWindow *self) {
  GList *l;
...
static gboolean files_has_file(NautilusPropertiesWindow *self) {
  GList *l;
...
static void start_long_operation(NautilusPropertiesWindow *self) {
...
static void end_long_operation(NautilusPropertiesWindow *self) {
...
static void permission_change_callback(NautilusFile *file, GFile *res_loc,
...
static void update_permissions(NautilusPropertiesWindow *self,
                               guint32 vfs_new_perm, guint32 vfs_mask,
                               gboolean is_folder,
  ...
  for (l = self->target_files; l != NULL; l = l->next) {
    ...
    if (use_original) {
      gpointer ptr;
      if (g_hash_table_lookup_extended(self->initial_permissions, file, NULL,
                                       &ptr)) {
        permissions =
            (permissions & ~vfs_mask) | (GPOINTER_TO_INT(ptr) & vfs_mask);
      }
    } else {
    ...
    g_object_ref(self);
    nautilus_file_set_permissions(file, permissions, permission_change_callback,
                                  self);
  }
}
...
initial_permission_state_consistent(NautilusPropertiesWindow *self,
                                    guint32 mask, gboolean is_folder,
  ...
  for (l = self->target_files; l != NULL; l = l->next) {
    ...
    permissions =
        GPOINTER_TO_INT(g_hash_table_lookup(self->initial_permissions, file));

    if (first) {
      if ((permissions & mask) != mask && (permissions & mask) != 0) {
...
static void permission_button_toggled(GtkToggleButton *button,
                                      NautilusPropertiesWindow *self) {
  ...
  permission_mask =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "permission"));
  is_folder = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "is-folder"));
  is_special =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "is-special"));
  ...
  if (gtk_toggle_button_get_active(button) &&
    ...
    if (initial_permission_state_consistent(self, permission_mask, is_folder,
                                            is_special)) {
      inconsistent = FALSE;
      on = TRUE;
    }
  ...
  g_signal_handlers_block_by_func(G_OBJECT(button),
                                  G_CALLBACK(permission_button_toggled), self);

  gtk_toggle_button_set_active(button, on);
  gtk_toggle_button_set_inconsistent(button, inconsistent);

  g_signal_handlers_unblock_by_func(
      G_OBJECT(button), G_CALLBACK(permission_button_toggled), self);

  update_permissions(self, on ? permission_mask : 0, permission_mask, is_folder,
                     is_special, inconsistent);
}
...
static void permission_button_update(GtkToggleButton *button,
  ...
  for (l = self->target_files; l != NULL; l = l->next) {
    ...
    if (!is_special && ((nautilus_file_is_directory(file) && !is_folder) ||
                        (!nautilus_file_is_directory(file) && is_folder))) {
      continue;
    }
...
static void setup_execute_checkbox_with_label(NautilusPropertiesWindow *self,
  ...
  self->permission_buttons =
      g_list_prepend(self->permission_buttons, self->execute_checkbox);

  g_signal_connect_object(self->execute_checkbox, "toggled",
                          G_CALLBACK(permission_button_toggled), self, 0);

  a11y_enabled =
      GTK_IS_ACCESSIBLE(gtk_widget_get_accessible(self->execute_checkbox));
...
static guint32 permission_to_vfs(PermissionType type, PermissionValue perm) {
  guint32 vfs_perm;
...
static PermissionValue permission_from_vfs(PermissionType type,
                                           guint32 vfs_perm) {
...
static void permission_combo_changed(GtkWidget *combo,
                                     NautilusPropertiesWindow *self) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  ...
  gtk_tree_model_get(model, &iter, COLUMN_VALUE, &new_perm, COLUMN_USE_ORIGINAL,
                     &use_original, -1);
  vfs_new_perm = permission_to_vfs(type, new_perm);

  update_permissions(self, vfs_new_perm, vfs_mask, is_folder, FALSE,
                     use_original);
}

static void permission_combo_add_multiple_choice(GtkComboBox *combo,
                                                 GtkTreeIter *iter) {
  GtkTreeModel *model;
  GtkListStore *store;
  gboolean found;
  ...
  if (!found) {
    gtk_list_store_append(store, iter);
    gtk_list_store_set(store, iter, COLUMN_NAME, "---", COLUMN_VALUE, 0,
...
static void permission_combo_update(GtkComboBox *combo,
  ...
  if (all_same) {
    ...
    if (!found) {
      ...
      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, 0, str->str, 1, all_perm, -1);
    }
  } else {
  ...
  g_signal_handlers_block_by_func(G_OBJECT(combo),
                                  G_CALLBACK(permission_combo_changed), self);
...
static void setup_permissions_combo_box(GtkComboBox *combo, PermissionType type,
                                        gboolean is_folder) {
  ...
  if (is_folder) {
    if (type != PERMISSION_USER) {
      ...
      gtk_list_store_set(store, &iter,
                          ...
                          */
                         COLUMN_NAME, _("None"), COLUMN_VALUE, 0, COLUMN_ID,
                         "none", -1);
    }
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_NAME, _("List files only"),
                       COLUMN_VALUE, PERMISSION_READ, COLUMN_ID, "r", -1);
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_NAME, _("Access files"),
                       COLUMN_VALUE, PERMISSION_READ | PERMISSION_EXEC,
                       COLUMN_ID, "rx", -1);
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_NAME, _("Create and delete files"),
                       COLUMN_VALUE,
    ...
    if (type != PERMISSION_USER) {
      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, COLUMN_NAME, _("None"), COLUMN_VALUE, 0,
                         COLUMN_ID, "none", -1);
    }
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_NAME, _("Read-only"), COLUMN_VALUE,
                       PERMISSION_READ, COLUMN_ID, "r", -1);
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, COLUMN_NAME, _("Read and write"),
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo), cell, "text",
                                 COLUMN_NAME, NULL);
}

static gboolean all_can_get_permissions(GList *file_list) {
  GList *l;
  for (l = file_list; l != NULL; l = l->next) {
    NautilusFile *file;
...
static GHashTable *get_initial_permissions(GList *file_list) {
  GHashTable *ret;
  GList *l;
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    permissions = nautilus_file_get_permissions(file);
    g_hash_table_insert(ret, file, GINT_TO_POINTER(permissions));
  }

  return ret;
}

static void create_simple_permissions(NautilusPropertiesWindow *self,
                                      GtkGrid *page_grid) {
  ...
  if (!is_multi_file_window(self) &&
    ...
    owner_combo_box = gtk_stack_get_child_by_name(
        GTK_STACK(self->owner_value_stack), "combo_box");
    gtk_stack_set_visible_child(GTK_STACK(self->owner_value_stack),
                                owner_combo_box);
    setup_owner_combo_box(owner_combo_box, get_target_file(self));
  } else {
    /* Static text in this case. */
    owner_value_label = gtk_stack_get_child_by_name(
        GTK_STACK(self->owner_value_stack), "label");
    gtk_stack_set_visible_child(GTK_STACK(self->owner_value_stack),
                                owner_value_label);
    ...
    self->value_fields = g_list_prepend(self->value_fields, owner_value_label);
  }
  if (has_directory && has_file) {
    ...
    g_signal_connect(self->owner_folder_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->owner_file_access_combo);
    g_signal_connect(self->owner_file_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  } else {
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->owner_access_combo);
    g_signal_connect(self->owner_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  }
  ...
  if (!is_multi_file_window(self) &&
    ...
    /* Combo box in this case. */
    group_combo_box = gtk_stack_get_child_by_name(
        GTK_STACK(self->group_value_stack), "combo_box");
    gtk_stack_set_visible_child(GTK_STACK(self->group_value_stack),
                                group_combo_box);
    setup_group_combo_box(group_combo_box, get_target_file(self));
  } else {
    group_value_label = gtk_stack_get_child_by_name(
        GTK_STACK(self->group_value_stack), "label");
    gtk_stack_set_visible_child(GTK_STACK(self->group_value_stack),
                                group_value_label);
    ...
    self->value_fields = g_list_prepend(self->value_fields, group_value_label);
  }
  ...
  if (has_directory && has_file) {
    ...
    g_signal_connect(self->group_folder_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->group_file_access_combo);
    g_signal_connect(self->group_file_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  } else {
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->group_access_combo);
    g_signal_connect(self->group_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  }
  ...
  if (has_directory && has_file) {
    ...
    g_signal_connect(self->others_folder_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->others_file_access_combo);
    g_signal_connect(self->others_file_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  } else {
    ...
    self->permission_combos =
        g_list_prepend(self->permission_combos, self->others_access_combo);
    g_signal_connect(self->others_access_combo, "changed",
                     G_CALLBACK(permission_combo_changed), self);
  }

  if (!has_directory) {
    setup_execute_checkbox_with_label(self, UNIX_PERM_USER_EXEC |
                                                UNIX_PERM_GROUP_EXEC |
                                                UNIX_PERM_OTHER_EXEC);
  }
}

static void set_recursive_permissions_done(gboolean success,
                                           gpointer callback_data) {
  g_autoptr(NautilusPropertiesWindow) self =
      NAUTILUS_PROPERTIES_WINDOW(callback_data);
  end_long_operation(self);
...
static void on_change_permissions_response(GtkDialog *dialog, int response,
                                           NautilusPropertiesWindow *self) {
  ...
  for (l = self->change_permission_combos; l != NULL; l = l->next) {
    ...
    type =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(combo), "permission-type"));
    is_folder =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(combo), "is-folder"));
  ...
  for (l = self->target_files; l != NULL; l = l->next) {
    ...
    if (nautilus_file_is_directory(file) &&
      ...
      nautilus_file_set_permissions_recursive(
          uri, file_permission, file_permission_mask, dir_permission,
          dir_permission_mask, set_recursive_permissions_done, self);
    }
  }
  g_clear_pointer(&self->change_permission_combos, g_list_free);
  gtk_widget_destroy(GTK_WIDGET(dialog));
...
static void set_active_from_umask(GtkComboBox *combo, PermissionType type,
                                  gboolean is_folder) {
  mode_t initial;
  mode_t mask;
  mode_t p;
...
static void on_change_permissions_clicked(GtkWidget *button,
                                          NautilusPropertiesWindow *self) {
  GtkWidget *dialog;
  GtkComboBox *combo;
  ...
  dialog = GTK_WIDGET(gtk_builder_get_object(change_permissions_builder,
                                             "change_permissions_dialog"));
  gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(self));

  /* Owner Permissions */
  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "file_owner_combo"));
  setup_permissions_combo_box(combo, PERMISSION_USER, FALSE);
  self->change_permission_combos =
      g_list_prepend(self->change_permission_combos, combo);
  set_active_from_umask(combo, PERMISSION_USER, FALSE);

  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "folder_owner_combo"));
  ...
  /* Group Permissions */
  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "file_group_combo"));
  ...
  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "folder_group_combo"));
  ...
  /* Others Permissions */
  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "file_other_combo"));
  ...
  combo = GTK_COMBO_BOX(
      gtk_builder_get_object(change_permissions_builder, "folder_other_combo"));
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(on_change_permissions_response), self);
  gtk_widget_show_all(dialog);
}

static void setup_permissions_page(NautilusPropertiesWindow *self) {
  GList *file_list;
  ...
  if (all_can_get_permissions(file_list) &&
      all_can_get_permissions(self->target_files)) {
    ...
    /* Stash a copy of the file attribute name in this field for the callback's
     * sake. */
    g_object_set_data_full(G_OBJECT(self->security_context_value_label),
                           "file_attribute", g_strdup("selinux_context"),
                           g_free);

    self->value_fields =
        g_list_prepend(self->value_fields, self->security_context_value_label);
#endif
    ...
    if (self->has_recursive_apply) {
      ...
      g_signal_connect(self->change_permissions_button, "clicked",
                       G_CALLBACK(on_change_permissions_clicked), self);
    }
  } else {
    /*
    ...
    if (!is_multi_file_window(self)) {
      ...
      prompt_text = g_strdup_printf(
          _("The permissions of “%s” could not be determined."), file_name);
      gtk_label_set_text(GTK_LABEL(self->permission_indeterminable_label),
                         prompt_text);
    }
...
static void append_extension_pages(NautilusPropertiesWindow *self) {
  ...
  for (p = providers; p != NULL; p = p->next) {
    ...
    pages = nautilus_property_page_provider_get_pages(provider,
                                                      self->original_files);
    ...
    for (l = pages; l != NULL; l = l->next) {
      ...
      g_object_get(G_OBJECT(page), "page", &page_widget, "label", &label, NULL);

      gtk_notebook_append_page(self->notebook, page_widget, label);
      gtk_container_child_set(GTK_CONTAINER(self->notebook), page_widget,
                              "tab-expand", TRUE, NULL);

      g_object_set_data(G_OBJECT(page_widget), "is-extension-page",
                        GINT_TO_POINTER(TRUE));
    }
  }
}

static gboolean should_show_permissions(NautilusPropertiesWindow *self) {
  GList *l;
...
static char *get_pending_key(GList *file_list) {
  GList *uris = NULL;
  GList *l;
  GString *key;
  char *ret;
...
static StartupData *
startup_data_new(GList *original_files, GList *target_files,
                 const char *pending_key, GtkWidget *parent_widget,
                 GtkWindow *parent_window, const char *startup_id,
                 NautilusPropertiesWindowCallback callback,
                 gpointer callback_data, NautilusPropertiesWindow *window) {
  StartupData *data;
  GList *l;
  ...
  data->pending_files = g_hash_table_new(g_direct_hash, g_direct_equal);
...
static void startup_data_free(StartupData *data) {
...
static gboolean should_show_open_with(NautilusPropertiesWindow *self) {
  NautilusFile *file;
  ...
  if (is_multi_file_window(self)) {
    ...
    if (!file_list_attributes_identical(self->target_files, "mime_type")) {
      return FALSE;
    }
...
static void add_clicked_cb(GtkButton *button, gpointer user_data) {
  NautilusPropertiesWindow *self = NAUTILUS_PROPERTIES_WINDOW(user_data);
  g_autoptr(GAppInfo) info = NULL;
  g_autoptr(GError) error = NULL;

  info =
      gtk_app_chooser_get_app_info(GTK_APP_CHOOSER(self->app_chooser_widget));
  ...
  if (error != NULL) {
    ...
    message =
        g_strdup_printf(_("Error while adding “%s”: %s"),
                        g_app_info_get_display_name(info), error->message);
    show_dialog(_("Could not add application"), message,
...
static void remove_clicked_cb(GtkMenuItem *item, gpointer user_data) {
  NautilusPropertiesWindow *self = NAUTILUS_PROPERTIES_WINDOW(user_data);
  g_autoptr(GAppInfo) info = NULL;
  ...
  if (info) {
    ...
    if (!g_app_info_remove_supports_type(info, self->content_type, &error)) {
      show_dialog(_("Could not forget association"), error->message,
                  GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self))),
                  GTK_MESSAGE_ERROR);
    }
...
static void populate_popup_cb(GtkAppChooserWidget *widget, GtkMenu *menu,
                              GAppInfo *app, gpointer user_data) {
  GtkWidget *item;
  NautilusPropertiesWindow *self = NAUTILUS_PROPERTIES_WINDOW(user_data);
  ...
  if (g_app_info_can_remove_supports_type(app)) {
    ...
    g_signal_connect(item, "activate", G_CALLBACK(remove_clicked_cb), self);
  }
}

static void reset_clicked_cb(GtkButton *button, gpointer user_data) {
  NautilusPropertiesWindow *self;
...
static void set_as_default_clicked_cb(GtkButton *button, gpointer user_data) {
  NautilusPropertiesWindow *self = NAUTILUS_PROPERTIES_WINDOW(user_data);
  g_autoptr(GAppInfo) info = NULL;
  g_autoptr(GError) error = NULL;
  ...
  g_app_info_set_as_default_for_type(info, self->content_type, &error);

  if (error != NULL) {
    g_autofree gchar *message = NULL;

    message = g_strdup_printf(
        _("Error while setting “%s” as default application: %s"),
        g_app_info_get_display_name(info), error->message);
    show_dialog(_("Could not set as default"), message,
                GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self))),
                GTK_MESSAGE_ERROR);
  }
...
static gint app_compare(gconstpointer a, gconstpointer b) {
  return !g_app_info_equal(G_APP_INFO(a), G_APP_INFO(b));
}

static gboolean app_info_can_add(GAppInfo *info, const gchar *content_type) {
...
static void application_selected_cb(GtkAppChooserWidget *widget, GAppInfo *info,
                                    gpointer user_data) {
  NautilusPropertiesWindow *self = NAUTILUS_PROPERTIES_WINDOW(user_data);
...
static void application_chooser_apply_labels(NautilusPropertiesWindow *self) {
  ...
  if (num_files > 1) {
    ...
    label = g_strdup_printf(_("Open all files of type “%s” with"), description);
  } else {
    ...
    label = g_strdup_printf(
        _("Select an application to open “%s” and other files of type “%s”"),
...
static void setup_app_chooser_area(NautilusPropertiesWindow *self) {
  g_autoptr(GAppInfo) info = NULL;

  self->app_chooser_widget = gtk_app_chooser_widget_new(self->content_type);
  gtk_widget_set_vexpand(self->app_chooser_widget, TRUE);
  gtk_box_pack_start(GTK_BOX(self->app_chooser_widget_box),
                     self->app_chooser_widget, FALSE, TRUE, 0);

  gtk_app_chooser_widget_set_show_default(
      GTK_APP_CHOOSER_WIDGET(self->app_chooser_widget), TRUE);
  gtk_app_chooser_widget_set_show_fallback(
      GTK_APP_CHOOSER_WIDGET(self->app_chooser_widget), TRUE);
  gtk_app_chooser_widget_set_show_other(
      GTK_APP_CHOOSER_WIDGET(self->app_chooser_widget), TRUE);
  gtk_widget_show(self->app_chooser_widget);
  g_signal_connect(self->reset_button, "clicked", G_CALLBACK(reset_clicked_cb),
                   self);
  g_signal_connect(self->add_button, "clicked", G_CALLBACK(add_clicked_cb),
                   self);
  g_signal_connect(self->set_as_default_button, "clicked",
                   G_CALLBACK(set_as_default_clicked_cb), self);
  ...
  g_signal_connect(self->app_chooser_widget, "application-selected",
                   G_CALLBACK(application_selected_cb), self);
  g_signal_connect(self->app_chooser_widget, "populate-popup",
                   G_CALLBACK(populate_popup_cb), self);

  application_chooser_apply_labels(self);
}

static void setup_open_with_page(NautilusPropertiesWindow *self) {
  GList *files = NULL;
  NautilusFile *target_file;
...
create_properties_window(StartupData *startup_data) {
  ...
  window = NAUTILUS_PROPERTIES_WINDOW(
      g_object_new(NAUTILUS_TYPE_PROPERTIES_WINDOW, NULL));

  window->original_files =
      nautilus_file_list_copy(startup_data->original_files);
  ...
  for (l = window->original_files; l != NULL; l = l->next) {
    ...
    attributes =
        NAUTILUS_FILE_ATTRIBUTES_FOR_ICON | NAUTILUS_FILE_ATTRIBUTE_INFO;

    nautilus_file_monitor_add(file, &window->original_files, attributes);
  }
  ...
  for (l = window->target_files; l != NULL; l = l->next) {
    g_signal_connect_object(NAUTILUS_FILE(l->data), "changed",
                            G_CALLBACK(file_changed_callback), G_OBJECT(window),
                            0);
  }
...
static GList *get_target_file_list(GList *original_files) {
  return g_list_copy_deep(original_files,
                          (GCopyFunc)get_target_file_for_original_file, NULL);
}

static void add_window(NautilusPropertiesWindow *self) {
  if (!is_multi_file_window(self)) {
    g_hash_table_insert(windows, get_original_file(self), self);
    g_object_set_data(G_OBJECT(self), "window_key", get_original_file(self));
  }
}

static void remove_window(NautilusPropertiesWindow *self) {
  gpointer key;
...
static NautilusPropertiesWindow *get_existing_window(GList *file_list) {
  if (!file_list->next) {
...
static void properties_window_finish(StartupData *data) {
  gboolean cancel_timed_wait;

  if (data->parent_widget != NULL) {
    g_signal_handlers_disconnect_by_data(data->parent_widget, data);
  }
  if (data->window != NULL) {
    g_signal_handlers_disconnect_by_data(data->window, data);
  }
...
static void cancel_create_properties_window_callback(gpointer callback_data) {
  StartupData *data;
...
static void cancel_call_when_ready_callback(gpointer key, gpointer value,
                                            gpointer user_data) {
  nautilus_file_cancel_call_when_ready(NAUTILUS_FILE(key),
                                       is_directory_ready_callback, user_data);
}

static void remove_pending(StartupData *startup_data,
                           gboolean cancel_call_when_ready,
                           gboolean cancel_timed_wait) {
  if (cancel_call_when_ready) {
    g_hash_table_foreach(startup_data->pending_files,
                         cancel_call_when_ready_callback, startup_data);
  }
  if (cancel_timed_wait) {
    eel_timed_wait_stop(cancel_create_properties_window_callback, startup_data);
  }
...
static gboolean widget_on_destroy(GtkWidget *widget, gpointer user_data) {
...
void nautilus_properties_window_present(
    GList *original_files, GtkWidget *parent_widget, const gchar *startup_id,
    NautilusPropertiesWindowCallback callback, gpointer callback_data) {
  GList *l, *next;
  GtkWindow *parent_window;
  StartupData *startup_data;
  ...
  if (parent_widget) {
    parent_window =
        GTK_WINDOW(gtk_widget_get_ancestor(parent_widget, GTK_TYPE_WINDOW));
  } else {
    parent_window = NULL;
  }

  startup_data = startup_data_new(original_files, target_files, pending_key,
                                  parent_widget, parent_window, startup_id,
  ...
  g_hash_table_insert(pending_lists, startup_data->pending_key,
  ...
  eel_timed_wait_start(cancel_create_properties_window_callback, startup_data,
  ...
  for (l = startup_data->target_files; l != NULL; l = next) {
    ...
    nautilus_file_call_when_ready(NAUTILUS_FILE(l->data),
                                  ...
                                  is_directory_ready_callback, startup_data);
  }
}
...
static void real_dispose(GObject *object) {
  ...
  g_list_foreach(self->original_files, (GFunc)nautilus_file_monitor_remove,
                 &self->original_files);
  g_clear_list(&self->original_files, (GDestroyNotify)nautilus_file_unref);

  g_list_foreach(self->target_files, (GFunc)nautilus_file_monitor_remove,
                 &self->target_files);
  ...
  g_clear_handle_id(&self->update_directory_contents_timeout_id,
                    g_source_remove);
...
/* icon selection callback to set the image of the file object to the selected
 * file */
static void set_icon(const char *icon_uri, NautilusPropertiesWindow *self) {
  NautilusFile *file;
  ...
  if (icon_path != NULL) {
    ...
    for (l = self->original_files; l != NULL; l = l->next) {
      ...
      real_icon_uri = g_file_get_relative_path(icon_location, file_location);
...
static void
custom_icon_file_chooser_response_cb(GtkDialog *dialog, gint response,
                                     NautilusPropertiesWindow *self) {
  switch (response) {
...
static void select_image_button_callback(GtkWidget *widget,
  ...
  if (dialog == NULL) {
    dialog = gtk_file_chooser_dialog_new(
        _("Select Custom Icon"), GTK_WINDOW(self), GTK_FILE_CHOOSER_ACTION_OPEN,
        _("_Revert"), GTK_RESPONSE_NO, _("_Cancel"), GTK_RESPONSE_CANCEL,
        _("_Open"), GTK_RESPONSE_OK, NULL);
    gtk_file_chooser_add_shortcut_folder(
        GTK_FILE_CHOOSER(dialog),
        g_get_user_special_dir(G_USER_DIRECTORY_PICTURES), NULL);
  ...
  /* it's likely that the user wants to pick an icon that is inside a local
   * directory */
  ...
  for (l = self->original_files; l != NULL; l = l->next) {
    ...
    file = NAUTILUS_FILE(l->data);
    image_path = nautilus_file_get_metadata(
        file, NAUTILUS_METADATA_KEY_CUSTOM_ICON, NULL);
  ...
  gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog), GTK_RESPONSE_NO,
                                    revert_is_sensitive);
...
nautilus_properties_window_class_init(NautilusPropertiesWindowClass *klass) {
  ...
  g_signal_new("close", G_OBJECT_CLASS_TYPE(klass),
               G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL,
               g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Escape, 0, "close", 0);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/ui/nautilus-properties-window.ui");
  ...
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       type_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       link_target_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       link_target_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       contents_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       contents_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       contents_spinner);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       size_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       size_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       parent_folder_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       parent_folder_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
  ...
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       volume_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       trashed_on_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       trashed_on_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       accessed_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       accessed_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       spacer_2);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       modified_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
  ...
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       spacer_3);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       free_space_title_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       free_space_value_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       volume_widget_box);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       open_in_disks_button);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       pie_chart);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       used_color);
  ...
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_folder_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_file_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_folder_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       owner_file_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_value_stack);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_folder_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_file_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_folder_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       group_file_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_folder_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_file_access_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_folder_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       others_file_access_combo);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       execute_label);
  gtk_widget_class_bind_template_child(widget_class, NautilusPropertiesWindow,
                                       execute_checkbox);
...
static void nautilus_properties_window_init(NautilusPropertiesWindow *self) {
