...
G_DEFINE_TYPE_WITH_PRIVATE(NautilusApplication, nautilus_application,
...
void nautilus_application_set_accelerator(GApplication *app,
    ...
    gtk_application_set_accels_for_action(GTK_APPLICATION(app), action_name,
                                          vaccels);
}
...
GList *nautilus_application_get_windows(NautilusApplication *self) {
...
static gboolean check_required_directories(NautilusApplication *self) {
    char *user_directory;
    GSList *directories;
    gboolean ret;
    ...
    if (directories != NULL) {
        ...
        for (l = directories->next; l != NULL; l = l->next) {
            g_string_append_printf(directories_as_string, ", %s",
                                   (const char *)l->data);
        }
        ...
        if (failed_count == 1) {
            detail_string =
                g_strdup_printf(_("Unable to create a required folder. "
                                  "Please create the following folder, or "
                                  "set permissions such that it can be created:\n%s"),
                                directories_as_string->str);
        } else {
            detail_string = g_strdup_printf(
                                _("Unable to create required folders. "
...
static void menu_provider_items_updated_handler(NautilusMenuProvider *provider,
        GtkWidget *parent_window,
        gpointer data) {
    g_signal_emit_by_name(nautilus_signaller_get_current(), "popup-menu-changed");
}

static void menu_provider_init_callback(void) {
    GList *providers;
    GList *l;
...
NautilusWindow *nautilus_application_create_window(NautilusApplication *self,
        GdkScreen *screen) {
    ...
    maximized = g_settings_get_boolean(nautilus_window_state,
...
get_window_slot_for_location(NautilusApplication *self, GFile *location) {
    NautilusApplicationPrivate *priv;
...
void nautilus_application_open_location_full(NautilusApplication *self,
        GFile *location,
        NautilusOpenFlags flags,
        GList *selection,
        NautilusWindow *target_window,
        NautilusWindowSlot *target_slot) {
    NAUTILUS_APPLICATION_CLASS(G_OBJECT_GET_CLASS(self))
    ->open_location_full(self, location, flags, selection, target_window,
                         target_slot);
}

static void real_open_location_full(NautilusApplication *self, GFile *location,
     ...
     * gtk_application_get_active_window, see
     * https://bugzilla.gnome.org/show_bug.cgi?id=756499 so what we do is never
     * rely on this on the callers, but would be cool to make it work withouth
     * explicitly setting the active window on the callers. */
    active_window =
        NAUTILUS_WINDOW(gtk_application_get_active_window(GTK_APPLICATION(self)));
    ...
    nautilus_profile_start("Application opening location, old: %s, new: %s",
                           old_uri, new_uri);
...
static NautilusWindow *open_window(NautilusApplication *self, GFile *location) {
    NautilusWindow *window;
    ...
    if (location != NULL) {
        nautilus_application_open_location_full(self, location, 0, NULL, window,
                                                NULL);
    } else {
        GFile *home;
...
void nautilus_application_open_location(NautilusApplication *self,
    ...
    nautilus_application_open_location_full(self, location, 0, sel_list, window,
                                            slot);
 ...
 */
static void nautilus_application_open(GApplication *app, GFile **files,
    ...
    for (idx = 0; idx < n_files; idx++) {
        ...
        if (!slot) {
            ...
            nautilus_application_open_location_full(NAUTILUS_APPLICATION(app), file,
                                                    0, NULL, NULL, slot);
        }
    }
}
...
static gboolean do_cmdline_sanity_checks(NautilusApplication *self,
...
static int do_perform_self_checks(void) {
...
static void nautilus_application_select(NautilusApplication *self,
                                        GFile **files, gint len) {
    int i;
    GFile *file;
    GFile *parent;
...
static void action_new_window(GSimpleAction *action, GVariant *parameter,
                              gpointer user_data) {
    ...
    nautilus_application_open_location_full(
        application, home, NAUTILUS_OPEN_FLAG_NEW_WINDOW, NULL, NULL, NULL);
}

static void action_clone_window(GSimpleAction *action, GVariant *parameter,
                                gpointer user_data) {
    NautilusWindowSlot *active_slot = NULL;
    ...
    if (current_view != NULL && nautilus_view_is_searching(current_view)) {
    ...
    nautilus_application_open_location_full(
        NAUTILUS_APPLICATION(application), location,
...
static void action_preferences(GSimpleAction *action, GVariant *parameter,
                               gpointer user_data) {
    GtkApplication *application = user_data;
    nautilus_preferences_window_show(
...
static void action_about(GSimpleAction *action, GVariant *parameter,
    ...
    nautilus_window_show_about_dialog(
        NAUTILUS_WINDOW(gtk_application_get_active_window(application)));
}
...
static void action_help(GSimpleAction *action, GVariant *parameter,
    ...
    if (error) {
        dialog = gtk_message_dialog_new(
                     window ? GTK_WINDOW(window) : NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
...
static void action_kill(GSimpleAction *action, GVariant *parameter,
                        gpointer user_data) {
    GtkApplication *application = user_data;
...
static void action_show_hide_sidebar(GObject *object, GParamSpec *pspec,
                                     gpointer *user_data) {
...
static void action_show_help_overlay(GSimpleAction *action, GVariant *state,
                                     gpointer user_data) {
    GtkApplication *application = user_data;
    GtkWindow *window = gtk_application_get_active_window(application);

    g_action_group_activate_action(G_ACTION_GROUP(window), "show-help-overlay",
                                   NULL);
}

static gboolean variant_get_mapping(GValue *value, GVariant *variant,
                                    gpointer user_data) {
    g_value_set_variant(value, variant);
    return TRUE;
}

static GVariant *variant_set_mapping(const GValue *value,
...
static void nautilus_init_application_actions(NautilusApplication *app) {
    ...
    sidebar_action =
        g_action_map_lookup_action(G_ACTION_MAP(app), "show-hide-sidebar");
    g_signal_connect(sidebar_action, "notify::state",
                     G_CALLBACK(action_show_hide_sidebar), app);
    g_settings_bind_with_mapping(
        nautilus_window_state, NAUTILUS_WINDOW_STATE_START_WITH_SIDEBAR,
        sidebar_action, "state", G_SETTINGS_BIND_DEFAULT, variant_get_mapping,
        variant_set_mapping, NULL, NULL);

    nautilus_application_set_accelerator(G_APPLICATION(app), "app.clone-window",
                                         "<Primary>n");
    nautilus_application_set_accelerator(G_APPLICATION(app), "app.help", "F1");
    nautilus_application_set_accelerator(G_APPLICATION(app), "app.quit",
                                         "<Primary>q");
    nautilus_application_set_accelerator(G_APPLICATION(app),
                                         "app.show-hide-sidebar", "F9");
}

static void nautilus_application_activate(GApplication *app) {
    GFile **files;
...
static gint nautilus_application_handle_file_args(NautilusApplication *self,
        GVariantDict *options) {
    GFile **files;
    GFile *file;
    gint idx, len;
    ...
    if (g_variant_dict_contains(options, "select")) {
        ...
        /* Create new windows */
        nautilus_application_open(
            G_APPLICATION(self), files, len,
...
nautilus_application_command_line(GApplication *application,
    ...
    if (g_variant_dict_contains(options, "quit")) {
        ...
        g_action_group_activate_action(G_ACTION_GROUP(application), "kill", NULL);
        goto out;
    }
...
static void nautilus_application_init(NautilusApplication *self) {
    static const GOptionEntry options[] = {
        ...
        {"quit", 'q', 0, G_OPTION_ARG_NONE, NULL, N_("Quit Nautilus."), NULL},
         ...
         */
        {   "browser", '\0', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, NULL, NULL,
            NULL
        },
        {   "geometry", 'g', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_STRING, NULL, NULL,
            NULL
        },
        {   "no-default-window", 'n', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, NULL,
            NULL, NULL
        },
        {   "no-desktop", '\0', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE, NULL, NULL,
            NULL
        },
...
static void theme_changed(GtkSettings *settings) {
    ...
    if (g_str_equal(theme, "Adwaita") || g_str_equal(theme, "Adwaita-dark")) {
        if (provider == NULL) {
            ...
            file =
                g_file_new_for_uri("resource:///org/gnome/nautilus/css/Adwaita.css");
    ...
    if (permanent_provider == NULL) {
        ...
        file =
            g_file_new_for_uri("resource:///org/gnome/nautilus/css/nautilus.css");
         ...
         */
        gtk_style_context_add_provider_for_screen(
            screen, GTK_STYLE_PROVIDER(permanent_provider),
...
static void setup_theme_extensions(void) {
...
void nautilus_application_send_notification(NautilusApplication *self,
    ...
    g_application_send_notification(G_APPLICATION(self), notification_id,
                                    notification);
}
...
static void on_application_shutdown(GApplication *application,
                                    gpointer user_data) {
    NautilusApplication *self = NAUTILUS_APPLICATION(application);
    NautilusApplicationPrivate *priv;
...
static void icon_theme_changed_callback(GtkIconTheme *icon_theme,
                                        gpointer user_data) {
...
void nautilus_application_startup_common(NautilusApplication *self) {
    NautilusApplicationPrivate *priv;
     ...
     * is called for us.
     */
    G_APPLICATION_CLASS(nautilus_application_parent_class)
    ...
    priv->progress_handler =
        nautilus_progress_persistence_handler_new(G_OBJECT(self));
    ...
    g_signal_connect_object(gtk_icon_theme_get_default(), "changed",
                            G_CALLBACK(icon_theme_changed_callback), NULL, 0);
}
...
static gboolean nautilus_application_dbus_register(GApplication *app,
    ...
    if (!nautilus_shell_search_provider_register(priv->search_provider,
            connection, error)) {
        return FALSE;
    }

    priv->previewer_selection_id =
        nautilus_previewer_connect_selection_event(connection);

    return TRUE;
}

static void nautilus_application_dbus_unregister(GApplication *app,
        GDBusConnection *connection,
        const gchar *object_path) {
    NautilusApplication *self = NAUTILUS_APPLICATION(app);
    NautilusApplicationPrivate *priv;
...
static void update_dbus_opened_locations(NautilusApplication *self) {
    NautilusApplicationPrivate *priv;
    gint i;
    GList *l, *sl;
    ...
    g_variant_builder_init(&windows_to_locations_builder,
    ...
    for (l = priv->windows; l != NULL; l = l->next) {
        ...
        for (sl = nautilus_window_get_slots(window); sl; sl = sl->next) {
            ...
            if (location != NULL) {
                ...
                GList *found =
                    g_list_find_custom(locations, uri, (GCompareFunc)g_strcmp0);
        ...
        g_variant_builder_add(&windows_to_locations_builder, "{sas}", path,
                              &locations_in_window_builder);
    ...
    windows_to_locations =
        g_variant_ref_sink(g_variant_builder_end(&windows_to_locations_builder));
    nautilus_freedesktop_dbus_set_open_windows_with_locations(
        priv->fdb_manager, windows_to_locations);
...
static void on_slot_added(NautilusWindow *window, NautilusWindowSlot *slot,
                          NautilusApplication *self) {
...
static void on_slot_removed(NautilusWindow *window, NautilusWindowSlot *slot,
                            NautilusApplication *self) {
    update_dbus_opened_locations(self);
...
static void nautilus_application_window_added(GtkApplication *app,
        GtkWindow *window) {
    NautilusApplication *self = NAUTILUS_APPLICATION(app);
    NautilusApplicationPrivate *priv;

    priv = nautilus_application_get_instance_private(self);
    GTK_APPLICATION_CLASS(nautilus_application_parent_class)
    ->window_added(app, window);
...
static void nautilus_application_window_removed(GtkApplication *app,
        GtkWindow *window) {
    NautilusApplication *self = NAUTILUS_APPLICATION(app);
    NautilusApplicationPrivate *priv;
...
static gint nautilus_application_handle_local_options(GApplication *app,
        GVariantDict *options) {
    gchar *cwd;
...
static void nautilus_application_class_init(NautilusApplicationClass *class) {
    ...
    application_class->handle_local_options =
        nautilus_application_handle_local_options;
...
NautilusApplication *nautilus_application_new(void) {
    return g_object_new(
               NAUTILUS_TYPE_APPLICATION, "application-id", APPLICATION_ID, "flags",
               G_APPLICATION_HANDLES_COMMAND_LINE | G_APPLICATION_HANDLES_OPEN,
               "inactivity-timeout", 12000, NULL);
}

void nautilus_application_search(NautilusApplication *self,
                                 NautilusQuery *query) {
    g_autoptr(GFile) location = NULL;
    NautilusWindow *window;
