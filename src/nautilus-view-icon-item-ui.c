...
G_DEFINE_TYPE(NautilusViewIconItemUi, nautilus_view_icon_item_ui,
              GTK_TYPE_FLOW_BOX_CHILD)

enum { PROP_0, PROP_MODEL, N_PROPS };

#define EXTRA_WIDTH_FOR_TEXT 36

static void update_icon(NautilusViewIconItemUi *self) {
  ...
  icon_pixbuf = nautilus_file_get_icon_pixbuf(file, icon_size, TRUE, 1, flags);
  ...
  if (thumbnail_path != NULL && nautilus_file_should_show_thumbnail(file)) {
...
static void update_captions(NautilusViewIconItemUi *self) {
  NautilusFile *file;
  GtkWidget *const caption_labels[] = {
      self->first_caption, self->second_caption, self->third_caption};
  G_STATIC_ASSERT(G_N_ELEMENTS(caption_labels) ==
...
static void on_file_changed(NautilusViewIconItemUi *self) {
  NautilusFile *file;
...
static void on_view_item_size_changed(GObject *object, GParamSpec *pspec,
                                      gpointer user_data) {
...
static void set_model(NautilusViewIconItemUi *self,
                      NautilusViewItemModel *model);
...
nautilus_view_icon_item_ui_class_init(NautilusViewIconItemUiClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("model", "Item model",
                          "The item model that this UI reprensents",
                          NAUTILUS_TYPE_VIEW_ITEM_MODEL, G_PARAM_READWRITE));

  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/nautilus/ui/nautilus-view-icon-item-ui.ui");

  gtk_widget_class_bind_template_child(widget_class, NautilusViewIconItemUi,
                                       fixed_height_box);
  gtk_widget_class_bind_template_child(widget_class, NautilusViewIconItemUi,
                                       icon);
  gtk_widget_class_bind_template_child(widget_class, NautilusViewIconItemUi,
...
static void nautilus_view_icon_item_ui_init(NautilusViewIconItemUi *self) {
