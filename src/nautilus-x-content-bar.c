...
enum { CONTENT_BAR_RESPONSE_APP = 1 };

G_DEFINE_TYPE(NautilusXContentBar, nautilus_x_content_bar, GTK_TYPE_INFO_BAR)

static void content_bar_response_cb(GtkInfoBar *infobar, gint response_id,
                                    gpointer user_data) {
    ...
    if (bar->x_content_types == NULL || bar->mount == NULL) {
        return;
    }

    /* FIXME */
    default_app =
        g_app_info_get_default_for_type(bar->x_content_types[response_id], FALSE);
    if (default_app != NULL) {
        nautilus_launch_application_for_mount(
            default_app, bar->mount,
...
nautilus_x_content_bar_set_x_content_types(NautilusXContentBar *bar,
    ...
    if (!should_handle_content_types(x_content_types)) {
        g_warning("Content types in content types bar cannot be handled. Check "
                  "before creating the content bar if they can be handled.");
        return;
    }
    ...
    case 2: {
        message = get_message_for_two_content_types(
                      (const char *const *)bar->x_content_types);
    }
    break;
...
static void nautilus_x_content_bar_set_mount(NautilusXContentBar *bar,
        GMount *mount) {
    if (bar->mount != NULL) {
...
static void nautilus_x_content_bar_set_property(GObject *object, guint prop_id,
        const GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_x_content_bar_get_property(GObject *object, guint prop_id,
        GValue *value,
        GParamSpec *pspec) {
    NautilusXContentBar *bar = NAUTILUS_X_CONTENT_BAR(object);
...
static void nautilus_x_content_bar_finalize(GObject *object) {
    NautilusXContentBar *bar = NAUTILUS_X_CONTENT_BAR(object);
...
static void nautilus_x_content_bar_class_init(NautilusXContentBarClass *klass) {
    ...
    g_object_class_install_property(
        object_class, PROP_MOUNT,
        g_param_spec_object("mount", "The GMount to run programs for",
                            "The GMount to run programs for", G_TYPE_MOUNT,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

    g_object_class_install_property(
        object_class, PROP_X_CONTENT_TYPES,
        g_param_spec_boxed("x-content-types",
                           "The x-content types for the cluebar",
                           "The x-content types for the cluebar", G_TYPE_STRV,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void nautilus_x_content_bar_init(NautilusXContentBar *bar) {
    ...
    g_signal_connect(bar, "response", G_CALLBACK(content_bar_response_cb), bar);
}

GtkWidget *nautilus_x_content_bar_new(GMount *mount,
                                      const char *const *x_content_types) {
    return g_object_new(NAUTILUS_TYPE_X_CONTENT_BAR, "message-type",
                        GTK_MESSAGE_QUESTION, "mount", mount, "x-content-types",
