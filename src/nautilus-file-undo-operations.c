...
G_DEFINE_TYPE_WITH_PRIVATE(NautilusFileUndoInfo, nautilus_file_undo_info,
                           G_TYPE_OBJECT)

enum { PROP_OP_TYPE = 1, PROP_ITEM_COUNT, N_PROPERTIES };
...
/* description helpers */
static void nautilus_file_undo_info_init(NautilusFileUndoInfo *self) {
...
static void nautilus_file_undo_info_get_property(GObject *object,
        guint property_id,
        GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_file_undo_info_set_property(GObject *object,
        guint property_id,
        const GValue *value,
        GParamSpec *pspec) {
    NautilusFileUndoInfo *self;
...
nautilus_file_redo_info_warn_redo(NautilusFileUndoInfo *self,
    ...
    g_critical("Object %p of type %s does not implement redo_func!!", self,
...
nautilus_file_undo_info_warn_undo(NautilusFileUndoInfo *self,
                                  GtkWindow *parent_window,
                                  NautilusFileOperationsDBusData *dbus_data) {
    g_critical("Object %p of type %s does not implement undo_func!!", self,
               G_OBJECT_TYPE_NAME(self));
}

static void nautilus_file_undo_info_strings_func(NautilusFileUndoInfo *self,
        gchar **undo_label,
        gchar **undo_description,
        gchar **redo_label,
        gchar **redo_description) {
...
static void nautilus_file_undo_info_finalize(GObject *object) {
    NautilusFileUndoInfo *self;
    NautilusFileUndoInfoPrivate *priv;
...
nautilus_file_undo_info_class_init(NautilusFileUndoInfoClass *klass) {
    ...
    properties[PROP_OP_TYPE] =
        g_param_spec_int("op-type", "Undo info op type", "Type of undo operation",
                         0, NAUTILUS_FILE_UNDO_OP_NUM_TYPES - 1, 0,
                         G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_ITEM_COUNT] =
        g_param_spec_int("item-count", "Number of items", "Number of items", 0,
                         G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
...
nautilus_file_undo_info_get_op_type(NautilusFileUndoInfo *self) {
    ...
    g_return_val_if_fail(NAUTILUS_IS_FILE_UNDO_INFO(self),
...
static gint nautilus_file_undo_info_get_item_count(NautilusFileUndoInfo *self) {
    NautilusFileUndoInfoPrivate *priv;
...
void nautilus_file_undo_info_apply_async(
    NautilusFileUndoInfo *self, gboolean undo, GtkWindow *parent_window,
    ...
    priv->apply_async_task =
        g_task_new(G_OBJECT(self), NULL, callback, user_data);

    if (undo) {
        NAUTILUS_FILE_UNDO_INFO_CLASS(G_OBJECT_GET_CLASS(self))
        ->undo_func(self, parent_window, dbus_data);
    } else {
        NAUTILUS_FILE_UNDO_INFO_CLASS(G_OBJECT_GET_CLASS(self))
        ->redo_func(self, parent_window, dbus_data);
    }
}
...
static void file_undo_info_op_res_free(gpointer data) {
...
void nautilus_file_undo_info_get_strings(NautilusFileUndoInfo *self,
        ...
        gchar **redo_description) {
    NAUTILUS_FILE_UNDO_INFO_CLASS(G_OBJECT_GET_CLASS(self))
    ->strings_func(self, undo_label, undo_description, redo_label,
                   redo_description);
}

static void file_undo_info_complete_apply(NautilusFileUndoInfo *self,
        gboolean success,
        gboolean user_cancel) {
    NautilusFileUndoInfoPrivate *priv;
    FileUndoInfoOpRes *op_res;
...
static void file_undo_info_transfer_callback(GHashTable *debuting_uris,
        gboolean success,
        gpointer user_data) {
...
static void file_undo_info_operation_callback(NautilusFile *file,
        GFile *result_location,
        GError *error,
        gpointer user_data) {
    NautilusFileUndoInfo *self = user_data;

    file_undo_info_complete_apply(
        self, (error == NULL),
        g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED));
}

static void file_undo_info_delete_callback(GHashTable *debuting_uris,
        gboolean user_cancel,
        gpointer user_data) {
    NautilusFileUndoInfo *self = user_data;
...
G_DEFINE_TYPE(NautilusFileUndoInfoExt, nautilus_file_undo_info_ext,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static char *ext_get_first_target_short_name(NautilusFileUndoInfoExt *self) {
    GList *targets_first;
    char *file_name = NULL;

    targets_first = g_queue_peek_head_link(self->destinations);

    if (targets_first != NULL && targets_first->data != NULL) {
...
static void ext_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
    ...
    if (op_type == NAUTILUS_FILE_UNDO_OP_MOVE) {
        if (count > 1) {
            *undo_description =
                g_strdup_printf(ngettext("Move %d item back to “%s”",
                                         "Move %d items back to “%s”", count),
                                count, source);
            *redo_description = g_strdup_printf(
                                    ngettext("Move %d item to “%s”", "Move %d items to “%s”", count),
                                    count, destination);

            *undo_label = g_strdup_printf(
                              ngettext("_Undo Move %d item", "_Undo Move %d items", count), count);
            *redo_label = g_strdup_printf(
                              ngettext("_Redo Move %d item", "_Redo Move %d items", count), count);
        } else {
            *undo_description =
        ...
        *redo_label = g_strdup(_("_Redo Restore from Trash"));
        ...
        if (count > 1) {
            *undo_description =
                g_strdup_printf(ngettext("Move %d item back to trash",
                                         "Move %d items back to trash", count),
                                count);
            *redo_description =
                g_strdup_printf(ngettext("Restore %d item from trash",
        ...
        if (count > 1) {
            *undo_description = g_strdup_printf(
                                    ngettext("Delete %d copied item", "Delete %d copied items", count),
                                    count);
            *redo_description = g_strdup_printf(
                                    ngettext("Copy %d item to “%s”", "Copy %d items to “%s”", count),
                                    count, destination);

            *undo_label = g_strdup_printf(
                              ngettext("_Undo Copy %d item", "_Undo Copy %d items", count), count);
            *redo_label = g_strdup_printf(
                              ngettext("_Redo Copy %d item", "_Redo Copy %d items", count), count);
        } else {
        ...
        if (count > 1) {
            *undo_description =
                g_strdup_printf(ngettext("Delete %d duplicated item",
                                         "Delete %d duplicated items", count),
                                count);
            *redo_description =
                g_strdup_printf(ngettext("Duplicate %d item in “%s”",
            ...
            *undo_description = g_strdup_printf(_("Delete “%s”"), name);
            *redo_description =
                g_strdup_printf(_("Duplicate “%s” in “%s”"), name, destination);
        ...
        if (count > 1) {
            *undo_description =
                g_strdup_printf(ngettext("Delete links to %d item",
                                         "Delete links to %d items", count),
                                count);
            *redo_description =
                g_strdup_printf(ngettext("Create links to %d item",
...
ext_create_link_redo_func(NautilusFileUndoInfoExt *self,
    ...
    nautilus_file_operations_link(g_queue_peek_head_link(self->sources),
                                  self->dest_dir, parent_window, dbus_data,
                                  file_undo_info_transfer_callback, self);
}

static void ext_duplicate_redo_func(NautilusFileUndoInfoExt *self,
                                    GtkWindow *parent_window,
                                    NautilusFileOperationsDBusData *dbus_data) {
...
static void ext_copy_redo_func(NautilusFileUndoInfoExt *self,
                               GtkWindow *parent_window,
                               NautilusFileOperationsDBusData *dbus_data) {
...
static void ext_redo_func(NautilusFileUndoInfo *info, GtkWindow *parent_window,
                          NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT(info);
    NautilusFileUndoOp op_type = nautilus_file_undo_info_get_op_type(info);
...
static void ext_restore_undo_func(NautilusFileUndoInfoExt *self,
                                  GtkWindow *parent_window,
                                  NautilusFileOperationsDBusData *dbus_data) {
    nautilus_file_operations_trash_or_delete_async(
        g_queue_peek_head_link(self->destinations), parent_window, dbus_data,
        file_undo_info_delete_callback, self);
}

static void ext_move_undo_func(NautilusFileUndoInfoExt *self,
                               GtkWindow *parent_window,
                               NautilusFileOperationsDBusData *dbus_data) {
    nautilus_file_operations_move_async(
        g_queue_peek_head_link(self->destinations), self->src_dir, parent_window,
        dbus_data, file_undo_info_transfer_callback, self);
}
...
ext_copy_duplicate_undo_func(NautilusFileUndoInfoExt *self,
    ...
    nautilus_file_operations_delete_async(files, parent_window, dbus_data,
                                          file_undo_info_delete_callback, self);

    g_list_free(files);
}

static void ext_undo_func(NautilusFileUndoInfo *info, GtkWindow *parent_window,
                          NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoExt *self = NAUTILUS_FILE_UNDO_INFO_EXT(info);
    NautilusFileUndoOp op_type = nautilus_file_undo_info_get_op_type(info);
...
static void nautilus_file_undo_info_ext_init(NautilusFileUndoInfoExt *self) {}

static void nautilus_file_undo_info_ext_finalize(GObject *obj) {
...
nautilus_file_undo_info_ext_new(NautilusFileUndoOp op_type, gint item_count,
                                GFile *src_dir, GFile *target_dir) {
    NautilusFileUndoInfoExt *self;

    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_EXT, "op-type", op_type,
                        "item-count", item_count, NULL);
...
void nautilus_file_undo_info_ext_add_origin_target_pair(
    NautilusFileUndoInfoExt *self, GFile *origin, GFile *target) {
...
G_DEFINE_TYPE(NautilusFileUndoInfoCreate, nautilus_file_undo_info_create,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static void create_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
                                gchar **undo_description, gchar **redo_label,
                                gchar **redo_description) {
...
static void create_callback(GFile *new_file, gboolean success,
...
create_from_template_redo_func(NautilusFileUndoInfoCreate *self,
    ...
    nautilus_file_operations_new_file_from_template(
        NULL, parent_uri, new_name, self->template, create_callback, self);
...
static void create_folder_redo_func(NautilusFileUndoInfoCreate *self,
                                    GtkWindow *parent_window,
                                    NautilusFileOperationsDBusData *dbus_data) {
    GFile *parent;
    gchar *parent_uri;
    ...
    parent = g_file_get_parent(self->target_file);
    parent_uri = g_file_get_uri(parent);
    nautilus_file_operations_new_folder(NULL, dbus_data, parent_uri, name,
...
static void create_empty_redo_func(NautilusFileUndoInfoCreate *self,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data) {
    GFile *parent;
    gchar *parent_uri;
    ...
    parent_uri = g_file_get_uri(parent);
    new_name = g_file_get_parse_name(self->target_file);
    nautilus_file_operations_new_file(NULL, parent_uri, new_name, self->template,
                                      self->length, create_callback, self);
...
static void create_redo_func(NautilusFileUndoInfo *info,
                             GtkWindow *parent_window,
                             NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE(info);
    NautilusFileUndoOp op_type = nautilus_file_undo_info_get_op_type(info);
...
static void create_undo_func(NautilusFileUndoInfo *info,
                             GtkWindow *parent_window,
                             NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoCreate *self = NAUTILUS_FILE_UNDO_INFO_CREATE(info);
    GList *files = NULL;
...
static void nautilus_file_undo_info_create_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_create_class_init(
    NautilusFileUndoInfoCreateClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
nautilus_file_undo_info_create_new(NautilusFileUndoOp op_type) {
    return g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_CREATE, "op-type", op_type,
                        "item-count", 1, NULL);
}
...
G_DEFINE_TYPE(NautilusFileUndoInfoRename, nautilus_file_undo_info_rename,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static void rename_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
                                gchar **undo_description, gchar **redo_label,
                                gchar **redo_description) {
    ...
    *undo_description =
        g_strdup_printf(_("Rename “%s” as “%s”"), new_name, old_name);
    *redo_description =
        g_strdup_printf(_("Rename “%s” as “%s”"), old_name, new_name);
...
static void rename_redo_func(NautilusFileUndoInfo *info,
                             GtkWindow *parent_window,
                             NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME(info);
    NautilusFile *file;
...
static void rename_undo_func(NautilusFileUndoInfo *info,
                             GtkWindow *parent_window,
                             NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoRename *self = NAUTILUS_FILE_UNDO_INFO_RENAME(info);
    NautilusFile *file;
...
static void nautilus_file_undo_info_rename_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_rename_class_init(
    NautilusFileUndoInfoRenameClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *nautilus_file_undo_info_rename_new(void) {
    return g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_RENAME, "op-type",
                        NAUTILUS_FILE_UNDO_OP_RENAME, "item-count", 1, NULL);
}

void nautilus_file_undo_info_rename_set_data_pre(
    NautilusFileUndoInfoRename *self, GFile *old_file, gchar *old_display_name,
    gchar *new_display_name) {
...
void nautilus_file_undo_info_rename_set_data_post(
    NautilusFileUndoInfoRename *self, GFile *new_file) {
...
G_DEFINE_TYPE(NautilusFileUndoInfoBatchRename,
              nautilus_file_undo_info_batch_rename,
              NAUTILUS_TYPE_FILE_UNDO_INFO);

static void batch_rename_strings_func(NautilusFileUndoInfo *info,
                                      gchar **undo_label,
                                      gchar **undo_description,
                                      gchar **redo_label,
                                      gchar **redo_description) {
    NautilusFileUndoInfoBatchRename *self =
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(info);

    *undo_description =
        g_strdup_printf(ngettext("Batch rename %d file", "Batch rename %d files",
...
static void batch_rename_redo_func(NautilusFileUndoInfo *info,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoBatchRename *self =
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(info);
    ...
    batch_rename_sort_lists_for_rename(&files, &self->new_display_names,
                                       &self->old_display_names, &self->new_files,
                                       &self->old_files, TRUE);

    nautilus_file_batch_rename(files, self->new_display_names,
                               file_undo_info_operation_callback, self);
}

static void batch_rename_undo_func(NautilusFileUndoInfo *info,
                                   GtkWindow *parent_window,
                                   NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoBatchRename *self =
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(info);
    ...
    batch_rename_sort_lists_for_rename(&files, &self->old_display_names,
                                       &self->new_display_names, &self->old_files,
                                       &self->new_files, TRUE);

    nautilus_file_batch_rename(files, self->old_display_names,
                               file_undo_info_operation_callback, self);
}

static void nautilus_file_undo_info_batch_rename_init(
    NautilusFileUndoInfoBatchRename *self) {}

static void nautilus_file_undo_info_batch_rename_finalize(GObject *obj) {
    GList *l;
    GFile *file;
    GString *string;
    NautilusFileUndoInfoBatchRename *self =
        NAUTILUS_FILE_UNDO_INFO_BATCH_RENAME(obj);
    ...
    G_OBJECT_CLASS(nautilus_file_undo_info_batch_rename_parent_class)
    ->finalize(obj);
}

static void nautilus_file_undo_info_batch_rename_class_init(
    NautilusFileUndoInfoBatchRenameClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
nautilus_file_undo_info_batch_rename_new(gint item_count) {
    return g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_BATCH_RENAME, "op-type",
                        NAUTILUS_FILE_UNDO_OP_BATCH_RENAME, "item-count",
                        item_count, NULL);
}

void nautilus_file_undo_info_batch_rename_set_data_pre(
    NautilusFileUndoInfoBatchRename *self, GList *old_files) {
    GList *l;
    GString *old_name;
    GFile *file;
...
void nautilus_file_undo_info_batch_rename_set_data_post(
    NautilusFileUndoInfoBatchRename *self, GList *new_files) {
    GList *l;
    GString *new_name;
    GFile *file;
...
G_DEFINE_TYPE(NautilusFileUndoInfoStarred, nautilus_file_undo_info_starred,
              NAUTILUS_TYPE_FILE_UNDO_INFO);

enum { PROP_FILES = 1, PROP_STARRED, NUM_PROPERTIES };

static void starred_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
                                 gchar **undo_description, gchar **redo_label,
                                 gchar **redo_description) {
    ...
    if (self->starred) {
        *undo_description =
            g_strdup_printf(ngettext("Unstar %d file", "Unstar %d files",
                                     g_list_length(self->files)),
                            g_list_length(self->files));
        *redo_description = g_strdup_printf(
                                ngettext("Star %d file", "Star %d files", g_list_length(self->files)),
                                g_list_length(self->files));
...
static void on_undo_starred_tags_updated(GObject *object, GAsyncResult *res,
        gpointer user_data) {
    GTask *task;
...
static void starred_redo_func(NautilusFileUndoInfo *info,
                              GtkWindow *parent_window,
                              NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoStarred *self = NAUTILUS_FILE_UNDO_INFO_STARRED(info);
    ...
    if (self->starred) {
        nautilus_tag_manager_star_files(tag_manager, G_OBJECT(info), self->files,
                                        on_undo_starred_tags_updated, NULL);
    } else {
        nautilus_tag_manager_unstar_files(tag_manager, G_OBJECT(info), self->files,
                                          on_undo_starred_tags_updated, NULL);
    }
}

static void starred_undo_func(NautilusFileUndoInfo *info,
                              GtkWindow *parent_window,
                              NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoStarred *self = NAUTILUS_FILE_UNDO_INFO_STARRED(info);
    g_autoptr(NautilusTagManager) tag_manager = nautilus_tag_manager_get();
...
static void nautilus_file_undo_info_starred_set_property(GObject *object,
        guint prop_id,
        const GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_file_undo_info_starred_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_starred_class_init(
    NautilusFileUndoInfoStarredClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
    ...
    g_object_class_install_property(
        oclass, PROP_FILES,
        g_param_spec_pointer("files", "files",
                             "The files for which to undo star/unstar",
                             G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property(
        oclass, PROP_STARRED,
        g_param_spec_boolean("starred", "starred",
                             "Whether the files were starred or unstarred", FALSE,
                             G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}
...
NautilusFileUndoInfo *nautilus_file_undo_info_starred_new(GList *files,
    ...
    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_STARRED, "op-type",
                        NAUTILUS_FILE_UNDO_OP_STARRED, "item-count",
                        g_list_length(files), "files", files, "starred", starred,
                        NULL);
...
static void trash_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
                               gchar **undo_description, gchar **redo_label,
                               gchar **redo_description) {
    ...
    if (count != 1) {
        *undo_description =
            g_strdup_printf(ngettext("Restore %d item from trash",
                            ...
                            count);
        *redo_description = g_strdup_printf(
                                ngettext("Move %d item to trash", "Move %d items to trash", count),
                                count);
    } else {
        GList *keys;
        char *name, *orig_path;
...
static void trash_redo_func_callback(GHashTable *debuting_uris,
                                     gboolean user_cancel, gpointer user_data) {
    ...
    if (!user_cancel) {
        new_trashed_files = g_hash_table_new_full(
        ...
        for (l = keys; l != NULL; l = l->next) {
            file = l->data;
            g_hash_table_insert(new_trashed_files, g_object_ref(file),
                                GSIZE_TO_POINTER(updated_trash_time));
        }
...
static void trash_redo_func(NautilusFileUndoInfo *info,
                            GtkWindow *parent_window,
                            NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH(info);
    ...
    if (g_hash_table_size(self->trashed) > 0) {
        ...
        nautilus_file_operations_trash_or_delete_async(
            locations, parent_window, dbus_data, trash_redo_func_callback, self);

        g_list_free(locations);
    }
}

static void trash_retrieve_files_to_restore_thread(GTask *task,
        gpointer source_object,
        gpointer task_data,
        GCancellable *cancellable) {
    NautilusFileUndoInfoTrash *self =
        NAUTILUS_FILE_UNDO_INFO_TRASH(source_object);
    ...
    enumerator = g_file_enumerate_children(
                     trash,
                     G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_TRASH_DELETION_DATE
    ...
    if (enumerator) {
        ...
        while ((info = g_file_enumerator_next_file(enumerator, NULL, &error)) !=
            ...
            origpath = g_file_info_get_attribute_byte_string(
...
static void trash_retrieve_files_ready(GObject *source, GAsyncResult *res,
                                       gpointer user_data) {
    ...
    if (error == NULL && g_hash_table_size(files_to_restore) > 0) {
        ...
        for (l = gfiles_in_trash; l != NULL; l = l->next) {
            ...
            g_file_move(item, dest, G_FILE_COPY_NOFOLLOW_SYMLINKS, NULL, NULL, NULL,
                        NULL);
        }
...
static void trash_undo_func(NautilusFileUndoInfo *info,
                            GtkWindow *parent_window,
                            NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoTrash *self = NAUTILUS_FILE_UNDO_INFO_TRASH(info);
...
static void nautilus_file_undo_info_trash_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_trash_class_init(
    NautilusFileUndoInfoTrashClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *nautilus_file_undo_info_trash_new(gint item_count) {
    return g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_TRASH, "op-type",
                        NAUTILUS_FILE_UNDO_OP_MOVE_TO_TRASH, "item-count",
                        item_count, NULL);
}
...
void nautilus_file_undo_info_trash_add_file(NautilusFileUndoInfoTrash *self,
    ...
    g_hash_table_insert(self->trashed, g_object_ref(file),
...
G_DEFINE_TYPE(NautilusFileUndoInfoRecPermissions,
              nautilus_file_undo_info_rec_permissions,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static void rec_permissions_strings_func(NautilusFileUndoInfo *info,
        gchar **undo_label,
        gchar **undo_description,
        gchar **redo_label,
        gchar **redo_description) {
    NautilusFileUndoInfoRecPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS(info);
    char *name;

    name = g_file_get_path(self->dest_dir);

    *undo_description = g_strdup_printf(
                            _("Restore original permissions of items enclosed in “%s”"), name);
    *redo_description =
        g_strdup_printf(_("Set permissions of items enclosed in “%s”"), name);
...
static void rec_permissions_callback(gboolean success, gpointer callback_data) {
    file_undo_info_transfer_callback(NULL, success, callback_data);
}

static void
rec_permissions_redo_func(NautilusFileUndoInfo *info, GtkWindow *parent_window,
                          NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoRecPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS(info);
    gchar *parent_uri;
    ...
    nautilus_file_set_permissions_recursive(
        parent_uri, self->file_permissions, self->file_mask,
        self->dir_permissions, self->dir_mask, rec_permissions_callback, self);
    g_free(parent_uri);
}

static void
rec_permissions_undo_func(NautilusFileUndoInfo *info, GtkWindow *parent_window,
                          NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoRecPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS(info);
    ...
    if (g_hash_table_size(self->original_permissions) > 0) {
        ...
        for (l = gfiles_list; l != NULL; l = l->next) {
            item = l->data;
            perm = GPOINTER_TO_UINT(
                       g_hash_table_lookup(self->original_permissions, item));
            dest = g_file_new_for_uri(item);
            g_file_set_attribute_uint32(dest, G_FILE_ATTRIBUTE_UNIX_MODE, perm,
...
static void nautilus_file_undo_info_rec_permissions_init(
    NautilusFileUndoInfoRecPermissions *self) {
    self->original_permissions =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
}

static void nautilus_file_undo_info_rec_permissions_finalize(GObject *obj) {
    NautilusFileUndoInfoRecPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_REC_PERMISSIONS(obj);

    g_hash_table_destroy(self->original_permissions);
    g_clear_object(&self->dest_dir);

    G_OBJECT_CLASS(nautilus_file_undo_info_rec_permissions_parent_class)
    ->finalize(obj);
}

static void nautilus_file_undo_info_rec_permissions_class_init(
    NautilusFileUndoInfoRecPermissionsClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *nautilus_file_undo_info_rec_permissions_new(
    GFile *dest, guint32 file_permissions, guint32 file_mask,
    guint32 dir_permissions, guint32 dir_mask) {
    NautilusFileUndoInfoRecPermissions *self;

    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_REC_PERMISSIONS, "op-type",
...
void nautilus_file_undo_info_rec_permissions_add_file(
    NautilusFileUndoInfoRecPermissions *self, GFile *file, guint32 permission) {
    gchar *original_uri = g_file_get_uri(file);
    g_hash_table_insert(self->original_permissions, original_uri,
...
G_DEFINE_TYPE(NautilusFileUndoInfoPermissions,
              nautilus_file_undo_info_permissions, NAUTILUS_TYPE_FILE_UNDO_INFO)

static void permissions_strings_func(NautilusFileUndoInfo *info,
                                     gchar **undo_label,
                                     gchar **undo_description,
                                     gchar **redo_label,
                                     gchar **redo_description) {
    NautilusFileUndoInfoPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_PERMISSIONS(info);
    gchar *name;
...
static void permissions_real_func(NautilusFileUndoInfoPermissions *self,
                                  guint32 permissions) {
    NautilusFile *file;
...
static void permissions_redo_func(NautilusFileUndoInfo *info,
                                  GtkWindow *parent_window,
                                  NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_PERMISSIONS(info);
...
static void permissions_undo_func(NautilusFileUndoInfo *info,
                                  GtkWindow *parent_window,
                                  NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_PERMISSIONS(info);
...
static void nautilus_file_undo_info_permissions_init(
    NautilusFileUndoInfoPermissions *self) {}

static void nautilus_file_undo_info_permissions_finalize(GObject *obj) {
    NautilusFileUndoInfoPermissions *self =
        NAUTILUS_FILE_UNDO_INFO_PERMISSIONS(obj);
    g_clear_object(&self->target_file);

    G_OBJECT_CLASS(nautilus_file_undo_info_permissions_parent_class)
    ->finalize(obj);
}

static void nautilus_file_undo_info_permissions_class_init(
    NautilusFileUndoInfoPermissionsClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *nautilus_file_undo_info_permissions_new(
    GFile *file, guint32 current_permissions, guint32 new_permissions) {
    NautilusFileUndoInfoPermissions *self;

    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_PERMISSIONS, "op-type",
                        NAUTILUS_FILE_UNDO_OP_SET_PERMISSIONS, "item-count", 1,
                        NULL);
...
static void ownership_strings_func(NautilusFileUndoInfo *info,
                                   gchar **undo_label, gchar **undo_description,
                                   gchar **redo_label,
                                   gchar **redo_description) {
    ...
    if (op_type == NAUTILUS_FILE_UNDO_OP_CHANGE_OWNER) {
        ...
        *redo_description = g_strdup_printf(_("Set group of “%s” to “%s”"), name,
                                            self->new_ownership);
        ...
        *undo_description = g_strdup_printf(_("Restore owner of “%s” to “%s”"),
                                            name, self->original_ownership);
        *redo_description = g_strdup_printf(_("Set owner of “%s” to “%s”"), name,
                                            self->new_ownership);
...
static void ownership_real_func(NautilusFileUndoInfoOwnership *self,
                                const gchar *ownership) {
    NautilusFileUndoOp op_type =
        nautilus_file_undo_info_get_op_type(NAUTILUS_FILE_UNDO_INFO(self));
    NautilusFile *file;

    file = nautilus_file_get(self->target_file);

    if (op_type == NAUTILUS_FILE_UNDO_OP_CHANGE_OWNER) {
        nautilus_file_set_owner(file, ownership, file_undo_info_operation_callback,
                                self);
    } else if (op_type == NAUTILUS_FILE_UNDO_OP_CHANGE_GROUP) {
        nautilus_file_set_group(file, ownership, file_undo_info_operation_callback,
                                self);
    }
...
static void ownership_redo_func(NautilusFileUndoInfo *info,
                                GtkWindow *parent_window,
                                NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP(info);
...
static void ownership_undo_func(NautilusFileUndoInfo *info,
                                GtkWindow *parent_window,
                                NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoOwnership *self = NAUTILUS_FILE_UNDO_INFO_OWNERSHIP(info);
...
static void nautilus_file_undo_info_ownership_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_ownership_class_init(
    NautilusFileUndoInfoOwnershipClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *
nautilus_file_undo_info_ownership_new(NautilusFileUndoOp op_type, GFile *file,
                                      const char *current_data,
                                      const char *new_data) {
    NautilusFileUndoInfoOwnership *self;

    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_OWNERSHIP, "item-count", 1,
                        "op-type", op_type, NULL);
...
G_DEFINE_TYPE(NautilusFileUndoInfoExtract, nautilus_file_undo_info_extract,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static void extract_callback(GList *outputs, gpointer callback_data) {
    NautilusFileUndoInfoExtract *self =
        NAUTILUS_FILE_UNDO_INFO_EXTRACT(callback_data);
    gboolean success;
...
static void extract_strings_func(NautilusFileUndoInfo *info, gchar **undo_label,
                                 gchar **undo_description, gchar **redo_label,
                                 gchar **redo_description) {
    ...
    *redo_label = g_strdup(_("_Redo Extract"));
    ...
    if (total_outputs == 1) {
        ...
        *undo_description =
            g_strdup_printf(ngettext("Delete %d extracted file",
                                     "Delete %d extracted files", total_outputs),
                            total_outputs);
    }
    ...
    if (total_sources == 1) {
        ...
        *redo_description = g_strdup_printf(
                                ngettext("Extract %d file", "Extract %d files", total_sources),
                                total_sources);
    }
}

static void extract_redo_func(NautilusFileUndoInfo *info,
                              GtkWindow *parent_window,
                              NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoExtract *self = NAUTILUS_FILE_UNDO_INFO_EXTRACT(info);

    nautilus_file_operations_extract_files(
        self->sources, self->destination_directory, parent_window, dbus_data,
        extract_callback, self);
}

static void extract_undo_func(NautilusFileUndoInfo *info,
                              GtkWindow *parent_window,
                              NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoExtract *self = NAUTILUS_FILE_UNDO_INFO_EXTRACT(info);

    nautilus_file_operations_delete_async(self->outputs, parent_window, dbus_data,
                                          file_undo_info_delete_callback, self);
}
...
static void nautilus_file_undo_info_extract_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_extract_class_init(
    NautilusFileUndoInfoExtractClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
void nautilus_file_undo_info_extract_set_outputs(
    NautilusFileUndoInfoExtract *self, GList *outputs) {
    if (self->outputs) {
        g_list_free_full(self->outputs, g_object_unref);
    }
    self->outputs = g_list_copy_deep(outputs, (GCopyFunc)g_object_ref, NULL);
}
...
nautilus_file_undo_info_extract_new(GList *sources,
    ...
    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_EXTRACT, "item-count", 1,
                        "op-type", NAUTILUS_FILE_UNDO_OP_EXTRACT, NULL);
...
G_DEFINE_TYPE(NautilusFileUndoInfoCompress, nautilus_file_undo_info_compress,
              NAUTILUS_TYPE_FILE_UNDO_INFO)

static void compress_callback(GFile *new_file, gboolean success,
                              gpointer callback_data) {
    NautilusFileUndoInfoCompress *self =
        NAUTILUS_FILE_UNDO_INFO_COMPRESS(callback_data);
...
static void compress_strings_func(NautilusFileUndoInfo *info,
                                  gchar **undo_label, gchar **undo_description,
                                  gchar **redo_label,
                                  gchar **redo_description) {
    ...
    *undo_description = g_strdup_printf(_("Delete “%s”"), output_name);
    ...
    if (sources_count == 1) {
        ...
        *redo_description = g_strdup_printf(
                                ngettext("Compress %d file", "Compress %d files", sources_count),
                                sources_count);
    }
...
static void compress_redo_func(NautilusFileUndoInfo *info,
                               GtkWindow *parent_window,
                               NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoCompress *self = NAUTILUS_FILE_UNDO_INFO_COMPRESS(info);

    nautilus_file_operations_compress(
        self->sources, self->output, self->format, self->filter, self->passphrase,
        parent_window, dbus_data, compress_callback, self);
}

static void compress_undo_func(NautilusFileUndoInfo *info,
                               GtkWindow *parent_window,
                               NautilusFileOperationsDBusData *dbus_data) {
    NautilusFileUndoInfoCompress *self = NAUTILUS_FILE_UNDO_INFO_COMPRESS(info);
    GList *files = NULL;
...
static void nautilus_file_undo_info_compress_finalize(GObject *obj) {
...
static void nautilus_file_undo_info_compress_class_init(
    NautilusFileUndoInfoCompressClass *klass) {
    GObjectClass *oclass = G_OBJECT_CLASS(klass);
    NautilusFileUndoInfoClass *iclass = NAUTILUS_FILE_UNDO_INFO_CLASS(klass);
...
NautilusFileUndoInfo *
nautilus_file_undo_info_compress_new(GList *sources, GFile *output,
                                     AutoarFormat format, AutoarFilter filter,
                                     const gchar *passphrase) {
    NautilusFileUndoInfoCompress *self;

    self = g_object_new(NAUTILUS_TYPE_FILE_UNDO_INFO_COMPRESS, "item-count", 1,
                        "op-type", NAUTILUS_FILE_UNDO_OP_COMPRESS, NULL);
