...
static void nautilus_query_get_property(GObject *object, guint prop_id,
...
static void nautilus_query_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
    NautilusQuery *self = NAUTILUS_QUERY(object);
...
static void nautilus_query_class_init(NautilusQueryClass *class) {
    ...
    g_object_class_install_property(
        gobject_class, PROP_DATE_RANGE,
        g_param_spec_pointer("date-range", "Date range of the query",
                             "The range date of the query", G_PARAM_READWRITE));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("location", "Location of the query",
                            "The location of the query", G_TYPE_FILE,
                            G_PARAM_READWRITE));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_MIMETYPES,
        g_param_spec_pointer("mimetypes", "MIME types of the query",
                             "The MIME types of the query", G_PARAM_READWRITE));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_RECURSIVE,
        g_param_spec_enum(
            "recursive", "Whether the query is being performed on subdirectories",
            "Whether the query is being performed on subdirectories or not",
            NAUTILUS_TYPE_QUERY_RECURSIVE, NAUTILUS_QUERY_RECURSIVE_ALWAYS,
            G_PARAM_READWRITE));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_SEARCH_TYPE,
        g_param_spec_enum(
            "search-type", "Type of the query", "The type of the query",
            NAUTILUS_TYPE_QUERY_SEARCH_TYPE,
            NAUTILUS_QUERY_SEARCH_TYPE_LAST_MODIFIED, G_PARAM_READWRITE));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_SEARCHING,
        g_param_spec_boolean("searching", "Whether the query is being performed",
                             "Whether the query is being performed or not", FALSE,
                             G_PARAM_READWRITE));
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_SHOW_HIDDEN,
        g_param_spec_boolean("show-hidden", "Show hidden files",
                             "Whether the search should show hidden files", FALSE,
                             G_PARAM_READWRITE));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("text", "Text of the search",
                            "The text string of the search", NULL,
                            G_PARAM_READWRITE));
}
...
gdouble nautilus_query_matches_string(NautilusQuery *query,
                                      const gchar *string) {
    ...
    /* The rank value depends on the numbers of letters before and after the
     * match. To make the prefix matches prefered over sufix ones, the number of
     * letters after the match is divided by a factor, so that it decreases the
     * rank by a smaller amount.
     */
    retval = MAX(MIN_RANK, MAX_RANK - (gdouble)(ptr - prepared_string) -
                 (gdouble)nonexact_malus / RANK_SCALE_FACTOR);
    g_free(prepared_string);
...
void nautilus_query_set_location(NautilusQuery *query, GFile *location) {
    g_return_if_fail(NAUTILUS_IS_QUERY(query));
 ...
 *
 * Returns: (transfer container) A #GPtrArray reference with MIME type name
 * strings.
 */
 ...
 * afterwards.
 */
void nautilus_query_set_mime_types(NautilusQuery *query,
                                   GPtrArray *mime_types) {
    g_return_if_fail(NAUTILUS_IS_QUERY(query));
...
void nautilus_query_set_show_hidden_files(NautilusQuery *query,
        gboolean show_hidden) {
    g_return_if_fail(NAUTILUS_IS_QUERY(query));
 ...
 *
 * Returns: (transfer full): the #GptrArray composed of #GDateTime representing
 * the date range.
 */
...
void nautilus_query_set_searching(NautilusQuery *query, gboolean searching) {
    g_return_if_fail(NAUTILUS_IS_QUERY(query));
