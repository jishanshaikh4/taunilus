...
struct {
    ...
    {N_("Anything"), {NULL}},
    {N_("Files"), {"application/octet-stream", "text/plain", NULL}},
    {N_("Folders"), {"inode/directory", NULL}},
    {   N_("Documents"),
        {   "application/rtf", "application/msword", "application/vnd.sun.xml.writer",
            ...
            "application/x-kword-crypt", "application/x-lyx",
    ...
    {   N_("Music"),
        {   "application/ogg", "audio/x-vorbis+ogg", "audio/ac3", "audio/basic",
            "audio/midi", "audio/x-flac", "audio/mp4", "audio/mpeg", "audio/x-mpeg",
            "audio/x-ms-asx", "audio/x-pn-realaudio", NULL
        }
    },
    {   N_("PDF / PostScript"),
        {   "application/pdf", "application/postscript", "application/x-dvi",
            "image/x-eps", "image/vnd.djvu+multipage", NULL
        }
    },
    {   N_("Picture"),
    ...
    {   N_("Presentation"),
        {   "application/vnd.ms-powerpoint", "application/vnd.sun.xml.impress",
            ...
            "application/x-magicpoint", "application/x-kpresenter",
            "application/"
...
static gboolean is_sandboxed(void) {
    static gboolean ret;
...
static void launch_location_update_from_file(LaunchLocation *location,
        NautilusFile *file) {
    nautilus_file_unref(location->file);
    g_free(location->uri);
    location->file = nautilus_file_ref(file);
...
static void launch_location_update_from_uri(LaunchLocation *location,
        const char *uri) {
    nautilus_file_unref(location->file);
    g_free(location->uri);
...
static LaunchLocation *find_launch_location_for_file(GList *list,
        NautilusFile *file) {
    LaunchLocation *location;
    GList *l;
...
static int file_compare_by_mime_type(NautilusFile *file_a,
                                     NautilusFile *file_b) {
...
static int file_compare_by_parent_uri(NautilusFile *file_a,
                                      NautilusFile *file_b) {
...
GAppInfo *nautilus_mime_get_default_application_for_files(GList *files) {
    ...
    sorted_files =
        g_list_sort(g_list_copy(files), (GCompareFunc)file_compare_by_mime_type);

    app = NULL;
    for (l = sorted_files; l != NULL; l = l->next) {
        file = l->data;

        if (l->prev && file_compare_by_mime_type(file, l->prev->data) == 0 &&
...
static void trash_or_delete_files(GtkWindow *parent_window, const GList *files,
    ...
    nautilus_file_operations_trash_or_delete_async(locations, parent_window, NULL,
            NULL, NULL);
...
static void trash_symbolic_link_cb(GtkDialog *dialog, gint response_id,
                                   gpointer user_data) {
...
static void report_broken_symbolic_link(GtkWindow *parent_window,
                                        NautilusFile *file) {
    char *target_path;
    char *display_name;
    char *prompt;
    ...
    if (can_trash) {
        prompt = g_strdup_printf(_("The link “%s” is broken. Move it to Trash?"),
                                 display_name);
    } else {
    ...
    if (!can_trash) {
        dialog =
            eel_show_simple_dialog(GTK_WIDGET(parent_window), GTK_MESSAGE_WARNING,
                                   prompt, detail, _("_Cancel"), NULL);
        goto out;
    }

    dialog = eel_show_yes_no_dialog(prompt, detail, _("Mo_ve to Trash"),
                                    _("_Cancel"), parent_window);
    ...
    g_signal_connect(G_OBJECT(dialog), "response",
                     G_CALLBACK(trash_symbolic_link_cb), data);
...
static void list_to_parameters_foreach(GAppInfo *application, GList *uris,
                                       GList **ret) {
...
static GList *make_activation_parameters(GList *uris, GList **unhandled_uris) {
    ...
    *unhandled_uris = NULL;
    ...
    app_table = g_hash_table_new_full(
                    (GHashFunc)mime_application_hash, (GEqualFunc)g_app_info_equal,
                    (GDestroyNotify)g_object_unref, (GDestroyNotify)g_list_free);
    ...
    for (l = uris; l != NULL; l = l->next) {
        ...
        if (app != NULL) {
            ...
            if (g_hash_table_lookup_extended(app_table, app, (gpointer *)&old_app,
                                             (gpointer *)&app_uris)) {
...
static gboolean file_was_cancelled(NautilusFile *file) {
    ...
    return error != NULL && error->domain == G_IO_ERROR &&
...
static void activation_parameters_free(ActivateParameters *parameters) {
    ...
    if (parameters->slot) {
        g_object_remove_weak_pointer(G_OBJECT(parameters->slot),
                                     (gpointer *)&parameters->slot);
    }
...
static void application_launch_async_parameters_free(
    ApplicationLaunchAsyncParameters *parameters) {
...
static void activation_start_timed_cancel(ActivateParameters *parameters) {
    ...
    eel_timed_wait_start_with_duration(
        DELAY_UNTIL_CANCEL_MSECS, cancel_activate_callback, parameters,
        parameters->timed_wait_prompt, parameters->parent_window);
}
...
static void activate_mount_op_active(GtkMountOperation *operation,
                                     GParamSpec *pspec,
...
static GtkDialog *show_confirm_multiple(GtkWindow *parent_window,
                                        int window_count, int tab_count) {
    GtkDialog *dialog;
    char *prompt;
    char *detail;
    ...
    if (tab_count > 0 && window_count > 0) {
        ...
        detail = g_strdup_printf(
                     ngettext("This will open %d separate tab and window.",
                              "This will open %d separate tabs and windows.", count),
                     count);
    } else if (tab_count > 0) {
        detail =
            g_strdup_printf(ngettext("This will open %d separate tab.",
                                     "This will open %d separate tabs.", tab_count),
                            tab_count);
    } else {
    ...
    dialog = eel_show_yes_no_dialog(prompt, detail, _("_OK"), _("_Cancel"),
                                    parent_window);
    g_free(detail);
...
static void
on_confirm_multiple_windows_response(GtkDialog *dialog, int response_id,
                                     ActivateParameters *parameters) {
    gtk_widget_destroy(GTK_WIDGET(dialog));
...
static void activate_parameters_install_free(
    ...
    if (parameters_install->slot) {
        g_object_remove_weak_pointer(G_OBJECT(parameters_install->slot),
                                     (gpointer *)&parameters_install->slot);
    }
    if (parameters_install->parent_window) {
        g_object_remove_weak_pointer(
            G_OBJECT(parameters_install->parent_window),
...
static char *get_application_no_mime_type_handler_message(NautilusFile *file,
        char *uri) {
...
static void open_with_response_cb(GtkDialog *dialog, gint response_id,
                                  gpointer user_data) {
    GtkWindow *parent_window;
    NautilusFile *file;
    GList files;
...
static void choose_program(GtkDialog *message_dialog, int response,
                           gpointer callback_data) {
    GtkWidget *dialog;
    NautilusFile *file;
    GFile *location;
    ...
    dialog = gtk_app_chooser_dialog_new(parameters->parent_window,
                                        GTK_DIALOG_MODAL, location);
    g_object_set_data_full(G_OBJECT(dialog), "mime-action:file",
                           nautilus_file_ref(file),
                           (GDestroyNotify)nautilus_file_unref);

    gtk_widget_show(dialog);

    g_signal_connect(dialog, "response", G_CALLBACK(open_with_response_cb),
                     parameters);
...
static void show_unhandled_type_error(ActivateParametersInstall *parameters) {
    ...
    char *error_message = get_application_no_mime_type_handler_message(
                              parameters->file, parameters->uri);
    if (g_content_type_is_unknown(mime_type)) {
        dialog = gtk_message_dialog_new(parameters->parent_window,
                                        GTK_DIALOG_DESTROY_WITH_PARENT |
                                        GTK_DIALOG_MODAL,
                                        GTK_MESSAGE_ERROR, 0, "%s", error_message);
        gtk_message_dialog_format_secondary_text(
            GTK_MESSAGE_DIALOG(dialog), _("The file is of an unknown type"));
    } else {
        char *text;
        text =
            g_strdup_printf(_("There is no application installed for “%s” files"),
    ...
    gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Select Application"),
                          GTK_RESPONSE_ACCEPT);
...
static void search_for_application_dbus_call_notify_cb(GDBusProxy *proxy,
        GAsyncResult *result,
        gpointer user_data) {
    ...
    if (variant == NULL) {
        if (!g_dbus_error_is_remote_error(error) ||
            ...
            message = g_strdup_printf(
                          ...
                          error->message);
            show_dialog(_("Unable to search for application"), message,
                        parameters_install->parent_window, GTK_MESSAGE_ERROR);
            g_free(message);
        } else {
...
search_for_application_mime_type(ActivateParametersInstall *parameters_install,
    ...
    g_dbus_proxy_call(
        parameters_install->proxy, "InstallMimeTypes",
        g_variant_new_parsed("([%s], %s, %s, [{%s, %v}])", mime_type,
                             "hide-confirm-search", APPLICATION_ID,
                             "desktop-startup-id",
                             g_variant_new_take_string(desktop_startup_id)),
        G_DBUS_CALL_FLAGS_NONE, G_MAXINT /* no timeout */, NULL /* cancellable */,
...
static void application_unhandled_file_install(
...
static void pk_proxy_appeared_cb(GObject *source, GAsyncResult *res,
                                 gpointer user_data) {
    ActivateParametersInstall *parameters_install = user_data;
    ...
    mime_type = nautilus_file_get_mime_type(parameters_install->file);
    error_message = get_application_no_mime_type_handler_message(
                        parameters_install->file, parameters_install->uri);
    ...
    dialog = gtk_message_dialog_new(
                 ...
                 GTK_BUTTONS_NONE, "%s", error_message);
    gtk_message_dialog_format_secondary_text(
    ...
    gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Search in Software"),
                          GTK_RESPONSE_YES);
...
static void application_unhandled_uri(ActivateParameters *parameters,
                                      char *uri) {
    ...
    /* copy the parts of parameters we are interested in as the orignal will be
     * unref'd */
    parameters_install = g_new0(ActivateParametersInstall, 1);
    parameters_install->slot = parameters->slot;
    g_object_add_weak_pointer(G_OBJECT(parameters_install->slot),
                              (gpointer *)&parameters_install->slot);
    if (parameters->parent_window) {
        parameters_install->parent_window = parameters->parent_window;
        g_object_add_weak_pointer(G_OBJECT(parameters_install->parent_window),
                                  (gpointer *)&parameters_install->parent_window);
    }
    parameters_install->activation_directory =
        g_strdup(parameters->activation_directory);
    ...
    show_install_mime = g_settings_get_boolean(
                            nautilus_preferences, NAUTILUS_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
    ...
    g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL,
                             ...
                             "/org/freedesktop/PackageKit",
                             "org.freedesktop.PackageKit.Modify2", NULL,
                             pk_proxy_appeared_cb, parameters_install);
...
static void launch_default_for_uris_callback(GObject *source_object,
        GAsyncResult *res,
        gpointer user_data) {
    ...
    if (!g_queue_is_empty(params->uris)) {
        nautilus_launch_default_for_uri_async(
            g_queue_peek_head(params->uris), activation_params->parent_window,
            activation_params->cancellable, launch_default_for_uris_callback,
            params);
    } else {
...
static void activate_files(ActivateParameters *parameters) {
    ...
    if (parameters->open_in_app_uris != NULL) {
        if (is_sandboxed()) {
            ...
            parameters->open_in_app_parameters = make_activation_parameters(
                    g_queue_peek_head_link(parameters->open_in_app_uris),
    ...
    if (parameters->user_confirmation &&
        ...
        pause_activation_timed_cancel(parameters);
        dialog =
            show_confirm_multiple(parameters->parent_window, num_windows, num_tabs);
        g_signal_connect(dialog, "response",
                         G_CALLBACK(on_confirm_multiple_windows_response),
                         parameters);
    } else {
        activate_files_internal(parameters);
    }
...
static void activate_files_internal(ActivateParameters *parameters) {
    ...
    for (l = g_queue_peek_head_link(parameters->launch_files); l != NULL;
            l = l->next) {
    ...
    for (l = g_queue_peek_head_link(parameters->launch_in_terminal_files);
            l != NULL; l = l->next) {
        g_autofree char *uri = NULL;
        g_autofree char *executable_path = NULL;
        g_autofree char *quoted_path = NULL;
    ...
    if (parameters->slot != NULL) {
        ...
        for (l = g_queue_peek_head_link(parameters->open_in_view_files); l != NULL;
                l = l->next) {
            g_autofree char *uri = NULL;
            ...
            if (g_file_is_native(location) &&
                    (nautilus_file_is_in_admin(file) || !nautilus_file_can_read(file) ||
            ...
            /* FIXME: we need to pass the parent_window, but we only use it for the
             * current active window, which nautilus-application should take care of.
             * However is not working and creating regressions in some cases. Until we
             * figure out what's going on, continue to use the parameters->slot to
             * make splicit the window we want to use for activating the files */
            nautilus_application_open_location_full(
                NAUTILUS_APPLICATION(g_application_get_default()),
                location_with_permissions, parameters->flags, NULL, NULL,
                parameters->slot);
        }
    }
    ...
    if (!g_queue_is_empty(parameters->open_in_app_uris) && is_sandboxed()) {
        ...
        nautilus_launch_default_for_uri_async(
            uri, parameters->parent_window, parameters->cancellable,
            launch_default_for_uris_callback, async_params);
        return;
    }
...
static void activation_mount_not_mounted_callback(GObject *source_object,
        GAsyncResult *res,
        gpointer user_data) {
    ...
    if (!g_file_mount_enclosing_volume_finish(G_FILE(source_object), res,
        ...
        if (error->domain != G_IO_ERROR ||
            ...
            show_dialog(_("Unable to access location"), error->message,
                        parameters->parent_window, GTK_MESSAGE_ERROR);
        }
    ...
    parameters->not_mounted =
        g_list_delete_link(parameters->not_mounted, parameters->not_mounted);
    nautilus_file_unref(file);
...
static void activation_mount_not_mounted(ActivateParameters *parameters) {
    ...
    if (parameters->not_mounted != NULL) {
        ...
        g_file_mount_enclosing_volume(
            location, 0, mount_op, parameters->cancellable,
    ...
    nautilus_file_list_call_when_ready(
        ...
        &parameters->files_handle, activate_callback, parameters);
    ...
    for (l = parameters->locations; l != NULL; l = next) {
        ...
        if (file_was_not_mounted(file)) {
            if (parameters->tried_mounting) {
                ...
                parameters->not_mounted =
                    g_list_prepend(parameters->not_mounted, nautilus_file_ref(file));
            }
            continue;
        }
    }
...
static void activate_activation_uris_ready_callback(GList *files_ignore,
        gpointer callback_data) {
    ActivateParameters *parameters = callback_data;
    GList *l, *next, *files;
    NautilusFile *file;
...
static void activate_regular_files(ActivateParameters *parameters) {
    GList *l, *files;
    NautilusFile *file;
    LaunchLocation *location;
    ...
    nautilus_file_list_call_when_ready(
        files, nautilus_mime_actions_get_required_file_attributes(),
        &parameters->files_handle, activate_activation_uris_ready_callback,
        parameters);
    nautilus_file_list_free(files);
}

static void activation_mountable_mounted(NautilusFile *file,
        GFile *result_location, GError *error,
        gpointer callback_data) {
    ActivateParameters *parameters = callback_data;
...
static void activation_mount_mountables(ActivateParameters *parameters) {
    ...
    if (parameters->mountables != NULL) {
        ...
        g_signal_connect(mount_op, "notify::is-showing",
                         G_CALLBACK(activate_mount_op_active), parameters);
        nautilus_file_mount(file, mount_op, parameters->cancellable,
                            activation_mountable_mounted, parameters);
        g_object_unref(mount_op);
        return;
    }
...
static void activation_mountable_started(NautilusFile *file,
        GFile *gfile_of_file, GError *error,
        gpointer callback_data) {
    ActivateParameters *parameters = callback_data;
    LaunchLocation *location;

    /* Remove from list of files that have to be mounted */
    parameters->start_mountables =
        g_list_remove(parameters->start_mountables, file);
    nautilus_file_unref(file);
    ...
    if (error == NULL) {
        ...
        if (error->domain != G_IO_ERROR ||
                (error->code != G_IO_ERROR_CANCELLED &&
                 error->code != G_IO_ERROR_FAILED_HANDLED)) {
            show_dialog(_("Unable to start location"), error->message,
                        parameters->parent_window, GTK_MESSAGE_ERROR);
        }
...
static void activation_start_mountables(ActivateParameters *parameters) {
    ...
    if (parameters->start_mountables != NULL) {
        ...
        g_signal_connect(start_op, "notify::is-showing",
                         G_CALLBACK(activate_mount_op_active), parameters);
        nautilus_file_start(file, start_op, parameters->cancellable,
                            activation_mountable_started, parameters);
...
void nautilus_mime_activate_files(GtkWindow *parent_window,
    ...
    g_object_add_weak_pointer(G_OBJECT(parameters->slot),
                              (gpointer *)&parameters->slot);
    if (parent_window) {
        parameters->parent_window = parent_window;
        g_object_add_weak_pointer(G_OBJECT(parameters->parent_window),
                                  (gpointer *)&parameters->parent_window);
    }
    ...
    if (file_count == 1) {
        ...
        parameters->timed_wait_prompt = g_strdup_printf(
                                            ngettext("Opening %d item.", "Opening %d items.", file_count),
                                            file_count);
    }
    ...
    for (l = parameters->locations; l != NULL; l = next) {
        ...
        if (nautilus_file_can_mount(file)) {
            parameters->mountables =
                g_list_prepend(parameters->mountables, nautilus_file_ref(file));
        }

        if (nautilus_file_can_start(file)) {
            parameters->start_mountables =
                g_list_prepend(parameters->start_mountables, nautilus_file_ref(file));
        }
    }
...
void nautilus_mime_activate_file(GtkWindow *parent_window,
                                 NautilusWindowSlot *slot, NautilusFile *file,
                                 const char *launch_directory,
                                 NautilusOpenFlags flags) {
    GList *files;
    ...
    nautilus_mime_activate_files(parent_window, slot, files, launch_directory,
                                 flags, FALSE);
    g_list_free(files);
}
