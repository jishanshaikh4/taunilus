 ...
 * XDS support: Benedikt Meurer <benny@xfce.org> (adapted by Amos Brocco
...
static const GtkTargetEntry drag_types[] = {
    {NAUTILUS_ICON_DND_GNOME_ICON_LIST_TYPE, 0,
    ...
    {NAUTILUS_ICON_DND_XDNDDIRECTSAVE_TYPE, 0,
     NAUTILUS_ICON_DND_XDNDDIRECTSAVE}, /* XDS Protocol Type */
...
static void gtk_tree_view_vertical_autoscroll(GtkTreeView *tree_view) {
  ...
  if (offset > 0) {
    offset =
        y - (visible_rect.y + visible_rect.height - 2 * AUTO_SCROLL_MARGIN);
    if (offset < 0) {
      return;
    }
  }
...
static int scroll_timeout(gpointer data) {
...
static void remove_scroll_timeout(NautilusTreeViewDragDest *dest) {
...
static int expand_timeout(gpointer data) {
  GtkTreeView *tree_view;
...
static void remove_expand_timer(NautilusTreeViewDragDest *dest) {
...
static void set_drag_dest_row(NautilusTreeViewDragDest *dest,
                              GtkTreePath *path) {
  if (path) {
    gtk_tree_view_set_drag_dest_row(dest->details->tree_view, path,
...
static void clear_drag_dest_row(NautilusTreeViewDragDest *dest) {
  gtk_tree_view_set_drag_dest_row(dest->details->tree_view, NULL, 0);
}

static gboolean get_drag_data(NautilusTreeViewDragDest *dest,
  ...
  gtk_drag_get_data(GTK_WIDGET(dest->details->tree_view), context, target,
                    time);

  return TRUE;
}

static void remove_hover_timer(NautilusTreeViewDragDest *dest) {
...
static void free_drag_data(NautilusTreeViewDragDest *dest) {
...
static gboolean hover_timer(gpointer user_data) {
...
static void check_hover_timer(NautilusTreeViewDragDest *dest, const char *uri) {
...
static void check_expand_timer(NautilusTreeViewDragDest *dest,
                               GtkTreePath *drop_path,
  ...
  if (dest->details->expand_id == 0 && drop_path != NULL) {
    ...
    if (gtk_tree_model_iter_has_child(model, &drop_iter)) {
      dest->details->expand_id = g_timeout_add_seconds(
          HOVER_EXPAND_TIMEOUT, expand_timeout, dest->details->tree_view);
    }
  }
}

static char *get_root_uri(NautilusTreeViewDragDest *dest) {
  char *uri;
...
static NautilusFile *file_for_path(NautilusTreeViewDragDest *dest,
                                   GtkTreePath *path) {
  NautilusFile *file;
  char *uri;
...
static char *get_drop_target_uri_for_path(NautilusTreeViewDragDest *dest,
                                          GtkTreePath *path) {
  NautilusFile *file;
  char *target = NULL;
  gboolean can;
  ...
  can = nautilus_drag_can_accept_info(file, dest->details->drag_type,
...
static void check_hover_expand_timer(NautilusTreeViewDragDest *dest,
                                     GtkTreePath *path, GtkTreePath *drop_path,
                                     GtkTreePath *old_drop_path) {
  gboolean use_tree = g_settings_get_boolean(
...
static GtkTreePath *get_drop_path(NautilusTreeViewDragDest *dest,
                                  GtkTreePath *path) {
  NautilusFile *file;
  GtkTreePath *ret;
  ...
  while (file == NULL /* dummy row */ ||
         !nautilus_drag_can_accept_info(file, dest->details->drag_type,
                                        dest->details->drag_list)) {
...
static guint get_drop_action(NautilusTreeViewDragDest *dest,
                             GdkDragContext *context, GtkTreePath *path) {
  char *drop_target;
  int action;
  ...
  case NAUTILUS_ICON_DND_GNOME_ICON_LIST: {
    nautilus_drag_default_drop_action_for_icons(
        context, drop_target, dest->details->drag_list, 0, &action);
  } break;
...
static gboolean drag_motion_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  action = 0;
  gtk_tree_view_convert_bin_window_to_widget_coords(GTK_TREE_VIEW(widget), 0, 0,
                                                    NULL, &bin_y);
  if (bin_y <= y) {
  ...
  gtk_tree_view_get_drag_dest_row(GTK_TREE_VIEW(widget), &old_drop_path, NULL);
  ...
  if (dest->details->scroll_id == 0) {
    dest->details->scroll_id =
        g_timeout_add(150, scroll_timeout, dest->details->tree_view);
  }
...
static char *get_drop_target_uri_at_pos(NautilusTreeViewDragDest *dest, int x,
                                        int y) {
  ...
  gtk_tree_view_get_dest_row_at_pos(dest->details->tree_view, x, y, &path,
                                    &pos);
  if (pos == GTK_TREE_VIEW_DROP_BEFORE || pos == GTK_TREE_VIEW_DROP_AFTER) {
    gtk_tree_path_free(path);
    path = NULL;
...
static void receive_uris(NautilusTreeViewDragDest *dest,
                         GdkDragContext *context, GList *source_uris, int x,
                         int y) {
  char *drop_target;
  ...
  if (real_action > 0) {
    if (!nautilus_drag_uris_local(drop_target, source_uris) ||
      ...
      g_signal_emit(dest, signals[MOVE_COPY_ITEMS], 0, source_uris, drop_target,
                    real_action, x, y);
    }
  }
...
static void receive_dropped_icons(NautilusTreeViewDragDest *dest,
...
static void receive_dropped_uri_list(NautilusTreeViewDragDest *dest,
                                     GdkDragContext *context, int x, int y) {
  char *drop_target;
  ...
  g_signal_emit(dest, signals[HANDLE_URI_LIST], 0,
                ...
                drop_target, gdk_drag_context_get_selected_action(context), x,
                y);

  g_free(drop_target);
}

static void receive_dropped_text(NautilusTreeViewDragDest *dest,
                                 GdkDragContext *context, int x, int y) {
  char *drop_target;
  guchar *text;
  ...
  g_signal_emit(dest, signals[HANDLE_TEXT], 0, (char *)text, drop_target,
                gdk_drag_context_get_selected_action(context), x, y);

  g_free(text);
  g_free(drop_target);
}

static void receive_dropped_raw(NautilusTreeViewDragDest *dest,
  ...
  g_signal_emit(dest, signals[HANDLE_RAW], 0, raw_data, length, drop_target,
...
static void receive_dropped_netscape_url(NautilusTreeViewDragDest *dest,
                                         GdkDragContext *context, int x,
                                         int y) {
  char *drop_target;
...
static gboolean receive_xds(NautilusTreeViewDragDest *dest, GtkWidget *widget,
                            guint32 time, GdkDragContext *context, int x,
                            int y) {
  GFile *location;
  ...
  if (selection_format == 8 && selection_length == 1 &&
      selection_data[0] == 'F') {
    gtk_drag_get_data(widget, context,
                      gdk_atom_intern(NAUTILUS_ICON_DND_RAW_TYPE, FALSE), time);
    return FALSE;
  } else if (selection_format == 8 && selection_length == 1 &&
             selection_data[0] == 'S') {
...
static gboolean
drag_data_received_callback(GtkWidget *widget, GdkDragContext *context, int x,
                            int y, GtkSelectionData *selection_data, guint info,
                            guint32 time, gpointer data) {
  NautilusTreeViewDragDest *dest;
  const gchar *tmp;
  int length;
...
static char *get_direct_save_filename(GdkDragContext *context) {
  guchar *prop_text;
  gint prop_len;
...
static gboolean set_direct_save_uri(NautilusTreeViewDragDest *dest,
                                    GdkDragContext *context, int x, int y) {
  GFile *base, *child;
  char *drop_uri;
  char *filename, *uri;
...
static void tree_view_weak_notify(gpointer user_data, GObject *object) {
  NautilusTreeViewDragDest *dest;
...
static void nautilus_tree_view_drag_dest_dispose(GObject *object) {
  NautilusTreeViewDragDest *dest;
...
static void nautilus_tree_view_drag_dest_finalize(GObject *object) {
  NautilusTreeViewDragDest *dest;
...
static void nautilus_tree_view_drag_dest_init(NautilusTreeViewDragDest *dest) {
  dest->details = G_TYPE_INSTANCE_GET_PRIVATE(
      dest, NAUTILUS_TYPE_TREE_VIEW_DRAG_DEST, NautilusTreeViewDragDestDetails);
}
...
nautilus_tree_view_drag_dest_class_init(NautilusTreeViewDragDestClass *class) {
  ...
  signals[GET_ROOT_URI] =
      g_signal_new("get-root-uri", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, get_root_uri),
                   NULL, NULL, g_cclosure_marshal_generic, G_TYPE_STRING, 0);
  signals[GET_FILE_FOR_PATH] = g_signal_new(
      "get-file-for-path", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, get_file_for_path), NULL,
      NULL, g_cclosure_marshal_generic, NAUTILUS_TYPE_FILE, 1,
      GTK_TYPE_TREE_PATH);
  signals[MOVE_COPY_ITEMS] = g_signal_new(
      "move-copy-items", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, move_copy_items), NULL,
      NULL,

      g_cclosure_marshal_generic, G_TYPE_NONE, 3, G_TYPE_POINTER, G_TYPE_STRING,
      GDK_TYPE_DRAG_ACTION);
  signals[HANDLE_NETSCAPE_URL] = g_signal_new(
      "handle-netscape-url", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, handle_netscape_url), NULL,
      NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 3, G_TYPE_STRING,
      G_TYPE_STRING, GDK_TYPE_DRAG_ACTION);
  signals[HANDLE_URI_LIST] = g_signal_new(
      "handle-uri-list", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, handle_uri_list), NULL,
      NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 3, G_TYPE_STRING,
      G_TYPE_STRING, GDK_TYPE_DRAG_ACTION);
  signals[HANDLE_TEXT] =
      g_signal_new("handle-text", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, handle_text),
                   NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 3,
                   G_TYPE_STRING, G_TYPE_STRING, GDK_TYPE_DRAG_ACTION);
  signals[HANDLE_RAW] = g_signal_new(
      "handle-raw", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, handle_raw), NULL, NULL,
      g_cclosure_marshal_generic, G_TYPE_NONE, 5, G_TYPE_POINTER, G_TYPE_INT,
      G_TYPE_STRING, G_TYPE_STRING, GDK_TYPE_DRAG_ACTION);
  signals[HANDLE_HOVER] = g_signal_new(
      "handle-hover", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusTreeViewDragDestClass, handle_hover), NULL, NULL,
...
nautilus_tree_view_drag_dest_new(GtkTreeView *tree_view) {
  ...
  g_object_weak_ref(G_OBJECT(dest->details->tree_view), tree_view_weak_notify,
                    dest);

  gtk_drag_dest_set(
      GTK_WIDGET(tree_view), 0, drag_types, G_N_ELEMENTS(drag_types),
  ...
  g_signal_connect_object(tree_view, "drag-motion",
                          G_CALLBACK(drag_motion_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag-leave",
                          G_CALLBACK(drag_leave_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag-drop",
                          G_CALLBACK(drag_drop_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag-data-received",
                          G_CALLBACK(drag_data_received_callback), dest, 0);
