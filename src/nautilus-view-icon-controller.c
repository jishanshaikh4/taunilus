...
G_DEFINE_TYPE(NautilusViewIconController, nautilus_view_icon_controller,
...
get_sorts_constants_from_action_target_name(const gchar *action_target_name) {
  ...
  for (i = 0; i < G_N_ELEMENTS(sorts_constants); i++) {
    if (g_strcmp0(sorts_constants[i].action_target_name, action_target_name) ==
        0) {
...
static const SortConstants *get_default_sort_order(NautilusFile *file) {
...
static const SortConstants *get_directory_sort_by(NautilusFile *file) {
  ...
  reversed = nautilus_file_get_boolean_metadata(
...
static void set_directory_sort_metadata(NautilusFile *file,
                                        const SortConstants *sort) {
  const SortConstants *default_sort;

  default_sort = get_default_sort_order(file);

  nautilus_file_set_metadata(file, NAUTILUS_METADATA_KEY_ICON_VIEW_SORT_BY,
                             default_sort->metadata_name, sort->metadata_name);
  nautilus_file_set_boolean_metadata(
      file, NAUTILUS_METADATA_KEY_ICON_VIEW_SORT_REVERSED,
      default_sort->reversed, sort->reversed);
}

static void update_sort_order_from_metadata_and_preferences(
    NautilusViewIconController *self) {
  const SortConstants *default_directory_sort;
  GActionGroup *view_action_group;

  default_directory_sort = get_directory_sort_by(
      nautilus_files_view_get_directory_as_file(NAUTILUS_FILES_VIEW(self)));
  view_action_group =
      nautilus_files_view_get_action_group(NAUTILUS_FILES_VIEW(self));
  g_action_group_change_action_state(
      view_action_group, "sort",
      g_variant_new_string(
          get_sorts_constants_from_sort_type(default_directory_sort->sort_type,
                                             default_directory_sort->reversed)
              ->action_target_name));
}

static void real_begin_loading(NautilusFilesView *files_view) {
...
static void real_clear(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);

  nautilus_view_model_remove_all_items(self->model);
}

static void real_file_changed(NautilusFilesView *files_view, NautilusFile *file,
...
static GList *real_get_selection(NautilusFilesView *files_view) {
  NautilusViewIconController *self;
...
static gboolean real_is_empty(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);

  return g_list_model_get_n_items(G_LIST_MODEL(self->model)) == 0;
}

static void real_end_file_changes(NautilusFilesView *files_view) {}

static void real_remove_file(NautilusFilesView *files_view, NautilusFile *file,
                             NautilusDirectory *directory) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  NautilusViewItemModel *item_model;
...
static GQueue *convert_glist_to_queue(GList *list) {
  GList *l;
  GQueue *queue;
...
static GQueue *convert_files_to_item_models(NautilusViewIconController *self,
                                            GQueue *files) {
  GList *l;
  GQueue *models;
  ...
  for (l = g_queue_peek_head_link(files); l != NULL; l = l->next) {
    ...
    item_model = nautilus_view_item_model_new(
        NAUTILUS_FILE(l->data), get_icon_size_for_zoom_level(self->zoom_level));
...
static void real_set_selection(NautilusFilesView *files_view,
                               GList *selection) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  ...
  selection_item_models =
      nautilus_view_model_get_items_from_files(self->model, selection_files);

  gtk_flow_box_unselect_all(self->view_ui);
  for (GList *l = g_queue_peek_head_link(selection_item_models); l != NULL;
       l = l->next) {
    GtkWidget *item_ui;

    item_ui =
        nautilus_view_item_model_get_item_ui(NAUTILUS_VIEW_ITEM_MODEL(l->data));
...
static void real_select_all(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  gtk_flow_box_select_all(self->view_ui);
}

static void real_invert_selection(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
...
static GtkWidget *get_first_selected_item_ui(NautilusViewIconController *self) {
...
static void reveal_item_ui(NautilusViewIconController *self,
                           GtkWidget *item_ui) {
  int item_y;
  int item_height;

  gtk_widget_translate_coordinates(item_ui, GTK_WIDGET(self->view_ui), 0, 0,
                                   NULL, &item_y);
...
static void real_reveal_selection(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  GtkWidget *item_ui;
...
static gboolean showing_search_directory(NautilusFilesView *view) {
  NautilusFile *file;
...
static void real_update_actions_state(NautilusFilesView *files_view) {
  GAction *action;
  GActionGroup *view_action_group;

  NAUTILUS_FILES_VIEW_CLASS(nautilus_view_icon_controller_parent_class)
      ->update_actions_state(files_view);
...
static void real_bump_zoom_level(NautilusFilesView *files_view,
                                 int zoom_increment) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  ...
  if (new_level >= NAUTILUS_GRID_ZOOM_LEVEL_SMALL &&
    ...
    g_action_group_change_action_state(self->action_group, "zoom-to-level",
...
static gint get_default_zoom_level(void) {
...
static void set_captions_from_preferences(NautilusViewIconController *self) {
  ...
  n_captions_for_zoom_level =
      MIN(self->zoom_level, G_N_ELEMENTS(self->caption_attributes));
  ...
  for (gint i = 0, quark_i = 0;
       value[i] != NULL && quark_i < n_captions_for_zoom_level; i++) {
...
static void set_icon_size(NautilusViewIconController *self, gint icon_size) {
  NautilusViewItemModel *current_item_model;
  guint i = 0;

  while ((current_item_model = NAUTILUS_VIEW_ITEM_MODEL(
              g_list_model_get_item(G_LIST_MODEL(self->model), i)))) {
    nautilus_view_item_model_set_icon_size(
        current_item_model, get_icon_size_for_zoom_level(self->zoom_level));
    i++;
  }
}

static void set_zoom_level(NautilusViewIconController *self, guint new_level) {
...
static void real_restore_standard_zoom_level(NautilusFilesView *files_view) {
  NautilusViewIconController *self;

  self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  g_action_group_change_action_state(
      self->action_group, "zoom-to-level",
      g_variant_new_int32(NAUTILUS_GRID_ZOOM_LEVEL_LARGE));
}

static gfloat real_get_zoom_level_percentage(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
...
static gboolean real_is_zoom_level_default(NautilusFilesView *files_view) {
  NautilusViewIconController *self;
  guint icon_size;
...
static gboolean real_can_zoom_in(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);

  return self->zoom_level < NAUTILUS_GRID_ZOOM_LEVEL_LARGEST;
}

static gboolean real_can_zoom_out(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);

  return self->zoom_level > NAUTILUS_GRID_ZOOM_LEVEL_SMALL;
}

static GdkRectangle *get_rectangle_for_item_ui(NautilusViewIconController *self,
                                               GtkWidget *item_ui) {
  ...
  content_widget =
      nautilus_files_view_get_content_widget(NAUTILUS_FILES_VIEW(self));
  vadjustment =
      gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(content_widget));
  hadjustment =
      gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(content_widget));
...
real_reveal_for_selection_context_menu(NautilusFilesView *files_view) {
  ...
  if (item_ui == NULL ||
      !gtk_flow_box_child_is_selected(GTK_FLOW_BOX_CHILD(item_ui))) {
...
static void set_click_mode_from_settings(NautilusViewIconController *self) {
  int click_policy;
...
static void real_click_policy_changed(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  set_click_mode_from_settings(self);
}

static void activate_selection_on_click(NautilusViewIconController *self,
...
static void on_button_press_event(GtkGestureMultiPress *gesture, gint n_press,
  ...
  gtk_widget_translate_coordinates(
      ...
      GTK_WIDGET(self->view_ui), x, y, &view_x, &view_y);
  ...
  if (child_at_pos != NULL) {
    ...
    self->activate_on_release =
        (self->single_click_mode && button == GDK_BUTTON_PRIMARY &&
         n_press == 1 && !selection_mode);
    ...
    if (!selection_mode) {
      ...
      item_model =
          g_list_model_get_item(G_LIST_MODEL(self->model),
    ...
    if (button == GDK_BUTTON_PRIMARY && n_press == 2) {
      ...
      nautilus_files_view_pop_up_selection_context_menu(
          NAUTILUS_FILES_VIEW(self), event);
    }
  } else {
...
static void on_click_released(GtkGestureMultiPress *gesture, gint n_press,
                              gdouble x, gdouble y, gpointer user_data) {
...
static void on_click_stopped(GtkGestureMultiPress *gesture,
                             gpointer user_data) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(user_data);
...
static void on_longpress_gesture_pressed_callback(GtkGestureLongPress *gesture,
                                                  gdouble x, gdouble y,
                                                  gpointer user_data) {
  NautilusViewIconController *self;
  GtkFlowBoxChild *child_at_pos;
  ...
  event = (GdkEvent *)gtk_gesture_get_last_event(GTK_GESTURE(gesture),
                                                 event_sequence);
...
static int real_compare_files(NautilusFilesView *files_view,
                              NautilusFile *file1, NautilusFile *file2) {
  GActionGroup *view_action_group;
  GAction *action;
  ...
  directories_first =
      nautilus_files_view_should_sort_directories_first(files_view);

  return nautilus_file_compare_for_sort(file1, file2, sort_constants->sort_type,
                                        directories_first,
                                        sort_constants->reversed);
}

static void real_end_loading(NautilusFilesView *files_view,
...
get_first_visible_item_ui(NautilusViewIconController *self) {
  ...
  gtk_widget_translate_coordinates(GTK_WIDGET(child_at_0),
                                   GTK_WIDGET(self->view_ui), 0, 0, &x0, &y0);
  scrolled_y = gtk_adjustment_get_value(self->vadjustment);

  return gtk_flow_box_get_child_at_pos(self->view_ui, x0, MAX(y0, scrolled_y));
}

static char *real_get_first_visible_file(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
...
static void scroll_to_file_data_free(ScrollToFileData *data) {
  g_free(data->uri);
  g_free(data);
}

static gboolean scroll_to_file_on_idle(ScrollToFileData *data) {
...
static void real_scroll_to_file(NautilusFilesView *files_view,
                                const char *uri) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  ...
  handle_id =
      g_idle_add_full(G_PRIORITY_LOW, (GSourceFunc)scroll_to_file_on_idle, data,
                      (GDestroyNotify)scroll_to_file_data_free);
  self->scroll_to_file_handle_id = handle_id;
}

static void real_sort_directories_first_changed(NautilusFilesView *files_view) {
...
static void action_sort_order_changed(GSimpleAction *action, GVariant *value,
                                      gpointer user_data) {
  const gchar *target_name;
  const SortConstants *sort_constants;
  NautilusViewModelSortData sort_data;
  ...
  if (g_strcmp0(g_variant_get_string(value, NULL),
                g_variant_get_string(g_action_get_state(G_ACTION(action)),
                                     NULL)) == 0) {
    return;
  }
...
static void real_add_files(NautilusFilesView *files_view, GList *files) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
...
static guint real_get_view_id(NautilusFilesView *files_view) {
  return NAUTILUS_VIEW_GRID_ID;
}

static GIcon *real_get_icon(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);

  return self->view_icon;
}

static void real_select_first(NautilusFilesView *files_view) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(files_view);
  NautilusViewItemModel *item;
  NautilusFile *file;
  g_autoptr(GList) selection = NULL;

  item = NAUTILUS_VIEW_ITEM_MODEL(
...
static void real_preview_selection_event(NautilusFilesView *files_view,
  ...
  count = (direction == GTK_DIR_RIGHT || direction == GTK_DIR_DOWN) ? 1 : -1;
...
static void on_captions_preferences_changed(NautilusViewIconController *self) {
  set_captions_from_preferences(self);
...
static void on_default_sort_order_changed(NautilusViewIconController *self) {
  update_sort_order_from_metadata_and_preferences(self);
}
...
static void prioritize_thumbnailing_on_idle(NautilusViewIconController *self) {
  gdouble page_size;
  ...
  for (next_index = first_index + 1; next_index < G_MAXINT; next_index++) {
    ...
    if (gtk_widget_translate_coordinates(GTK_WIDGET(next_child),
                                         GTK_WIDGET(first_visible_child), 0, 0,
                                         NULL, &y)) {
      if (y > page_size) {
        break;
      }
...
static void on_ui_selected_children_changed(GtkFlowBox *box,
                                            gpointer user_data) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(user_data);
...
static void bind_item_ui(GtkWidget **child, NautilusViewItemModel *item_model,
                         gpointer user_data) {
...
static void setup_item_ui(GtkWidget **child, gpointer user_data) {
  NautilusViewIconController *self = NAUTILUS_VIEW_ICON_CONTROLLER(user_data);
  NautilusViewIconItemUi *item_ui;

  item_ui = nautilus_view_icon_item_ui_new();
  nautilus_view_item_ui_set_caption_attributes(item_ui,
                                               self->caption_attributes);

  *child = GTK_WIDGET(item_ui);
  gtk_widget_show_all(*child);
}

static GtkWidget *create_widget_func(gpointer item, gpointer user_data) {
...
static GtkFlowBox *create_view_ui(NautilusViewIconController *self) {
  GtkWidget *widget;
  GtkFlowBox *flowbox;
...
static void constructed(GObject *object) {
  ...
  g_signal_connect(vadjustment, "changed", (GCallback)on_vadjustment_changed,
                   self);
  g_signal_connect(vadjustment, "value-changed",
                   (GCallback)on_vadjustment_changed, self);
  ...
  g_signal_connect(self->view_ui, "selected-children-changed",
                   (GCallback)on_ui_selected_children_changed, self);

  gtk_flow_box_bind_model(self->view_ui, G_LIST_MODEL(self->model),
                          create_widget_func, self, NULL);

  self->view_icon = g_themed_icon_new("view-grid-symbolic");

  /* Compensating for the lack of event boxen to allow clicks outside the flow
   * box. */
  self->multi_press_gesture =
      gtk_gesture_multi_press_new(GTK_WIDGET(content_widget));
  gtk_event_controller_set_propagation_phase(
      GTK_EVENT_CONTROLLER(self->multi_press_gesture), GTK_PHASE_CAPTURE);
  ...
  gtk_event_controller_set_propagation_phase(
      GTK_EVENT_CONTROLLER(longpress_gesture), GTK_PHASE_CAPTURE);
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(self->action_group),
                                  view_icon_actions,
                                  G_N_ELEMENTS(view_icon_actions), self);
...
static void nautilus_view_icon_controller_class_init(
    NautilusViewIconControllerClass *klass) {
  ...
  files_view_class->sort_directories_first_changed =
      real_sort_directories_first_changed;
  ...
  files_view_class->restore_standard_zoom_level =
      real_restore_standard_zoom_level;
  ...
  files_view_class->compute_rename_popover_pointing_to =
      real_compute_rename_popover_pointing_to;
  files_view_class->reveal_for_selection_context_menu =
      real_reveal_for_selection_context_menu;
...
nautilus_view_icon_controller_init(NautilusViewIconController *self) {
  ...
  g_signal_connect_swapped(nautilus_preferences,
                           ...
                           G_CALLBACK(on_default_sort_order_changed), self);
  ...
  g_signal_connect_swapped(nautilus_icon_view_preferences,
                           ...
                           G_CALLBACK(on_captions_preferences_changed), self);
}

NautilusViewIconController *
nautilus_view_icon_controller_new(NautilusWindowSlot *slot) {
  return g_object_new(NAUTILUS_TYPE_VIEW_ICON_CONTROLLER, "window-slot", slot,
                      NULL);
}
