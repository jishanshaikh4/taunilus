...
static void apply_columns_settings(NautilusListView *list_view,
                                   char **column_order, char **visible_columns);
...
static void on_clipboard_owner_changed(GtkClipboard *clipboard, GdkEvent *event,
                                       gpointer user_data);
static void popup_column_header_menu(NautilusListView *list_view, gdouble x,
                                     gdouble y);

G_DEFINE_TYPE(NautilusListView, nautilus_list_view, NAUTILUS_TYPE_FILES_VIEW);

static const char *default_search_visible_columns[] = {"name", "size", "where",
                                                       NULL};

static const char *default_search_columns_order[] = {"name", "size", "where",
                                                     NULL};

static const char *default_recent_visible_columns[] = {"name", "where",
                                                       "recency", NULL};

static const char *default_recent_columns_order[] = {"name", "where", "recency",
                                                     NULL};
...
static const gchar *get_default_sort_order(NautilusFile *file,
                                           gboolean *reversed) {
...
static void list_selection_changed_callback(GtkTreeSelection *selection,
                                            gpointer user_data) {
...
static void activate_selected_items(NautilusListView *view) {
  GList *file_list;

  file_list = nautilus_list_view_get_selection(NAUTILUS_FILES_VIEW(view));
  if (file_list != NULL) {
    nautilus_files_view_activate_files(NAUTILUS_FILES_VIEW(view), file_list, 0,
                                       TRUE);
    nautilus_file_list_free(file_list);
  }
}

static void activate_selected_items_alternate(NautilusListView *view,
                                              NautilusFile *file,
  ...
  nautilus_files_view_activate_files(NAUTILUS_FILES_VIEW(view), file_list,
                                     flags, TRUE);
  nautilus_file_list_free(file_list);
}

static gboolean button_event_modifies_selection(const GdkEvent *event) {
...
static int get_click_policy(void) {
...
on_event_controller_motion_motion(GtkEventControllerMotion *controller,
  ...
  gtk_tree_view_convert_widget_to_bin_window_coords(GTK_TREE_VIEW(widget), x, y,
                                                    &x_in_bin, &y_in_bin);
  gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), x_in_bin, y_in_bin,
                                &view->details->hover_path, NULL, NULL, NULL);
...
static void row_activated_callback(GtkTreeView *treeview, GtkTreePath *path,
                                   GtkTreeViewColumn *column,
                                   NautilusListView *view) {
  activate_selected_items(view);
}

static gboolean check_starred_status(GtkTreeModel *model, GtkTreePath *path,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(model), iter,
                     NAUTILUS_LIST_MODEL_FILE_COLUMN, &file, -1);
...
static void on_starred_files_changed(NautilusTagManager *tag_manager,
                                     GList *changed_files, gpointer user_data) {
  NautilusListView *list_view;

  list_view = NAUTILUS_LIST_VIEW(user_data);

  gtk_tree_model_foreach(GTK_TREE_MODEL(list_view->details->model),
                         check_starred_status, changed_files);
}

static void on_star_cell_renderer_clicked(GtkTreePath *path,
                                          NautilusListView *list_view) {
  ...
  if (nautilus_tag_manager_file_is_starred(list_view->details->tag_manager,
                                           uri)) {
    nautilus_tag_manager_unstar_files(list_view->details->tag_manager,
                                      G_OBJECT(list_view), selection, NULL,
...
on_tree_view_multi_press_gesture_pressed(GtkGestureMultiPress *gesture,
  ...
  nautilus_list_model_set_drag_view(
      NAUTILUS_LIST_MODEL(gtk_tree_view_get_model(tree_view)), tree_view, bin_x,
      bin_y);
  ...
  if (!gtk_tree_view_get_path_at_pos(tree_view, bin_x, bin_y, &path, &column,
                                     NULL, NULL)) {
    if (is_simple_click) {
    ...
    if (button == GDK_BUTTON_SECONDARY) {
      nautilus_files_view_pop_up_background_context_menu(
          NAUTILUS_FILES_VIEW(view), event);
    }
  ...
  show_expanders = g_settings_get_boolean(
      nautilus_list_view_preferences, NAUTILUS_PREFERENCES_LIST_VIEW_USE_TREE);
  ...
  on_star = (g_hash_table_lookup(view->details->columns, "starred") == column &&
             !gtk_tree_view_is_blank_at_pos(tree_view, bin_x, bin_y, NULL, NULL,
                                            NULL, NULL));
  ...
  if (is_simple_click && on_star) {
    ...
    if (!on_expander && view->details->first_click_path &&
    ...
    /* Don't open a new tab if we are in single click mode (this would open 2
     * tabs), or if CTRL or SHIFT is pressed.
     */
...
on_tree_view_multi_press_gesture_released(GtkGestureMultiPress *gesture,
  ...
  gtk_tree_view_convert_widget_to_bin_window_coords(tree_view, x, y, &x_in_bin,
                                                    &y_in_bin);

  if (!gtk_tree_view_get_path_at_pos(tree_view, x_in_bin, y_in_bin, &path, NULL,
                                     NULL, NULL)) {
    return;
  }
...
on_event_controller_key_key_pressed(GtkEventControllerKey *controller,
  ...
  if (keyval == GDK_KEY_space) {
    ...
    if (!gtk_widget_has_focus(
            GTK_WIDGET(NAUTILUS_LIST_VIEW(view)->details->tree_view))) {
      return GDK_EVENT_PROPAGATE;
    }
...
static void subdirectory_done_loading_callback(NautilusDirectory *directory,
                                               NautilusListView *view) {
  nautilus_list_model_subdirectory_done_loading(view->details->model,
                                                directory);
}

static void row_expanded_callback(GtkTreeView *treeview, GtkTreeIter *iter,
                                  GtkTreePath *path, gpointer callback_data) {
  NautilusListView *view;
  ...
  if (!nautilus_list_model_load_subdirectory(view->details->model, path,
                                             &directory)) {
    return;
  }
...
static void unload_delay_data_free(UnloadDelayData *unload_data) {
...
unload_delay_data_new(NautilusFile *file, NautilusDirectory *parent_directory,
                      NautilusListView *view) {
...
static gboolean unload_file_timeout(gpointer data) {
  ...
  if (nautilus_list_model_get_tree_iter_from_file(
          model, unload_data->file, unload_data->directory, &iter)) {
...
static void row_collapsed_callback(GtkTreeView *treeview, GtkTreeIter *iter,
                                   GtkTreePath *path, gpointer callback_data) {
  NautilusListView *view;
  NautilusFile *file;
  NautilusDirectory *directory;
  ...
  gtk_tree_model_get(model, iter, NAUTILUS_LIST_MODEL_FILE_COLUMN, &file, -1);
  ...
  if (gtk_tree_model_iter_parent(model, &parent, iter)) {
    gtk_tree_model_get(model, &parent, NAUTILUS_LIST_MODEL_SUBDIRECTORY_COLUMN,
                       &directory, -1);
  }

  unload_data = unload_delay_data_new(file, directory, view);
  g_timeout_add_seconds(COLLAPSE_TO_UNLOAD_DELAY, unload_file_timeout,
                        unload_data);
...
static void subdirectory_unloaded_callback(NautilusListModel *model,
                                           NautilusDirectory *directory,
                                           gpointer callback_data) {
  NautilusListView *view;
  ...
  g_signal_handlers_disconnect_by_func(
      directory, G_CALLBACK(subdirectory_done_loading_callback), view);
  nautilus_files_view_remove_subdirectory(NAUTILUS_FILES_VIEW(view), directory);
}

static gboolean test_expand_row_callback(GtkTreeView *tree_view,
...
static void nautilus_list_view_reveal_selection(NautilusFilesView *view) {
  ...
  if (selection != NULL) {
    ...
    file = selection->data;
    if (nautilus_list_model_get_first_iter_for_file(list_view->details->model,
                                                    file, &iter)) {
      path = gtk_tree_model_get_path(GTK_TREE_MODEL(list_view->details->model),
                                     &iter);

      gtk_tree_view_scroll_to_cell(list_view->details->tree_view, path, NULL,
                                   FALSE, 0.0, 0.0);
...
static gboolean sort_criterion_changes_due_to_user(GtkTreeView *tree_view) {
  GList *columns, *p;
  GtkTreeViewColumn *column;
...
static void sort_column_changed_callback(GtkTreeSortable *sortable,
                                         NautilusListView *view) {
  NautilusFile *file;
  ...
  sort_attr = nautilus_list_model_get_attribute_from_sort_column_id(
      view->details->model, sort_column_id);

  default_sort_column_id =
      nautilus_list_model_get_sort_column_id_from_attribute(
          view->details->model, g_quark_from_string(get_default_sort_order(
                                    file, &default_sort_reversed)));
  ...
  if (view->details->last_sort_attr != sort_attr &&
    ...
    /* at this point, the sort order is always GTK_SORT_ASCENDING, if the sort
     * column ID switched. Invert the sort order, if it's the default criterion
     * with a reversed preference,
    ...
    if (reversed) {
      g_signal_handlers_block_by_func(sortable, sort_column_changed_callback,
                                      view);
      gtk_tree_sortable_set_sort_column_id(
          GTK_TREE_SORTABLE(view->details->model), sort_column_id,
          GTK_SORT_DESCENDING);
      g_signal_handlers_unblock_by_func(sortable, sort_column_changed_callback,
                                        view);
    }
  }
...
static char *get_root_uri_callback(NautilusTreeViewDragDest *dest,
                                   gpointer user_data) {
  NautilusListView *view;
...
static NautilusFile *get_file_for_path_callback(NautilusTreeViewDragDest *dest,
                                                GtkTreePath *path,
                                                gpointer user_data) {
  NautilusListView *view;
...
static void list_view_handle_uri_list(NautilusTreeViewDragDest *dest,
                                      const char *item_uris,
                                      const char *target_uri,
                                      GdkDragAction action,
                                      NautilusListView *view) {
  nautilus_files_view_handle_uri_list_drop(NAUTILUS_FILES_VIEW(view), item_uris,
                                           target_uri, action);
}

static void list_view_handle_text(NautilusTreeViewDragDest *dest,
                                  const char *text, const char *target_uri,
                                  GdkDragAction action,
                                  NautilusListView *view) {
  nautilus_files_view_handle_text_drop(NAUTILUS_FILES_VIEW(view), text,
                                       target_uri, action);
}

static void list_view_handle_raw(NautilusTreeViewDragDest *dest,
                                 const char *raw_data, int length,
                                 const char *target_uri,
                                 const char *direct_save_uri,
                                 GdkDragAction action, NautilusListView *view) {
  nautilus_files_view_handle_raw_drop(NAUTILUS_FILES_VIEW(view), raw_data,
                                      length, target_uri, direct_save_uri,
                                      action);
}

static void list_view_handle_hover(NautilusTreeViewDragDest *dest,
                                   const char *target_uri,
                                   NautilusListView *view) {
  nautilus_files_view_handle_hover(NAUTILUS_FILES_VIEW(view), target_uri);
}

static void move_copy_items_callback(NautilusTreeViewDragDest *dest,
                                     const GList *item_uris,
                                     const char *target_uri, guint action,
                                     gpointer user_data) {
  NautilusFilesView *view = user_data;

  nautilus_clipboard_clear_if_colliding_uris(GTK_WIDGET(view), item_uris);
  nautilus_files_view_move_copy_items(view, item_uris, target_uri, action);
}

static void column_header_menu_toggled(GtkCheckMenuItem *menu_item,
                                       NautilusListView *list_view) {
  NautilusFile *file;
  ...
  file =
      nautilus_files_view_get_directory_as_file(NAUTILUS_FILES_VIEW(list_view));
  ...
  nautilus_file_set_metadata_list(
      file, NAUTILUS_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, list);
...
static void column_header_menu_use_default(GtkMenuItem *menu_item,
                                           NautilusListView *list_view) {
  NautilusFile *file;
  ...
  nautilus_file_set_metadata_list(
      file, NAUTILUS_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, NULL);
  nautilus_file_set_metadata_list(
      file, NAUTILUS_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS, NULL);
...
static void popup_column_header_menu(NautilusListView *list_view, gdouble x,
  ...
  visible_columns_hash = g_hash_table_new_full(
  ...
  for (l = all_columns; l != NULL; l = l->next) {
    ...
    g_object_get(G_OBJECT(l->data), "name", &name, "label", &label, NULL);
    lowercase = g_ascii_strdown(name, -1);

    menu_item = gtk_check_menu_item_new_with_label(label);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);

    g_object_set_data_full(G_OBJECT(menu_item), "column-name", name, g_free);
    ...
    g_signal_connect(menu_item, "toggled",
                     G_CALLBACK(column_header_menu_toggled), list_view);
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(column_header_menu_use_default), list_view);
...
static void apply_columns_settings(NautilusListView *list_view,
  ...
  for (l = view_columns; l != NULL; l = l->next) {
    gtk_tree_view_move_column_after(list_view->details->tree_view, l->data,
                                    prev_view_column);
...
static void starred_cell_data_func(GtkTreeViewColumn *column,
  ...
  gtk_tree_model_get(model, iter, view->details->file_name_column_num, &text,
                     -1);
  ...
  if (file == NULL) {
    /* This row is a label, not a file */
    g_object_set(renderer, "icon-name", NULL, "mode",
  ...
  if (nautilus_tag_manager_file_is_starred(view->details->tag_manager, uri)) {
    g_object_set(renderer, "icon-name", "starred-symbolic", NULL);
  } else {
    g_object_set(renderer, "icon-name", "non-starred-symbolic", NULL);
  }
...
static void filename_cell_data_func(GtkTreeViewColumn *column,
  ...
  if (NAUTILUS_IS_SEARCH_DIRECTORY(directory)) {
    query = nautilus_search_directory_get_query(
  ...
  if (query && nautilus_query_get_search_content(query) ==
    ...
    if (file != NULL) {
      ...
      if (snippet) {
        replaced_text = g_regex_replace(view->details->regex, snippet, -1, 0,
                                        " ", G_REGEX_MATCH_NEWLINE_ANY, NULL);

        escaped_text = g_markup_escape_text(replaced_text, -1);

        g_string_append_printf(
            display_text, " <small><span alpha='50%%'><b>%s</b></span></small>",
            escaped_text);
      }
    }
    nautilus_file_unref(file);
  ...
  g_object_set(G_OBJECT(renderer), "markup", display_text->str, "underline",
               underline, NULL);
...
static void location_cell_data_func(GtkTreeViewColumn *column,
  ...
  g_object_set(G_OBJECT(renderer), "text", where, NULL);
...
static void where_cell_data_func(GtkTreeViewColumn *column,
                                 GtkCellRenderer *renderer, GtkTreeModel *model,
                                 GtkTreeIter *iter, NautilusListView *view) {
  location_cell_data_func(column, renderer, model, iter, view, FALSE);
}

static void trash_orig_path_cell_data_func(GtkTreeViewColumn *column,
                                           GtkCellRenderer *renderer,
                                           GtkTreeModel *model,
                                           GtkTreeIter *iter,
                                           NautilusListView *view) {
...
static gint nautilus_list_view_get_icon_padding_for_zoom_level(
...
static void set_up_pixbuf_size(NautilusListView *view) {
  int icon_size, icon_padding;

  /* Make all rows the same size. */
  icon_size = nautilus_list_model_get_icon_size_for_zoom_level(
      view->details->zoom_level);
  icon_padding = nautilus_list_view_get_icon_padding_for_zoom_level(
      view->details->zoom_level);
  gtk_cell_renderer_set_fixed_size(
      GTK_CELL_RENDERER(view->details->pixbuf_cell), -1,
      icon_size + 2 * icon_padding);

  /* FIXME: https://bugzilla.gnome.org/show_bug.cgi?id=641518 */
  gtk_tree_view_columns_autosize(view->details->tree_view);
}

static gint get_icon_scale_callback(NautilusListModel *model,
                                    NautilusListView *view) {
  return gtk_widget_get_scale_factor(GTK_WIDGET(view->details->tree_view));
}

static void on_longpress_gesture_pressed_event(GtkGestureLongPress *gesture,
                                               gdouble x, gdouble y,
                                               gpointer user_data) {
...
static void on_tree_view_drag_gesture_drag_begin(GtkGestureDrag *gesture,
                                                 gdouble start_x,
                                                 gdouble start_y,
                                                 gpointer user_data) {
...
static void on_tree_view_drag_gesture_drag_update(GtkGestureDrag *gesture,
                                                  gdouble offset_x,
                                                  gdouble offset_y,
                                                  gpointer user_data) {
  GdkEventSequence *sequence;
...
static void list_view_use_tree_changed_callback(gpointer callback_data) {
  GtkTreeView *tree_view;
...
static void create_and_set_up_tree_view(NautilusListView *view) {
  ...
  content_widget =
      nautilus_files_view_get_content_widget(NAUTILUS_FILES_VIEW(view));
  view->details->tree_view = GTK_TREE_VIEW(gtk_tree_view_new());
  view->details->columns = g_hash_table_new_full(g_str_hash, g_str_equal,
  ...
  g_signal_connect_object(view->details->drag_dest, "get-root-uri",
                          G_CALLBACK(get_root_uri_callback), view, 0);
  g_signal_connect_object(view->details->drag_dest, "get-file-for-path",
                          G_CALLBACK(get_file_for_path_callback), view, 0);
  g_signal_connect_object(view->details->drag_dest, "move-copy-items",
  ...
  controller =
      gtk_event_controller_motion_new(GTK_WIDGET(view->details->tree_view));
  ...
  controller =
      gtk_event_controller_key_new(GTK_WIDGET(view->details->tree_view));
  ...
  gtk_tree_view_set_model(view->details->tree_view,
  ...
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(view->details->tree_view),
      GTK_SELECTION_MULTIPLE);

  g_settings_bind(
      nautilus_list_view_preferences, NAUTILUS_PREFERENCES_LIST_VIEW_USE_TREE,
      view->details->tree_view, "show-expanders", G_SETTINGS_BIND_DEFAULT);
  ...
  for (l = nautilus_columns; l != NULL; l = l->next) {
    ...
    g_object_get(nautilus_column, "name", &name, "label", &label, "xalign",
                 &xalign, "default-sort-order", &sort_order, NULL);

    column_num =
        nautilus_list_model_add_column(view->details->model, nautilus_column);
    ...
    if (!strcmp(name, "name")) {
      ...
      gtk_tree_view_column_set_sort_column_id(view->details->file_name_column,
                                              column_num);
      gtk_tree_view_column_set_title(view->details->file_name_column,
                                     _("Name"));
      ...
      gtk_tree_view_column_pack_start(view->details->file_name_column, cell,
                                      FALSE);
      g_object_set(cell, "xpad", 6, NULL);
      g_settings_bind(nautilus_list_view_preferences,
                      NAUTILUS_PREFERENCES_LIST_VIEW_USE_TREE, cell, "visible",
      ...
      gtk_tree_view_column_set_attributes(
          view->details->file_name_column, cell, "surface",
      ...
      g_object_set(cell, "ellipsize", PANGO_ELLIPSIZE_END,
                   "single-paragraph-mode", FALSE, "width-chars", 30, "xpad", 5,
                   NULL);

      gtk_tree_view_column_pack_start(view->details->file_name_column, cell,
                                      TRUE);
      gtk_tree_view_column_set_cell_data_func(
          view->details->file_name_column, cell,
          (GtkTreeCellDataFunc)filename_cell_data_func, view, NULL);
    } else {
      if (g_strcmp0(name, "starred") == 0) {
        cell = gtk_cell_renderer_pixbuf_new();
        g_object_set(cell, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE, NULL);

        column = gtk_tree_view_column_new_with_attributes("", cell, NULL);
        ...
        column = gtk_tree_view_column_new_with_attributes(label, cell, "text",
                                                          column_num, NULL);

        pango_attr_list_insert(
            attr_list, pango_attr_foreground_alpha_new(ALPHA_55_PERCENT));
        g_object_set(cell, "attributes", attr_list, NULL);
        pango_attr_list_unref(attr_list);
      }

      gtk_tree_view_column_set_alignment(column, xalign);
      g_object_set(cell, "xalign", xalign, "xpad", 5, NULL);
      ...
      g_hash_table_insert(view->details->columns, g_strdup(name), column);
      ...
      if (!strcmp(name, "where")) {
        gtk_tree_view_column_set_cell_data_func(
            column, cell, (GtkTreeCellDataFunc)where_cell_data_func, view,
            NULL);
      } else if (!strcmp(name, "trash_orig_path")) {
        gtk_tree_view_column_set_cell_data_func(
            column, cell, (GtkTreeCellDataFunc)trash_orig_path_cell_data_func,
            view, NULL);
      } else if (!strcmp(name, "starred")) {
        gtk_tree_view_column_set_cell_data_func(
            column, cell, (GtkTreeCellDataFunc)starred_cell_data_func, view,
            NULL);
      }
    }
    g_free(name);
  ...
  default_visible_columns = g_settings_get_strv(
...
static void nautilus_list_view_add_files(NautilusFilesView *view,
                                         GList *files) {
  NautilusListModel *model;
  GList *l;
...
static char **get_default_visible_columns(NautilusListView *list_view) {
  ...
  return g_settings_get_strv(
      nautilus_list_view_preferences,
      NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
}

static GList *get_default_visible_columns_as_list(NautilusListView *list_view) {
  GList *res = NULL;
  gint i = 0;
  gchar **array;
...
static char **get_visible_columns(NautilusListView *list_view) {
  ...
  can_star_current_directory = nautilus_tag_manager_can_star_contents(
      list_view->details->tag_manager, location);
  ...
  for (l = visible_columns; l != NULL; l = l->next) {
    if (g_strcmp0(l->data, "starred") != 0 ||
        (g_strcmp0(l->data, "starred") == 0 &&
         (can_star_current_directory || is_starred))) {
...
static char **get_column_order(NautilusListView *list_view) {
  ...
  column_order = nautilus_file_get_metadata_list(
      file, NAUTILUS_METADATA_KEY_LIST_VIEW_COLUMN_ORDER);
...
static void check_allow_sort(NautilusListView *list_view) {
  GList *column_names;
  GList *l;
  NautilusFile *file;
  GtkTreeViewColumn *column;
  ...
  file =
      nautilus_files_view_get_directory_as_file(NAUTILUS_FILES_VIEW(list_view));
  allow_sorting =
      !(nautilus_file_is_in_recent(file) || nautilus_file_is_in_search(file));
  ...
  for (l = column_names; l != NULL; l = l->next) {
    ...
    if (allow_sorting) {
      sort_column_id = nautilus_list_model_get_sort_column_id_from_attribute(
          list_view->details->model, g_quark_from_string(l->data));
      /* Restore its original sorting id. We rely on that the keys of the
       * hashmap use the same string than the sort criterias */
...
static void set_columns_settings_from_metadata_and_preferences(
    NautilusListView *list_view) {
  char **column_order;
  char **visible_columns;
...
set_sort_order_from_metadata_and_preferences(NautilusListView *list_view) {
  ...
  if (!(nautilus_file_is_in_recent(file) || nautilus_file_is_in_search(file))) {
    sort_attribute = nautilus_file_get_metadata(
        file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL);
    sort_column_id = nautilus_list_model_get_sort_column_id_from_attribute(
        list_view->details->model, g_quark_from_string(sort_attribute));
    g_free(sort_attribute);

    if (sort_column_id == -1) {
      sort_column_id = nautilus_list_model_get_sort_column_id_from_attribute(
          list_view->details->model, g_quark_from_string(default_sort_order));
    }

    sort_reversed = nautilus_file_get_boolean_metadata(
        file, NAUTILUS_METADATA_KEY_LIST_VIEW_SORT_REVERSED,
        default_sort_reversed);
  } else {
    /* Make sure we use the default one and not one that the user used
     * previously of the change to not allow sorting on search and recent, or
     * the case that the user or some app modified directly the metadata */
    sort_column_id = nautilus_list_model_get_sort_column_id_from_attribute(
        list_view->details->model, g_quark_from_string(default_sort_order));
...
static NautilusListZoomLevel get_default_zoom_level(void) {
  NautilusListZoomLevel default_zoom_level;

  default_zoom_level =
      g_settings_get_enum(nautilus_list_view_preferences,
...
static void nautilus_list_view_begin_loading(NautilusFilesView *view) {
  NautilusListView *list_view;

  list_view = NAUTILUS_LIST_VIEW(view);

  nautilus_list_view_sort_directories_first_changed(
      NAUTILUS_FILES_VIEW(list_view));
...
static void nautilus_list_view_clear(NautilusFilesView *view) {
  NautilusListView *list_view;
  GtkTreeView *tree_view;
...
static void nautilus_list_view_file_changed(NautilusFilesView *view,
                                            NautilusFile *file,
                                            NautilusDirectory *directory) {
...
static void tree_selection_has_common_parent_foreach_func(GtkTreeModel *model,
                                                          GtkTreePath *path,
                                                          GtkTreeIter *iter,
                                                          gpointer user_data) {
...
static void tree_selection_has_common_parent(GtkTreeSelection *selection,
                                             gboolean *is_common,
                                             gboolean *is_root) {
  HasCommonParentData data;
  ...
  gtk_tree_selection_selected_foreach(
      selection, tree_selection_has_common_parent_foreach_func, &data);
...
static char *nautilus_list_view_get_backing_uri(NautilusFilesView *view) {
  NautilusListView *list_view;
  NautilusListModel *list_model;
  NautilusFile *file;
  GtkTreeView *tree_view;
  ...
  return NAUTILUS_FILES_VIEW_CLASS(nautilus_list_view_parent_class)
      ->get_backing_uri(view);
}

static void nautilus_list_view_get_selection_foreach_func(GtkTreeModel *model,
                                                          GtkTreePath *path,
                                                          GtkTreeIter *iter,
                                                          gpointer data) {
  GList **list;
...
static GList *nautilus_list_view_get_selection(NautilusFilesView *view) {
  ...
  gtk_tree_selection_selected_foreach(
      gtk_tree_view_get_selection(NAUTILUS_LIST_VIEW(view)->details->tree_view),
      nautilus_list_view_get_selection_foreach_func, &list);

  return g_list_reverse(list);
}

static void nautilus_list_view_get_selection_for_file_transfer_foreach_func(
...
static gboolean nautilus_list_view_is_empty(NautilusFilesView *view) {
  return nautilus_list_model_is_empty(NAUTILUS_LIST_VIEW(view)->details->model);
}

static void nautilus_list_view_end_file_changes(NautilusFilesView *view) {
  NautilusListView *list_view;
...
static void nautilus_list_view_remove_file(NautilusFilesView *view,
                                           NautilusFile *file,
                                           NautilusDirectory *directory) {
  GtkTreePath *path;
  ...
  if (nautilus_list_model_get_tree_iter_from_file(list_view->details->model,
                                                  file, directory, &iter)) {
    ...
    if (gtk_tree_selection_path_is_selected(selection, file_path)) {
      /* get reference for next element in the list view. If the element to be
       * deleted is the last one, get reference to previous element. If there is
       * only one element in view no need to select anything.
       */
      temp_iter = iter;
...
static void nautilus_list_view_set_selection(NautilusFilesView *view,
                                             GList *selection) {
  NautilusListView *list_view;
  NautilusListModel *model;
  GtkTreeView *tree_view;
  ...
  g_signal_handlers_block_by_func(tree_selection,
                                  list_selection_changed_callback, view);
  ...
  for (node = selection; node != NULL; node = node->next) {
    ...
    for (l = iters; l != NULL; l = l->next) {
      ...
      gtk_tree_selection_select_iter(tree_selection, (GtkTreeIter *)l->data);
    }
...
static void nautilus_list_view_invert_selection(NautilusFilesView *view) {
  NautilusListView *list_view;
  GtkTreeSelection *tree_selection;
  GList *node;
  GList *iters, *l;
  ...
  gtk_tree_selection_selected_foreach(
      tree_selection, nautilus_list_view_get_selection_foreach_func,
      &selection);
  ...
  for (node = selection; node != NULL; node = node->next) {
    file = node->data;
    iters = nautilus_list_model_get_all_iters_for_file(
        list_view->details->model, file);

    for (l = iters; l != NULL; l = l->next) {
      gtk_tree_selection_unselect_iter(tree_selection, (GtkTreeIter *)l->data);
    }
    g_list_free_full(iters, g_free);
  }
...
static void nautilus_list_view_select_all(NautilusFilesView *view) {
  gtk_tree_selection_select_all(gtk_tree_view_get_selection(
      NAUTILUS_LIST_VIEW(view)->details->tree_view));
}

static void nautilus_list_view_select_first(NautilusFilesView *view) {
  GtkTreeSelection *selection;
  GtkTreeIter iter;

  if (!gtk_tree_model_get_iter_first(
          GTK_TREE_MODEL(NAUTILUS_LIST_VIEW(view)->details->model), &iter)) {
    return;
  }
  selection =
      gtk_tree_view_get_selection(NAUTILUS_LIST_VIEW(view)->details->tree_view);
...
static void nautilus_list_view_zoom_to_level(NautilusFilesView *view,
                                             gint zoom_level) {
  NautilusListView *list_view;
  ...
  g_action_group_change_action_state(nautilus_files_view_get_action_group(view),
                                     "zoom-to-level",
                                     g_variant_new_int32(zoom_level));

  nautilus_files_view_update_toolbar_menus(view);
}

static void action_zoom_to_level(GSimpleAction *action, GVariant *state,
                                 gpointer user_data) {
  NautilusFilesView *view;
  NautilusListZoomLevel zoom_level;
...
static void column_chooser_changed_callback(NautilusColumnChooser *chooser,
                                            NautilusListView *view) {
  NautilusFile *file;
  char **visible_columns;
  char **column_order;
  ...
  nautilus_column_chooser_get_settings(chooser, &visible_columns,
                                       &column_order);
  ...
  list = g_list_reverse(list);
  nautilus_file_set_metadata_list(
      file, NAUTILUS_METADATA_KEY_LIST_VIEW_COLUMN_ORDER, list);
  g_list_free(list);
...
static void column_chooser_set_from_arrays(NautilusColumnChooser *chooser,
                                           NautilusListView *view,
                                           char **visible_columns,
                                           char **column_order) {
  ...
  nautilus_column_chooser_set_settings(chooser, visible_columns, column_order);

  g_signal_handlers_unblock_by_func(
      chooser, G_CALLBACK(column_chooser_changed_callback), view);
}

static void column_chooser_set_from_settings(NautilusColumnChooser *chooser,
                                             NautilusListView *view) {
  char **visible_columns;
  char **column_order;
  ...
  column_chooser_set_from_arrays(chooser, view, visible_columns, column_order);

  g_strfreev(visible_columns);
  g_strfreev(column_order);
}

static void column_chooser_use_default_callback(NautilusColumnChooser *chooser,
                                                NautilusListView *view) {
  NautilusFile *file;
  char **default_columns;
  char **default_order;
  ...
  column_chooser_set_from_arrays(chooser, view, default_columns, default_order);

  g_strfreev(default_columns);
  g_strfreev(default_order);
}

static GtkWidget *create_column_editor(NautilusListView *view) {
  ...
  builder = gtk_builder_new_from_resource(
      "/org/gnome/nautilus/ui/nautilus-list-view-column-editor.ui");
  ...
  g_signal_connect(column_chooser, "changed",
                   G_CALLBACK(column_chooser_changed_callback), view);
  g_signal_connect(column_chooser, "use-default",
                   G_CALLBACK(column_chooser_use_default_callback), view);

  column_chooser_set_from_settings(NAUTILUS_COLUMN_CHOOSER(column_chooser),
                                   view);

  return window;
}

static void action_visible_columns(GSimpleAction *action, GVariant *state,
                                   gpointer user_data) {
  NautilusListView *list_view;
...
static void nautilus_list_view_set_zoom_level(NautilusListView *view,
  ...
  gtk_tree_view_column_set_attributes(
      view->details->file_name_column,
      GTK_CELL_RENDERER(view->details->pixbuf_cell), "surface", column, NULL);
  set_up_pixbuf_size(view);
}

static void nautilus_list_view_bump_zoom_level(NautilusFilesView *view,
                                               int zoom_increment) {
  NautilusListView *list_view;
  gint new_level;
...
static gboolean nautilus_list_view_can_zoom_in(NautilusFilesView *view) {
  g_return_val_if_fail(NAUTILUS_IS_LIST_VIEW(view), FALSE);

  return NAUTILUS_LIST_VIEW(view)->details->zoom_level <
         NAUTILUS_LIST_ZOOM_LEVEL_LARGER;
}

static gboolean nautilus_list_view_can_zoom_out(NautilusFilesView *view) {
  g_return_val_if_fail(NAUTILUS_IS_LIST_VIEW(view), FALSE);
...
nautilus_list_view_click_policy_changed(NautilusFilesView *directory_view) {
  ...
  if (get_click_policy() == NAUTILUS_CLICK_POLICY_DOUBLE) {
    if (view->details->hover_path != NULL) {
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(view->details->model), &iter,
...
static void default_visible_columns_changed_callback(gpointer callback_data) {
  NautilusListView *list_view;
...
static void default_column_order_changed_callback(gpointer callback_data) {
  NautilusListView *list_view;
...
nautilus_list_view_sort_directories_first_changed(NautilusFilesView *view) {
  ...
  nautilus_list_model_set_should_sort_directories_first(
      list_view->details->model,
      nautilus_files_view_should_sort_directories_first(view));
}

static int nautilus_list_view_compare_files(NautilusFilesView *view,
                                            NautilusFile *file1,
                                            NautilusFile *file2) {
  NautilusListView *list_view;

  list_view = NAUTILUS_LIST_VIEW(view);
  return nautilus_list_model_compare_func(list_view->details->model, file1,
                                          file2);
}

static void nautilus_list_view_dispose(GObject *object) {
  NautilusListView *list_view;
  ...
  g_signal_handlers_disconnect_by_func(clipboard, on_clipboard_owner_changed,
                                       list_view);
  g_signal_handlers_disconnect_by_func(
      nautilus_preferences, default_sort_order_changed_callback, list_view);
...
static void nautilus_list_view_finalize(GObject *object) {
  NautilusListView *list_view;
  ...
  g_signal_handlers_disconnect_by_func(list_view->details->tag_manager,
                                       on_starred_files_changed, list_view);
...
nautilus_list_view_get_first_visible_file(NautilusFilesView *view) {
  ...
  if (gtk_tree_view_get_path_at_pos(list_view->details->tree_view, 0, 0, &path,
                                    NULL, NULL, NULL)) {
    gtk_tree_model_get_iter(GTK_TREE_MODEL(list_view->details->model), &iter,
                            path);

    gtk_tree_path_free(path);

    gtk_tree_model_get(GTK_TREE_MODEL(list_view->details->model), &iter,
                       NAUTILUS_LIST_MODEL_FILE_COLUMN, &file, -1);
    if (file) {
      char *uri;
...
static void nautilus_list_view_scroll_to_file(NautilusListView *view,
  ...
  if (!nautilus_list_model_get_first_iter_for_file(view->details->model, file,
                                                   &iter)) {
    return;
  }
  ...
  gtk_tree_view_scroll_to_cell(view->details->tree_view, path, NULL, TRUE, 0.0,
                               0.0);

  gtk_tree_path_free(path);
}

static void list_view_scroll_to_file(NautilusFilesView *view, const char *uri) {
  NautilusFile *file;
...
static void update_clipboard_status(NautilusListView *view) {
...
static void nautilus_list_view_end_loading(NautilusFilesView *view,
                                           gboolean all_files_seen) {
  update_clipboard_status(NAUTILUS_LIST_VIEW(view));
}

static guint nautilus_list_view_get_id(NautilusFilesView *view) {
  return NAUTILUS_VIEW_LIST_ID;
}

static GdkRectangle *get_rectangle_for_path(NautilusListView *list_view,
                                            GtkTreePath *path) {
  ...
  gtk_tree_view_get_cell_area(tree_view, path,
                              list_view->details->file_name_column, rect);
   ...
   */
  gtk_tree_view_convert_bin_window_to_widget_coords(tree_view, 0, 0, NULL,
                                                    &header_height);

  rect->y = CLAMP(rect->y, header_height,
                  gtk_widget_get_allocated_height(GTK_WIDGET(list_view)) -
                      rect->height);
  /* End of workaround */
...
nautilus_list_view_reveal_for_selection_context_menu(NautilusFilesView *view) {
  ...
  gtk_tree_view_get_cursor(tree_view, &path, NULL);
  if (path == NULL ||
      !gtk_tree_selection_path_is_selected(tree_selection, path)) {
    GList *list;
...
nautilus_list_view_preview_selection_event(NautilusFilesView *view,
  ...
  if (moved) {
    g_signal_handlers_block_by_func(selection, list_selection_changed_callback,
                                    view);
...
static void nautilus_list_view_class_init(NautilusListViewClass *class) {
  ...
  nautilus_files_view_class->bump_zoom_level =
      nautilus_list_view_bump_zoom_level;
  ...
  nautilus_files_view_class->get_zoom_level_percentage =
      nautilus_list_view_get_zoom_level_percentage;
  nautilus_files_view_class->is_zoom_level_default =
      nautilus_list_view_is_zoom_level_default;
  nautilus_files_view_class->click_policy_changed =
      nautilus_list_view_click_policy_changed;
  ...
  nautilus_files_view_class->get_backing_uri =
      nautilus_list_view_get_backing_uri;
  ...
  nautilus_files_view_class->reveal_selection =
      nautilus_list_view_reveal_selection;
  ...
  nautilus_files_view_class->invert_selection =
      nautilus_list_view_invert_selection;
  ...
  nautilus_files_view_class->sort_directories_first_changed =
      nautilus_list_view_sort_directories_first_changed;
  nautilus_files_view_class->end_file_changes =
      nautilus_list_view_end_file_changes;
  nautilus_files_view_class->get_view_id = nautilus_list_view_get_id;
  nautilus_files_view_class->get_first_visible_file =
      nautilus_list_view_get_first_visible_file;
  ...
  nautilus_files_view_class->preview_selection_event =
      nautilus_list_view_preview_selection_event;
}

static void nautilus_list_view_init(NautilusListView *list_view) {
  GActionGroup *view_action_group;
  GtkClipboard *clipboard;

  list_view->details = g_new0(NautilusListViewDetails, 1);

  /* ensure that the zoom level is always set before settings up the tree view
   * columns */
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(default_sort_order_changed_callback), list_view);
  g_signal_connect_swapped(
      nautilus_preferences,
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(default_visible_columns_changed_callback), list_view);
  g_signal_connect_swapped(
      nautilus_list_view_preferences,
      "changed::" NAUTILUS_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER,
      G_CALLBACK(default_column_order_changed_callback), list_view);
  ...
  view_action_group =
      nautilus_files_view_get_action_group(NAUTILUS_FILES_VIEW(list_view));
  g_action_map_add_action_entries(G_ACTION_MAP(view_action_group),
                                  list_view_entries,
                                  G_N_ELEMENTS(list_view_entries), list_view);
  /* Keep the action synced with the actual value, so the toolbar can poll it */
  g_action_group_change_action_state(
      nautilus_files_view_get_action_group(NAUTILUS_FILES_VIEW(list_view)),
      "zoom-to-level", g_variant_new_int32(get_default_zoom_level()));

  list_view->details->regex =
      g_regex_new("\\R+", 0, G_REGEX_MATCH_NEWLINE_ANY, NULL);

  list_view->details->tag_manager = nautilus_tag_manager_get();
  list_view->details->starred_cancellable = g_cancellable_new();

  g_signal_connect(list_view->details->tag_manager, "starred-changed",
                   (GCallback)on_starred_files_changed, list_view);
}

NautilusFilesView *nautilus_list_view_new(NautilusWindowSlot *slot) {
  return g_object_new(NAUTILUS_TYPE_LIST_VIEW, "window-slot", slot, NULL);
}
