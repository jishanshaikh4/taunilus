...
enum { FILES_ADDED, FILES_CHANGED, DONE_LOADING, LOAD_ERROR, LAST_SIGNAL };

enum { PROP_LOCATION = 1, NUM_PROPERTIES };
...
static gboolean real_contains_file(NautilusDirectory *self,
                                   NautilusFile *file) {
...
static gboolean is_tentative(NautilusFile *file, gpointer callback_data) {
...
static GList *real_get_file_list(NautilusDirectory *directory) {
  ...
  tentative_files = nautilus_file_list_filter(
      directory->details->file_list, &non_tentative_files, is_tentative, NULL);
  nautilus_file_list_free(tentative_files);

  return non_tentative_files;
}

static gboolean real_is_editable(NautilusDirectory *directory) { return TRUE; }
...
static NautilusFile *real_new_file_from_filename(NautilusDirectory *directory,
  ...
  if (NAUTILUS_IS_SEARCH_DIRECTORY(directory)) {
    if (self_owned) {
      file = NAUTILUS_FILE(
          g_object_new(NAUTILUS_TYPE_SEARCH_DIRECTORY_FILE, NULL));
    } else {
...
static void nautilus_directory_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void nautilus_directory_get_property(GObject *object, guint property_id,
...
static void nautilus_directory_class_init(NautilusDirectoryClass *klass) {
  ...
  signals[FILES_ADDED] = g_signal_new(
      "files-added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusDirectoryClass, files_added), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[FILES_CHANGED] = g_signal_new(
      "files-changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusDirectoryClass, files_changed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[DONE_LOADING] = g_signal_new(
      "done-loading", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusDirectoryClass, done_loading), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[LOAD_ERROR] = g_signal_new(
      "load-error", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(NautilusDirectoryClass, load_error), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  properties[PROP_LOCATION] = g_param_spec_object(
      "location", "The location", "The location of this directory", G_TYPE_FILE,
...
static void nautilus_directory_init(NautilusDirectory *directory) {
  directory->details = G_TYPE_INSTANCE_GET_PRIVATE(
      (directory), NAUTILUS_TYPE_DIRECTORY, NautilusDirectoryDetails);
...
static void collect_all_directories(gpointer key, gpointer value,
...
static void async_state_changed_one(gpointer key, gpointer value,
                                    gpointer user_data) {
  NautilusDirectory *directory;
...
static void async_data_preference_changed_callback(gpointer callback_data) {
  g_assert(callback_data == NULL);
 ...
 */
NautilusFile *nautilus_directory_get_existing_corresponding_file(
    NautilusDirectory *directory) {
  NautilusFile *file;
  char *uri;
...
NautilusFile *nautilus_directory_new_file_from_filename(
    NautilusDirectory *directory, const char *filename, gboolean self_owned) {
  return NAUTILUS_DIRECTORY_CLASS(G_OBJECT_GET_CLASS(directory))
      ->new_file_from_filename(directory, filename, self_owned);
}

static GList *nautilus_directory_provider_get_all(void) {
  GIOExtensionPoint *extension_point;
  GList *extensions;

  extension_point = g_io_extension_point_lookup(
      NAUTILUS_DIRECTORY_PROVIDER_EXTENSION_POINT_NAME);
  if (extension_point == NULL) {
    g_warning("Directory provider extension point not registered. Did you call "
...
static NautilusDirectory *nautilus_directory_new(GFile *location) {
  ...
  handling_instance =
      g_object_new(handling_provider_type, "location", location, NULL);
...
static void add_to_hash_table(NautilusDirectory *directory, NautilusFile *file,
                              GList *node) {
  gchar *name;
  ...
  g_assert(name != NULL);
  g_assert(node != NULL);
  g_assert(g_hash_table_lookup(directory->details->file_hash, name) == NULL);
  g_hash_table_insert(directory->details->file_hash, name, node);
}

static GList *extract_from_hash_table(NautilusDirectory *directory,
                                      NautilusFile *file) {
...
void nautilus_directory_remove_file(NautilusDirectory *directory,
  ...
  directory->details->file_list =
      g_list_remove_link(directory->details->file_list, node);
  g_list_free_1(node);
...
void nautilus_directory_emit_files_added(NautilusDirectory *directory,
  ...
  if (added_files != NULL) {
    g_signal_emit(directory, signals[FILES_ADDED], 0, added_files);
  }
...
void nautilus_directory_emit_files_changed(NautilusDirectory *directory,
  ...
  if (changed_files != NULL) {
    g_signal_emit(directory, signals[FILES_CHANGED], 0, changed_files);
  }
  nautilus_profile_end(NULL);
}
...
void nautilus_directory_emit_done_loading(NautilusDirectory *directory) {
  g_signal_emit(directory, signals[DONE_LOADING], 0);
}

void nautilus_directory_emit_load_error(NautilusDirectory *directory,
                                        GError *error) {
  g_signal_emit(directory, signals[LOAD_ERROR], 0, error);
}

/* Return a directory object for this one's parent. */
static NautilusDirectory *get_parent_directory(GFile *location) {
  NautilusDirectory *directory;
  GFile *parent;
 ...
 */
static NautilusDirectory *get_parent_directory_if_exists(GFile *location) {
  NautilusDirectory *directory;
  GFile *parent;
...
static void hash_table_list_prepend(GHashTable *table, gconstpointer key,
                                    gpointer data) {
  GList *list;
...
static void call_files_added_free_list(gpointer key, gpointer value,
                                       gpointer user_data) {
  g_assert(NAUTILUS_IS_DIRECTORY(key));
  g_assert(value != NULL);
  g_assert(user_data == NULL);

  g_signal_emit(key, signals[FILES_ADDED], 0, value);
  g_list_free(value);
}

static void call_files_changed_common(NautilusDirectory *self,
                                      GList *file_list) {
  GList *node;
  NautilusFile *file;
...
static void call_files_changed_free_list(gpointer key, gpointer value,
                                         gpointer user_data) {
  g_assert(value != NULL);
  g_assert(user_data == NULL);
...
static void call_files_changed_unref_free_list(gpointer key, gpointer value,
                                               gpointer user_data) {
  g_assert(value != NULL);
  g_assert(user_data == NULL);
...
static void call_get_file_info_free_list(gpointer key, gpointer value,
                                         gpointer user_data) {
  NautilusDirectory *directory;
  GList *files;
...
static void invalidate_count_and_unref(gpointer key, gpointer value,
                                       gpointer user_data) {
  g_assert(NAUTILUS_IS_DIRECTORY(key));
  g_assert(value == key);
  g_assert(user_data == NULL);
...
static void collect_parent_directories(GHashTable *hash_table,
                                       NautilusDirectory *directory) {
...
void nautilus_directory_notify_files_added(GList *files) {
  ...
  for (p = files; p != NULL; p = p->next) {
    ...
    if (file && file->details->is_added) {
      ...
      hash_table_list_prepend(added_lists, directory, g_object_ref(location));
    }
    nautilus_file_unref(file);
    nautilus_directory_unref(directory);
  }
...
void nautilus_directory_notify_files_removed(GList *files) {
  ...
  for (p = files; p != NULL; p = p->next) {
    ...
    if (file != NULL && !nautilus_file_rename_in_progress(file)) {
      ...
      hash_table_list_prepend(changed_lists, directory,
                              nautilus_file_ref(file));
    }
    nautilus_file_unref(file);
  }
...
static void change_directory_location(NautilusDirectory *directory,
                                      GFile *new_location) {
...
static void collect_directories_by_container(gpointer key, gpointer value,
                                             gpointer callback_data) {
  NautilusDirectory *directory;
  ...
  if (g_file_has_prefix(location, collect_data->container) ||
    ...
    collect_data->directories =
        g_list_prepend(collect_data->directories, directory);
  }
}

static GList *nautilus_directory_moved_internal(GFile *old_location,
                                                GFile *new_location) {
  ...
  g_hash_table_foreach(directories, collect_directories_by_container,
                       &collection);
  ...
  for (node = collection.directories; node != NULL; node = node->next) {
    ...
    if (g_file_equal(directory->details->location, old_location)) {
      ...
      relative_path =
          g_file_get_relative_path(old_location, directory->details->location);
      if (relative_path != NULL) {
        new_directory_location =
            g_file_resolve_relative_path(new_location, relative_path);
    ...
    if (new_directory_location) {
      ...
      if (directory->details->as_file != NULL) {
        affected_files = g_list_prepend(
            affected_files, nautilus_file_ref(directory->details->as_file));
      }
      affected_files =
          g_list_concat(affected_files,
...
void nautilus_directory_moved(const char *old_uri, const char *new_uri) {
  GList *list, *node;
  GHashTable *hash;
  NautilusFile *file;
  GFile *old_location;
...
void nautilus_directory_notify_files_moved(GList *file_pairs) {
  ...
  for (p = file_pairs; p != NULL; p = p->next) {
    ...
    if (file == NULL) {
      ...
      new_files_list = g_list_prepend(new_files_list, to_location);
    } else {
      NautilusDirectory *directory;
      ...
      nautilus_directory_cancel_loading_file_attributes(old_directory, file,
                                                        cancel_attributes);
      ...
      nautilus_file_update_name_and_directory(file, name, new_directory);
      g_free(name);

      /* Update file attributes */
      nautilus_file_invalidate_attributes(file, NAUTILUS_FILE_ATTRIBUTE_INFO);

      hash_table_list_prepend(changed_lists, old_directory, file);
      if (old_directory != new_directory) {
        hash_table_list_prepend(added_lists, new_directory, file);
      }
...
void nautilus_directory_call_when_ready(NautilusDirectory *directory,
  ...
  NAUTILUS_DIRECTORY_CLASS(G_OBJECT_GET_CLASS(directory))
...
void nautilus_directory_file_monitor_add(NautilusDirectory *directory,
  ...
  NAUTILUS_DIRECTORY_CLASS(G_OBJECT_GET_CLASS(directory))
      ->file_monitor_add(directory, client, monitor_hidden_files,
                         file_attributes, callback, callback_data);
}
 ...
 **/
GList *nautilus_directory_list_ref(GList *list) {
 ...
 * @list: GList of directories.
 **/
void nautilus_directory_list_unref(GList *list) {
 ...
 * @list: GList of directories.
 **/
void nautilus_directory_list_free(GList *list) {
 ...
 * @list: GList of directories.
 **/
GList *nautilus_directory_list_copy(GList *list) {
  return g_list_copy(nautilus_directory_list_ref(list));
}

static int compare_by_uri(NautilusDirectory *a, NautilusDirectory *b) {
  char *uri_a, *uri_b;
  int res;
...
static int compare_by_uri_cover(gconstpointer a, gconstpointer b) {
 ...
 * @list: GList of directories.
 **/
GList *nautilus_directory_list_sort_by_uri(GList *list) {
...
static void got_files_callback(NautilusDirectory *directory, GList *files,
                               gpointer callback_data) {
  g_assert(NAUTILUS_IS_DIRECTORY(directory));
...
void nautilus_self_check_directory(void) {
  NautilusDirectory *directory;
  NautilusFile *file;
  ...
  nautilus_directory_file_monitor_add(directory, &data_dummy, TRUE, 0, NULL,
                                      NULL);
   ...
   *  EEL_CHECK_STRING_RESULT (nautilus_file_get_metadata (file, "test",
   * "default"), "value");
   *
   *  nautilus_file_set_boolean_metadata (file, "test_boolean", TRUE, TRUE);
   *  EEL_CHECK_BOOLEAN_RESULT (nautilus_file_get_boolean_metadata (file,
   * "test_boolean", TRUE), TRUE); nautilus_file_set_boolean_metadata (file,
   * "test_boolean", TRUE, FALSE); EEL_CHECK_BOOLEAN_RESULT
   * (nautilus_file_get_boolean_metadata (file, "test_boolean", TRUE), FALSE);
   *  EEL_CHECK_BOOLEAN_RESULT (nautilus_file_get_boolean_metadata (NULL,
   * "test_boolean", TRUE), TRUE);
   *
   *  nautilus_file_set_integer_metadata (file, "test_integer", 0, 17);
   *  EEL_CHECK_INTEGER_RESULT (nautilus_file_get_integer_metadata (file,
   * "test_integer", 0), 17); nautilus_file_set_integer_metadata (file,
   * "test_integer", 0, -1); EEL_CHECK_INTEGER_RESULT
   * (nautilus_file_get_integer_metadata (file, "test_integer", 0), -1);
   *  nautilus_file_set_integer_metadata (file, "test_integer", 42, 42);
   *  EEL_CHECK_INTEGER_RESULT (nautilus_file_get_integer_metadata (file,
   * "test_integer", 42), 42); EEL_CHECK_INTEGER_RESULT
   * (nautilus_file_get_integer_metadata (NULL, "test_integer", 42), 42);
   *  EEL_CHECK_INTEGER_RESULT (nautilus_file_get_integer_metadata (file,
   * "nonexistent_key", 42), 42);
   */

  EEL_CHECK_BOOLEAN_RESULT(
      nautilus_directory_get_by_uri("file:///etc") == directory, TRUE);
  nautilus_directory_unref(directory);
  ...
  nautilus_directory_call_when_ready(directory,
                                     ...
                                     TRUE, got_files_callback, &data_dummy);
