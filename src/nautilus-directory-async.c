...
typedef struct {
  NautilusFile *file; /* Which file, NULL means all. */
  gboolean
      monitor_hidden_files; /* defines whether "all" includes hidden files */
...
static gboolean request_is_satisfied(NautilusDirectory *directory,
                                     NautilusFile *file, Request request);
...
static gboolean istr_equal(gconstpointer v, gconstpointer v2) {
...
static void add_istr_to_list(gpointer key, gpointer value,
                             gpointer callback_data) {
  GList **list;
...
static void istr_set_destroy(GHashTable *table) { g_hash_table_destroy(table); }

static void request_counter_add_request(RequestCounter counter,
                                        Request request) {
  guint i;
...
static void request_counter_remove_request(RequestCounter counter,
                                           Request request) {
  guint i;
...
static void get_one_value_callback(gpointer key, gpointer value,
                                   gpointer callback_data) {
...
static void deep_count_cancel(NautilusDirectory *directory) {
  if (directory->details->deep_count_in_progress != NULL) {
    ...
    g_cancellable_cancel(
        directory->details->deep_count_in_progress->cancellable);
...
static void mime_list_cancel(NautilusDirectory *directory) {
  if (directory->details->mime_list_in_progress != NULL) {
    g_cancellable_cancel(
        directory->details->mime_list_in_progress->cancellable);
  }
}
...
static void new_files_cancel(NautilusDirectory *directory) {
  ...
  if (directory->details->new_files_in_progress != NULL) {
    for (l = directory->details->new_files_in_progress; l != NULL;
         l = l->next) {
      state = l->data;
...
static Monitor *find_monitor(NautilusDirectory *directory, NautilusFile *file,
                             gconstpointer client) {
  GList *l;
...
static gboolean insert_new_monitor(NautilusDirectory *directory,
                                   Monitor *monitor) {
  GList *list;
  ...
  if (list == NULL) {
    ...
    g_hash_table_insert(directory->details->monitor_table, monitor->file, list);
  } else {
    list = g_list_append(list, monitor);
  }
...
static Monitor *remove_monitor_from_table(NautilusDirectory *directory,
                                          NautilusFile *file,
                                          gconstpointer client) {
...
static void remove_monitor(NautilusDirectory *directory, NautilusFile *file,
                           gconstpointer client) {
  Monitor *monitor;
...
nautilus_directory_set_up_request(NautilusFileAttributes file_attributes) {
  ...
  if ((file_attributes & NAUTILUS_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES) !=
      0) {
...
void nautilus_directory_monitor_add_internal(
    NautilusDirectory *directory, NautilusFile *file, gconstpointer client,
    gboolean monitor_hidden_files, NautilusFileAttributes file_attributes,
    NautilusDirectoryCallback callback, gpointer callback_data) {
  Monitor *monitor;
  GList *file_list;
  char *file_uri = NULL;
  char *dir_uri = NULL;
  ...
  nautilus_profile_start("uri %s file-uri %s client %p", dir_uri, file_uri,
                         client);
  g_free(dir_uri);
  g_free(file_uri);
  ...
  if (directory->details->monitor == NULL) {
    directory->details->monitor =
        nautilus_monitor_directory(directory->details->location);
  }
  ...
  if (REQUEST_WANTS_TYPE(monitor->request, REQUEST_FILE_INFO) &&
    ...
    directory->details->mime_db_monitor = g_signal_connect_object(
        nautilus_signaller_get_current(), "mime-data-changed",
...
static gboolean should_skip_file(NautilusDirectory *directory,
                                 GFileInfo *info) {
...
static gboolean dequeue_pending_idle_callback(gpointer callback_data) {
  ...
  nautilus_profile_start("nitems %d",
                         g_list_length(directory->details->pending_file_info));
  ...
  if (directory->details->directory_loaded &&
    ...
    if (dir_load_state) {
      ...
      file->details->mime_list =
          istr_set_get_as_list(dir_load_state->load_mime_list_hash);
...
static void directory_load_one(NautilusDirectory *directory, GFileInfo *info) {
  ...
  if (g_file_info_get_name(info) == NULL) {
    ...
    uri = nautilus_directory_get_uri(directory);
    g_warning("Got GFileInfo with NULL name in %s, ignoring. This shouldn't "
              "happen unless the gvfs backend is broken.\n",
              uri);
    g_free(uri);
...
static void directory_load_done(NautilusDirectory *directory, GError *error) {
  ...
  if (error != NULL) {
     ...
     */
    for (node = directory->details->file_list; node != NULL;
         node = node->next) {
...
static int ready_callback_key_compare(gconstpointer a, gconstpointer b) {
  ...
  if (callback_a->file == NULL) {
    /* ANSI C doesn't allow ordered compares of function pointers, so we cast
     * them to normal pointers to make some overly pedantic compilers (*cough*
     * HP-UX *cough*) compile this. Of course, on any compiler where ordered
     * function pointers actually break this probably won't work, but at least
     * it will compile on platforms where it works, but stupid compilers won't
     * let you use it.
     */
    if ((void *)callback_a->callback.directory <
        (void *)callback_b->callback.directory) {
      return -1;
    }
    if ((void *)callback_a->callback.directory >
        (void *)callback_b->callback.directory) {
...
static int ready_callback_key_compare_only_active(gconstpointer a,
                                                  gconstpointer b) {
...
static void ready_callback_call(NautilusDirectory *directory,
  ...
  if (callback->file != NULL) {
    ...
    (*callback->callback.directory)(directory, file_list,
...
void nautilus_directory_call_when_ready_internal(
    NautilusDirectory *directory, NautilusFile *file,
    NautilusFileAttributes file_attributes, gboolean wait_for_file_list,
  ...
  if (g_list_find_custom(directory->details->call_when_ready_list, &callback,
  ...
  directory->details->call_when_ready_list =
      g_list_prepend(directory->details->call_when_ready_list,
...
gboolean nautilus_directory_check_if_ready_internal(
    NautilusDirectory *directory, NautilusFile *file,
    NautilusFileAttributes file_attributes) {
  Request request;
...
static void remove_callback_link_keep_data(NautilusDirectory *directory,
                                           GList *link) {
  ReadyCallback *callback;
  ...
  directory->details->call_when_ready_list =
      g_list_remove_link(directory->details->call_when_ready_list, link);
...
void nautilus_directory_cancel_callback_internal(
    NautilusDirectory *directory, NautilusFile *file,
    NautilusDirectoryCallback directory_callback,
    NautilusFileCallback file_callback, gpointer callback_data) {
  ReadyCallback callback;
  ...
  do {
    node = g_list_find_custom(directory->details->call_when_ready_list,
                              &callback, ready_callback_key_compare);
    if (node != NULL) {
...
static void new_files_state_unref(NewFilesState *state) {
  ...
  if (state->count == 0) {
    if (state->directory) {
      state->directory->details->new_files_in_progress = g_list_remove(
          state->directory->details->new_files_in_progress, state);
    }
...
static void new_files_callback(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
  NautilusDirectory *directory;
  GFileInfo *info;
  NewFilesState *state;
...
void nautilus_directory_get_info_for_new_files(NautilusDirectory *directory,
  ...
  for (l = location_list; l != NULL; l = l->next) {
    ...
    g_file_query_info_async(location, NAUTILUS_FILE_DEFAULT_ATTRIBUTES, 0,
                            G_PRIORITY_DEFAULT, state->cancellable,
                            new_files_callback, state);
  }

  directory->details->new_files_in_progress =
      g_list_prepend(directory->details->new_files_in_progress, state);
}
...
void nautilus_async_destroying_file(NautilusFile *file) {
  ...
  /* Check for callbacks. */
  for (node = directory->details->call_when_ready_list; node != NULL;
       node = next) {
    next = node->next;
    callback = node->data;
...
static gboolean has_problem(NautilusDirectory *directory, NautilusFile *file,
                            FileCheck problem) {
  GList *node;
...
static gboolean call_ready_callbacks(NautilusDirectory *directory) {
  ...
  /* Check if any callbacks are satisifed and mark them for call them if they
   * are. */
  for (node = directory->details->call_when_ready_list; node != NULL;
       node = next) {
    next = node->next;
...
static GList *lookup_monitors(GHashTable *monitor_table, NautilusFile *file) {
...
static GList *lookup_all_files_monitors(GHashTable *monitor_table) {
...
gboolean nautilus_directory_is_anyone_monitoring_file_list(
    NautilusDirectory *directory) {
...
static void more_files_callback(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  DirectoryLoadState *state;
  NautilusDirectory *directory;
  GError *error;
  ...
  if (files == NULL) {
    ...
    g_file_enumerator_next_files_async(
        state->enumerator, DIRECTORY_LOAD_ITEMS_PER_CALLBACK,
        G_PRIORITY_DEFAULT, state->cancellable, more_files_callback, state);
  }
...
static void start_monitoring_file_list(NautilusDirectory *directory) {
  ...
  DEBUG("load_directory called to monitor file list of %p",
        directory->details->location);

  directory->details->directory_load_in_progress = state;

  g_file_enumerate_children_async(
      directory->details->location, NAUTILUS_FILE_DEFAULT_ATTRIBUTES,
      0,                  /* flags */
      G_PRIORITY_DEFAULT, /* prio */
      state->cancellable, enumerate_children_callback, state);
}

/* Stop monitoring the file list if it is being monitored. */
void nautilus_directory_stop_monitoring_file_list(
    NautilusDirectory *directory) {
  if (!directory->details->file_list_monitored) {
    g_assert(directory->details->directory_load_in_progress == NULL);
    return;
 ...
 */
void nautilus_directory_invalidate_count_and_mime_list(
    NautilusDirectory *directory) {
  NautilusFile *file;
...
void nautilus_directory_force_reload_internal(
    NautilusDirectory *directory, NautilusFileAttributes file_attributes) {
  nautilus_profile_start(NULL);
...
static gboolean monitor_includes_file(const Monitor *monitor,
                                      NautilusFile *file) {
  ...
  return nautilus_file_should_show(file, monitor->monitor_hidden_files);
}

static gboolean is_wanted_by_monitor(NautilusFile *file, GList *monitors,
...
static gboolean is_needy(NautilusFile *file, FileCheck check_missing,
                         RequestType request_type_wanted) {
  NautilusDirectory *directory;
  GList *node;
  ReadyCallback *callback;
...
static void directory_count_stop(NautilusDirectory *directory) {
  ...
  if (directory->details->count_in_progress != NULL) {
    ...
    if (file != NULL) {
      g_assert(NAUTILUS_IS_FILE(file));
      g_assert(file->details->directory == directory);
      if (is_needy(file, should_get_directory_count_now,
...
static void count_children_done(NautilusDirectory *directory,
                                NautilusFile *count_file, gboolean succeeded,
                                int count) {
...
static void count_more_files_callback(GObject *source_object, GAsyncResult *res,
                                      gpointer user_data) {
  ...
  if (files == NULL) {
    ...
    g_file_enumerator_next_files_async(state->enumerator,
                                       ...
                                       G_PRIORITY_DEFAULT, state->cancellable,
                                       count_more_files_callback, state);
  }
...
static void count_children_callback(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  DirectoryCountState *state;
  GFileEnumerator *enumerator;
  NautilusDirectory *directory;
...
static void directory_count_start(NautilusDirectory *directory,
                                  NautilusFile *file, gboolean *doing_io) {
  DirectoryCountState *state;
  GFile *location;
  ...
  *doing_io = TRUE;
  ...
  g_file_enumerate_children_async(
      ...
      G_PRIORITY_DEFAULT,                  /* prio */
      state->cancellable, count_children_callback, state);
...
static void deep_count_next_dir(DeepCountState *state) {
  ...
  if (state->deep_count_subdirectories != NULL) {
    ...
    state->deep_count_subdirectories =
        g_list_remove(state->deep_count_subdirectories, location);
...
static void deep_count_more_files_callback(GObject *source_object,
                                           GAsyncResult *res,
                                           gpointer user_data) {
  ...
  if (files == NULL) {
    ...
    g_file_enumerator_next_files_async(
        state->enumerator, DIRECTORY_LOAD_ITEMS_PER_CALLBACK, G_PRIORITY_LOW,
...
static void deep_count_callback(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  DeepCountState *state;
  GFileEnumerator *enumerator;
  NautilusFile *file;
...
static void deep_count_load(DeepCountState *state, GFile *location) {
  ...
  g_file_enumerate_children_async(
      state->deep_count_location,
      G_FILE_ATTRIBUTE_STANDARD_NAME
      "," G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_STANDARD_SIZE
      "," G_FILE_ATTRIBUTE_STANDARD_IS_HIDDEN
      "," G_FILE_ATTRIBUTE_STANDARD_IS_BACKUP "," G_FILE_ATTRIBUTE_ID_FILESYSTEM
      "," G_FILE_ATTRIBUTE_UNIX_INODE,
      G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, /* flags */
      G_PRIORITY_LOW,                      /* prio */
      state->cancellable, deep_count_callback, state);
}
...
static void deep_count_got_info(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  GFileInfo *info;
  const char *id;
...
static void deep_count_start(NautilusDirectory *directory, NautilusFile *file,
                             gboolean *doing_io) {
  GFile *location;
  DeepCountState *state;
  ...
  *doing_io = TRUE;
  ...
  location = nautilus_file_get_location(file);
  g_file_query_info_async(location, G_FILE_ATTRIBUTE_ID_FILESYSTEM,
                          G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                          G_PRIORITY_DEFAULT, NULL, deep_count_got_info, state);
  g_object_unref(location);
}
...
static void mime_list_callback(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
  MimeListState *state;
  NautilusDirectory *directory;
  GError *error;
  ...
  if (files == NULL) {
    ...
    g_file_enumerator_next_files_async(
        state->enumerator, DIRECTORY_LOAD_ITEMS_PER_CALLBACK,
        G_PRIORITY_DEFAULT, state->cancellable, mime_list_callback, state);
  }
...
static void list_mime_enum_callback(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  MimeListState *state;
  GFileEnumerator *enumerator;
  NautilusDirectory *directory;
...
static void mime_list_start(NautilusDirectory *directory, NautilusFile *file,
                            gboolean *doing_io) {
  MimeListState *state;
  GFile *location;
  ...
  *doing_io = TRUE;
  ...
  g_file_enumerate_children_async(
      location, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, 0, /* flags */
      G_PRIORITY_LOW,                                      /* prio */
      state->cancellable, list_mime_enum_callback, state);
  g_object_unref(location);
}
...
static void query_info_callback(GObject *source_object, GAsyncResult *res,
                                gpointer user_data) {
  NautilusDirectory *directory;
...
static void file_info_start(NautilusDirectory *directory, NautilusFile *file,
                            gboolean *doing_io) {
  GFile *location;
  GetInfoState *state;
  ...
  *doing_io = TRUE;
  ...
  g_file_query_info_async(location, NAUTILUS_FILE_DEFAULT_ATTRIBUTES, 0,
                          G_PRIORITY_DEFAULT, state->cancellable,
                          query_info_callback, state);
  g_object_unref(location);
}

static void thumbnail_done(NautilusDirectory *directory, NautilusFile *file,
                           GdkPixbuf *pixbuf) {
...
static void thumbnail_got_pixbuf(NautilusDirectory *directory,
                                 NautilusFile *file, GdkPixbuf *pixbuf) {
  nautilus_directory_ref(directory);
...
static void thumbnail_loader_size_prepared(GdkPixbufLoader *loader, int width,
  ...
  max_thumbnail_size = NAUTILUS_GRID_ICON_SIZE_LARGEST *
...
static GdkPixbuf *get_pixbuf_for_content(goffset file_len,
                                         char *file_contents) {
  gboolean res;
  ...
  g_signal_connect(loader, "size-prepared",
                   G_CALLBACK(thumbnail_loader_size_prepared), NULL);

  /* For some reason we have to write in chunks, or gdk-pixbuf fails */
  res = TRUE;
  while (res && file_len > 0) {
    chunk_len = file_len;
    res = gdk_pixbuf_loader_write(loader, (guchar *)file_contents, chunk_len,
                                  NULL);
...
static void thumbnail_read_callback(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  ...
  result = g_file_load_contents_finish(G_FILE(source_object), res,
                                       &file_contents, &file_size, NULL, NULL);
...
static void thumbnail_start(NautilusDirectory *directory, NautilusFile *file,
                            gboolean *doing_io) {
  GFile *location;
  ThumbnailState *state;
...
static void find_enclosing_mount_callback(GObject *source_object,
                                          GAsyncResult *res,
                                          gpointer user_data) {
  GMount *mount;
  MountState *state;
...
static void mount_start(NautilusDirectory *directory, NautilusFile *file,
                        gboolean *doing_io) {
  GFile *location;
  MountState *state;
  ...
  *doing_io = TRUE;
  ...
  if (file->details->type == G_FILE_TYPE_MOUNTABLE) {
    ...
    g_file_find_enclosing_mount_async(location, G_PRIORITY_DEFAULT,
                                      state->cancellable,
                                      find_enclosing_mount_callback, state);
  }
  g_object_unref(location);
}
...
static void filesystem_info_cancel(NautilusDirectory *directory) {
  if (directory->details->filesystem_info_state != NULL) {
    g_cancellable_cancel(
        directory->details->filesystem_info_state->cancellable);
...
static void got_filesystem_info(FilesystemInfoState *state, GFileInfo *info) {
  ...
  if (info != NULL) {
    file->details->filesystem_use_preview = g_file_info_get_attribute_uint32(
        info, G_FILE_ATTRIBUTE_FILESYSTEM_USE_PREVIEW);
    file->details->filesystem_readonly = g_file_info_get_attribute_boolean(
        info, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY);
    filesystem_type = g_file_info_get_attribute_string(
        info, G_FILE_ATTRIBUTE_FILESYSTEM_TYPE);
    file->details->filesystem_remote = g_file_info_get_attribute_boolean(
...
static void filesystem_info_start(NautilusDirectory *directory,
                                  NautilusFile *file, gboolean *doing_io) {
  GFile *location;
  FilesystemInfoState *state;
  ...
  *doing_io = TRUE;
  ...
  g_file_query_filesystem_info_async(location,
                                     ...
                                     G_PRIORITY_DEFAULT, state->cancellable,
                                     query_filesystem_info_callback, state);
  g_object_unref(location);
}
...
static void finish_info_provider(NautilusDirectory *directory,
                                 NautilusFile *file,
                                 NautilusInfoProvider *provider) {
  file->details->pending_info_providers =
      g_list_remove(file->details->pending_info_providers, provider);
...
static gboolean info_provider_idle_callback(gpointer user_data) {
  ...
  if (response->handle != directory->details->extension_info_in_progress ||
    ...
    g_warning("Unexpected plugin response.  This probably indicates a bug in a "
              "Nautilus extension: handle=%p",
              response->handle);
  } else {
    NautilusFile *file;
    async_job_end(directory, "extension info");
...
static void info_provider_callback(NautilusInfoProvider *provider,
                                   NautilusOperationHandle *handle,
                                   NautilusOperationResult result,
                                   gpointer user_data) {
  InfoProviderResponse *response;
  ...
  response->directory->details->extension_info_idle = g_idle_add_full(
      G_PRIORITY_DEFAULT_IDLE, info_provider_idle_callback, response, g_free);
}

static void extension_info_start(NautilusDirectory *directory,
                                 NautilusFile *file, gboolean *doing_io) {
  NautilusInfoProvider *provider;
  NautilusOperationResult result;
  NautilusOperationHandle *handle;
  ...
  *doing_io = TRUE;
  ...
  update_complete =
      g_cclosure_new(G_CALLBACK(info_provider_callback), directory, NULL);
  g_closure_set_marshal(update_complete, g_cclosure_marshal_generic);

  result = nautilus_info_provider_update_file_info(
      provider, NAUTILUS_FILE_INFO(file), update_complete, &handle);
...
static void start_or_stop_io(NautilusDirectory *directory) {
  ...
  while (
      !nautilus_file_queue_is_empty(directory->details->high_priority_queue)) {
  ...
  while (
      !nautilus_file_queue_is_empty(directory->details->low_priority_queue)) {
...
static void cancel_directory_count_for_file(NautilusDirectory *directory,
                                            NautilusFile *file) {
  if (directory->details->count_in_progress != NULL &&
      directory->details->count_in_progress->count_file == file) {
    directory_count_cancel(directory);
...
static void cancel_deep_counts_for_file(NautilusDirectory *directory,
                                        NautilusFile *file) {
  if (directory->details->deep_count_file == file) {
    deep_count_cancel(directory);
  }
...
static void cancel_mime_list_for_file(NautilusDirectory *directory,
                                      NautilusFile *file) {
  if (directory->details->mime_list_in_progress != NULL &&
      directory->details->mime_list_in_progress->mime_list_file == file) {
    mime_list_cancel(directory);
...
static void cancel_file_info_for_file(NautilusDirectory *directory,
                                      NautilusFile *file) {
  if (directory->details->get_info_file == file) {
    file_info_cancel(directory);
  }
...
static void cancel_thumbnail_for_file(NautilusDirectory *directory,
                                      NautilusFile *file) {
  if (directory->details->thumbnail_state != NULL &&
      directory->details->thumbnail_state->file == file) {
    thumbnail_cancel(directory);
...
static void cancel_mount_for_file(NautilusDirectory *directory,
                                  NautilusFile *file) {
  if (directory->details->mount_state != NULL &&
      directory->details->mount_state->file == file) {
    mount_cancel(directory);
...
static void cancel_filesystem_info_for_file(NautilusDirectory *directory,
                                            NautilusFile *file) {
  if (directory->details->filesystem_info_state != NULL &&
      directory->details->filesystem_info_state->file == file) {
    filesystem_info_cancel(directory);
...
void nautilus_directory_remove_file_from_work_queue(
    NautilusDirectory *directory, NautilusFile *file) {
