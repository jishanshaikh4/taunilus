...
static GType nautilus_view_model_get_item_type(GListModel *list) {
    return NAUTILUS_TYPE_VIEW_ITEM_MODEL;
}

static guint nautilus_view_model_get_n_items(GListModel *list) {
...
static gpointer nautilus_view_model_get_item(GListModel *list, guint position) {
    NautilusViewModel *self = NAUTILUS_VIEW_MODEL(list);
...
static void nautilus_view_model_list_model_init(GListModelInterface *iface) {
...
G_DEFINE_TYPE_WITH_CODE(
    NautilusViewModel, nautilus_view_model, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_LIST_MODEL,
                          nautilus_view_model_list_model_init))

enum { PROP_0, PROP_SORT_TYPE, N_PROPS };
...
static void dispose(GObject *object) {
    ...
    if (self->internal_model != NULL) {
        g_signal_handlers_disconnect_by_func(self->internal_model,
                                             g_list_model_items_changed, self);
...
static void nautilus_view_model_class_init(NautilusViewModelClass *klass) {
...
static void nautilus_view_model_init(NautilusViewModel *self) {}

static gint compare_data_func(gconstpointer a, gconstpointer b,
                              gpointer user_data) {
    ...
    file_a =
        nautilus_view_item_model_get_file(NAUTILUS_VIEW_ITEM_MODEL((gpointer)a));
    file_b =
        nautilus_view_item_model_get_file(NAUTILUS_VIEW_ITEM_MODEL((gpointer)b));

    return nautilus_file_compare_for_sort(
               file_a, file_b, self->sort_data->sort_type,
               self->sort_data->directories_first, self->sort_data->reversed);
}

NautilusViewModel *nautilus_view_model_new() {
...
GQueue *nautilus_view_model_get_items_from_files(NautilusViewModel *self,
    ...
    for (l = g_queue_peek_head_link(files); l != NULL; l = l->next) {
        ...
        while ((item_model =
                    g_list_model_get_item(G_LIST_MODEL(self->internal_model), i))) {
            NautilusFile *file2;
...
void nautilus_view_model_add_item(NautilusViewModel *self,
    ...
    g_hash_table_insert(self->map_files_to_model,
                        nautilus_view_item_model_get_file(item), item);
    g_list_store_insert_sorted(self->internal_model, item, compare_data_func,
                               self);
}

void nautilus_view_model_add_items(NautilusViewModel *self, GQueue *items) {
    g_autofree gpointer *array = NULL;
    GList *l;
    int i = 0;

    array =
        g_malloc_n(g_queue_get_length(items), sizeof(NautilusViewItemModel *));

    for (l = g_queue_peek_head_link(items); l != NULL; l = l->next) {
        array[i] = l->data;
        g_hash_table_insert(self->map_files_to_model,
                            nautilus_view_item_model_get_file(l->data), l->data);
        i++;
    }

    g_list_store_splice(
        self->internal_model,
        g_list_model_get_n_items(G_LIST_MODEL(self->internal_model)), 0, array,
        g_queue_get_length(items));
