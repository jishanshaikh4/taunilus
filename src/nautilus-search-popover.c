...
enum { PROP_0, PROP_QUERY, PROP_FTS_ENABLED, LAST_PROP };

enum { MIME_TYPE, TIME_TYPE, DATE_RANGE, LAST_SIGNAL };
...
/* Range on dates are partially implemented. For now just use it for
 * differentation between a exact day or a range of a first day until now.
 */
static void setup_date(NautilusSearchPopover *popover, NautilusQuery *query) {
    ...
    if (date_range) {
        ...
        g_signal_handlers_block_by_func(popover->calendar, calendar_day_selected,
                                        popover);
        ...
        g_signal_handlers_unblock_by_func(popover->calendar, calendar_day_selected,
                                          popover);
    }
}

static void query_date_changed(GObject *object, GParamSpec *pspec,
                               NautilusSearchPopover *popover) {
    setup_date(popover, NAUTILUS_QUERY(object));
}

static void clear_date_button_clicked(GtkButton *button,
                                      NautilusSearchPopover *popover) {
    nautilus_search_popover_reset_date_range(popover);
}

static void date_entry_activate(GtkEntry *entry,
                                NautilusSearchPopover *popover) {
    if (gtk_entry_get_text_length(entry) > 0) {
        ...
        date_time =
            g_date_time_new_local(g_date_get_year(date), g_date_get_month(date),
                                  g_date_get_day(date), 0, 0, 0);
...
static void dates_listbox_row_activated(GtkListBox *listbox, GtkListBoxRow *row,
                                        NautilusSearchPopover *popover) {
    GDateTime *date;
    GDateTime *now;
...
static void listbox_header_func(GtkListBoxRow *row, GtkListBoxRow *before,
    ...
    show_separator =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(row), "show-separator"));
...
static void select_date_button_clicked(GtkButton *button,
                                       NautilusSearchPopover *popover) {
    /* Hide the type selection widgets when date selection
     * widgets are shown.
     */
    gtk_stack_set_visible_child_name(GTK_STACK(popover->type_stack),
                                     "type-button");

    show_date_selection_widgets(popover, TRUE);
}

static void select_type_button_clicked(GtkButton *button,
                                       NautilusSearchPopover *popover) {
    GtkListBoxRow *selected_row;

    selected_row =
        gtk_list_box_get_selected_row(GTK_LIST_BOX(popover->type_listbox));
...
static void toggle_calendar_icon_clicked(GtkEntry *entry,
    ...
    current_visible_child =
        gtk_stack_get_visible_child_name(GTK_STACK(popover->around_stack));
...
static void types_listbox_row_activated(GtkListBox *listbox, GtkListBoxRow *row,
                                        NautilusSearchPopover *popover) {
    gint group;
...
static void search_time_type_changed(GtkToggleButton *button,
                                     NautilusSearchPopover *popover) {
    NautilusQuerySearchType type = -1;

    if (gtk_toggle_button_get_active(
                GTK_TOGGLE_BUTTON(popover->last_modified_button))) {
        type = NAUTILUS_QUERY_SEARCH_TYPE_LAST_MODIFIED;
    } else if (gtk_toggle_button_get_active(
                   GTK_TOGGLE_BUTTON(popover->last_used_button))) {
...
static void search_fts_mode_changed(GtkToggleButton *button,
                                    NautilusSearchPopover *popover) {
    if (gtk_toggle_button_get_active(
                GTK_TOGGLE_BUTTON(popover->full_text_search_button)) &&
            popover->fts_enabled == FALSE) {
        popover->fts_enabled = TRUE;
        g_settings_set_boolean(nautilus_preferences,
                               NAUTILUS_PREFERENCES_FTS_ENABLED, TRUE);
        g_object_notify(G_OBJECT(popover), "fts-enabled");
    } else if (gtk_toggle_button_get_active(
                   GTK_TOGGLE_BUTTON(popover->filename_search_button)) &&
               popover->fts_enabled == TRUE) {
        popover->fts_enabled = FALSE;
        g_settings_set_boolean(nautilus_preferences,
                               NAUTILUS_PREFERENCES_FTS_ENABLED, FALSE);
        g_object_notify(G_OBJECT(popover), "fts-enabled");
    }
}
...
static GtkWidget *create_row_for_label(const gchar *text,
                                       gboolean show_separator) {
    GtkWidget *row;
    GtkWidget *label;
    ...
    g_object_set_data(G_OBJECT(row), "show-separator",
                      GINT_TO_POINTER(show_separator));

    label = g_object_new(GTK_TYPE_LABEL, "label", text, "hexpand", TRUE, "xalign",
                         0.0, "margin-start", 6, NULL);
...
static void fill_fuzzy_dates_listbox(NautilusSearchPopover *popover) {
    ...
    while (days <= max_days) {
        ...
        g_object_set_data_full(G_OBJECT(row), "date", g_date_time_ref(current_date),
...
static void fill_types_listbox(NautilusSearchPopover *popover) {
    GtkWidget *row;
    int i;
    gint n_groups;
...
static void show_date_selection_widgets(NautilusSearchPopover *popover,
    ...
    gtk_stack_set_visible_child_name(GTK_STACK(popover->around_stack),
                                     "date-list");
    ...
    gtk_revealer_set_reveal_child(GTK_REVEALER(popover->around_revealer),
                                  visible);
}

static void on_other_types_dialog_response(GtkDialog *dialog, gint response_id,
        NautilusSearchPopover *popover) {
    if (response_id == GTK_RESPONSE_OK) {
        ...
        gtk_tree_model_get(store, &iter, 0, &description, 1, &mimetype, -1);
...
static void show_other_types_dialog(NautilusSearchPopover *popover) {
    ...
    dialog = gtk_dialog_new_with_buttons(
                 _("Select type"), GTK_WINDOW(toplevel),
                 GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT |
                 GTK_DIALOG_USE_HEADER_BAR,
                 _("_Cancel"), GTK_RESPONSE_CANCEL, _("Select"), GTK_RESPONSE_OK, NULL);
    ...
    column = gtk_tree_view_column_new_with_attributes("Name", renderer, "text", 0,
             NULL);
...
static void nautilus_search_popover_closed(GtkPopover *popover) {
    ...
    g_signal_handlers_unblock_by_func(self->calendar, calendar_day_selected,
                                      self);
}

static void nautilus_search_popover_get_property(GObject *object, guint prop_id,
        GValue *value,
        GParamSpec *pspec) {
...
static void nautilus_search_popover_set_property(GObject *object, guint prop_id,
        const GValue *value,
        GParamSpec *pspec) {
    NautilusSearchPopover *self;
...
nautilus_search_popover_class_init(NautilusSearchPopoverClass *klass) {
    ...
    signals[DATE_RANGE] = g_signal_new(
                              "date-range", NAUTILUS_TYPE_SEARCH_POPOVER, G_SIGNAL_RUN_LAST, 0, NULL,
                              NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 1, G_TYPE_PTR_ARRAY);

    signals[MIME_TYPE] =
        g_signal_new("mime-type", NAUTILUS_TYPE_SEARCH_POPOVER, G_SIGNAL_RUN_LAST,
                     0, NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 2,
                     G_TYPE_INT, G_TYPE_STRING);

    signals[TIME_TYPE] = g_signal_new(
                             "time-type", NAUTILUS_TYPE_SEARCH_POPOVER, G_SIGNAL_RUN_LAST, 0, NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("query", "Query of the popover",
                            "The current query being edited", NAUTILUS_TYPE_QUERY,
                            G_PARAM_READWRITE));

    g_object_class_install_property(
        object_class, PROP_FTS_ENABLED,
        g_param_spec_boolean("fts-enabled", "fts enabled", "fts enabled", FALSE,
                             G_PARAM_READWRITE));

    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/gnome/nautilus/ui/nautilus-search-popover.ui");

    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         around_revealer);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         around_stack);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
    ...
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         last_used_button);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         last_modified_button);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         created_button);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         full_text_search_button);
    gtk_widget_class_bind_template_child(widget_class, NautilusSearchPopover,
                                         filename_search_button);

    gtk_widget_class_bind_template_callback(widget_class, calendar_day_selected);
    gtk_widget_class_bind_template_callback(widget_class,
                                            clear_date_button_clicked);
    gtk_widget_class_bind_template_callback(widget_class, date_entry_activate);
    gtk_widget_class_bind_template_callback(widget_class,
                                            dates_listbox_row_activated);
    gtk_widget_class_bind_template_callback(widget_class,
                                            select_date_button_clicked);
    gtk_widget_class_bind_template_callback(widget_class,
                                            select_type_button_clicked);
    gtk_widget_class_bind_template_callback(widget_class,
                                            toggle_calendar_icon_clicked);
    gtk_widget_class_bind_template_callback(widget_class,
                                            types_listbox_row_activated);
    gtk_widget_class_bind_template_callback(widget_class,
                                            search_time_type_changed);
    gtk_widget_class_bind_template_callback(widget_class,
                                            search_fts_mode_changed);
}

static void nautilus_search_popover_init(NautilusSearchPopover *self) {
    ...
    gtk_list_box_select_row(
        GTK_LIST_BOX(self->type_listbox),
        gtk_list_box_get_row_at_index(GTK_LIST_BOX(self->type_listbox), 0));

    filter_time_type =
        g_settings_get_enum(nautilus_preferences, "search-filter-time-type");
    if (filter_time_type == NAUTILUS_QUERY_SEARCH_TYPE_LAST_MODIFIED) {
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(self->last_modified_button),
                                     TRUE);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(self->last_used_button),
                                     FALSE);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(self->created_button),
                                     FALSE);
    ...
    if (self->fts_enabled) {
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(self->full_text_search_button), TRUE);
    } else {
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(self->filename_search_button), TRUE);
    }
}
...
void nautilus_search_popover_set_query(NautilusSearchPopover *popover,
    ...
    if (popover->query != query) {
        ...
        if (previous_query) {
            g_signal_handlers_disconnect_by_func(previous_query, query_date_changed,
                                                 popover);
        }
        ...
        if (query) {
            ...
            g_signal_connect(query, "notify::date", G_CALLBACK(query_date_changed),
                             popover);
        } else {
...
void nautilus_search_popover_reset_mime_types(NautilusSearchPopover *popover) {
    ...
    gtk_list_box_select_row(
        GTK_LIST_BOX(popover->type_listbox),
...
void nautilus_search_popover_reset_date_range(NautilusSearchPopover *popover) {
    ...
    gtk_list_box_select_row(
        GTK_LIST_BOX(popover->dates_listbox),
