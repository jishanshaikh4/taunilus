/*
 * nautilus-progress-persistence-handler.c: file operation progress notification
 * handler.
 *
...
G_DEFINE_TYPE(NautilusProgressPersistenceHandler,
              nautilus_progress_persistence_handler, G_TYPE_OBJECT);
 ...
 * - in the same case, but when the window is not showing, we update the
 * resident notification, changing its message
 * - when one file operation finishes, if it's not the last one, we only update
 * the resident notification's message
...
static void show_file_transfers(NautilusProgressPersistenceHandler *self) {
    GFile *home;
...
static void action_show_file_transfers(GSimpleAction *action,
                                       GVariant *parameter,
                                       gpointer user_data) {
...
static void progress_persistence_handler_update_notification(
    NautilusProgressPersistenceHandler *self) {
    ...
    nautilus_application_send_notification(self->app, "progress", notification);

    g_object_unref(notification);
    g_free(body);
}

void nautilus_progress_persistence_handler_make_persistent(
    NautilusProgressPersistenceHandler *self) {
    GList *windows;
...
static void progress_persistence_handler_show_complete_notification(
    NautilusProgressPersistenceHandler *self) {
    ...
    nautilus_application_send_notification(self->app, "transfer-complete",
                                           complete_notification);

    g_object_unref(complete_notification);
}

static void progress_persistence_handler_hide_notification(
    NautilusProgressPersistenceHandler *self) {
    if (!server_has_persistence()) {
        return;
    }

    nautilus_application_withdraw_notification(self->app, "progress");
}
...
progress_info_finished_cb(NautilusProgressInfo *info,
    ...
    last_active_window =
        gtk_application_get_active_window(GTK_APPLICATION(self->app));
    ...
    if (self->active_infos > 0) {
        ...
        if ((last_active_window == NULL) ||
                !gtk_window_is_active(last_active_window)) {
...
static void handle_new_progress_info(NautilusProgressPersistenceHandler *self,
                                     NautilusProgressInfo *info) {
    GList *windows;
    g_signal_connect(info, "finished", G_CALLBACK(progress_info_finished_cb),
                     self);
...
static void timeout_data_free(TimeoutData *data) {
...
static TimeoutData *timeout_data_new(NautilusProgressPersistenceHandler *self,
                                     NautilusProgressInfo *info) {
    TimeoutData *retval;
...
static gboolean new_op_started_timeout(TimeoutData *data) {
...
static void release_application(NautilusProgressInfo *info,
                                NautilusProgressPersistenceHandler *self) {
    /* release the GApplication hold we acquired */
    g_application_release(g_application_get_default());
}

static void progress_info_started_cb(NautilusProgressInfo *info,
                                     NautilusProgressPersistenceHandler *self) {
    TimeoutData *data;
    ...
    g_signal_connect(info, "finished", G_CALLBACK(release_application), self);
...
static void new_progress_info_cb(NautilusProgressInfoManager *manager,
                                 NautilusProgressInfo *info,
                                 NautilusProgressPersistenceHandler *self) {
    g_signal_connect(info, "started", G_CALLBACK(progress_info_started_cb), self);
}

static void nautilus_progress_persistence_handler_dispose(GObject *obj) {
    NautilusProgressPersistenceHandler *self =
        NAUTILUS_PROGRESS_PERSISTENCE_HANDLER(obj);

    g_clear_object(&self->manager);

    G_OBJECT_CLASS(nautilus_progress_persistence_handler_parent_class)
    ->dispose(obj);
}
...
static gboolean server_has_persistence(void) {
    ...
    result = g_dbus_connection_call_sync(
                 ...
                 "org.freedesktop.Notifications", "GetCapabilities", g_variant_new("()"),
...
static void nautilus_progress_persistence_handler_init(
    NautilusProgressPersistenceHandler *self) {
...
static void nautilus_progress_persistence_handler_class_init(
    NautilusProgressPersistenceHandlerClass *klass) {
    GObjectClass *oclass;
...
nautilus_progress_persistence_handler_new(GObject *app) {
    ...
    g_action_map_add_action_entries(
        ...
        G_N_ELEMENTS(progress_persistence_entries), self);
    return self;
}
