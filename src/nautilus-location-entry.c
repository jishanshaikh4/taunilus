...
enum { NAUTILUS_DND_URI_LIST, NAUTILUS_DND_TEXT_PLAIN, NAUTILUS_DND_NTARGETS };
...
enum { CANCEL, LOCATION_CHANGED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL];

G_DEFINE_TYPE_WITH_PRIVATE(NautilusLocationEntry, nautilus_location_entry,
                           GTK_TYPE_ENTRY);
...
static void emit_location_changed(NautilusLocationEntry *entry) {
    GFile *location;
...
nautilus_location_entry_update_action(NautilusLocationEntry *entry) {
    ...
    if (priv->last_location == NULL) {
        nautilus_location_entry_set_secondary_action(
            entry, NAUTILUS_LOCATION_ENTRY_ACTION_GOTO);
        return;
    }
    ...
    if (g_file_equal(priv->last_location, location)) {
        nautilus_location_entry_set_secondary_action(
            entry, NAUTILUS_LOCATION_ENTRY_ACTION_CLEAR);
    } else {
        nautilus_location_entry_set_secondary_action(
            entry, NAUTILUS_LOCATION_ENTRY_ACTION_GOTO);
    }
...
void nautilus_location_entry_set_location(NautilusLocationEntry *entry,
    ...
    if (!priv->last_location || !g_file_equal(priv->last_location, location)) {
...
static void do_open_windows(OpenWindowsOnDragData *data,
    ...
    if (new_windows_for_extras) {
        ...
        for (i = 1; data->names[i] != NULL; ++i) {
            ...
            nautilus_application_open_location_full(
                NAUTILUS_APPLICATION(g_application_get_default()), location,
...
static void confirm_multiple_windows_cb(GtkDialog *dialog, gint response_id,
                                        gpointer user_data) {
...
static void drag_data_received_callback(GtkWidget *widget,
                                        GdkDragContext *context, int x, int y,
                                        GtkSelectionData *data, guint info,
                                        guint32 time, gpointer callback_data) {
    char **names;
    int name_count;
    GtkWidget *window;
    char *prompt;
    ...
    names =
        g_uri_list_extract_uris((const gchar *)gtk_selection_data_get_data(data));
    ...
    if (name_count > 1) {
        ...
        dialog = eel_show_simple_dialog(window, GTK_MESSAGE_QUESTION, prompt,
                                        detail, _("_Cancel"), _("_OK"), NULL);

        /* calls gtk_drag_finish and do_open_windows, frees op_data */
        g_signal_connect(dialog, "response",
                         G_CALLBACK(confirm_multiple_windows_cb), op_data);
...
static void set_prefix_dimming(GtkCellRenderer *completion_cell,
                               char *user_location) {
...
static gboolean update_completions_store(gpointer callback_data) {
    ...
    if (!g_path_is_absolute(user_location) && uri_scheme == NULL &&
            user_location[0] != '~') {
        is_relative = TRUE;
        absolute_location =
            g_build_filename(priv->current_directory, user_location, NULL);
    } else {
        absolute_location = g_steal_pointer(&user_location);
    }

    completions =
        g_filename_completer_get_completions(priv->completer, absolute_location);
...
static void nautilus_location_entry_dispose(GObject *object) {
    NautilusLocationEntry *entry;
    NautilusLocationEntryPrivate *priv;
...
static void on_has_focus_changed(GObject *object, GParamSpec *pspec,
                                 gpointer user_data) {
    NautilusLocationEntry *entry;
    NautilusLocationEntryPrivate *priv;
...
static void nautilus_location_entry_text_changed(NautilusLocationEntry *entry,
        GParamSpec *pspec) {
    NautilusLocationEntryPrivate *priv;
...
static void nautilus_location_entry_icon_release(GtkEntry *gentry,
...
static void after_text_change(NautilusLocationEntry *self, gboolean insert) {
    NautilusLocationEntryPrivate *priv =
        nautilus_location_entry_get_instance_private(self);
...
static void on_after_insert_text(GtkEditable *editable, const gchar *text,
...
static void on_after_delete_text(GtkEditable *editable, gint start_pos,
...
static void nautilus_location_entry_activate(GtkEntry *entry) {
...
static void nautilus_location_entry_cancel(NautilusLocationEntry *entry) {
    NautilusLocationEntryPrivate *priv;
...
nautilus_location_entry_class_init(NautilusLocationEntryClass *class) {
    ...
    signals[CANCEL] = g_signal_new(
                          ...
                          G_STRUCT_OFFSET(NautilusLocationEntryClass, cancel), NULL, NULL,
                          g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    signals[LOCATION_CHANGED] = g_signal_new(
                                    "location-changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, 0, NULL,
                                    NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 1, G_TYPE_OBJECT);

    binding_set = gtk_binding_set_by_class(class);
    gtk_binding_entry_add_signal(binding_set, GDK_KEY_Escape, 0, "cancel", 0);
}

void nautilus_location_entry_set_secondary_action(
    NautilusLocationEntry *entry,
    NautilusLocationEntryAction secondary_action) {
    NautilusLocationEntryPrivate *priv;
    ...
    case NAUTILUS_LOCATION_ENTRY_ACTION_CLEAR: {
        gtk_entry_set_icon_from_icon_name(
            GTK_ENTRY(entry), GTK_ENTRY_ICON_SECONDARY, "edit-clear-symbolic");
    }
    break;

    case NAUTILUS_LOCATION_ENTRY_ACTION_GOTO: {
        gtk_entry_set_icon_from_icon_name(
            GTK_ENTRY(entry), GTK_ENTRY_ICON_SECONDARY, "go-next-symbolic");
    }
    break;
...
static void editable_activate_callback(GtkEntry *entry, gpointer user_data) {
...
static void editable_changed_callback(GtkEntry *entry, gpointer user_data) {
...
static void nautilus_location_entry_init(NautilusLocationEntry *entry) {
    ...
    g_signal_connect(entry, "notify::has-focus", G_CALLBACK(on_has_focus_changed),
                     NULL);
    ...
    /* Drag dest. */
    gtk_drag_dest_set(GTK_WIDGET(entry), GTK_DEST_DEFAULT_ALL, drop_types,
    ...
    g_signal_connect_object(entry, "activate",
                            G_CALLBACK(editable_activate_callback), entry,
                            G_CONNECT_AFTER);
    ...
    gtk_event_controller_set_propagation_phase(priv->controller,
            GTK_PHASE_BUBBLE);
    g_signal_connect(priv->controller, "key-pressed",
                     G_CALLBACK(nautilus_location_entry_key_pressed), NULL);
    g_signal_connect_after(entry, "insert-text", G_CALLBACK(on_after_insert_text),
                           NULL);
    g_signal_connect_after(entry, "delete-text", G_CALLBACK(on_after_delete_text),
                           NULL);
    ...
    gtk_entry_completion_set_model(priv->completion,
                                   GTK_TREE_MODEL(priv->completions_store));

    g_object_set(priv->completion, "text-column", 0, "inline-completion", FALSE,
                 "inline-selection", TRUE, "popup-single-match", TRUE, NULL);

    priv->completion_cell = gtk_cell_renderer_text_new();
    g_object_set(priv->completion_cell, "xpad", 6, NULL);

    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(priv->completion),
                               priv->completion_cell, FALSE);
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(priv->completion),
                                  priv->completion_cell, "text", 0);
