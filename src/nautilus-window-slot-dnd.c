...
static void switch_tab(NautilusDragSlotProxyInfo *drag_info) {
  ...
  notebook = gtk_widget_get_ancestor(GTK_WIDGET(drag_info->target_slot),
                                     NAUTILUS_TYPE_NOTEBOOK);
...
static void switch_location(NautilusDragSlotProxyInfo *drag_info) {
  GFile *location;
  GtkWidget *window;
  ...
  nautilus_application_open_location_full(
      ...
      NAUTILUS_OPEN_FLAG_DONT_MAKE_ACTIVE, NULL, NAUTILUS_WINDOW(window), NULL);
  g_object_unref(location);
}

static gboolean slot_proxy_switch_location_timer(gpointer user_data) {
...
slot_proxy_check_switch_location_timer(NautilusDragSlotProxyInfo *drag_info,
  ...
  drag_info->switch_location_timer =
      g_timeout_add(HOVER_TIMEOUT, slot_proxy_switch_location_timer, drag_info);
}
...
static gboolean slot_proxy_drag_motion(GtkWidget *widget,
                                       GdkDragContext *context, int x, int y,
                                       unsigned int time, gpointer user_data) {
  ...
  if (drag_info->have_data && drag_info->have_valid_data) {
    if (drag_info->info == NAUTILUS_ICON_DND_GNOME_ICON_LIST) {
      nautilus_drag_default_drop_action_for_icons(
          context, target_uri, drag_info->data.selection_list, 0, &action);
    } else if (drag_info->info == NAUTILUS_ICON_DND_URI_LIST) {
      action =
          nautilus_drag_default_drop_action_for_uri_list(context, target_uri);
...
static void drag_info_free(gpointer user_data) {
  NautilusDragSlotProxyInfo *drag_info = user_data;
...
static void drag_info_clear(NautilusDragSlotProxyInfo *drag_info) {
  slot_proxy_remove_switch_location_timer(drag_info);
...
static void slot_proxy_drag_leave(GtkWidget *widget, GdkDragContext *context,
                                  unsigned int time, gpointer user_data) {
  NautilusDragSlotProxyInfo *drag_info;
...
static gboolean slot_proxy_drag_drop(GtkWidget *widget, GdkDragContext *context,
...
static void slot_proxy_handle_drop(GtkWidget *widget, GdkDragContext *context,
                                   unsigned int time,
                                   NautilusDragSlotProxyInfo *drag_info) {
  GtkWidget *window;
  NautilusWindowSlot *target_slot;
...
static void slot_proxy_drag_data_received(GtkWidget *widget,
                                          GdkDragContext *context, int x, int y,
...
void nautilus_drag_slot_proxy_init(GtkWidget *widget, NautilusFile *target_file,
                                   NautilusWindowSlot *target_slot) {
  NautilusDragSlotProxyInfo *drag_info;
  ...
  drag_info->is_notebook =
      (g_object_get_data(G_OBJECT(widget), "nautilus-notebook-tab") != NULL);
  ...
  g_signal_connect(widget, "drag-motion", G_CALLBACK(slot_proxy_drag_motion),
                   drag_info);
  g_signal_connect(widget, "drag-drop", G_CALLBACK(slot_proxy_drag_drop),
                   drag_info);
  g_signal_connect(widget, "drag-data-received",
                   G_CALLBACK(slot_proxy_drag_data_received), drag_info);
  g_signal_connect(widget, "drag-leave", G_CALLBACK(slot_proxy_drag_leave),
                   drag_info);
}
