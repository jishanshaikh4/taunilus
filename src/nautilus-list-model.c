 ...
 *
 *  Authors: Anders Carlsson <andersca@gnu.org>, Soeren Sandmann
...
enum { SUBDIRECTORY_UNLOADED, GET_ICON_SCALE, LAST_SIGNAL };

static GQuark attribute_name_q, attribute_modification_date_q,
...
typedef struct {
    ...
    GHashTable
    *top_reverse_map; /* map from files in top dir to GSequenceIter's */
...
G_DEFINE_TYPE_WITH_CODE(
    NautilusListModel, nautilus_list_model, G_TYPE_OBJECT,
...
static void file_entry_free(FileEntry *file_entry) {
...
static int nautilus_list_model_get_n_columns(GtkTreeModel *tree_model) {
    NautilusListModelPrivate *priv;

    priv =
        nautilus_list_model_get_instance_private(NAUTILUS_LIST_MODEL(tree_model));

    return NAUTILUS_LIST_MODEL_NUM_COLUMNS + priv->columns->len;
}

static GType nautilus_list_model_get_column_type(GtkTreeModel *tree_model,
        int index) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static void nautilus_list_model_ptr_to_iter(NautilusListModel *model,
        GSequenceIter *ptr,
        GtkTreeIter *iter) {
    NautilusListModelPrivate *priv;
...
static gboolean nautilus_list_model_get_iter(GtkTreeModel *tree_model,
        GtkTreeIter *iter,
        GtkTreePath *path) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static GtkTreePath *nautilus_list_model_get_path(GtkTreeModel *tree_model,
        GtkTreeIter *iter) {
    GtkTreePath *path;
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static gint nautilus_list_model_get_icon_scale(NautilusListModel *model) {
    gint retval = -1;

    g_signal_emit(model, list_model_signals[GET_ICON_SCALE], 0, &retval);

    if (retval == -1) {
        retval = gdk_monitor_get_scale_factor(
...
guint nautilus_list_model_get_icon_size_for_zoom_level(
...
static void nautilus_list_model_get_value(GtkTreeModel *tree_model,
    ...
    case NAUTILUS_LIST_MODEL_LARGER_ICON_COLUMN: {
        ...
        if (file != NULL) {
            ...
            if (priv->drag_view != NULL) {
                ...
                gtk_tree_view_get_drag_dest_row(priv->drag_view, &path_a, NULL);
                if (path_a != NULL) {
            ...
            icon = nautilus_file_get_icon_pixbuf(file, icon_size, TRUE, icon_scale,
                                                 flags);

            if (priv->highlight_files != NULL &&
                    g_list_find_custom(priv->highlight_files, file,
    ...
    default: {
        if (column >= NAUTILUS_LIST_MODEL_NUM_COLUMNS &&
                column < NAUTILUS_LIST_MODEL_NUM_COLUMNS + priv->columns->len) {
            NautilusColumn *nautilus_column;
            GQuark attribute;
            nautilus_column =
                priv->columns->pdata[column - NAUTILUS_LIST_MODEL_NUM_COLUMNS];

            g_value_init(value, G_TYPE_STRING);
            g_object_get(nautilus_column, "attribute_q", &attribute, NULL);
            if (file != NULL) {
                str =
                    nautilus_file_get_string_attribute_with_default_q(file, attribute);
...
static gboolean nautilus_list_model_iter_next(GtkTreeModel *tree_model,
        GtkTreeIter *iter) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static gboolean nautilus_list_model_iter_children(GtkTreeModel *tree_model,
        GtkTreeIter *iter,
        GtkTreeIter *parent) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static gboolean nautilus_list_model_iter_has_child(GtkTreeModel *tree_model,
        GtkTreeIter *iter) {
    FileEntry *file_entry;
    ...
    return (file_entry->files != NULL &&
            g_sequence_get_length(file_entry->files) > 0);
}

static int nautilus_list_model_iter_n_children(GtkTreeModel *tree_model,
        GtkTreeIter *iter) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
    GSequence *files;
...
static gboolean nautilus_list_model_iter_nth_child(GtkTreeModel *tree_model,
        GtkTreeIter *iter,
...
static gboolean nautilus_list_model_iter_parent(GtkTreeModel *tree_model,
        GtkTreeIter *iter,
        GtkTreeIter *child) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static GSequenceIter *lookup_file(NautilusListModel *model, NautilusFile *file,
    ...
    parent_ptr = NULL;
    if (directory) {
        parent_ptr = g_hash_table_lookup(priv->directory_reverse_map, directory);
    }
...
static void dir_to_iters(struct GetIters *data, GHashTable *reverse_map) {
    GSequenceIter *ptr;
...
static void file_to_iter_cb(gpointer key, gpointer value, gpointer user_data) {
...
GList *nautilus_list_model_get_all_iters_for_file(NautilusListModel *model,
    ...
    g_hash_table_foreach(priv->directory_reverse_map, file_to_iter_cb, &data);
...
gboolean nautilus_list_model_get_tree_iter_from_file(
    NautilusListModel *model, NautilusFile *file, NautilusDirectory *directory,
    GtkTreeIter *iter) {
    GSequenceIter *ptr;
...
static int nautilus_list_model_file_entry_compare_func(gconstpointer a,
    ...
    if (file_entry1->file != NULL && file_entry2->file != NULL) {
        result = nautilus_file_compare_for_sort_by_attribute_q(
                     file_entry1->file, file_entry2->file, priv->sort_attribute,
                     priv->sort_directories_first, (priv->order == GTK_SORT_DESCENDING));
...
int nautilus_list_model_compare_func(NautilusListModel *model,
    ...
    priv = nautilus_list_model_get_instance_private(model);
    result = nautilus_file_compare_for_sort_by_attribute_q(
                 file1, file2, priv->sort_attribute, priv->sort_directories_first,
                 (priv->order == GTK_SORT_DESCENDING));

    return result;
}

static void nautilus_list_model_sort_file_entries(NautilusListModel *model,
        GSequence *files,
        GtkTreePath *path) {
    ...
    gtk_tree_model_rows_reordered(GTK_TREE_MODEL(model), path,
                                  has_iter ? &iter : NULL, new_order);

    g_free(old_order);
    g_free(new_order);
}

static void nautilus_list_model_sort(NautilusListModel *model) {
    GtkTreePath *path;
    NautilusListModelPrivate *priv;
...
static gboolean nautilus_list_model_get_sort_column_id(
...
static void nautilus_list_model_set_sort_column_id(GtkTreeSortable *sortable,
        gint sort_column_id,
        GtkSortType order) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
    ...
    priv->sort_attribute = nautilus_list_model_get_attribute_from_sort_column_id(
...
static void add_dummy_row(NautilusListModel *model, FileEntry *parent_entry) {
    NautilusListModelPrivate *priv;
    ...
    dummy_file_entry->ptr = g_sequence_insert_sorted(
                                parent_entry->files, dummy_file_entry,
...
gboolean nautilus_list_model_add_file(NautilusListModel *model,
    ...
    file_entry->ptr = g_sequence_insert_sorted(
                          files, file_entry, nautilus_list_model_file_entry_compare_func, model);
...
void nautilus_list_model_file_changed(NautilusListModel *model,
    ...
    g_sequence_sort_changed(ptr, nautilus_list_model_file_entry_compare_func,
                            model);
    ...
    if (pos_before != pos_after) {
        ...
        gtk_tree_model_rows_reordered(GTK_TREE_MODEL(model), parent_path,
                                      has_iter ? &iter : NULL, new_order);
...
static void nautilus_list_model_remove(NautilusListModel *model,
                                       GtkTreeIter *iter) {
    NautilusListModelPrivate *priv;
    ...
    if (file_entry->files != NULL) {
        while (g_sequence_get_length(file_entry->files) > 0) {
            ...
            if (child_file_entry->file != NULL) {
                nautilus_list_model_remove_file(model, child_file_entry->file,
    ...
    if (file_entry->subdirectory != NULL) {
        g_signal_emit(model, list_model_signals[SUBDIRECTORY_UNLOADED], 0,
                      file_entry->subdirectory);
        g_hash_table_remove(priv->directory_reverse_map, file_entry->subdirectory);
    }
    ...
    if (parent_file_entry &&
            g_sequence_get_length(parent_file_entry->files) == 0) {
...
void nautilus_list_model_remove_file(NautilusListModel *model,
    ...
    if (nautilus_list_model_get_tree_iter_from_file(model, file, directory,
            &iter)) {
        nautilus_list_model_remove(model, &iter);
    }
}

static void nautilus_list_model_clear_directory(NautilusListModel *model,
        GSequence *files) {
    NautilusListModelPrivate *priv;
    GtkTreeIter iter;
    FileEntry *file_entry;
...
gboolean nautilus_list_model_load_subdirectory(NautilusListModel *model,
        GtkTreePath *path,
        NautilusDirectory **directory) {
    NautilusListModelPrivate *priv;
    GtkTreeIter iter;
    ...
    file_entry->subdirectory = subdirectory,
                g_hash_table_insert(priv->directory_reverse_map, subdirectory,
                                    file_entry->ptr);
...
void nautilus_list_model_set_should_sort_directories_first(
    NautilusListModel *model, gboolean sort_directories_first) {
    NautilusListModelPrivate *priv;
...
int nautilus_list_model_get_sort_column_id_from_attribute(
    NautilusListModel *model, GQuark attribute) {
    NautilusListModelPrivate *priv;
    guint i;
    ...
    for (i = 0; i < priv->columns->len; i++) {
        ...
        g_object_get(G_OBJECT(column), "attribute_q", &column_attribute, NULL);
...
int nautilus_list_model_get_column_id_from_zoom_level(
    NautilusListZoomLevel zoom_level) {
    switch (zoom_level) {
    case NAUTILUS_LIST_ZOOM_LEVEL_SMALL: {
...
void nautilus_list_model_set_drag_view(NautilusListModel *model,
                                       GtkTreeView *view, int drag_begin_x,
                                       int drag_begin_y) {
    NautilusListModelPrivate *priv;
...
GtkTargetList *nautilus_list_model_get_drag_target_list() {
...
static void nautilus_list_model_dispose(GObject *object) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
    int i;
...
static void nautilus_list_model_finalize(GObject *object) {
    NautilusListModel *model;
    NautilusListModelPrivate *priv;
...
static void nautilus_list_model_init(NautilusListModel *model) {
    NautilusListModelPrivate *priv;
...
static void nautilus_list_model_class_init(NautilusListModelClass *klass) {
    ...
    list_model_signals[SUBDIRECTORY_UNLOADED] = g_signal_new(
                "subdirectory-unloaded", NAUTILUS_TYPE_LIST_MODEL, G_SIGNAL_RUN_FIRST,
                G_STRUCT_OFFSET(NautilusListModelClass, subdirectory_unloaded), NULL,
                NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
                NAUTILUS_TYPE_DIRECTORY);

    list_model_signals[GET_ICON_SCALE] =
        g_signal_new("get-icon-scale", NAUTILUS_TYPE_LIST_MODEL,
                     G_SIGNAL_RUN_FIRST | G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
                     G_TYPE_INT, 0);
}
...
void nautilus_list_model_subdirectory_done_loading(
    NautilusListModel *model, NautilusDirectory *directory) {
    NautilusListModelPrivate *priv;
    GtkTreeIter iter;
    GtkTreePath *path;
...
static void refresh_row(gpointer data, gpointer user_data) {
    NautilusFile *file;
    NautilusListModel *model;
    GList *iters, *l;
    GtkTreePath *path;
