...
enum { PROP_VIEW = 1, NUM_PROPERTIES };
...
static void selection_changed_cb(NautilusFilesView *view,
  ...
  gtk_info_bar_set_response_sensitive(GTK_INFO_BAR(bar),
                                      TRASH_BAR_RESPONSE_RESTORE, (count > 0));
}

static void connect_view_and_update_button(NautilusTrashBar *bar) {
  bar->selection_handler_id = g_signal_connect(
      bar->view, "selection-changed", G_CALLBACK(selection_changed_cb), bar);

  selection_changed_cb(bar->view, bar);
}

static void nautilus_trash_bar_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void nautilus_trash_bar_dispose(GObject *obj) {
...
static void nautilus_trash_bar_class_init(NautilusTrashBarClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "view", "view", "the NautilusFilesView", NAUTILUS_TYPE_FILES_VIEW,
...
static void trash_bar_response_cb(GtkInfoBar *infobar, gint response_id,
  ...
  case TRASH_BAR_RESPONSE_AUTODELETE: {
    ...
    app_info = g_app_info_create_from_commandline(
        "gnome-control-center usage", NULL, G_APP_INFO_CREATE_NONE, NULL);

    g_app_info_launch(app_info, NULL, NULL, &error);

    if (error) {
      show_dialog(_("There was an error launching the application."),
                  error->message, GTK_WINDOW(window), GTK_MESSAGE_ERROR);
    }
  } break;
...
static void nautilus_trash_bar_init(NautilusTrashBar *bar) {
  ...
  subtitle_text =
      _("Trashed items are automatically deleted after a period of time");
  subtitle = gtk_label_new(subtitle_text);
  gtk_widget_set_tooltip_text(subtitle, subtitle_text);
  gtk_label_set_ellipsize(GTK_LABEL(subtitle), PANGO_ELLIPSIZE_END);

  g_settings_bind(gnome_privacy_preferences, "remove-old-trash-files", subtitle,
  ...
  w = gtk_info_bar_add_button(GTK_INFO_BAR(bar), _("_Settings"),
                              TRASH_BAR_RESPONSE_AUTODELETE);
  gtk_widget_set_tooltip_text(w,
                              _("Display system controls for trash content"));

  w = gtk_info_bar_add_button(GTK_INFO_BAR(bar), _("_Restore"),
                              TRASH_BAR_RESPONSE_RESTORE);
  gtk_widget_set_tooltip_text(
      w, _("Restore selected items to their original position"));

  w = gtk_info_bar_add_button(
      GTK_INFO_BAR(bar),
      /* Translators: "Empty" is an action (for the trash) , not a state */
      _("_Empty…"), TRASH_BAR_RESPONSE_EMPTY);
  gtk_widget_set_tooltip_text(w, _("Delete all items in the Trash"));

  g_signal_connect_object(nautilus_trash_monitor_get(), "trash-state-changed",
                          G_CALLBACK(nautilus_trash_bar_trash_state_changed),
                          bar, 0);
  nautilus_trash_bar_trash_state_changed(nautilus_trash_monitor_get(), FALSE,
                                         bar);

  g_signal_connect(bar, "response", G_CALLBACK(trash_bar_response_cb), bar);
}

GtkWidget *nautilus_trash_bar_new(NautilusFilesView *view) {
  return g_object_new(NAUTILUS_TYPE_TRASH_BAR, "view", view, "message-type",
                      GTK_MESSAGE_QUESTION, NULL);
}
