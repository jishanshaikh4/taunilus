...
gboolean nautilus_uri_parse(const char *uri, char **host, guint16 *port,
                            char **userinfo) {
    char *tmp_str;
    const char *start, *p;
    char c;
...
char *nautilus_compute_title_for_location(GFile *location) {
    NautilusFile *file;
    GMount *mount;
    char *title;
...
char *nautilus_get_user_directory(void) {
    ...
    user_directory = g_build_filename(g_get_user_config_dir(),
                                      NAUTILUS_USER_DIRECTORY_NAME, NULL);
...
gboolean nautilus_is_home_directory_file(GFile *dir, const char *filename) {
    char *dirname;
...
GFile *nautilus_generate_unique_file_in_directory(GFile *directory,
    ...
    while (g_file_query_exists(child, NULL)) {
        ...
        filename = g_strdup_printf("%s (%d)%s", basename_without_extension, copy,
                                   extension ? extension : "");
...
GFile *nautilus_find_existing_uri_in_hierarchy(GFile *location) {
    ...
    while (location != NULL) {
        info = g_file_query_info(location, G_FILE_ATTRIBUTE_STANDARD_NAME, 0, NULL,
                                 NULL);
        g_object_unref(info);
        if (info != NULL) {
            return location;
...
static gboolean have_program_in_path(const char *name) {
    gchar *path;
    gboolean result;
...
static GIcon *special_directory_get_icon(GUserDirectory directory,
        gboolean symbolic) {
#define ICON_CASE(x)                                                           \
  case G_USER_DIRECTORY_##x:                                                   \
    return (symbolic) ? g_themed_icon_new(NAUTILUS_ICON_FOLDER_##x)            \
                      : g_themed_icon_new(NAUTILUS_ICON_FULLCOLOR_FOLDER_##x);
    ...
    default: {
        return (symbolic) ? g_themed_icon_new(NAUTILUS_ICON_FOLDER)
               : g_themed_icon_new(NAUTILUS_ICON_FULLCOLOR_FOLDER);
    }
    }
...
nautilus_trashed_files_get_original_directories(GList *files,
    ...
    for (l = files; l != NULL; l = l->next) {
        ...
        if (original_dir != NULL) {
            if (directories == NULL) {
                directories = g_hash_table_new_full(
                                  g_direct_hash, g_direct_equal, (GDestroyNotify)nautilus_file_unref,
            ...
            *unhandled_files =
                g_list_append(*unhandled_files, nautilus_file_ref(file));
        }
...
static GList *locations_from_file_list(GList *file_list) {
    NautilusFile *file;
    GList *l, *ret;
...
static void ensure_dirs_task_ready_cb(GObject *_source, GAsyncResult *res,
                                      gpointer user_data) {
    ...
    for (l = original_dirs; l != NULL; l = l->next) {
        ...
        nautilus_file_operations_move_async(locations, original_dir_location,
                                            data->parent_window, NULL, NULL, NULL);
...
static void ensure_dirs_task_thread_func(GTask *task, gpointer source,
        gpointer task_data,
    ...
    for (l = original_dirs; l != NULL; l = l->next) {
        ...
        g_file_make_directory_with_parents(original_dir_location, cancellable,
                                           NULL);
        g_object_unref(original_dir_location);
    }
...
void nautilus_restore_files_from_trash(GList *files, GtkWindow *parent_window) {
    NautilusFile *file;
    GHashTable *original_dirs_hash;
    ...
    original_dirs_hash =
        nautilus_trashed_files_get_original_directories(files, &unhandled_files);

    for (l = unhandled_files; l != NULL; l = l->next) {
        file = NAUTILUS_FILE(l->data);
        file_name = nautilus_file_get_display_name(file);
        message = g_strdup_printf(
                      _("Could not determine original location of “%s” "), file_name);
        g_free(file_name);

        show_dialog(message, _("The item cannot be restored from trash"),
                    parent_window, GTK_MESSAGE_WARNING);
        g_free(message);
    }
...
static void get_types_cb(GObject *source_object, GAsyncResult *res,
                         gpointer user_data) {
    ...
    g_object_set_data_full(source_object, "nautilus-content-type-cache",
                           g_strdupv(types), (GDestroyNotify)g_strfreev);
...
void nautilus_get_x_content_types_for_mount_async(
    GMount *mount, NautilusMountGetContent callback, GCancellable *cancellable,
    gpointer user_data) {
    char **cached;
    GetContentTypesData *data;
    ...
    g_mount_guess_content_type(mount, FALSE, cancellable, get_types_cb, data);
}
...
char *get_message_for_two_content_types(const char *const *content_types) {
    ...
    if (strcmp(content_types[0], "x-content/image-dcf") == 0 ||
        ...
        message =
            g_strdup_printf("%s/%s", get_message_for_content_type(content_types[0]),
...
gboolean should_handle_content_types(const char *const *content_types) {
    int i;
...
char *nautilus_get_common_filename_prefix(GList *file_list,
    ...
    result_files = nautilus_get_common_filename_prefix_from_filenames(
                       file_names, min_required_len);
...
char *nautilus_get_common_filename_prefix_from_filenames(GList *filenames,
        int min_required_len) {
    ...
    common_prefix =
        eel_str_get_common_prefix(stripped_filenames, min_required_len);
    ...
    if (name_max == -1) {
        ...
        if (path_max != -1) {
            max_child_name_length =
                CLAMP((path_max - 1) - strlen(path), 0, max_child_name_length);
        }
    }
...
void nautilus_self_check_file_utilities(void) {}
