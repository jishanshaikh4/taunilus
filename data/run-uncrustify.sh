...
if [ ! -x "$DATA/lineup-parameters" ]; then
  echo "Script lineup-parameters does not exists."
  exit 1
fi

for DIR in "$DATA/../"{src,test,libnautilus-extension,eel,extensions}; do
  for FILE in "$(find "$DIR" -name "*.c" -not -path "*/gtk/*" -not -path "*/animation/*" -not -path "*/audio-video-properties/*")"; do
